"""No-clobber freezer for the latent V12R8/Q3 checkpoint-zero runtime.
...
try:
    ...
    from . import run_h0_v12r8_zero_checkpoint as runner
    ...
    import run_h0_v12r8_zero_checkpoint as runner


class ZeroCheckpointFreezeDeferredError(RuntimeError):
    """Raised before I/O while R8/Q3 terminal PASS evidence is unavailable."""
...
def build_lock_payload(
    *,
    ...
    if upstream.get("passed") is not True:
        _fail("R8 and Q3 terminal semantic prerequisite gate failed")
    ...
    if source_closure.get("candidate_module") != candidate_module:
        _fail("source closure candidate differs from terminal R8/Q3 candidate")
    ...
    payload = {
        ...
        "candidate_binding_state": "BOUND_AFTER_R8_AND_Q3_TERMINAL_PASS",
        "binding": binding,
...
def validate_prerequisites() -> dict[str, Any]:
    """Return the live semantic R8/Q3 prerequisite payload without publication."""
    ...
    if gate.get("passed") is not True:
        _fail("R8/Q3 semantic terminal gate failed")
    return payload
