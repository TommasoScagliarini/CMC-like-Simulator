"""Pure fail-closed gates for the deferred V12R8 checkpoint-zero port."""
...
def upstream_terminal_gate(payload: Any) -> dict[str, Any]:
    """Require semantic terminal PASS from R8 and Q3 for one exact candidate."""

    data = _mapping(payload)
    r8_ledger = _mapping(data.get("v12r8_terminal"))
    q3_ledger = _mapping(data.get("v12r8_q3_terminal"))
    attestations = _mapping(data.get("semantic_attestations"))
    r8_module = _mapping(r8_ledger.get("candidate_module"))
    q3_module = _mapping(q3_ledger.get("candidate_module"))
    candidate_id = r8_ledger.get("candidate_id")
    r8_endpoint = contract.R8_TERMINAL_ENDPOINT
    q3_endpoint = contract.Q3_TERMINAL_ENDPOINT
    checks = {
        "r8_semantic_verifier": _semantic_attestation_valid(
        ...
        "r8_terminal_pass": r8_ledger.get("schema_version")
        == r8_endpoint["schema_version"]
        and r8_ledger.get("status") == r8_endpoint["required_status"]
        and r8_ledger.get("passed") is True
        and r8_ledger.get("terminal") is True
        and r8_ledger.get("protocol_id") == r8_endpoint["protocol_id"]
        and r8_ledger.get("pipeline_id") == r8_endpoint["pipeline_id"]
        and r8_ledger.get("error") is None
        and r8_ledger.get("candidate_selection_rule")
        == contract.CANDIDATE_SELECTION_RULE
        and r8_ledger.get("actor_updates") == r8_endpoint["expected_actor_updates"]
        and r8_ledger.get("critic_updates") == r8_endpoint["expected_critic_updates"]
        and r8_ledger.get("ppo_updates") == r8_endpoint["expected_ppo_updates"]
        and r8_ledger.get("checkpoint_zero_created") is False
        and r8_ledger.get("positive_morphology_enabled") is False
        and r8_ledger.get("runtime_promoted") is False
        and r8_ledger.get("qualification_executed") is False
        and r8_ledger.get("q3_paths_opened") == []
        and r8_ledger.get("next_stage") == "WAIT_SEPARATE_V12R8Q3_PROTOCOL"
        and artifact_record_valid(
            r8_ledger.get("candidate_freeze"),
            expected_path=contract.R8_CANDIDATE_FREEZE_ENDPOINT["path"],
        )
        and artifact_record_valid(
            r8_ledger.get("final_development_receipt"),
            expected_path=contract.R8_FINAL_DEVELOPMENT_ENDPOINT["path"],
        ),
        ...
        and q3_ledger.get("next_stage") == contract.q3.NEXT_STAGE_AFTER_Q3_PASS
        and artifact_record_valid(
        ...
        and candidate_id
        == contract.candidate_id_for_tree(r8_module.get("tree_sha256", "")),
    }
    passed = all(checks.values())
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            "PASS_H0_V12R8_ZERO_UPSTREAM_PREREQUISITES"
            if passed
            else "FAIL_H0_V12R8_ZERO_UPSTREAM_PREREQUISITES"
        ),
        "passed": passed,
        "checks": checks,
        "candidate_id": candidate_id if passed else None,
        "candidate_module": r8_module if passed else None,
    }
...
def runtime_audit_gate(
    ...
    checks = {
        ...
        and data.get("status") == "COMPLETE_H0_V12R8_ZERO_CHECKPOINT_AUDIT"
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            "PASS_H0_V12R8_ZERO_CHECKPOINT_AUDIT"
            if passed
            else "FAIL_H0_V12R8_ZERO_CHECKPOINT_AUDIT"
        ),
        "passed": passed,
        "checks": checks,
    }
...
def terminal_receipt_gate(value: Any) -> dict[str, Any]:
    ...
    checks = {
        ...
        and audit_gate.get("status") == "PASS_H0_V12R8_ZERO_CHECKPOINT_AUDIT"
