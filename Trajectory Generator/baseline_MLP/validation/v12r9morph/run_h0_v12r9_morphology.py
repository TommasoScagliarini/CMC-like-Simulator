"""Latent-live one-shot V12R9 positive morphology qualification.

Importing this module is inert.  The live path becomes reachable only after
the native R9, R9-Q3 and R9-zero semantic verifiers all return terminal PASS
...
for _root in (
    HERE,
    BASELINE_ROOT,
    VALIDATION_ROOT / "v12r9",
    VALIDATION_ROOT / "v12r9q3",
    VALIDATION_ROOT / "v12r9q3" / "runtime",
    VALIDATION_ROOT / "v12r9zero",
...
def require_repository_root_cwd(
    ...
    """Reject relative training/handoff commands outside the repository root."""
    ...
    if observed_real != expected_real:
        raise MorphologyExecutionError(
            f"V12R9 morphology requires cwd=repository_root; observed {observed}"
        )
    return {
        "passed": True,
...
def semantic_upstream_payload() -> dict[str, Any]:
    ...
    try:
        import run_h0_v12r9_q3_qualification as q3_runner
        import run_h0_v12r9_recovery as r9_runner

        r9_result = r9_runner.verify_terminal_ledger()
        q3_result = q3_runner.verify_terminal_ledger()
        zero_result = zero_runner.verify_terminal_pass()
    except BaseException as exc:
        raise MorphologyExecutionError(
            f"native R9/Q3/zero terminal verification failed: {exc}"
        ) from exc
    if not all(
        isinstance(item, Mapping) for item in (r9_result, q3_result, zero_result)
    ):
        raise MorphologyExecutionError("native terminal verifier returned non-mapping")
    r9_payload = dict(r9_result)
    q3_payload = dict(q3_result)
    zero_payload = dict(zero_result)
    payload = {
        "v12r9_terminal": r9_payload,
        "v12r9_q3_terminal": q3_payload,
        "v12r9_zero_terminal": zero_payload,
        "semantic_attestations": {
            "v12r9_terminal": _attestation(contract.R9_TERMINAL_ENDPOINT, r9_payload),
            "v12r9_q3_terminal": _attestation(
                contract.Q3_TERMINAL_ENDPOINT, q3_payload
            ),
            "v12r9_zero_terminal": _attestation(
...
def condition_snapshot() -> dict[str, Any]:
    ...
    try:
        import prepare_h0_v12r9_q3_noise_tapes as noise
    except BaseException as exc:
...
def inspect_q3_control_capabilities() -> dict[str, Any]:
    ...
    for case_id in contract.CASE_IDS:
        ...
        if (
            ...
            or len(rows) != contract.EXPECTED_STEPS_PER_ROLLOUT
        ):
...
def checkpoint_module_snapshot(binding: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "checkpoint": _same_json(
            audit.get("checkpoint_tree"), binding.get("checkpoint")
        ),
...
def verify_lock_payload(
    ...
    *,
    ...
    capability = gates.q3_control_capability_gate(lock.get("q3_control_capabilities"))
    ...
    checks = {
        ...
        and lock.get("required_working_directory")
        ...
        "training_validation": _same_json(
            lock.get("training_validation_endpoint"),
...
def verify_lock() -> dict[str, Any]:
    ...
    try:
        ...
        raise MorphologyExecutionError(
            f"protocol freeze verification failed: {exc}"
        ) from exc
...
def build_execution_plan(lock_payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "status": "PLANNED_H0_V12R9_MORPH_PHYSICAL_AB",
...
def _persist_arm(
    *,
    ...
    if not _same_json(summary["streams"], _streams_from_trace(trace_rows)):
        raise MorphologyExecutionError(
            f"collector stream digest drifted: {case_id}/{role}"
        )
...
def verify_arm(*, lock: Mapping[str, Any], case_id: str, role: str) -> dict[str, Any]:
    paths = _arm_paths(case_id, role)
    ...
    if (
        set(trace_wrapper) != {"rows"}
        or set(reward_wrapper) != {"rows"}
        or set(causal_wrapper) != {"rows"}
    ):
    ...
    checks["gate"] = (
        _same_json(observed_gate, expected_gate) and expected_gate.get("passed") is True
    )
    expected_receipt = _arm_receipt(summary, expected_gate)
    checks["receipt"] = _same_json(strict_mapping(paths["receipt"]), expected_receipt)
    if not all(checks.values()):
        failed = sorted(name for name, passed in checks.items() if not passed)
        raise MorphologyExecutionError(
...
def _pair_receipt(case_id: str, gate: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "pair_gate": artifact_record(
            resolve_relative(contract.pair_gate_path(case_id))
        ),
...
def build_training_handoff(
    ...
    return {
        ...
        "required_training_launcher": contract.TRAINING_ENTRYPOINT.as_posix(),
        "required_working_directory": contract.REQUIRED_WORKING_DIRECTORY,
...
def execute(*, collector: ArmCollector | None = None) -> dict[str, Any]:
    ...
    selected = collector
    if selected is None:
        from h0_v12r9_morphology_physical_rollout import collect_physical_arm
    ...
    try:
        ...
        receipt = build_terminal_receipt(lock_payload=lock, pair_results=pair_results)
        if gates.terminal_receipt_gate(receipt).get("passed") is not True:
        ...
        write_json_exclusive(resolve_relative(contract.TRAINING_HANDOFF_PATH), handoff)
        write_json_exclusive(
...
def verify_terminal_ledger() -> dict[str, Any]:
    ...
    checks = {
        ...
        and ledger.get("required_working_directory")
        == contract.REQUIRED_WORKING_DIRECTORY
