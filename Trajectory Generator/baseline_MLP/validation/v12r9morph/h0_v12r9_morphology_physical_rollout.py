"""Physical control/positive collector for the V12R9 morphology A/B.
...
Q3_ROOT = VALIDATION_ROOT / "v12r9q3"
...
def build_env_config(
    ...
    *,
    ...
    import h0_v12r9_q3_physical_rollout as q3_physical
    ...
    fixed = {
        ...
        "binary_phase_detector_profile": result.get("binary_phase_detector_profile"),
        "detector_sample_dt_s": result.get("detector_sample_dt_s"),
        "binary_phase_debounce_s": result.get("binary_phase_debounce_s"),
        "binary_phase_event_contract_id": result.get("binary_phase_event_contract_id"),
        "policy_step_s": result.get("segment_duration"),
    }
    expected_fixed = {key: contract.V26_RUNTIME_CONFIG[key] for key in fixed}
...
class _RecordingEnv:
    def __init__(
        self, env: Any, *, role: str, reward_mapping: Mapping[str, Any]
    ) -> None:
        self._env = env
        self.role = role
    ...
    def step(self, action: Any) -> Any:
        ...
        observation_after = np.ascontiguousarray(observation, dtype=np.float32).reshape(
            -1
        )
        if (
        ...
        expected_term = (
            0.0 if self.role == contract.CONTROL_ROLE else 0.0025 * morphology_loss
        )
        ...
        self.steps.append(
            {
                ...
                "applied_action": np.ascontiguousarray(action, dtype=np.float32)
                .reshape(-1)
                .tolist(),
...
def _canonical_surfaces(
    *,
    ...
    for index, (recorded, mature_raw) in enumerate(
        ...
        event = {
            "binary_phase_sensor_samples": recorded["binary_phase_sensor_samples"],
...
def collect_physical_arm(
    *,
    ...
    import h0_v12r9_q3_physical_rollout as q3_physical
    import prepare_h0_v12r9_q3_noise_tapes as q3_noise
    ...
    def patched_builder(selected_case: Mapping[str, Any]) -> dict[str, Any]:
        return build_env_config(selected_case, role=role, base_builder=original_builder)
    ...
    return {
        **surfaces,
        ...
        "physical_summary_proxy": {
            ...
            "raw_sensor_sample_count": physical_summary.get("raw_sensor_sample_count"),
