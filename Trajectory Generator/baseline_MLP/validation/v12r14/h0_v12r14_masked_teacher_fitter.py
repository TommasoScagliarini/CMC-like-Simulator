"""One-shot V12R14 actor builder from the frozen V8R1P1/V26 safe tape.
...
class V12R14MaskedTeacherFitError(RuntimeError):
    """Raised when any immutable input, fit gate, or artifact gate fails."""


SCHEMA_VERSION = 1214
REVISION = "2026-08-16"
PROTOCOL_ID = "AB06_H0_V12R14_V26_INVARIANT_SAFE_TEACHER"
FIT_CONTRACT_ID = "h0_v12r14_masked_v8r1p1_safe_teacher_w256_v1"
...
GLOBAL_RMSE_LIMIT = 5.0e-3
GLOBAL_MAX_ABS_LIMIT = 6.0e-2
PER_CASE_ACTION_RMSE_LIMIT = 5.5e-3
PER_TRAJECTORY_ACTION_RMSE_LIMIT = 5.5e-3
RESET_MAX_ABS_LIMIT = 3.0e-3
TRANSITION_MAX_ABS_LIMIT = 3.0e-2
TEMPORAL_FIRST_DIFFERENCE_MAX_ABS_LIMIT = 8.0e-2
...
RUN_ROOT = REVISION_ROOT / "h0_v12r14_run_20260816"
...
P1_PROTOCOL_FREEZE_PATH = REVISION_ROOT / "h0_v12r14_tape_dagger_protocol_freeze.json"
P1_EXECUTION_LOCK_PATH = REVISION_ROOT / "h0_v12r14_tape_dagger_execution_lock.json"
...
PROTOCOL_FREEZE_PATH = REVISION_ROOT / "h0_v12r14_protocol_freeze.json"
EXECUTION_LOCK_PATH = REVISION_ROOT / "h0_v12r14_execution_lock.json"
CANDIDATE_DIRNAMES = {
...
def _resolve(path: str | PurePath | Path) -> Path:
    ...
    try:
        ...
        raise V12R14MaskedTeacherFitError(
...
def _reject_link_components(path: Path, *, include_leaf: bool) -> None:
    ...
    try:
        ...
        raise V12R14MaskedTeacherFitError(f"unsafe path: {path}") from exc
...
def _record(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    if not _regular_file(resolved):
        raise V12R14MaskedTeacherFitError(f"unsafe or missing file: {resolved}")
    return {
...
def _tree_record(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R14MaskedTeacherFitError(f"missing tree: {root}") from exc
    if not stat.S_ISDIR(root_status.st_mode) or _is_link_or_reparse(root):
        raise V12R14MaskedTeacherFitError(f"unsafe tree: {root}")
    files: list[Path] = []
    for current_text, directory_names, file_names in os.walk(
        ...
        for name in directory_names:
            ...
            if _is_link_or_reparse(child) or not stat.S_ISDIR(os.lstat(child).st_mode):
                raise V12R14MaskedTeacherFitError(f"unsafe tree directory: {child}")
        for name in file_names:
            child = current / name
            if not _regular_file(child) or _is_link_or_reparse(child):
                raise V12R14MaskedTeacherFitError(f"unsafe tree file: {child}")
            files.append(child)
    ...
    for item in files:
        ...
        digest.update(b"\n")
    if not rows:
        raise V12R14MaskedTeacherFitError(f"empty tree: {root}")
    return {
...
def _strict_json_value(path: str | PurePath | Path) -> Any:
    ...
    try:
        ...
        raise V12R14MaskedTeacherFitError(f"invalid strict JSON: {resolved}") from exc
    return value
...
def _strict_json(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise V12R14MaskedTeacherFitError(f"expected JSON object: {resolved}")
    return value
...
def _validate_referenced_artifact(record: Mapping[str, Any]) -> None:
    if set(record) != {"path", "sha256", "size_bytes"}:
        raise V12R14MaskedTeacherFitError("referenced artifact record schema drifted")
...
def attest_locked_sources() -> dict[str, Any]:
    ...
    if not isinstance(completed, list) or len(completed) != len(CASE_IDS):
        raise V12R14MaskedTeacherFitError("V8R1P1 receipt index drifted")
    ...
    for expected_case, item in zip(CASE_IDS, completed, strict=True):
        if not isinstance(item, Mapping) or item.get("case_id") != expected_case:
            raise V12R14MaskedTeacherFitError("V8R1P1 receipt order drifted")
        receipt_record = item.get("receipt")
        if not isinstance(receipt_record, Mapping):
            raise V12R14MaskedTeacherFitError("missing V8R1P1 receipt record")
        canonical = V8R1P1_ROOT / expected_case / "receipt.json"
        if receipt_record.get("path") != canonical.as_posix():
            raise V12R14MaskedTeacherFitError("non-canonical V8R1P1 receipt path")
        ...
        for name in expected_artifact_keys - {"steps"}:
            ...
            if not isinstance(record, Mapping):
                raise V12R14MaskedTeacherFitError("artifact record is not a mapping")
    ...
    if not all(checks.values()):
        ...
        raise V12R14MaskedTeacherFitError(f"V8R1P1 source closure failed: {failed}")
    return {
        "source_h0": h0,
...
def _float32_vector(value: Any, *, width: int, label: str) -> np.ndarray:
    ...
    if result.shape != (width,) or not np.all(np.isfinite(result)):
        raise V12R14MaskedTeacherFitError(f"malformed finite {label}")
    return result
...
def load_locked_corpus() -> LockedCorpus:
    ...
    if (
        ...
        raise V12R14MaskedTeacherFitError("materialized corpus shape drifted")
...
def _validate_referenced_tree(record: Mapping[str, Any]) -> None:
    ...
    if set(record) != required:
        raise V12R14MaskedTeacherFitError("referenced tree record schema drifted")
...
def _validated_p0_terminal() -> dict[str, Any]:
    ...
    checks = {
        ...
        == "PASS_H0_V12R14_MASKED_SAFE_TEACHER_FIT_RECEIPT",
        "protocol": receipt.get("protocol_id") == PROTOCOL_ID,
        "gate_pass": gate.get("passed") is True,
        "gate_status": gate.get("status") == "PASS_H0_V12R14_MASKED_SAFE_TEACHER_FIT",
    ...
    if not all(checks.values()):
        failed = sorted(name for name, passed in checks.items() if not passed)
        raise V12R14MaskedTeacherFitError(f"P0 terminal drifted: {failed}")
    for name in ("corpus", "summary", "gate"):
        record = artifacts[name]
        if not isinstance(record, Mapping):
            raise V12R14MaskedTeacherFitError("P0 artifact record malformed")
        _validate_referenced_artifact(record)
    candidate = artifacts["candidate_module"]
    if not isinstance(candidate, Mapping):
        raise V12R14MaskedTeacherFitError("P0 candidate tree record malformed")
    ...
    if candidate.get("path") != _logical_path(expected_candidate_path):
        raise V12R14MaskedTeacherFitError("P0 candidate path is non-canonical")
    return {
...
def _p1_alpha_tag(alpha: float) -> str:
    ...
    if alpha not in tags:
        raise V12R14MaskedTeacherFitError("non-canonical P1 alpha")
    return tags[alpha]


def _p1_trajectory_id(alpha: float, case_id: str) -> str:
    if case_id not in CASE_IDS:
        raise V12R14MaskedTeacherFitError("non-canonical P1 trajectory identity")
...
def _reconstruct_p1_journals(
    *,
    ...
    if len(case_receipt_records) != len(P1_ALPHAS) * len(CASE_IDS):
        raise V12R14MaskedTeacherFitError("P1 journal receipt count drifted")
    observations: list[np.ndarray] = []
    ...
    for alpha in P1_ALPHAS:
        for case_id in CASE_IDS:
            ...
            receipt_checks = {
                ...
                == "PASS_H0_V12R14_P1_CANDIDATE_EXPOSED_COLLECTION",
                "protocol": receipt.get("protocol_id") == PROTOCOL_ID,
                ...
                "rows": type(receipt.get("row_count")) is int
                and 1 <= receipt["row_count"] <= ROWS_PER_CASE,
                "zero_updates": receipt.get("actor_updates") == 0
                and receipt.get("critic_updates") == 0
                and receipt.get("ppo_updates") == 0,
            ...
            case_rows = int(receipt["row_count"])
            step_records = artifacts["steps"]
            if not isinstance(step_records, list) or len(step_records) != case_rows:
                raise V12R14MaskedTeacherFitError(
            ...
            aggregate_checks = {
                ...
                "summary_rows": summary.get("steps") == case_rows
                and summary.get("sample_count") == case_rows
                and summary.get("persisted_label_count") == case_rows,
            ...
            transition_mask = np.zeros(case_rows, dtype=np.bool_)
            for event_step in event_steps:
                start = max(0, event_step - 1 - TRANSITION_RADIUS_STEPS)
                stop = min(case_rows, event_step + TRANSITION_RADIUS_STEPS)
...
def _load_p1_candidate_exposed(
    *, p0_corpus: LockedCorpus | None = None
    ...
    if p0_corpus.stage != "p0" or p0_corpus.actions.shape != (
        ...
        raise V12R14MaskedTeacherFitError("P1 target source is not canonical P0")
    ...
    artifacts = receipt.get("artifacts")
    per_trajectory_row_counts = receipt.get("per_trajectory_row_counts")
    counts_valid = (
        isinstance(per_trajectory_row_counts, list)
        and len(per_trajectory_row_counts) == trajectory_count
        and all(
            type(count) is int and 1 <= count <= ROWS_PER_CASE
            for count in per_trajectory_row_counts
        )
    )
    total_rows = int(sum(per_trajectory_row_counts)) if counts_valid else 0
    full_count = (
        sum(1 for count in per_trajectory_row_counts if count == ROWS_PER_CASE)
        if counts_valid
        else 0
    )
    receipt_checks = {
        "pass": receipt.get("passed") is True,
        "status": receipt.get("status")
        == "PASS_H0_V12R14_P1_CANDIDATE_EXPOSED_RECEIPT",
        "protocol": receipt.get("protocol_id") == PROTOCOL_ID,
        "source_candidate": receipt.get("source_candidate_id") == p0["candidate_id"],
        "rows": counts_valid
        and total_rows >= 1
        and receipt.get("row_count") == total_rows
        and receipt.get("maximum_row_count") == P1_ROW_COUNT
        and receipt.get("full_horizon_count") == full_count
        and receipt.get("physically_truncated_count") == trajectory_count - full_count,
    ...
    if not all(receipt_checks.values()):
        ...
        raise V12R14MaskedTeacherFitError(f"P1 receipt drifted: {failed}")
    for name in ("corpus", "corpus_manifest", "pipeline_ledger"):
        record = artifacts[name]
        if not isinstance(record, Mapping):
            raise V12R14MaskedTeacherFitError("P1 artifact record malformed")
        _validate_referenced_artifact(record)
    if artifacts["corpus"].get("path") != _logical_path(P1_CORPUS_PATH):
        raise V12R14MaskedTeacherFitError("P1 corpus path is non-canonical")
    if artifacts["corpus_manifest"].get("path") != _logical_path(
        P1_CORPUS_MANIFEST_PATH
    ):
        raise V12R14MaskedTeacherFitError("P1 corpus manifest path is non-canonical")
    if artifacts["pipeline_ledger"].get("path") != _logical_path(P1_LEDGER_PATH):
        raise V12R14MaskedTeacherFitError("P1 ledger path is non-canonical")
    ...
    if not isinstance(source_p0, Mapping):
        raise V12R14MaskedTeacherFitError("P1 source P0 tree malformed")
    _validate_referenced_tree(source_p0)
    if not _strict_equal(source_p0, p0["candidate_module"]):
        raise V12R14MaskedTeacherFitError("P1 source P0 candidate identity drifted")
    case_receipts = artifacts["case_receipts"]
    if not isinstance(case_receipts, list) or len(case_receipts) != trajectory_count:
        raise V12R14MaskedTeacherFitError("P1 case receipt index drifted")
    for record in case_receipts:
        if not isinstance(record, Mapping):
            raise V12R14MaskedTeacherFitError("P1 case receipt record malformed")
        _validate_referenced_artifact(record)
    ledger_checks = {
        "pass": ledger.get("passed") is True,
        "status": ledger.get("status")
        == "PASS_H0_V12R14_P1_CANDIDATE_EXPOSED_PIPELINE",
        ...
        "rows": counts_valid
        and ledger.get("row_count") == total_rows
        and ledger.get("maximum_row_count") == P1_ROW_COUNT
        and ledger.get("per_trajectory_row_counts") == per_trajectory_row_counts
        and ledger.get("full_horizon_count") == full_count
        and ledger.get("physically_truncated_count") == trajectory_count - full_count
    ...
    if not all(ledger_checks.values()):
        ...
        raise V12R14MaskedTeacherFitError(f"P1 ledger drifted: {failed}")
    try:
        with np.load(_resolve(P1_CORPUS_PATH), allow_pickle=False) as archive:
            if set(archive.files) != P1_CORPUS_KEYS:
                raise V12R14MaskedTeacherFitError("P1 NPZ key set drifted")
            arrays = {
                name: np.ascontiguousarray(archive[name]) for name in archive.files
            }
    except V12R14MaskedTeacherFitError:
        raise
    except Exception as exc:
        raise V12R14MaskedTeacherFitError("cannot read strict P1 corpus") from exc
    expected_shapes = {
        "observations": (total_rows, INPUT_DIM),
        "targets": (total_rows, ACTION_DIM),
        "case_ids": (total_rows,),
        "trajectory_ids": (total_rows,),
        "requested_alpha": (total_rows,),
        "effective_alpha": (total_rows,),
        "step_indices": (total_rows,),
        "recovery_mask": (total_rows,),
        "support_distance": (total_rows,),
        "transition_mask": (total_rows,),
    }
    expected_dtypes = {
    ...
    if any(arrays[name].shape != shape for name, shape in expected_shapes.items()):
        raise V12R14MaskedTeacherFitError("P1 corpus shape drifted")
    if any(arrays[name].dtype != dtype for name, dtype in expected_dtypes.items()):
        raise V12R14MaskedTeacherFitError("P1 corpus dtype drifted")
    if (
        arrays["case_ids"].dtype.kind != "U"
        or arrays["trajectory_ids"].dtype.kind != "U"
    ):
        raise V12R14MaskedTeacherFitError("P1 identity arrays must be Unicode")
    if not all(
        ...
        raise V12R14MaskedTeacherFitError("P1 corpus contains non-finite values")
    ordered_trajectories = tuple(dict.fromkeys(arrays["trajectory_ids"].astype(str)))
    plan_pairs = tuple((alpha, case_id) for alpha in P1_ALPHAS for case_id in CASE_IDS)
    case_offset = {
        case_id: index * ROWS_PER_CASE for index, case_id in enumerate(CASE_IDS)
    }
    expected_cases = np.concatenate(
        [
            np.repeat(np.asarray([case_id], dtype="U40"), count)
            for (_alpha, case_id), count in zip(
                plan_pairs, per_trajectory_row_counts, strict=True
            )
        ]
    )
    expected_steps = np.concatenate(
        [np.arange(1, count + 1, dtype=np.int64) for count in per_trajectory_row_counts]
    )
    expected_alphas = np.concatenate(
        [
            np.full(count, np.float32(alpha), dtype=np.float32)
            for (alpha, _case_id), count in zip(
                plan_pairs, per_trajectory_row_counts, strict=True
            )
        ]
    )
    expected_trajectory_rows = np.concatenate(
        [
            np.repeat(np.asarray([tid], dtype=arrays["trajectory_ids"].dtype), count)
            for tid, count in zip(
                expected_trajectory_ids, per_trajectory_row_counts, strict=True
            )
        ]
    )
    ...
    expected_targets = np.ascontiguousarray(
        np.concatenate(
            [
                p0_corpus.actions[case_offset[case_id] : case_offset[case_id] + count]
                for (_alpha, case_id), count in zip(
                    plan_pairs, per_trajectory_row_counts, strict=True
                )
            ]
    ...
    if not all(ordering_checks.values()):
        ...
        raise V12R14MaskedTeacherFitError(f"P1 corpus ordering drifted: {failed}")
    ...
    manifest_checks = {
        ...
        == "PASS_H0_V12R14_P1_CANDIDATE_EXPOSED_CORPUS_MANIFEST",
        ...
        "rows": counts_valid
        and manifest.get("row_count") == total_rows
        and manifest.get("maximum_row_count") == P1_ROW_COUNT
        and manifest.get("per_trajectory_row_counts") == per_trajectory_row_counts
        and manifest.get("full_horizon_count") == full_count
        and manifest.get("physically_truncated_count") == trajectory_count - full_count,
        ...
        and manifest.get("candidate_exposed_rows_for_p2") == total_rows
        and manifest.get("expected_cumulative_p2_rows") == P0_ROW_COUNT + total_rows,
    ...
    if not all(manifest_checks.values()):
        ...
        raise V12R14MaskedTeacherFitError(f"P1 corpus manifest drifted: {failed}")
    if receipt.get("array_hashes") != array_hashes:
        raise V12R14MaskedTeacherFitError("P1 receipt array hashes drifted")
    ...
    if not all(ledger_closure.values()):
        ...
        raise V12R14MaskedTeacherFitError(f"P1 ledger artifacts drifted: {failed}")
    return LockedCorpus(
        stage="p1",
        ...
        audit={
            "rows": total_rows,
            "maximum_rows": P1_ROW_COUNT,
            "per_trajectory_row_counts": list(per_trajectory_row_counts),
...
def load_fit_corpus(stage: str) -> LockedCorpus:
    ...
    if stage != "p2":
        raise V12R14MaskedTeacherFitError(f"unsupported fit stage: {stage!r}")
    ...
    p2_rows = P0_ROW_COUNT + int(p1.audit["rows"])
    if (
        observations.shape != (p2_rows, INPUT_DIM)
        or actions.shape != (p2_rows, ACTION_DIM)
        or not p2_rows <= P2_ROW_COUNT
        or len(set(trajectory_ids.astype(str))) != 24
        or int(np.count_nonzero(step_indices == 1)) != 24
    ):
        raise V12R14MaskedTeacherFitError("P2 canonical union closure failed")
    hashes = {
    ...
    return LockedCorpus(
        ...
        audit={
            "rows": p2_rows,
            "maximum_rows": P2_ROW_COUNT,
            "p0_rows": P0_ROW_COUNT,
            "p1_rows": int(p1.audit["rows"]),
            "p1_maximum_rows": P1_ROW_COUNT,
            "trajectory_count": 24,
            "reset_rows": 24,
...
def build_masked_normalization(observations: Any) -> MaskedNormalization:
    ...
    if (
        ...
        or not (len(raw) == P0_ROW_COUNT or P0_ROW_COUNT < len(raw) <= P2_ROW_COUNT)
    ):
        raise V12R14MaskedTeacherFitError("normalization requires exact P0 or P2 rows")
    if not np.all(np.isfinite(raw)):
        raise V12R14MaskedTeacherFitError("normalization rows are non-finite")
    ...
    if not _columns_vector_positive_zero(mean, MASKED_COLUMNS):
        raise V12R14MaskedTeacherFitError("masked normalization means are not +0")
...
def normalized_masked_observations(
    ...
    if raw.ndim != 2 or raw.shape[1] != INPUT_DIM or not np.all(np.isfinite(raw)):
        raise V12R14MaskedTeacherFitError("observations must be finite Nx35")
    ...
    if not _columns_positive_zero(result, MASKED_COLUMNS):
        raise V12R14MaskedTeacherFitError("imitation input mask is not positive-zero")
    return result
...
def _load_source_module_and_state() -> tuple[Any, dict[str, np.ndarray]]:
    ...
    if type(module) is not AsymmetricActorCriticTorchRLModule:
        raise V12R14MaskedTeacherFitError("source H0 RLModule class drifted")
    ...
    if not all(source_checks.values()):
        ...
        raise V12R14MaskedTeacherFitError(f"source H0 state drifted: {failed}")
    return module, state
...
def _new_normalized_model(
    ...
    if set(source) != STATE_KEYS:
        raise V12R14MaskedTeacherFitError("source H0 is not actor-only")
    model = nn.Sequential(
...
def _canonicalize_model_mask(model: Any) -> None:
    ...
    if not _columns_positive_zero(
        ...
        raise V12R14MaskedTeacherFitError("trainable first-layer mask drifted")
...
def _state_logits(state: Mapping[str, Any], observations: Any) -> np.ndarray:
    ...
    if result.shape != (len(x), LOGITS_DIM) or not np.all(np.isfinite(result)):
        raise V12R14MaskedTeacherFitError("candidate logits are malformed")
    return result
...
def adamw_rate(epoch: int) -> float:
    if type(epoch) is not int or epoch < 1 or epoch > ADAMW_EPOCHS:
        raise V12R14MaskedTeacherFitError(f"AdamW epoch outside fixed range: {epoch!r}")
...
def _milestone(
    ...
    *,
    ...
    if not all(math.isfinite(value) for value in values.values()):
        raise V12R14MaskedTeacherFitError("non-finite optimizer milestone")
    history.append(
        {
            "optimizer": optimizer,
            "index": int(index),
...
def prediction_metrics(
    ...
    if pred.shape != target.shape or pred.ndim != 2 or pred.shape[1] != ACTION_DIM:
        raise V12R14MaskedTeacherFitError("metric arrays have wrong shape")
    if (
        cases.shape != (rows,)
        or trajectories.shape != (rows,)
        or steps.shape != (rows,)
    ):
        raise V12R14MaskedTeacherFitError("metric identity arrays have wrong shape")
    error = pred.astype(np.float64) - target.astype(np.float64)

    if transitions.shape != (rows,) or not np.any(transitions):
        raise V12R14MaskedTeacherFitError("transition mask is malformed")
...
def fit_gate(
    *,
    ...
    checks = {
        "global_rmse_le_0p005": float(global_metrics.get("rmse", math.inf))
        <= GLOBAL_RMSE_LIMIT,
        "global_max_abs_le_0p06": float(global_metrics.get("max_abs_error", math.inf))
        <= GLOBAL_MAX_ABS_LIMIT,
        "every_case_and_action_rmse_le_0p0055": float(
            metrics.get("worst_per_case_or_action_rmse", math.inf)
        )
        <= PER_CASE_ACTION_RMSE_LIMIT,
        "every_trajectory_and_action_rmse_le_0p0055": float(
            metrics.get("worst_per_trajectory_or_action_rmse", math.inf)
        )
        <= PER_TRAJECTORY_ACTION_RMSE_LIMIT,
        "reset_max_abs_le_0p003": float(metrics.get("reset_max_abs_error", math.inf))
        <= RESET_MAX_ABS_LIMIT,
        "transition_window_max_abs_le_0p03": float(
            metrics.get("transition_window_max_abs_error", math.inf)
        )
        <= TRANSITION_MAX_ABS_LIMIT,
        "temporal_first_difference_max_abs_le_0p08": float(
...
def fit_candidate_in_memory(
    *,
    ...
    """Execute the sole fixed V12R14 fit without publishing artifacts."""
    ...
    try:
        ...
        mass_weights = None
        if corpus.stage == "p2":
            # Variable-length P1 collections: normalize each trajectory to the
            # full-horizon mass so a physically truncated boundary prefix is
            # not underrepresented in the recovery fit. P0 is untouched (all
            # tape trajectories are exactly ROWS_PER_CASE rows).
            trajectory_keys = corpus.trajectory_ids.astype(str)
            _, inverse, counts = np.unique(
                trajectory_keys, return_inverse=True, return_counts=True
            )
            factors = (float(ROWS_PER_CASE) / counts.astype(np.float64))[inverse]
            mass_weights = torch.as_tensor(
                np.ascontiguousarray(factors, dtype=np.float32),
                dtype=torch.float32,
            )
        ...
        for epoch in range(1, ADAMW_EPOCHS + 1):
            ...
            objective, mse, reset_mse = _fit_objective(
                model(x), y, reset_mask, mass_weights
            )
        ...
        _canonicalize_model_mask(model)
        if terminal is None:
            raise V12R14MaskedTeacherFitError("LBFGS never evaluated the objective")
        ...
        raise V12R14MaskedTeacherFitError("fixed V12R14 fit failed") from exc
    finally:
...
def _runtime_logits(module: Any, observations: np.ndarray) -> np.ndarray:
    ...
    if result.shape != (len(observations), LOGITS_DIM):
        raise V12R14MaskedTeacherFitError("runtime logits shape drifted")
    return result
...
def _save_rlmodule_no_clobber(module: Any, destination: Path) -> Any:
    ...
    if not output.parent.is_dir() or _is_link_or_reparse(output.parent):
        raise V12R14MaskedTeacherFitError("candidate parent is unsafe")
    if os.path.lexists(output):
        raise V12R14MaskedTeacherFitError("candidate destination exists/no-clobber")
    try:
        os.mkdir(output, 0o700)
    except OSError as exc:
        raise V12R14MaskedTeacherFitError("cannot exclusively claim candidate") from exc
...
def _actor_manifest(
    *, state: Mapping[str, Any], module_state_path: Path, stage: str
    ...
    return {
        "schema_version": 1,
        "status": "H0_V12R14_ACTOR_FEATURE_BUILD_EVIDENCE",
...
def _build_manifest(
    *,
    ...
    return {
        ...
        "status": "H0_V12R14_CANDIDATE_BUILD_EVIDENCE",
        "terminal_gate_pending_at_write": True,
        "protocol_id": PROTOCOL_ID,
        "fit_contract_id": FIT_CONTRACT_ID,
        "fit_stage": corpus.stage,
...
def save_candidate_and_manifests(
    *,
    ...
    if not _state_byte_exact(candidate_state, reloaded_state):
        raise V12R14MaskedTeacherFitError("candidate save/reload state drifted")
...
def runtime_and_transplant_audit(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="h0_v12r14_roundtrip_") as temporary:
...
def _stage_spec(stage: str) -> dict[str, Any]:
    ...
    return {
        ...
        "row_count_semantics": "exact" if stage == "p0" else "maximum",
    ...
    return {
        ...
        "stages": {
            stage: {
                ...
                "row_count_semantics": spec["row_count_semantics"],
...
def _governance_source_records() -> dict[str, dict[str, Any]]:
    return {
        ...
        "runner": _record(REVISION_ROOT / "run_h0_v12r14_masked_teacher_fit.py"),
        "tests": _record(REVISION_ROOT / "test_h0_v12r14_masked_teacher_fitter.py"),
    }
...
def protocol_freeze_payload() -> dict[str, Any]:
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "status": "FROZEN_H0_V12R14_MASKED_TEACHER_FIT_PROTOCOL",
        "passed": True,
        "protocol_id": PROTOCOL_ID,
        "fit_contract_id": FIT_CONTRACT_ID,
...
def execution_lock_payload() -> dict[str, Any]:
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "status": "LOCKED_H0_V12R14_MASKED_TEACHER_FIT_EXECUTION",
        "passed": True,
        "protocol_id": PROTOCOL_ID,
        "fit_contract_id": FIT_CONTRACT_ID,
        "authorized_stages": ["p0", "p2"],
...
def verify_execution_governance(stage: str) -> dict[str, Any]:
    ...
    if stage not in observed.get("authorized_stages", []):
        raise V12R14MaskedTeacherFitError(f"stage {stage} is not execution-locked")
    return {
...
def describe_protocol(stage: str = "p0") -> dict[str, Any]:
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "status": "H0_V12R14_MASKED_TEACHER_SOURCE_READY_UNEXECUTED",
        "passed": True,
        "fit_stage": stage,
...
def preflight(stage: str = "p0") -> dict[str, Any]:
    ...
    checks = {
        ...
        "locked_stage_rows_exact": (
            corpus.audit["rows"] == spec["row_count"]
            if spec["row_count_semantics"] == "exact"
            else P0_ROW_COUNT < corpus.audit["rows"] <= spec["row_count"]
        ),
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "status": (
            f"PASS_H0_V12R14_{stage.upper()}_MASKED_TEACHER_PREFLIGHT"
            if all(checks.values())
            else f"FAIL_H0_V12R14_{stage.upper()}_MASKED_TEACHER_PREFLIGHT"
        ),
        "passed": all(checks.values()),
        "fit_stage": stage,
...
def _write_failure_once(destination: Path, exc: BaseException, *, stage: str) -> None:
    ...
    payload = {
        "schema_version": SCHEMA_VERSION,
        "status": "FAIL_H0_V12R14_MASKED_SAFE_TEACHER_FIT_TERMINAL",
        "passed": False,
        "protocol_id": PROTOCOL_ID,
        "fit_contract_id": FIT_CONTRACT_ID,
        "fit_stage": stage,
...
def run_production_fit(
    *,
    ...
    if enforce_canonical and output != _resolve(spec["output"]):
        raise V12R14MaskedTeacherFitError("non-canonical production destination")
    if os.path.lexists(output):
        raise V12R14MaskedTeacherFitError(
            f"production destination occupied/no-clobber: {output}"
        )
    ...
    corpus = load_fit_corpus(stage)
    row_count_valid = (
        len(corpus.observations) == spec["row_count"]
        if spec["row_count_semantics"] == "exact"
        else P0_ROW_COUNT < len(corpus.observations) <= spec["row_count"]
    )
    if corpus.stage != stage or not row_count_valid:
        raise V12R14MaskedTeacherFitError("fit-stage corpus binding drifted")
    normalization = build_masked_normalization(corpus.observations)
    ...
    try:
        ...
        # Forensic invariant (V12R3 fit_p2 and V12R12 P0 lesson): the observed
        # metrics must reach disk BEFORE any gate can raise, so a terminal FAIL
        # always leaves the numbers that the successor design needs.
        _write_json_exclusive(
            output / "fit_metrics.json",
            {
                "schema_version": SCHEMA_VERSION,
                "status": "H0_V12R14_FIT_METRICS_PERSISTED_PRE_GATE",
                "terminal_gate_pending_at_write": True,
                "fit_stage": stage,
                "metrics": result.metrics,
                "optimizer_audit": result.optimizer_audit,
        ...
        final_gate = {
            ...
            "status": (
                (
                    "PASS_H0_V12R14_MASKED_SAFE_TEACHER_FIT"
                    if stage == "p0"
                    else "PASS_H0_V12R14_P2_MASKED_CANDIDATE_EXPOSED_FIT"
                )
                if all(final_checks.values())
                else (
                    "FAIL_H0_V12R14_MASKED_SAFE_TEACHER_FIT"
                    if stage == "p0"
                    else "FAIL_H0_V12R14_P2_MASKED_CANDIDATE_EXPOSED_FIT"
                )
            ),
        ...
        summary = {
            "schema_version": SCHEMA_VERSION,
            "status": (
                "COMPLETE_H0_V12R14_MASKED_SAFE_TEACHER_FIT"
                if stage == "p0"
                else "COMPLETE_H0_V12R14_P2_MASKED_CANDIDATE_EXPOSED_FIT"
            ),
            "passed": True,
            "protocol_id": PROTOCOL_ID,
            "fit_contract_id": FIT_CONTRACT_ID,
        ...
        receipt = {
            ...
            "status": (
                "PASS_H0_V12R14_MASKED_SAFE_TEACHER_FIT_RECEIPT"
                if stage == "p0"
                else "PASS_H0_V12R14_P2_MASKED_CANDIDATE_EXPOSED_FIT_RECEIPT"
            ),
            "passed": True,
            "protocol_id": PROTOCOL_ID,
            "fit_contract_id": FIT_CONTRACT_ID,
        ...
        if isinstance(exc, V12R14MaskedTeacherFitError):
            raise
        if not isinstance(exc, Exception):
            raise
        raise V12R14MaskedTeacherFitError("V12R14 production fit failed") from exc
