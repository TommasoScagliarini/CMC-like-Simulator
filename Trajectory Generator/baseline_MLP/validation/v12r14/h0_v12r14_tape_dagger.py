"""Fail-closed V12R14 P1 tape-reference candidate-exposed contract.
...
P1_PROTOCOL_FREEZE = REVISION_ROOT / "h0_v12r14_tape_dagger_protocol_freeze.json"
P1_EXECUTION_LOCK = REVISION_ROOT / "h0_v12r14_tape_dagger_execution_lock.json"
...
COLLECTION_STARTED_STATUS = "H0_V12R14_P1_CANDIDATE_EXPOSED_STARTED"
COLLECTION_COMPLETE_STATUS = "H0_V12R14_P1_CANDIDATE_EXPOSED_PERSISTED_UNGATED"
COLLECTION_PASS_STATUS = "PASS_H0_V12R14_P1_CANDIDATE_EXPOSED_COLLECTION"
COLLECTION_FAIL_STATUS = "FAIL_H0_V12R14_P1_CANDIDATE_EXPOSED_COLLECTION"
P1_RECEIPT_STATUS = "PASS_H0_V12R14_P1_CANDIDATE_EXPOSED_RECEIPT"
P1_LEDGER_STATUS = "PASS_H0_V12R14_P1_CANDIDATE_EXPOSED_PIPELINE"
P1_FAILURE_STATUS = "FAIL_H0_V12R14_P1_CANDIDATE_EXPOSED"
PROTOCOL_FREEZE_STATUS = "H0_V12R14_P1_CANDIDATE_EXPOSED_PROTOCOL_FROZEN"
EXECUTION_LOCK_STATUS = "H0_V12R14_P1_CANDIDATE_EXPOSED_EXECUTION_LOCKED"
...
class LockedTapeCorpus:
    ...
    def reference(self, case_id: str, step: int) -> TapeReferenceStep:
        if case_id not in self.rows_by_case:
            raise V12R14TapeDaggerError(f"unknown tape case: {case_id!r}")
        if type(step) is not int or not 1 <= step <= EXPECTED_STEPS:
            raise V12R14TapeDaggerError(f"invalid one-based tape step: {step!r}")
...
class SupportEnvelope:
    ...
    def query(self, observation: Any) -> SupportQuery:
        ...
        if not math.isfinite(distance):
            raise V12R14TapeDaggerError("support distance is non-finite")
        return SupportQuery(
...
def _resolve(relative: str | PurePosixPath | Path) -> Path:
    ...
    try:
        ...
        raise V12R14TapeDaggerError(f"path escapes repository: {relative}") from exc
    return absolute
...
def _mapping_json(path: str | PurePosixPath | Path) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise V12R14TapeDaggerError(f"expected JSON object: {path}")
    return dict(value)
...
def _tree_record(path: str | PurePosixPath | Path) -> dict[str, Any]:
    ...
    if not root.is_dir() or root.is_symlink():
        raise V12R14TapeDaggerError(f"unsafe or missing tree: {root}")
    entries = list(root.rglob("*"))
    if any(item.is_symlink() for item in entries):
        raise V12R14TapeDaggerError(f"empty or linked tree: {root}")
    files = sorted(
...
def _portable_rms_distance(left: Any, right: Any) -> float:
    ...
    if a.shape != (len(INVARIANT_COLUMNS),) or b.shape != a.shape:
        raise V12R14TapeDaggerError("portable RMS vectors are malformed")
    return math.sqrt(
...
def support_within_p99(distance: Any) -> bool:
    if (
        ...
        raise V12R14TapeDaggerError("support distance must be finite nonnegative")
...
def _canonical_nearest_rms(
    ...
    *,
    ...
    if (
        ...
        raise V12R14TapeDaggerError("nearest-neighbour inputs are malformed")
    ...
    if excluded_index is not None:
        if not 0 <= excluded_index < reference_array.shape[0]:
            raise V12R14TapeDaggerError("excluded nearest index is invalid")
        eligible[excluded_index] = False
    finite = np.isfinite(approximate) & eligible
    if not np.any(finite):
        raise V12R14TapeDaggerError("nearest-neighbour set is empty")
    ...
    if candidate_indices.size == 0:  # pragma: no cover - guarded by finite min.
        raise V12R14TapeDaggerError("nearest-neighbour candidate set is empty")
    candidates = [
        (
...
def _float32_vector(value: Any, *, width: int, label: str) -> np.ndarray:
    try:
        ...
        raise V12R14TapeDaggerError(f"{label} is not float32") from exc
    if result.shape != (width,) or not np.all(np.isfinite(result)):
        raise V12R14TapeDaggerError(f"{label} must be finite shape ({width},)")
    return result
...
def canonical_case(case_id: str) -> dict[str, Any]:
    ...
    if len(matches) != 1:
        raise V12R14TapeDaggerError(f"unknown canonical case: {case_id!r}")
...
def alpha_tag(alpha: float) -> str:
    ...
    if alpha not in mapping:
        raise V12R14TapeDaggerError(f"unregistered alpha: {alpha!r}")
...
def verify_runtime_source_closure() -> dict[str, Any]:
    ...
    if not isinstance(closure, Mapping) or len(closure) != 94:
        raise V12R14TapeDaggerError("R10 transitive runtime closure drifted")
    required = {
    ...
    if not required.issubset(closure):
        raise V12R14TapeDaggerError("runtime closure lacks required P1 primitives")
    verified: dict[str, dict[str, Any]] = {}
    for relative, expected in sorted(closure.items()):
        if not isinstance(relative, str) or not isinstance(expected, Mapping):
            raise V12R14TapeDaggerError("runtime closure record is malformed")
...
def source_snapshot(*, candidate_module: Path = P0_MODULE) -> dict[str, Any]:
    ...
    local_paths = {
        ...
        "runner": REVISION_ROOT / "run_h0_v12r14_tape_dagger.py",
    }
...
def verify_tape_source_closure() -> dict[str, Any]:
    ...
    if not all(ledger_checks.values()):
        ...
        raise V12R14TapeDaggerError(f"V8R1P1 ledger closure failed: {failed}")
    completed = ledger.get("completed_receipts")
    if not isinstance(completed, list) or len(completed) != EXPECTED_CASES:
        raise V12R14TapeDaggerError("V8R1P1 receipt index drifted")
    ...
    for case_id, binding in zip(CASE_IDS, completed, strict=True):
        if not isinstance(binding, Mapping) or binding.get("case_id") != case_id:
            raise V12R14TapeDaggerError("V8R1P1 receipt order drifted")
        ...
        if (
            ...
            raise V12R14TapeDaggerError(f"receipt binding drifted: {case_id}")
        ...
        if (
            ...
            raise V12R14TapeDaggerError(f"receipt content drifted: {case_id}")
        ...
        if gate.get("passed") is not True:
            raise V12R14TapeDaggerError(f"source tape gate is not PASS: {case_id}")
...
def load_locked_tape_corpus() -> LockedTapeCorpus:
    ...
    for case_id in CASE_IDS:
        ...
        if not isinstance(value, list) or len(value) != EXPECTED_STEPS:
            raise V12R14TapeDaggerError(f"source trace is incomplete: {case_id}")
        case_rows: list[TapeReferenceStep] = []
        for index, row in enumerate(value, start=1):
            if not isinstance(row, Mapping):
                raise V12R14TapeDaggerError(f"source row malformed: {case_id}/{index}")
    ...
    if any(arrays[name].shape != shape for name, shape in expected_shapes.items()):
        raise V12R14TapeDaggerError("source tape array shapes drifted")
    hashes = {name: array_sha256(value) for name, value in arrays.items()}
    if hashes != EXPECTED_TAPE_ARRAY_HASHES:
        raise V12R14TapeDaggerError(f"source tape arrays drifted: {hashes}")
...
def build_support_envelope(corpus: LockedTapeCorpus) -> SupportEnvelope:
    ...
    if not isinstance(corpus, LockedTapeCorpus):
        raise V12R14TapeDaggerError("support calibration requires locked tape")
    ...
    if (
        ...
        raise V12R14TapeDaggerError("support normalization is malformed")
    ...
    if hashes["raw"] != EXPECTED_SUPPORT_RAW_HASH:
        raise V12R14TapeDaggerError(f"support source rows drifted: {hashes['raw']}")
    if float(SUPPORT_P99_THRESHOLD).hex() != SUPPORT_P99_HEX:
        raise V12R14TapeDaggerError("serialized support p99 hex contract drifted")
    if float(SUPPORT_LOO_MAX).hex() != SUPPORT_LOO_MAX_HEX:
        raise V12R14TapeDaggerError("serialized support max hex contract drifted")
    if not math.isclose(
...
def select_tape_dagger_action(
    *,
    ...
    if requested_alpha not in ROUND_ALPHAS:
        raise V12R14TapeDaggerError("requested alpha is not preregistered")
    ...
    if not _bytes_equal(std, reference.teacher_std):
        raise V12R14TapeDaggerError("P0 logstd drifted from frozen tape sigma")
    ...
    try:
        ...
        raise V12R14TapeDaggerError("causal V10S latch input is malformed") from exc
    ...
    try:
        ...
        raise V12R14TapeDaggerError("V10S blend/noise semantics failed") from exc
    if observed_alpha != effective_alpha:
        raise V12R14TapeDaggerError("effective alpha drifted after latch selection")
    reasons: list[str] = []
    ...
    if recovery and effective_alpha != 0.0:
        raise V12R14TapeDaggerError("recovery row retained nonzero student weight")
