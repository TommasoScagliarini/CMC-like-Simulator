...
import h0_primary_split_v12r3_recovery_weighted_fitter as fit
...
def test_recovery_ramp_endpoints_and_midpoint_are_frozen() -> None:
    ...
    for invalid in (-1.0e-9, math.nan, math.inf, -math.inf):
        with pytest.raises(fit.V12R3RecoveryWeightedFitError):
...
def test_corpus_component_order_is_seed_then_label_then_matching_round() -> None:
    ...
    assert fit.expected_corpus_component_order("p1") == (
        ("v11_seed",),
        ("pure_observer", "p0"),
        ("v12r3_shielded", 1, minus),
        ("v12r3_shielded", 1, stochastic),
    )
    assert fit.expected_corpus_component_order("p3") == (
        ...
        ("v12r3_shielded", 1, stochastic),
        ("pure_observer", "p1"),
        ("v12r3_shielded", 2, minus),
        ("v12r3_shielded", 2, stochastic),
        ("pure_observer", "p2"),
        ("v12r3_shielded", 3, minus),
        ("v12r3_shielded", 3, stochastic),
    )
