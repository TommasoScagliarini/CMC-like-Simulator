...
def test_r3_is_additive_and_preserves_v12_topology_and_science() -> None:
    assert contract.SCHEMA_VERSION == 123
    assert "V12R3" in contract.PROTOCOL_ID
    assert "v12r3" in contract.RUN_ROOT.as_posix()
    ...
    assert contract.EXECUTION_AUTHORITY_SCOPE == ("V12R3_CORRECTIVE_ONE_SHOT_EXECUTION")


def test_r2_terminal_lineage_is_exact_and_never_a_r3_mutation_source() -> None:
    assert len(contract.V12R2_LINEAGE_ARTIFACTS) == 7
    assert contract.V12R2_TERMINAL_LINEAGE == {
        "artifacts": contract.V12R2_LINEAGE_ARTIFACTS,
        "run_tree": contract.V12R2_TERMINAL_RUN_TREE,
        "semantics": contract.V12R2_TERMINAL_SEMANTICS,
    }
    assert contract.V12R2_TERMINAL_RUN_TREE == {
        "path": (
            "Trajectory Generator/baseline_MLP/validation/v12r2/h0_v12r2_run_20260809"
        ),
        "tree_sha256": (
            "18a89b0fca12f3644164bee211a413a0dafcf8880a51513f53dc0d04bbe020a4"
        ),
        "file_count": 251,
        "total_size_bytes": 15_279_831,
    }
    r2_root = contract.V12R2_TERMINAL_RUN_TREE["path"]
    assert all(
        not path.as_posix().startswith(r2_root)
        for path in contract.declared_mutation_paths().values()
    )
...
def test_inherited_probe_gate_uses_r2_schema_and_paths() -> None:
    ...
    assert case["destination"].startswith(
        "Trajectory Generator/baseline_MLP/validation/v12r3/h0_v12r3_run_"
    )
    assert (
...
def test_design_audit_gate_binds_replay_sources_and_tolerance() -> None:
    ...
    terminal_drift = copy.deepcopy(payload)
    terminal_drift["v12r2_terminal_failure_lineage"]["semantics"][
        "environment_step_calls"
    ] += 1
    assert contract.design_audit_gate(terminal_drift)["passed"] is False
...
def test_execution_lock_gate_requires_exact_five_sources_and_one_shot() -> None:
    ...
    terminal_drift = copy.deepcopy(payload)
    terminal_drift["v12r2_terminal_failure_lineage"]["run_tree"]["tree_sha256"] = (
        "00" * 32
    )
    assert contract.execution_lock_gate(terminal_drift)["passed"] is False
...
def test_real_fit_gate_is_total_and_fail_closed(stage: str) -> None:
    ...
    assert gate["next_stage"] == "STOP_V12R3_FIT_FAILURE"
...
def test_real_contract_fit_gate_p3_synthetic_passes(
    ...
    assert "completed_v12r3_collection_rounds" not in summary["fit_counts"]
