"""Execute the frozen V12R3 autonomy-recovery pipeline exactly once.

This module is the execution overlay for the additive V12R3 protocol.  The
...
V12R3_ROOT = Path(__file__).resolve().parent
for _root in (
    REPO_ROOT,
    VALIDATION_ROOT,
    TRAJECTORY_ROOT,
...
import h0_primary_split_v12r3_pure_probe_observer_labeler as label_engine  # noqa: E402
import h0_primary_split_v12r3_recovery_weighted_fitter as fit_engine  # noqa: E402
import run_h0_primary_split_v12r3_design_audit as audit_engine  # noqa: E402


class V12R3ExecutionError(RuntimeError):
    """Raised when V12R3 cannot continue without violating the freeze."""
...
def _activity_increment(name: str, amount: int = 1) -> None:
    ...
    if type(amount) is not int or amount < 0:
        raise V12R3ExecutionError("stage activity increment is malformed")
...
def resolve_relative(path: str | os.PathLike[str] | PurePosixPath) -> Path:
    ...
    if not raw or pure.is_absolute() or ".." in pure.parts or pure.as_posix() != raw:
        raise V12R3ExecutionError(f"non-canonical repository path: {raw!r}")
...
def _mapping(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise V12R3ExecutionError(f"expected strict JSON object: {path}")
    return dict(value)
...
def _sequence(path: str | Path) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise V12R3ExecutionError(f"expected strict JSON array: {path}")
    return value
...
def _tree_record(path: str | Path) -> dict[str, Any]:
    ...
    if not root.is_dir():
        raise V12R3ExecutionError(f"artifact tree is missing: {root}")
    files = sorted(item for item in root.rglob("*") if item.is_file())
    if not files or any(item.is_symlink() for item in files):
        raise V12R3ExecutionError(f"artifact tree is empty or symlinked: {root}")
...
def _v12r1_lineage_records() -> dict[str, dict[str, Any]]:
    ...
    if records != contract.V12R1_LINEAGE_ARTIFACTS:
        raise V12R3ExecutionError("terminal V12R1 lineage drifted")
    return records


def _v12r2_terminal_lineage() -> dict[str, Any]:
    """Rebind the complete terminal R2 attempt without reusing its outputs."""

    artifacts = {
        name: _record(resolve_relative(artifact["path"]))
        for name, artifact in contract.V12R2_LINEAGE_ARTIFACTS.items()
    }
    if artifacts != contract.V12R2_LINEAGE_ARTIFACTS:
        raise V12R3ExecutionError("terminal V12R2 artifact lineage drifted")

    expected_tree = contract.V12R2_TERMINAL_RUN_TREE
    run_root = resolve_relative(expected_tree["path"])
    if any(item.is_symlink() for item in run_root.rglob("*")):
        raise V12R3ExecutionError("terminal V12R2 run tree contains a symlink")
    full_tree = _tree_record(run_root)
    run_tree = {
        "path": full_tree["path"],
        "tree_sha256": full_tree["tree_sha256"],
        "file_count": full_tree["file_count"],
        "total_size_bytes": sum(int(item["size_bytes"]) for item in full_tree["files"]),
    }
    if run_tree != expected_tree:
        raise V12R3ExecutionError("terminal V12R2 run tree drifted")

    ledger_artifact = contract.V12R2_LINEAGE_ARTIFACTS["v12r2_pipeline_ledger"]
    ledger = _mapping(resolve_relative(ledger_artifact["path"]))
    semantics = {
        "status": ledger.get("status"),
        "attempted_stage": ledger.get("attempted_stage"),
        "completed_stages": ledger.get("completed_stages"),
        "error": ledger.get("error"),
        "actor_fit_executions_confirmed": ledger.get("actor_fit_executions_confirmed"),
        "actor_updates_confirmed": ledger.get("actor_updates_confirmed"),
        "environment_reset_calls": ledger.get("environment_reset_calls"),
        "environment_step_calls": ledger.get("environment_step_calls"),
        "offline_teacher_label_calls_confirmed": ledger.get(
            "offline_teacher_label_calls_confirmed"
        ),
    }
    if semantics != contract.V12R2_TERMINAL_SEMANTICS:
        raise V12R3ExecutionError("terminal V12R2 semantics drifted")
    if (
        ledger.get("passed") is not False
        ...
        or ledger.get("pipeline_claim") != artifacts["v12r2_pipeline_claim"]
        or ledger.get("execution_lock") != artifacts["v12r2_execution_lock"]
        or ledger.get("completed_receipts")
        != [
            {
                "stage_id": "fit_p0",
                "receipt": artifacts["v12r2_fit_p0_receipt"],
            }
        ]
        or not isinstance(ledger.get("stage_activity"), list)
        or len(ledger["stage_activity"]) != 2
        or ledger["stage_activity"][1].get("terminal_failure", {}).get("artifact")
        != artifacts["v12r2_probe_p0_failure"]
    ):
        raise V12R3ExecutionError("terminal V12R2 causal chain drifted")
    ...
    if lineage != contract.V12R2_TERMINAL_LINEAGE:
        raise V12R3ExecutionError("terminal V12R2 lineage drifted")
    return lineage
...
def _inherited_runtime_evidence_audit() -> dict[str, Any]:
    ...
    if not isinstance(frozen_sources, Mapping) or not isinstance(
        ...
        raise V12R3ExecutionError("V11 execution lock evidence is malformed")
...
def _runtime_record() -> dict[str, Any]:
    ...
    if (
        ...
        raise V12R3ExecutionError("execution inference stack identity drifted")
    plugin_readiness = _platform_plugin_readiness()
    if plugin_readiness.get("passed") is not True:
        raise V12R3ExecutionError("platform plugin binaries are not training-ready")
    return {
...
def _platform_plugin_readiness() -> dict[str, Any]:
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V12R3_PLATFORM_PLUGINS"
            if all(checks.values())
            else "FAIL_H0_PRIMARY_SPLIT_V12R3_PLATFORM_PLUGINS"
        ),
...
def _physical_summary(audit: Mapping[str, Any], **kwargs: Any) -> dict[str, Any]:
    with _legacy_contract_bound():
        summary = dict(legacy_runner._physical_summary(audit, **kwargs))

    binary_gate = summary.get("binary_phase_event_gate")
    rows = kwargs.get("rows")
    if not isinstance(binary_gate, Mapping) or not isinstance(rows, Sequence):
        raise V12R3ExecutionError("binary detector prefix evidence is malformed")
    sample_count = binary_gate.get("sample_count")
    raw_sensor_sample_count = summary.get("raw_sensor_sample_count")
    control_window_count = summary.get("control_window_count")
    expected_sample_count = len(rows) * 10
    anomaly_fields = (
    ...
    checks = {
        "sample_count_positive": type(sample_count) is int and sample_count > 0,
        "sample_count_matches_raw_sensor": (
            type(raw_sensor_sample_count) is int
            and sample_count == raw_sensor_sample_count
        ),
        "raw_sensor_matches_control_windows": (
            type(control_window_count) is int
            and raw_sensor_sample_count == control_window_count
        ),
        "ten_samples_per_control_step": sample_count == expected_sample_count,
        **{
            f"zero_{field}": type(binary_gate.get(field)) is int
            and binary_gate[field] == 0
    ...
    prefix_integrity = {
        ...
        "raw_sensor_sample_count": raw_sensor_sample_count,
        "control_window_count": control_window_count,
        "expected_sample_count": expected_sample_count,
        "checks": checks,
    }
    summary["binary_event_prefix_integrity"] = prefix_integrity
    summary["binary_event_failure_count"] = int(prefix_integrity["passed"] is not True)
    return summary
...
def _assert_execution_authority() -> None:
    if EXECUTION_AUTHORITY != {
        ...
        raise V12R3ExecutionError("execution authority schema/identity drifted")
...
def _design_audit_gate(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if dict(payload) != verified:
        raise V12R3ExecutionError("design audit receipt/current bindings drifted")
    gate_function = getattr(contract, "design_audit_gate", None)
    if not callable(gate_function):
        raise V12R3ExecutionError("R2 contract lacks design_audit_gate")
    gate = dict(gate_function(payload))
    if gate.get("passed") is not True:
        failed = [
            name for name, value in gate.get("checks", {}).items() if value is not True
        ]
        raise V12R3ExecutionError(f"design audit is not PASS: {failed}")
    return gate
...
def build_execution_lock(*, require_unoccupied: bool = True) -> dict[str, Any]:
    ...
    if not PROTOCOL_FREEZE_PATH.is_file():
        raise V12R3ExecutionError("canonical R2 protocol freeze is missing")
    protocol = _mapping(PROTOCOL_FREEZE_PATH)
    if protocol.get("passed") is not True:
        raise V12R3ExecutionError("R2 protocol freeze is not PASS")
    ...
    r2_lineage = _v12r2_terminal_lineage()
    ...
    checks = {
        ...
        "v12r2_terminal_lineage_exact": (r2_lineage == contract.V12R2_TERMINAL_LINEAGE),
        "source_h0_exact": (
    ...
    payload = {
        ...
        "status": (
            getattr(
                ...
                "PASS_H0_PRIMARY_SPLIT_V12R3_EXECUTION_LOCK",
            )
            if passed
            else getattr(
                contract,
                "EXECUTION_LOCK_FAIL_STATUS",
                "FAIL_H0_PRIMARY_SPLIT_V12R3_EXECUTION_LOCK",
            )
        ),
        "passed": passed,
        "protocol_id": contract.PROTOCOL_ID,
        ...
        "v12r2_terminal_failure_lineage": r2_lineage,
        ...
        "next_stage": "EXECUTE_V12R3_ONCE" if passed else "STOP",
    }
    ...
    if require_unoccupied and (not passed or payload.get("passed") is not True):
        ...
        raise V12R3ExecutionError(f"execution lock preflight failed: {failed}")
    return payload


def prepare_execution_lock() -> dict[str, Any]:
    if os.path.lexists(LOCK_PATH):
        raise V12R3ExecutionError("execution lock exists/no-clobber")
...
def verify_execution_lock(*, require_run_root_absent: bool = False) -> dict[str, Any]:
    ...
    if (
        ...
        raise V12R3ExecutionError("protocol freeze content drifted")
    audit = _mapping(DESIGN_AUDIT_PATH)
    audit_gate = _design_audit_gate(audit)
    coverage = fit_engine.coverage_reference_audit()
    fit_contract = contract.fit_contract_self_check()
    ...
    if not all(value is True for value in checks.values()):
        failed = [name for name, value in checks.items() if value is not True]
        raise V12R3ExecutionError(f"execution lock bindings failed: {failed}")
    expected = {
        "schema_version": contract.SCHEMA_VERSION,
    ...
    if expected["contract_gate"].get("passed") is not True or observed != expected:
        raise V12R3ExecutionError("execution lock exact payload drifted")
    if require_run_root_absent and os.path.lexists(RUN_ROOT):
        raise V12R3ExecutionError("V12R3 run root already claimed")
    return observed


def _token_sha256(token: str) -> str:
    if not isinstance(token, str) or len(token) < 32:
        raise V12R3ExecutionError("execution token is malformed")
...
def _verify_pipeline_claim() -> dict[str, Any]:
    ...
    if not isinstance(token_sha256, str) or len(token_sha256) != 64:
        raise V12R3ExecutionError("pipeline claim token digest is malformed")
    expected = _pipeline_claim_payload(token_sha256)
    if claim != expected:
        raise V12R3ExecutionError("pipeline claim/current lock binding drifted")
    return claim
...
def _verify_worker_claim(stage_id: str) -> dict[str, Any]:
    ...
    if claim != expected:
        raise V12R3ExecutionError(f"worker claim chain drifted: {stage_id}")
    return claim
...
def _pipeline_claim_payload(token_sha256: str) -> dict[str, Any]:
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "CLAIM_H0_PRIMARY_SPLIT_V12R3_PIPELINE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_order": list(contract.STAGE_IDS),
...
def _worker_claim_payload(
    *, stage_id: str, token_sha256: str, previous_receipts: Sequence[Mapping[str, Any]]
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "CLAIM_H0_PRIMARY_SPLIT_V12R3_WORKER",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_id": stage_id,
...
def _write_worker_claim(stage_id: str, token_sha256: str) -> Path:
    ...
    if os.path.lexists(claim_path) or os.path.lexists(_stage_receipt_path(stage_id)):
        raise V12R3ExecutionError(f"stage already consumed: {stage_id}")
...
def _load_rollout_stack() -> tuple[Any, Any, Any, Any, Any, Any, Any]:
    ...
    if (
        ...
        raise V12R3ExecutionError("inference stack identity drifted")
...
def _verify_stage_receipt(stage_id: str) -> dict[str, Any]:
    ...
    if kind == "probe":
        ...
        if gate != expected or gate.get("integrity_passed") is not True:
            raise V12R3ExecutionError(f"probe closure drifted: {stage}")
        ...
        if receipt != expected_receipt or not current_bindings:
            raise V12R3ExecutionError(f"probe receipt/binding drifted: {stage}")
        return receipt
    if kind == "label":
    ...
    if kind == "collection":
        ...
        if gates != {"data_gate": data_gate, "latch_dependence_gate": latch_gate}:
            raise V12R3ExecutionError(f"collection gate closure drifted: {stage_id}")
        if data_gate.get("passed") is not True:
            raise V12R3ExecutionError(f"collection data gate failed: {stage_id}")
        import numpy as np
        ...
        trace_corpus_exact = (
            ...
            == {f"v12r3_collect_r{descriptor['round_index']}"}
            and set(label_arrays["origins"].astype(str).tolist())
            == {"V12R3_SHIELDED_SAME_STATE"}
        )
        ...
        if receipt != expected_receipt or not current_bindings:
            raise V12R3ExecutionError(f"collection receipt/binding drifted: {stage_id}")
        return receipt
    if kind == "freeze":
        summary = _mapping(
        ...
        if (
            ...
            raise V12R3ExecutionError("candidate freeze closure drifted")
        ...
        if receipt != expected_receipt or not current_bindings:
            raise V12R3ExecutionError("candidate freeze receipt/binding drifted")
        return receipt
    if kind == "final":
        ...
        if (
            ...
            raise V12R3ExecutionError(f"final closure drifted: {stage_id}")
        module = _tree_record(resolve_relative(contract.MODULE_PATHS["p3"]))
        ...
        if receipt != expected_receipt or not current_bindings:
            raise V12R3ExecutionError(f"final receipt/binding drifted: {stage_id}")
        return receipt
    ...
    if (
        ...
        raise V12R3ExecutionError("final aggregate closure drifted")
    ...
    if receipt != expected_receipt or not current_bindings:
        raise V12R3ExecutionError("final aggregate receipt/binding drifted")
    return receipt
...
def _publish_rollout_before_gate(
    ...
    *,
    ...
    # ``run_start`` is written before this function, while the shared writer
    # returns only the artifacts it finalizes.  Re-read it after finalization:
    # comparison with ``expected`` above preserves the intended TOCTOU check.
    persisted["run_start"] = _record(writer.run_start_path)
    for name, record in expected.items():
        if persisted.get(name) != record:
            raise V12R3ExecutionError(f"prospective rollout record drifted: {name}")
...
def _run_probe(stage: str) -> dict[str, Any]:
    ...
    if os.path.lexists(destination):
        raise V12R3ExecutionError(f"probe destination exists: {stage}")
    ...
    writer.start(
        {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "STARTED_H0_PRIMARY_SPLIT_V12R3_PURE_PROBE",
            "protocol_id": contract.PROTOCOL_ID,
            "pipeline_id": contract.PIPELINE_ID,
            "stage_id": stage_id,
            "fit_stage": stage,
    ...
    try:
        ...
        if not math.isfinite(body_weight_n) or body_weight_n <= 0.0:
            raise V12R3ExecutionError("body weight is malformed")
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if not isinstance(info, Mapping):
                raise V12R3ExecutionError("probe info is malformed")
        ...
        try:
            writer.publish_failure(
                end_reason="v12r3_pure_probe_failed",
                error=exc,
    ...
    if audit is None or replay_recorder is None:
        raise V12R3ExecutionError("probe audit/replay was not initialized")
    ...
    if replay.n_steps != len(rows) or replay.boundary_count != len(rows) + 1:
        raise V12R3ExecutionError("probe replay row/boundary count drifted")
    partial = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PERSISTED_H0_PRIMARY_SPLIT_V12R3_PROBE_BEFORE_GATE",
        "protocol_id": contract.PROTOCOL_ID,
        "stage_id": stage_id,
        "fit_stage": stage,
        "steps": len(rows),
    ...
    if gate.get("integrity_passed") is not True:
        raise V12R3ExecutionError(f"probe integrity failed: {stage}")
    if stage == "p3" and gate.get("passed") is not True:
        raise V12R3ExecutionError("P3 autonomy probe failed terminally")
    return receipt
...
def _run_label(stage: str) -> dict[str, Any]:
    ...
    if result.get("passed") is not True:
        raise V12R3ExecutionError(f"observer label gate failed: {stage}")
    _activity_increment(
        "offline_teacher_label_calls_confirmed",
...
def _run_collection(round_index: int, case_id: str) -> dict[str, Any]:
    ...
    destination = resolve_relative(case["destination"])
    if os.path.lexists(destination):
        raise V12R3ExecutionError(f"collection destination exists: {stage_id}")
    fit_stage = str(case["candidate_fit_stage"])
    ...
    writer.start(
        {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "STARTED_H0_PRIMARY_SPLIT_V12R3_SHIELDED_COLLECTION",
            "protocol_id": contract.PROTOCOL_ID,
            "pipeline_id": contract.PIPELINE_ID,
            "stage_id": stage_id,
            "round_index": round_index,
    ...
    try:
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if candidate_std.tobytes() != teacher_std.tobytes():
                raise V12R3ExecutionError("candidate/teacher logstd mismatch")
            ...
            if (
                ...
                raise V12R3ExecutionError("blend/latch/noise semantics drifted")
            applied = np.ascontiguousarray(
            ...
            if not isinstance(info, Mapping):
                raise V12R3ExecutionError("collection info is malformed")
            ...
            if step == 1 or step % 25 == 0:
                ...
                eta = elapsed / step * (contract.EXPECTED_STEPS - step)
                print(
                    f"[V12R3 collect r{round_index}/{case_id}] "
        ...
        try:
            writer.publish_failure(
                end_reason="v12r3_shielded_collection_failed",
                error=exc,
        ...
        env.close()
    if audit is None:
        raise V12R3ExecutionError("collection audit was not initialized")
    ...
    v10s_fit._write_npz_exclusive(
        label_path,
        {
            ...
            "tranche_ids": np.asarray(
                [f"v12r3_collect_r{round_index}"] * sample_count,
                dtype="U96",
            ),
            "origins": np.asarray(
                ["V12R3_SHIELDED_SAME_STATE"] * sample_count, dtype="U160"
            ),
        },
    )
    partial = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PERSISTED_H0_PRIMARY_SPLIT_V12R3_COLLECTION_BEFORE_GATE",
        "protocol_id": contract.PROTOCOL_ID,
        "stage_id": stage_id,
        "round_index": round_index,
        "case_id": case_id,
    ...
    if data_gate.get("passed") is not True:
        writer.publish_failure(
            end_reason="v12r3_collection_data_gate_failed",
            error="V12R3 collection integrity/physical gate failed",
            status=contract.COLLECTION_FAIL_STATUS,
...
def _run_candidate_freeze() -> dict[str, Any]:
    ...
    for path in (CANDIDATE_FREEZE_PATH, summary_path, gate_path):
        if os.path.lexists(path):
            raise V12R3ExecutionError(f"candidate freeze output exists: {path}")
    ...
    summary = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "COMPLETE_H0_PRIMARY_SPLIT_V12R3_CANDIDATE_FREEZE_UNGATED",
        "protocol_id": contract.PROTOCOL_ID,
    ...
    if gate.get("passed") is not True:
        raise V12R3ExecutionError("candidate freeze gate failed")
    receipt = {
        **summary,
        "status": contract.CANDIDATE_FREEZE_PASS_STATUS,
        "passed": True,
...
def _run_final(case_id: str) -> dict[str, Any]:
    ...
    destination = resolve_relative(case["destination"])
    if os.path.lexists(destination):
        raise V12R3ExecutionError(f"final destination exists: {case_id}")
    ...
    if freeze.get("candidate_id") != candidate_id:
        raise V12R3ExecutionError("candidate freeze identity drifted")
    candidate = RLModule.from_checkpoint(candidate_module_path)
    candidate.eval()
    innovations = _frozen_innovations(
        case_id,
    ...
    writer.start(
        {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "STARTED_H0_PRIMARY_SPLIT_V12R3_FINAL_ROLLOUT",
            "protocol_id": contract.PROTOCOL_ID,
            "pipeline_id": contract.PIPELINE_ID,
            "stage_id": stage_id,
            "case": copy.deepcopy(case),
    ...
    try:
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if not isinstance(info, Mapping):
                raise V12R3ExecutionError("final rollout info is malformed")
            physical = _consume_physical_step(
                audit,
                step=step,
                info=info,
        ...
        try:
            writer.publish_failure(
                end_reason="v12r3_final_rollout_failed",
                error=exc,
        ...
        env.close()
    if audit is None:
        raise V12R3ExecutionError("final audit was not initialized")
    partial = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PERSISTED_H0_PRIMARY_SPLIT_V12R3_FINAL_BEFORE_GATE",
        "protocol_id": contract.PROTOCOL_ID,
        "stage_id": stage_id,
        "case_id": case_id,
        "steps": len(rows),
    ...
    if gate.get("passed") is not True:
        writer.publish_failure(
            end_reason="v12r3_final_gate_failed",
            error="V12R3 final rollout physical/dependency gate failed",
            status=contract.FINAL_ROLLOUT_FAIL_STATUS,
            details={"gate": _record(writer.gate_path)},
        )
        raise V12R3ExecutionError(f"final rollout gate failed: {case_id}")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": contract.FINAL_ROLLOUT_PASS_STATUS,
        "passed": True,
...
def _run_finalize_development() -> dict[str, Any]:
    ...
    for path in (receipt_path, summary_path, gate_path):
        if os.path.lexists(path):
            raise V12R3ExecutionError(f"final aggregate output exists: {path}")
    freeze = _candidate_freeze_receipt()
    module = _tree_record(resolve_relative(contract.MODULE_PATHS["p3"]))
    candidate_id = contract.candidate_id("p3", module["tree_sha256"])
    ...
    forensic.write_json_exclusive(gate_path, gate)
    if gate.get("passed") is not True:
        raise V12R3ExecutionError("final development aggregate gate failed")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": contract.FINAL_DEVELOPMENT_PASS_STATUS,
        "passed": True,
...
def _run_fit(stage: str) -> dict[str, Any]:
    ...
    if result.get("passed") is not True:
        raise V12R3ExecutionError(f"fit gate failed: {stage}")
...
def _run_stage(stage_id: str) -> dict[str, Any]:
    ...
    raise V12R3ExecutionError(f"unknown stage kind: {kind!r}")
...
def _pipeline_activity_gate(
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V12R3_PIPELINE_ACTIVITY"
            if all(checks.values())
            else "FAIL_H0_PRIMARY_SPLIT_V12R3_PIPELINE_ACTIVITY"
        ),
        "passed": all(checks.values()),
        "protocol_id": contract.PROTOCOL_ID,
        "checks": checks,
...
def _ledger_payload(
    *,
    ...
    if passed and activity_gate.get("passed") is not True:
        raise V12R3ExecutionError("pipeline PASS activity accounting drifted")
...
def _claim_run_root() -> None:
    """Atomically claim the exact V12R3 run directory before its first file."""

    if RUN_ROOT.resolve().parent != resolve_relative(contract.RUN_ROOT.parent):
        raise V12R3ExecutionError("run-root parent drifted")
    for ancestor in (RUN_ROOT.parent, *RUN_ROOT.parent.parents):
        ...
        if os.path.lexists(ancestor) and ancestor.is_symlink():
            raise V12R3ExecutionError(f"run-root ancestor is a symlink: {ancestor}")
    RUN_ROOT.parent.mkdir(parents=True, exist_ok=True)
    try:
        RUN_ROOT.mkdir(exist_ok=False)
    except FileExistsError as exc:
        raise V12R3ExecutionError("V12R3 run root already claimed") from exc
...
def _publish_terminal_stage_failure(
    ...
    try:
        ...
        artifact = writer.publish_failure(
            end_reason="v12r3_stage_failed_terminally",
            error=error,
            status=status,
...
def _preclaim_failure_ledger(error: BaseException) -> dict[str, Any]:
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "FAIL_H0_PRIMARY_SPLIT_V12R3_PIPELINE_CLAIM_TERMINAL",
        "passed": False,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "execution_lock": _record(LOCK_PATH),
...
def execute_pipeline_once() -> dict[str, Any]:
    """Claim and execute the exact 26-stage V12R3 order once."""
    ...
    if os.path.lexists(PIPELINE_CLAIM_PATH) or os.path.lexists(PIPELINE_LEDGER_PATH):
        raise V12R3ExecutionError("pipeline is already claimed or terminal")
    ...
    try:
        for stage_id in contract.STAGE_IDS:
            ...
            _verify_worker_claim(stage_id)
            print(
                f"[V12R3 pipeline] start {len(completed) + 1:02d}/"
            ...
            attempted = None
            print(
                f"[V12R3 pipeline] pass  {len(completed):02d}/"
                f"{len(contract.STAGE_IDS):02d} {stage_id}",
                flush=True,
            )
        if tuple(completed) != tuple(contract.STAGE_IDS):
            raise V12R3ExecutionError("pipeline completed-stage order drifted")
        ledger = _ledger_payload(
            status="PASS_H0_PRIMARY_SPLIT_V12R3_PIPELINE",
            passed=True,
...
def _parser() -> argparse.ArgumentParser:
    ...
    action.add_argument(
        "--execute-once",
        action="store_true",
        help="claim and execute the frozen V12R3 pipeline once",
    )
    return parser
