"""Self-contained pure-probe replay and observer-only H0 labelling for V12R3.
...
REPLAY_SCHEMA_ID = "H0_V12R3_OBSERVER_REPLAY_BOUNDARIES_V2"
LABEL_CORPUS_SCHEMA_ID = "H0_V12R3_OBSERVER_LABEL_CORPUS_V1"
...
class V12R3ObserverLabelError(RuntimeError):
...
def _contract_module() -> Any:
    return importlib.import_module("h0_primary_split_v12r3_autonomy_recovery_contract")


def _finite(value: Any, *, label: str, nonnegative: bool = False) -> float:
    if isinstance(value, (bool, np.bool_)):
        raise V12R3ObserverLabelError(f"{label} must be numeric")
    try:
        result = float(value)
    except (TypeError, ValueError, OverflowError) as exc:
        raise V12R3ObserverLabelError(f"{label} must be numeric") from exc
    if not math.isfinite(result):
        raise V12R3ObserverLabelError(f"{label} must be finite")
    if nonnegative and result < 0.0:
        raise V12R3ObserverLabelError(f"{label} must be nonnegative")
    return result


def _native_bool(value: Any, *, label: str) -> bool:
    if not isinstance(value, (bool, np.bool_)):
        raise V12R3ObserverLabelError(f"{label} must be boolean")
    return bool(value)


def _utf8_array(value: str, *, label: str) -> np.ndarray:
    if not isinstance(value, str) or not value:
        raise V12R3ObserverLabelError(f"{label} must be nonempty text")
...
def _decode_utf8(array: np.ndarray, *, label: str, ascii_only: bool = False) -> str:
    if array.dtype != np.dtype(np.uint8) or array.ndim != 1 or array.size == 0:
        raise V12R3ObserverLabelError(f"{label} must be a nonempty uint8 vector")
    try:
        result = array.tobytes().decode("ascii" if ascii_only else "utf-8")
    except UnicodeDecodeError as exc:
        raise V12R3ObserverLabelError(f"{label} is not valid text") from exc
    if not result:
        raise V12R3ObserverLabelError(f"{label} must be nonempty")
    return result
...
def _reject_duplicate_pairs(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V12R3ObserverLabelError(f"legacy FSM config repeats JSON key {key!r}")
        result[key] = value
    return result


def _reject_json_constant(value: str) -> None:
    raise V12R3ObserverLabelError(
...
def canonical_legacy_fsm_config(runtime_phase_fsm: Any) -> tuple[bytes, str, str]:
    ...
    if config is None or not dataclasses.is_dataclass(config):
        raise V12R3ObserverLabelError("runtime phase FSM config must be a dataclass")
    try:
    ...
    if not module_name or not class_name or "<locals>" in class_name:
        raise V12R3ObserverLabelError("runtime phase FSM identity is unstable")
...
def _event_timestamp(
    ...
    *,
    ...
    if not primary_present and not fallback_present:
        if default is None:
            raise V12R3ObserverLabelError(f"{label} is missing")
    ...
    if (
        ...
        raise V12R3ObserverLabelError(f"{label} timestamp aliases disagree")
...
def _observation_fields(info: Mapping[str, Any]) -> tuple[float, float, float, float]:
    ...
    if not isinstance(observation, Mapping):
        raise V12R3ObserverLabelError("info.observation must be an object")
    return (
...
class ProbeReplayAccumulator:
    ...
    def __init__(
        ...
        *,
        ...
        if self._body_weight_n <= 0.0:
            raise V12R3ObserverLabelError("body_weight_n must be positive")
        if not isinstance(event_contract_id, str) or not event_contract_id:
            raise V12R3ObserverLabelError("event_contract_id must be nonempty")
    ...
    def _append_boundary(
        ...
        if not isinstance(info, Mapping):
            raise V12R3ObserverLabelError("boundary info must be an object")
        ...
        if not isinstance(detector, Mapping):
            raise V12R3ObserverLabelError("info.online_grf_detector must be an object")
        if reset_boundary:
            if detector:
                raise V12R3ObserverLabelError("reset detector stream must be empty")
            load_bw = 0.0
            in_contact = False
        else:
            left = detector.get("left")
            if not isinstance(left, Mapping):
                raise V12R3ObserverLabelError("left detector stream is missing")
        ...
        if reset_boundary and raw_events:
            raise V12R3ObserverLabelError("reset legacy event stream must be empty")
        previous_onset = (
        ...
        for raw_index, raw in enumerate(raw_events):
            ...
            if not isinstance(raw, Mapping):
                raise V12R3ObserverLabelError(f"{label} must be an object")
            side = str(raw.get("side", "")).strip().lower()
            event_type = str(raw.get("event", "")).strip().lower()
            if side not in {"left", "right"}:
                raise V12R3ObserverLabelError(f"{label}.side is unknown")
            if event_type not in {"heel_strike", "toe_off"}:
                raise V12R3ObserverLabelError(f"{label}.event is unknown")
            ...
            if not (
                ...
                raise V12R3ObserverLabelError(f"{label} timestamps are not causal")
            if side != "left":
                continue
            if (
    ...
    def append_step(
        ...
        *,
        ...
        if self._closed:
            raise V12R3ObserverLabelError("probe replay accumulator is closed")
    ...
    def arrays(self) -> dict[str, np.ndarray]:
        """Close the accumulator and return the exact V12R3 NPZ mapping."""

        if self._closed:
            raise V12R3ObserverLabelError("probe replay accumulator already closed")
...
class PureProbeReplayRecorder:
    ...
    def record_reset(
        ...
        *,
        ...
        if self._accumulator is not None:
            raise V12R3ObserverLabelError("probe reset was already recorded")
    ...
    def arrays(self) -> dict[str, np.ndarray]:
        if self._accumulator is None:
            raise V12R3ObserverLabelError("probe reset was not recorded")
...
def write_npz_exclusive(path: str | Path, arrays: Mapping[str, np.ndarray]) -> Path:
    ...
    if os.path.lexists(destination):
        raise V12R3ObserverLabelError(f"refusing to clobber: {destination}")
...
# Explicit runner-facing spelling requested by the V12R3 orchestrator.
...
def _strict_config(config_json: bytes) -> dict[str, Any]:
    ...
    except V12R3ObserverLabelError:
        raise
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise V12R3ObserverLabelError("legacy FSM config JSON is invalid") from exc
    if not isinstance(value, Mapping):
        raise V12R3ObserverLabelError("legacy FSM config JSON must be an object")
    try:
    ...
    if canonical != config_json:
        raise V12R3ObserverLabelError("legacy FSM config JSON bytes are noncanonical")
    if value.get("event_source") != "legacy_events":
        raise V12R3ObserverLabelError("legacy FSM config event source drifted")
    return dict(value)
...
def load_probe_replay_strict(
    ...
    if not source.is_file():
        raise V12R3ObserverLabelError(f"probe replay is missing: {source}")
    try:
    ...
    if actor.ndim != 2 or actor.shape[1] != ACTOR_FEATURE_COUNT or len(actor) < 1:
        raise V12R3ObserverLabelError("actor_observations must have shape (N,35)")
    n_steps = len(actor)
    boundaries = n_steps + 1
    if penetration.shape != (n_steps,):
        raise V12R3ObserverLabelError("previous_penetration_m shape drifted")
    for name in BOUNDARY_ARRAY_DTYPES:
        if arrays[name].shape != (boundaries,):
            raise V12R3ObserverLabelError(f"{name} must have shape (N+1,)")
    if arrays["body_weight_n"].shape != (1,):
        raise V12R3ObserverLabelError("body_weight_n must have shape (1,)")
    ...
    if not np.all(np.isfinite(actor)) or not np.all(np.isfinite(penetration)):
        raise V12R3ObserverLabelError("probe step arrays contain non-finite values")
    if np.any(penetration < 0.0):
        raise V12R3ObserverLabelError("previous penetration must be nonnegative")
    for name in BOUNDARY_ARRAY_DTYPES:
        if arrays[name].dtype.kind == "f" and not np.all(np.isfinite(arrays[name])):
            raise V12R3ObserverLabelError(f"{name} contains non-finite values")
    times = arrays["boundary_time_s"]
    if not np.all(np.diff(times) > TIME_TOLERANCE_S):
        raise V12R3ObserverLabelError("boundary times are not strictly increasing")
    body_weight = _finite(
    ...
    if arrays["legacy_left_normal_grf_bw"][0].tobytes() != np.float32(
        ...
        raise V12R3ObserverLabelError("reset detector boundary is not empty")
    if np.any(arrays["legacy_left_normal_grf_bw"] < 0.0):
        raise V12R3ObserverLabelError("legacy normal GRF must be nonnegative")
    ...
    if (
        ...
        raise V12R3ObserverLabelError("legacy FSM config SHA-256 mismatch")
    ...
    if offsets.shape != (boundaries + 1,):
        raise V12R3ObserverLabelError("event boundary offsets must have shape (N+2,)")
    if offsets[0] != 0 or np.any(np.diff(offsets) < 0):
        raise V12R3ObserverLabelError("event boundary offsets are malformed")
    event_count = int(offsets[-1])
    if offsets[1] != 0:
        raise V12R3ObserverLabelError("reset boundary must own zero events")
    for name in EVENT_ARRAY_DTYPES:
        if name == "legacy_left_event_boundary_offsets":
            continue
        if arrays[name].shape != (event_count,):
            raise V12R3ObserverLabelError(f"{name} must have shape (E,)")
    if any(str(value) != "left" for value in arrays["legacy_left_event_side"]):
        raise V12R3ObserverLabelError("event journal must contain only left events")
    if any(
        str(value) not in {"heel_strike", "toe_off"}
        for value in arrays["legacy_left_event_type"]
    ):
        raise V12R3ObserverLabelError("event journal contains an unknown event")
    ...
    if not (
        ...
        raise V12R3ObserverLabelError("event timestamps are non-finite or noncausal")
    if event_count > 1 and not np.all(np.diff(onset) > TIME_TOLERANCE_S):
        raise V12R3ObserverLabelError("left event onsets are not strictly increasing")
    for boundary in range(boundaries):
        start, stop = int(offsets[boundary]), int(offsets[boundary + 1])
        if start < 0 or stop > event_count:
            raise V12R3ObserverLabelError("event offsets escape the journal")
    ...
    for present_name, value_name in optional_float_pairs:
        ...
        if not np.all(np.isfinite(value)) or np.any(value < 0.0):
            raise V12R3ObserverLabelError(f"{value_name} is invalid")
    ...
    if (
        ...
        raise V12R3ObserverLabelError("V12R3 replay contract is not available")
    topology = selected_contract.replay_event_topology_gate(arrays, n_steps=n_steps)
    if topology.get("passed") is not True:
        raise V12R3ObserverLabelError(f"V12R3 replay topology gate failed: {topology}")
    return LoadedReplay(
        arrays=arrays,
        config=config,
...
def replay_teacher_views(
    ...
    *,
    ...
    try:
        ...
        raise V12R3ObserverLabelError("legacy FSM reconstruction failed") from exc
    ...
    if teacher_array.shape != (replay.n_steps, ACTOR_FEATURE_COUNT):
        raise V12R3ObserverLabelError("teacher replay result shape drifted")
...
def _default_mean_query(module: Any, teacher_view: np.ndarray) -> np.ndarray:
    ...
    if logits.shape != (1, 2 * ACTION_DIM) or not np.all(np.isfinite(logits)):
        raise V12R3ObserverLabelError("H0 policy logits are malformed")
...
def _coverage_from_mapping(value: Any, *, rows: int) -> CoverageResult:
    if isinstance(value, CoverageResult):
        ...
        raise V12R3ObserverLabelError("coverage evaluator returned an unknown value")
    if (
        ...
        raise V12R3ObserverLabelError("coverage arrays are malformed")
    return result


def _default_coverage_evaluator(observations: np.ndarray) -> CoverageResult:
    fitter = importlib.import_module("h0_primary_split_v12r3_recovery_weighted_fitter")
    evaluator = getattr(fitter, "evaluate_observer_coverage", None)
    if not callable(evaluator):
        evaluator = getattr(fitter, "observer_coverage", None)
    if not callable(evaluator):
        raise V12R3ObserverLabelError(
            "V12R3 fitter does not expose observer coverage evaluation"
        )
...
def observer_episode_weights(
    ...
    *,
    ...
    if (
        ...
        raise V12R3ObserverLabelError("observer episode weighting inputs are malformed")
    ...
    if (
        ...
        raise V12R3ObserverLabelError("observer episode weights are malformed")
...
def label_closed_probe_in_memory(
    ...
    *,
    ...
    if not callable(probe_closure_validator):
        raise V12R3ObserverLabelError("probe closure validator is required")
    contract = contract_module or _contract_module()
    if probe_stage not in tuple(contract.FIT_STAGES):
        raise V12R3ObserverLabelError(f"unknown probe stage: {probe_stage!r}")
    if not isinstance(case_id, str) or not case_id or len(case_id) > 64:
        raise V12R3ObserverLabelError("case_id must be nonempty and <=64 chars")
    ...
    try:
        ...
        raise V12R3ObserverLabelError("pure probe is not validly closed") from exc
    ...
    for index, teacher_view in enumerate(teacher_views.teacher_observations):
        ...
        if not np.all(np.isfinite(mean)) or np.any(np.abs(mean) > 1.0):
            raise V12R3ObserverLabelError("H0 mean is non-finite or out of bounds")
        actions.append(mean.copy())
        query_count += 1
    if query_count != replay.n_steps:
        raise V12R3ObserverLabelError("H0 teacher query count drifted")
...
def validate_observer_label_arrays(
    ...
    if set(arrays) != set(LABEL_ARRAY_DTYPES):
        raise V12R3ObserverLabelError("observer label corpus array schema drifted")
    for name, dtype in LABEL_ARRAY_DTYPES.items():
        array = np.asarray(arrays[name])
        if not _dtype_matches(array, dtype):
            raise V12R3ObserverLabelError(f"observer label {name} dtype drifted")
    expected_shapes = {
    ...
    if any(
        ...
        raise V12R3ObserverLabelError("observer label corpus shape drifted")
    for name in (
        "observations",
        "actions",
        "previous_penetration_m",
        ...
        if not np.all(np.isfinite(arrays[name])):
            raise V12R3ObserverLabelError(f"observer label {name} is non-finite")
    if tuple(arrays["actor_feature_names"].astype(str).tolist()) != tuple(
        coherent_teacher.EXPECTED_ACTOR_FEATURE_NAMES
    ):
        raise V12R3ObserverLabelError("observer actor feature layout drifted")
    if np.count_nonzero(arrays["reset_mask"]) != 1 or not bool(arrays["reset_mask"][0]):
        raise V12R3ObserverLabelError("observer reset mask drifted")
    if not np.array_equal(arrays["step_indices"], np.arange(1, rows + 1)):
        raise V12R3ObserverLabelError("observer step indices drifted")
...
def _artifact(path: str | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R3ObserverLabelError(f"cannot bind artifact: {path}") from exc
...
def _tree_record(path: str | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R3ObserverLabelError(f"cannot bind artifact tree: {path}") from exc
    ...
    *,
    ...
    def validate(replay: LoadedReplay) -> None:
        ...
        if (
            ...
            raise V12R3ObserverLabelError("probe evidence does not close this replay")
        if replay.event_contract_id != contract.EVENT_CONTRACT_ID:
            raise V12R3ObserverLabelError("probe replay event contract drifted")
...
def run_observer_label_stage(
    *,
    ...
    if stage not in tuple(contract.FIT_STAGES):
        raise V12R3ObserverLabelError(f"unknown label stage: {stage!r}")
    ...
    if enforce_canonical_destination and destination != canonical_destination:
        raise V12R3ObserverLabelError("observer label destination is not canonical")
    if os.path.lexists(destination):
        raise V12R3ObserverLabelError(
    ...
    summary = {
        ...
        "status": "H0_PRIMARY_SPLIT_V12R3_OBSERVER_LABELS_COMPLETE_UNGATED",
...
def _strict_json_mapping(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise V12R3ObserverLabelError(f"{label} is not strict JSON") from exc
    if not isinstance(value, Mapping):
        raise V12R3ObserverLabelError(f"{label} must be an object")
    return dict(value)
...
def verify_observer_label_stage(
    ...
    *,
    ...
    try:
        with np.load(corpus_path, allow_pickle=False) as archive:
            if set(archive.files) != set(LABEL_ARRAY_DTYPES):
                raise V12R3ObserverLabelError("observer corpus NPZ schema drifted")
            arrays = {
                name: np.ascontiguousarray(archive[name].copy())
                for name in archive.files
            }
    ...
    if type(rows) is not int or rows < 1:
        raise V12R3ObserverLabelError("observer labelled row count is malformed")
    ...
    if expected_ood.tobytes(order="C") != arrays["coverage_ood_mask"].tobytes(
        ...
        raise V12R3ObserverLabelError("observer coverage OOD mask drifted")
    ...
    if expected_raw.tobytes(order="C") != arrays["raw_sample_weights"].tobytes(
        ...
        raise V12R3ObserverLabelError("observer sample weights drifted")
    ...
    if (
        ...
        raise V12R3ObserverLabelError("observer label gate recomputation failed")
    ...
    if forensic.canonical_json_bytes(receipt) != forensic.canonical_json_bytes(
        ...
        raise V12R3ObserverLabelError("minimal observer label receipt drifted")
    current_bindings = (
    ...
    if not current_bindings:
        raise V12R3ObserverLabelError("observer label artifact binding drifted")
    return {
        **receipt,
        "receipt": _artifact(receipt_path),
        "summary": _artifact(summary_path),
