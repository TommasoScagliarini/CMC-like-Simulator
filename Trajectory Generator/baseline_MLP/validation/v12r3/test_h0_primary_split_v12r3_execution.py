"""Execution-surface tests for the additive, one-shot V12R3 pipeline.

These tests exercise orchestration only.  They never load a policy checkpoint,
fit an actor, reset an environment, or create a canonical V12R3 artifact.
"""
...
for _root in (REPO_ROOT, LOCAL_VALIDATION_ROOT, V12R3_ROOT):
    if str(_root) not in sys.path:
        sys.path.insert(0, str(_root))

import h0_primary_split_v12r3_autonomy_recovery_contract as contract  # noqa: E402
import run_h0_primary_split_v12r3_autonomy_recovery as runner  # noqa: E402
import run_h0_primary_split_v12r3_design_audit as audit_runner  # noqa: E402
import test_h0_primary_split_v12_autonomy_recovery_contract as v12_fixture  # noqa: E402
...
def test_execution_sources_authority_and_stage_topology_are_exact() -> None:
    ...
    assert (
        sum(
            name.endswith("_failure") and name.startswith("collect_")
    ...
    assert (
        sum(
            name.endswith("_failure") and name.startswith("final_") for name in manifest
        )
        == 6
    )


def test_current_platform_plugins_are_exact_and_windows_fails_closed_without_dlls(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    current = runner._platform_plugin_readiness()
    assert current["passed"] is True
    assert set(current["records"]) == {"sea", "online_grf"}

    monkeypatch.setattr(runner.platform, "system", lambda: "Windows")
    monkeypatch.setattr(runner.platform, "machine", lambda: "AMD64")
    windows = runner._platform_plugin_readiness()
    assert windows["passed"] is False
    assert windows["expected_paths"] == {
        "sea": "plugins/SEA_Plugin_BlackBox_mCMC_impedence_ff.dll",
        "online_grf": "plugins/OnlineGRFContact.dll",
    }
    assert windows["checks"]["all_required_binaries_recorded"] is False


def test_rollout_publisher_rebinds_run_start_after_real_finalization(
    ...
    writer.start({"kind": "probe", "closed": False})
    ...
    runner._publish_rollout_before_gate(
        ...
        rows=rows,
        partial={"steps": 1},
        summary={"steps": 1, "passed": False},
    )

    persisted = writer.finalized_artifact_records()
    assert runner._record(writer.run_start_path) == {
        "path": "rollout/run_start.json",
        "sha256": runner.forensic.sha256_file(writer.run_start_path),
        "size_bytes": writer.run_start_path.stat().st_size,
    }
    assert set(persisted) == {"trace", "partial_summary", "summary"}


def test_rollout_publisher_detects_run_start_toctou(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(runner, "REPO_ROOT", tmp_path)
    writer = runner.forensic.ForensicRolloutWriter(
    ...
    writer.start({"kind": "probe", "closed": False})
    writer.write_step(1, {"value": 7})
    original_finalize = writer.finalize_before_gate

    def finalize_then_tamper(**kwargs: object) -> dict[str, object]:
        records = original_finalize(**kwargs)
        writer.run_start_path.write_bytes(b'{"closed":true,"kind":"probe"}\n')
        return records

    monkeypatch.setattr(writer, "finalize_before_gate", finalize_then_tamper)
    with pytest.raises(runner.V12R3ExecutionError, match="run_start"):
        runner._publish_rollout_before_gate(
            writer,
            rows=[{"step": 1, "value": 7}],
            partial={"steps": 1},
...
def _physical_prefix_summary(
    stage: str, *, anomaly_counter: str | None = None
) -> dict[str, object]:
    class Legacy:
        @staticmethod
        def _finalize_accumulator(_value: object) -> dict[str, float]:
            return {"max": 0.0, "rms": 0.0}

        @staticmethod
        def _finalize_sea(_value: object) -> dict[str, object]:
            return {}

    binary_gate: dict[str, object] = {
        "passed": False,
        "sample_count": 2320,
        "event_count": 3,
    ...
    if anomaly_counter is not None:
        binary_gate[anomaly_counter] = 1

    class Collector:
        @staticmethod
        def _finalize_binary_event_gate_v26(
            _events: object, _sample_count: object
        ) -> dict[str, object]:
            return copy.deepcopy(binary_gate)

    audit = {
        "binary_events": [],
        "control_window_count": 2320,
        "raw_sensor_sample_count": 2320,
        "phase_valid_cycle_count": 1,
        "penetrations": [0.02517650964311425],
    ...
    return runner._physical_summary(
        audit,
        case=contract.canonical_probe_case(stage),
        rows=[{}] * 232,
        info={"end_reason": "grf_penetration"},
        terminated=True,
        truncated=False,
        actor_names=["actor"] * contract.EXPECTED_ACTOR_FEATURES,
        full_names=["observation"] * contract.EXPECTED_FULL_FEATURES,
        legacy=Legacy(),
        v26_collector=Collector(),
    )


@pytest.mark.parametrize("stage", ("p0", "p1", "p2"))
def test_clean_v26_physical_prefix_is_integrity_valid_and_recoverable(
    monkeypatch: pytest.MonkeyPatch, stage: str
) -> None:
    monkeypatch.setattr(v12_fixture, "contract", contract)
    physical = _physical_prefix_summary(stage)
    assert physical["binary_phase_event_gate"]["passed"] is False
    assert physical["binary_event_prefix_integrity"]["passed"] is True
    assert physical["binary_event_failure_count"] == 0

    summary = v12_fixture._failed_physical_probe(stage, steps=232)
    summary.update(physical)
    gate = contract.pure_probe_gate(summary, stage=stage)
...
def test_each_v26_prefix_anomaly_fails_probe_integrity(
    monkeypatch: pytest.MonkeyPatch, counter: str
) -> None:
    monkeypatch.setattr(v12_fixture, "contract", contract)
    physical = _physical_prefix_summary("p0", anomaly_counter=counter)
    assert physical["binary_event_prefix_integrity"]["passed"] is False
    assert physical["binary_event_failure_count"] == 1

    summary = v12_fixture._failed_physical_probe("p0", steps=232)
    summary.update(physical)
    gate = contract.pure_probe_gate(summary, stage="p0")
...
def test_stage_dispatch_is_exact_and_side_effect_free_when_stubbed(
    ...
    monkeypatch.setattr(
        runner,
        "_run_collection",
        lambda round_index, case_id: calls.append(("collection", round_index, case_id))
        or {"passed": True},
    )
    monkeypatch.setattr(
        runner,
...
def test_execute_pipeline_once_publishes_one_terminal_failure_and_stops(
    ...
    def fail_on_third(stage: str) -> dict[str, bool]:
        ...
        if stage == failed_stage:
            raise runner.V12R3ExecutionError("synthetic terminal failure")
        return {"passed": True}

    monkeypatch.setattr(runner, "_run_stage", fail_on_third)
    with pytest.raises(runner.V12R3ExecutionError, match="synthetic terminal"):
...
def test_pipeline_claim_publication_failure_writes_zero_activity_ledger(
    ...
    ledger = writes[0][1]
    assert ledger["status"] == ("FAIL_H0_PRIMARY_SPLIT_V12R3_PIPELINE_CLAIM_TERMINAL")
...
def test_execution_lock_gate_rejects_retry_even_if_runner_payload_is_pass_like() -> (
    None
):
    ...
    payload = {
        ...
        "v12r2_terminal_failure_lineage": copy.deepcopy(
            contract.V12R2_TERMINAL_LINEAGE
        ),
...
def test_run_root_claim_is_atomic_and_no_clobber(
    ...
    with pytest.raises(runner.V12R3ExecutionError, match="already claimed"):
        runner._claim_run_root()
...
def test_design_audit_build_calls_the_in_memory_fitter_exactly_once(
    ...
    monkeypatch.setattr(
        ...
        "_record",
        lambda path: (
            v12_record if path == contract.V12_PROTOCOL_FREEZE_PATH else protocol_record
        ),
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        audit_runner,
        "_v12r1_lineage_records",
        lambda: copy.deepcopy(contract.V12R1_LINEAGE_ARTIFACTS),
    )
    monkeypatch.setattr(
        audit_runner,
        "_v12r2_terminal_lineage",
        lambda: copy.deepcopy(contract.V12R2_TERMINAL_LINEAGE),
    )
