...
import h0_primary_split_v12r3_pure_probe_observer_labeler as labeler  # noqa: E402
...
def _actor(
    knee: float, knee_speed: float, ankle: float, ankle_speed: float
) -> np.ndarray:
...
def test_self_contained_replay_round_trip_and_column_24_is_mutable(
    ...
    replayed = labeler.replay_teacher_views(loaded, phase_fsm_factory=_fake_factory)
...
def test_missing_journal_array_and_tampered_delivery_are_rejected(
    ...
    with pytest.raises(labeler.V12R3ObserverLabelError, match="schema drifted"):
    ...
    with pytest.raises(
        labeler.V12R3ObserverLabelError,
...
def test_config_hash_and_noncanonical_config_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(labeler.V12R3ObserverLabelError, match="SHA-256 mismatch"):
    ...
    with pytest.raises(labeler.V12R3ObserverLabelError, match="noncanonical"):
        labeler.load_probe_replay_strict(path)
...
def test_object_dtype_cannot_cross_no_pickle_loader(tmp_path: Path) -> None:
    ...
    with pytest.raises(labeler.V12R3ObserverLabelError, match="pickle-free"):
        labeler.load_probe_replay_strict(path)
...
def test_h0_load_occurs_only_after_closure_and_exactly_n_queries(
    ...
    assert (
        labeler.math.fsum(result.arrays["normalized_sample_weights"].tolist()) == 500.0
    )
...
def test_failed_closure_prevents_fsm_replay_and_h0_load(tmp_path: Path) -> None:
    ...
    with pytest.raises(labeler.V12R3ObserverLabelError, match="not validly closed"):
...
def test_replay_npz_is_no_clobber(tmp_path: Path) -> None:
    path = tmp_path / "replay.npz"
    labeler.write_replay_npz_exclusive(path, _replay_arrays())
    with pytest.raises(labeler.V12R3ObserverLabelError, match="clobber"):
        labeler.write_replay_npz_exclusive(path, _replay_arrays())
