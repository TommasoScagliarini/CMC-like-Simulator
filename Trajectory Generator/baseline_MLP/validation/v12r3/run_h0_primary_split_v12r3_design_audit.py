"""Run or verify the sole V12R3 in-memory P0 numerical design audit.

``--prepare`` first verifies the immutable V12R3 protocol freeze, binds all
...
import freeze_h0_primary_split_v12r3_autonomy_recovery as protocol_freezer  # noqa: E402
import h0_primary_split_v12r3_autonomy_recovery_contract as contract  # noqa: E402
import h0_primary_split_v12r3_recovery_weighted_fitter as fitter  # noqa: E402


class V12R3DesignAuditError(RuntimeError):
...
def resolve_relative(path: str | os.PathLike[str] | PurePosixPath) -> Path:
    ...
    if (
        ...
        raise V12R3DesignAuditError(f"non-canonical repository path: {raw!r}")
...
def _execution_source_records() -> dict[str, dict[str, Any]]:
    ...
    if (
        ...
        raise V12R3DesignAuditError("execution source declaration drifted")
    try:
        return {name: _record(paths[name]) for name in expected_names}
    except Exception as exc:
        raise V12R3DesignAuditError(
...
def _v12r1_lineage_records() -> dict[str, dict[str, Any]]:
    ...
    if records != contract.V12R1_LINEAGE_ARTIFACTS:
        raise V12R3DesignAuditError("terminal V12R1 lineage drifted")
    return records


def _v12r2_terminal_lineage() -> dict[str, Any]:
    """Re-audit and return only the exact, non-reusable R2 lineage binding."""

    try:
        gate = protocol_freezer._v12r2_terminal_failure_gate()
    except Exception as exc:
        raise V12R3DesignAuditError(
            "terminal V12R2 lineage is missing or drifted"
        ) from exc
    lineage = gate.get("lineage")
    if gate.get("passed") is not True or lineage != contract.V12R2_TERMINAL_LINEAGE:
        failed = [
            name for name, value in gate.get("checks", {}).items() if value is not True
        ]
        raise V12R3DesignAuditError(f"terminal V12R2 lineage gate failed: {failed}")
    return copy.deepcopy(contract.V12R2_TERMINAL_LINEAGE)
...
def _runtime_record() -> dict[str, str]:
    ...
    for name in ("numpy", "scipy", "torch"):
        ...
        if not isinstance(value, str) or not value:
            raise V12R3DesignAuditError(f"runtime version unavailable: {name}")
        versions[name] = value
    return versions
...
def _verify_protocol_freeze() -> dict[str, Any]:
    try:
        ...
        raise V12R3DesignAuditError(
            "canonical V12R3 protocol freeze is missing or drifted"
        ) from exc
    if (
        ...
        raise V12R3DesignAuditError("protocol freeze is not design-only PASS")
    return dict(payload)
...
def _verify_protocol_freeze_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R3DesignAuditError(
            "canonical V12R3 protocol freeze is unreadable"
        ) from exc
    if (
        observed != expected
        ...
        raise V12R3DesignAuditError(
            "V12R3 protocol freeze/current design sources drifted"
        )
...
def build_design_audit() -> dict[str, Any]:
    ...
    if v12_record_before != expected_v12:
        raise V12R3DesignAuditError("immutable V12 parent freeze drifted")
    ...
    if fit_contract.get("passed") is not True:
        raise V12R3DesignAuditError("fit contract self-check is not PASS")
    r1_lineage = _v12r1_lineage_records()
    r2_lineage_before = _v12r2_terminal_lineage()
    ...
    if not isinstance(raw, Mapping):
        raise V12R3DesignAuditError("in-memory fitter returned a non-object")

    occupancy_after = _forbidden_execution_occupancy()
    sources_after = _execution_source_records()
    r2_lineage_after = _v12r2_terminal_lineage()
    runtime_after = _runtime_record()
    if occupancy_after != {name: True for name in occupancy_after}:
        raise V12R3DesignAuditError("in-memory audit opened execution scope")
    if os.path.lexists(RECEIPT_PATH):
        raise V12R3DesignAuditError("audit fitter unexpectedly created the receipt")
    if sources_after != sources_before:
        raise V12R3DesignAuditError("execution source changed during audit")
    if r2_lineage_after != r2_lineage_before:
        raise V12R3DesignAuditError("terminal V12R2 lineage changed during audit")
    if runtime_after != runtime_before:
        raise V12R3DesignAuditError("runtime identity changed during audit")
    if _record(contract.PROTOCOL_FREEZE_PATH) != protocol_record_before:
        raise V12R3DesignAuditError("V12R3 protocol freeze changed during audit")
    if _record(contract.V12_PROTOCOL_FREEZE_PATH) != v12_record_before:
        raise V12R3DesignAuditError("V12 protocol freeze changed during audit")
    ...
    payload.update(
        {
            ...
            "v12r2_terminal_failure_lineage": r2_lineage_before,
    ...
    if gate.get("passed") is not True:
        ...
        raise V12R3DesignAuditError(f"design audit contract gate failed: {failed}")
    payload["contract_gate"] = copy.deepcopy(gate)
    if contract.design_audit_gate(payload) != gate:
        raise V12R3DesignAuditError("design audit gate is not payload-stable")
    return payload
...
def _write_json_exclusive(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    if destination != RECEIPT_PATH:
        raise V12R3DesignAuditError(f"non-canonical audit receipt: {destination}")
    if _symlink_ancestors(destination):
        raise V12R3DesignAuditError("audit receipt has a symlink ancestor")
    try:
        parent_status = os.lstat(destination.parent)
    except FileNotFoundError as exc:
        raise V12R3DesignAuditError("audit receipt parent is missing") from exc
    if not stat.S_ISDIR(parent_status.st_mode):
        raise V12R3DesignAuditError("audit receipt parent is not a directory")
    if os.path.lexists(destination):
        raise V12R3DesignAuditError("audit receipt exists/no-clobber")
    ...
    try:
        ...
        raise V12R3DesignAuditError("audit receipt exists/no-clobber") from exc
    try:
...
def verify_design_audit() -> dict[str, Any]:
    ...
    if not stat.S_ISREG(status.st_mode) or _symlink_ancestors(RECEIPT_PATH):
        raise V12R3DesignAuditError("design audit receipt is not a safe regular file")
    try:
        payload = forensic.strict_json_load(RECEIPT_PATH)
    except Exception as exc:
        raise V12R3DesignAuditError("design audit receipt is not strict JSON") from exc
    if not isinstance(payload, Mapping):
        raise V12R3DesignAuditError("design audit receipt is not an object")
    observed = dict(payload)
    if RECEIPT_PATH.read_bytes() != forensic.canonical_json_bytes(observed):
        raise V12R3DesignAuditError("design audit receipt bytes are not canonical")

    gate = contract.design_audit_gate(observed)
    if gate.get("passed") is not True or observed.get("contract_gate") != gate:
        raise V12R3DesignAuditError("design audit contract gate drifted")
    current_bindings = {
        ...
        "v12r2_terminal_failure_lineage": _v12r2_terminal_lineage(),
    ...
    if drifted:
        raise V12R3DesignAuditError(f"design audit current bindings drifted: {drifted}")
    return observed
