...
import h0_primary_split_v12r1_recovery_weighted_fitter as fit
...
def test_recovery_ramp_endpoints_and_midpoint_are_frozen() -> None:
    ...
    for invalid in (-1.0e-9, math.nan, math.inf, -math.inf):
        with pytest.raises(fit.V12R1RecoveryWeightedFitError):
...
def test_corpus_component_order_is_seed_then_label_then_matching_round() -> None:
    ...
    assert fit.expected_corpus_component_order("p1") == (
        ("v11_seed",),
        ("pure_observer", "p0"),
        ("v12r1_shielded", 1, minus),
        ("v12r1_shielded", 1, stochastic),
    )
    assert fit.expected_corpus_component_order("p3") == (
        ...
        ("v12r1_shielded", 1, stochastic),
        ("pure_observer", "p1"),
        ("v12r1_shielded", 2, minus),
        ("v12r1_shielded", 2, stochastic),
        ("pure_observer", "p2"),
        ("v12r1_shielded", 3, minus),
        ("v12r1_shielded", 3, stochastic),
    )
...
def test_real_coverage_reference_hashes_and_p95_are_exact() -> None:
    ...
    assert hashes["observations"] == contract.COVERAGE_WEIGHTING[
        "reference_observations_sha256"
    ]
    assert hashes["normalization_mean"] == contract.COVERAGE_WEIGHTING[
        "normalization_mean_sha256"
    ]
    assert hashes["normalization_std"] == contract.COVERAGE_WEIGHTING[
        "normalization_std_sha256"
    ]
    assert hashes["normalized_features"] == contract.COVERAGE_WEIGHTING[
        "normalized_feature_matrix_sha256"
    ]
    assert hashes["nearest_indices"] == contract.COVERAGE_WEIGHTING[
        "loo_nearest_indices_sha256"
    ]
    assert hashes["loo_distances"] == contract.COVERAGE_WEIGHTING[
...
def test_new_row_query_reproduces_frozen_v11_failure_certificate() -> None:
    ...
    assert result["normalized_features_sha256"] == expected[
    ...
    assert bool(np.all(result["ood_mask"][200:])) is expected[
        "all_steps_201_through_259_ood"
    ]
...
def test_p0_reproduction_tolerance_is_taken_from_r1_contract() -> None:
    ...
    assert match["absolute_tolerance"] == contract.P0_REPRODUCTION_TOLERANCE[
        "absolute"
    ]
    assert match["relative_tolerance"] == contract.P0_REPRODUCTION_TOLERANCE[
        "relative"
    ]
