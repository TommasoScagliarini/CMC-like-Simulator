"""Environment-free tests for the historical V12R7-Q3 runtime source."""
...
import h0_v12r7_q3_physical_rollout as physical  # noqa: E402
...
import run_h0_v12r7_q3_qualification as runner  # noqa: E402
...
def _candidate() -> tuple[str, dict[str, Any]]:
    ...
    return contract.r7.candidate_id(module["tree_sha256"]), module
...
def test_imports_are_inert_and_deferred() -> None:
    ...
    assert state["successor_namespace"] == "v12r8q3"
...
import h0_v12r7_q3_physical_rollout
import run_h0_v12r7_q3_qualification
...
def test_tape_prepare_is_no_clobber_and_tamper_evident(tmp_path: Path) -> None:
    ...
    with pytest.raises(noise.V12R7Q3NoiseError, match="clobber"):
        noise.prepare(
            output_root=root,
            prerequisite_snapshot=snapshot,
            protocol_record=_record(contract.PROTOCOL_FREEZE_PATH.as_posix()),
    ...
    with pytest.raises(noise.V12R7Q3NoiseError, match="closure drifted"):
        noise.verify_manifest(noise_root=root, enforce_canonical_destination=False)
...
def test_comparison_metric_flattening_is_exact_and_finite() -> None:
    ...
    with pytest.raises(physical.V12R7Q3PhysicalRolloutError, match="missing"):
...
def test_rollout_summary_builder_preserves_fail_closed_fields() -> None:
    ...
    assert all(name in summary for name in contract.ZERO_REQUIRED_COUNTS)


def test_historical_r7_q3_cli_only_reports_deferred_without_publication(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert runner.main(["--deferred-preflight"]) == 0
    assert capsys.readouterr().out.strip() == (
        "BLOCKED_H0_V12R7_Q3_HISTORICAL_R7_TERMINAL_FAIL"
    )


def test_historical_namespace_rejects_all_canonical_mutation() -> None:
    with pytest.raises(freezer.V12R7Q3FreezeError, match="terminal FAIL"):
        freezer.freeze_protocol_and_lock()
    with pytest.raises(noise.V12R7Q3NoiseError, match="terminal FAIL"):
        noise.prepare()
    with pytest.raises(
        runner.V12R7Q3QualificationExecutionError, match="terminal FAIL"
    ):
        runner.build_execution_preflight()
