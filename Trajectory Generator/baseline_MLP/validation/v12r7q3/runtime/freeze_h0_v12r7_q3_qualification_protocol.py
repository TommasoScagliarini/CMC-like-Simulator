"""Freeze V12R7-Q3 only after five live semantic R7 verifiers pass.

Importing this module is inert.  Publication is explicit, exclusive, and
fail-closed; the canonical Q3 paths must all be absent when the live R7
...
class V12R7Q3FreezeError(RuntimeError):
    """Raised before any unsafe, premature, or drifting Q3 freeze."""


PROTOCOL_FREEZE_PASS_STATUS = "PASS_H0_V12R7_Q3_QUALIFICATION_PROTOCOL_FREEZE"
EXECUTION_LOCK_PASS_STATUS = "PASS_H0_V12R7_Q3_QUALIFICATION_EXECUTION_LOCK"
...
def resolve_relative(value: str | PurePosixPath) -> Path:
    ...
    if not artifacts.canonical_relative_path(raw):
        raise V12R7Q3FreezeError(f"non-canonical repository path: {raw!r}")
...
def _record(value: str | PurePosixPath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R7Q3FreezeError(f"unsafe or missing artifact: {path}") from exc
...
def _mapping(path: str | PurePosixPath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R7Q3FreezeError(f"strict JSON could not be read: {source}") from exc
    if not isinstance(value, Mapping):
        raise V12R7Q3FreezeError(f"expected JSON object: {source}")
    result = dict(value)
    if source.read_bytes() != forensic.canonical_json_bytes(result):
        raise V12R7Q3FreezeError(f"JSON is not canonical: {source}")
    return result


def official_r7_hooks() -> prerequisites.R7SemanticVerifierHooks:
    """Load the five public R7 verifiers lazily in the RLlib environment."""

    try:
        freezer = importlib.import_module("freeze_h0_v12r7_recovery")
        runner = importlib.import_module("run_h0_v12r7_recovery")
    except BaseException as exc:
        raise V12R7Q3FreezeError("R7 verifier modules are unavailable") from exc
    return prerequisites.R7SemanticVerifierHooks(
...
def live_r7_prerequisite_snapshot(
    *,
    hooks: prerequisites.R7SemanticVerifierHooks | None = None,
    ...
    if occupied:
        raise V12R7Q3FreezeError(f"Q3 namespace is already occupied: {occupied}")
    selected = official_r7_hooks() if hooks is None else hooks
    try:
        payloads = prerequisites.invoke_verifier_hooks(selected)
    except BaseException as exc:
        raise V12R7Q3FreezeError("five live R7 verifiers did not converge") from exc
    ...
    if gate.get("passed") is not True:
        ...
        raise V12R7Q3FreezeError(f"semantic R7 prerequisite gate failed: {failed}")
    return {
        "payloads": payloads,
        "records": records,
...
def source_closure() -> dict[str, dict[str, Any]]:
    ...
    if gate.get("passed") is not True:
        ...
        raise V12R7Q3FreezeError(f"Q3 source closure failed: {failed}")
    return records
...
def build_protocol_payload(
    *,
    ...
    if not isinstance(gate, Mapping) or gate.get("passed") is not True:
        raise V12R7Q3FreezeError("protocol requires a semantic terminal-PASS R7 gate")
    ...
    if closure_gate.get("passed") is not True:
        raise V12R7Q3FreezeError("protocol source closure is incomplete")
    return {
        ...
        "candidate_binding_state": "BOUND_FROM_R7_TERMINAL_PASS",
        ...
        "r7_prerequisite_records": copy.deepcopy(
...
def protocol_payload_gate(payload: Any) -> dict[str, Any]:
    ...
    checks = {
        ...
        and data.get("candidate_binding_state") == "BOUND_FROM_R7_TERMINAL_PASS"
        and data.get("candidate_selection_rule") == contract.CANDIDATE_SELECTION_RULE,
        "prerequisite": isinstance(data.get("r7_prerequisite_gate"), Mapping)
        and data["r7_prerequisite_gate"].get("passed") is True,
...
def build_execution_lock_payload(
    *,
    ...
    if protocol_payload_gate(protocol_payload).get("passed") is not True:
        raise V12R7Q3FreezeError("execution lock requires an exact protocol payload")
    expected_occupancy = {
    ...
    if dict(occupancy) != expected_occupancy:
        raise V12R7Q3FreezeError("execution lock occupancy is not pristine")
...
def freeze_protocol_and_lock() -> dict[str, Any]:
    """Publish both canonical Q3 locks exactly once after R7 terminal PASS."""

    if contract.HISTORICAL_TERMINAL_FAILURE is True:
        raise V12R7Q3FreezeError(
            "V12R7 ended in terminal FAIL; this historical Q3 namespace can never "
            f"publish (use a fresh {contract.SUCCESSOR_NAMESPACE} contract)"
        )
    if _q3_occupied():
        raise V12R7Q3FreezeError(f"Q3 freeze paths already occupied: {_q3_occupied()}")
    snapshot = live_r7_prerequisite_snapshot(require_q3_unopened=True)
    ...
    if protocol_gate.get("passed") is not True:
        raise V12R7Q3FreezeError("Q3 protocol pure gate failed")
...
def verify_protocol_freeze() -> dict[str, Any]:
    ...
    if protocol_payload_gate(payload).get("passed") is not True:
        raise V12R7Q3FreezeError("Q3 protocol freeze drifted")
    if payload.get("source_closure") != source_closure():
        raise V12R7Q3FreezeError("Q3 protocol source bytes drifted")
    return payload
...
def verify_execution_lock() -> dict[str, Any]:
    ...
    if not all(checks.values()):
        ...
        raise V12R7Q3FreezeError(f"Q3 execution lock drifted: {failed}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return 0 if args.preflight or args.freeze or args.verify else 1
