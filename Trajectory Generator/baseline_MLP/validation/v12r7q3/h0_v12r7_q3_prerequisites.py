"""Deferred R7 binding and semantic prerequisite verifier hooks for Q3.
...
try:
    from . import h0_v12r7_q3_artifacts as artifacts
    from . import h0_v12r7_q3_qualification_contract as contract
except ImportError:  # Direct test execution with this directory on sys.path.
    import h0_v12r7_q3_artifacts as artifacts
    import h0_v12r7_q3_qualification_contract as contract


class V12R7Q3PrerequisiteError(RuntimeError):
    """Raised unless the exact terminal-PASS R7 lineage is verified."""


@dataclass(frozen=True)
class R7SemanticVerifierHooks:
    """The only five live R7 verifier entry points accepted by this scaffold."""
...
R7_CANDIDATE_ID: str | None = None
R7_CANDIDATE_MODULE: dict[str, Any] | None = None
...
def validate_candidate_tree(candidate_id: Any, candidate_module: Any) -> dict[str, Any]:
    """Validate the exact five-file standard R7 actor tree and identity."""
    ...
    expected_id = contract.r7.candidate_id(module["tree_sha256"])
    if candidate_id != expected_id:
        raise artifacts.V12R7Q3ArtifactError(
            "R7 candidate identity does not bind the exact five-file tree"
        )
...
def validate_actor_feature_manifest(
    ...
    *,
    ...
    """Validate the R7 standard 35->512->512->2 actor feature contract."""
    ...
    semantic_ok = (
        ...
        and data.get("status") == contract.r7.ACTOR_FEATURE_MANIFEST_STATUS
        and data.get("topology_id") == contract.r7.TOPOLOGY_ID
        and data.get("fit_contract_id") == contract.r7.FIT_CONTRACT_ID
    ...
    if not (record_ok and semantic_ok):
        raise artifacts.V12R7Q3ArtifactError(
            "R7 actor feature manifest is not the exact standard W512 contract"
        )
...
def candidate_freeze_semantics(
    ...
    *,
    ...
    """Pure semantic verifier for the R7 candidate-freeze receipt."""
    ...
    try:
        ...
        manifest = None
    checks = {
        "status": data.get("status") == contract.r7.CANDIDATE_FREEZE_PASS_STATUS
        and data.get("passed") is True,
        "identity": data.get("protocol_id") == contract.r7.PROTOCOL_ID
        and data.get("pipeline_id") == contract.r7.PIPELINE_ID
...
def final_development_semantics(
    ...
    *,
    ...
    """Pure semantic verifier for the six-case R7 final-development receipt."""

    data = dict(payload) if isinstance(payload, Mapping) else {}
    rows = data.get("case_gates")
    rows_ok = isinstance(rows, list) and len(rows) == len(
        contract.r7.DEVELOPMENT_CASE_IDS
    )
    if rows_ok:
        for row, case_id in zip(rows, contract.r7.DEVELOPMENT_CASE_IDS, strict=True):
            ...
            if (
                ...
                break
    checks = {
        "status": data.get("status") == contract.r7.DEVELOPMENT_PASS_STATUS
        and data.get("passed") is True,
        "identity": data.get("protocol_id") == contract.r7.PROTOCOL_ID
        and data.get("pipeline_id") == contract.r7.PIPELINE_ID
...
def terminal_ledger_semantics(
    ...
    *,
    ...
    """Pure semantic verifier for the immutable terminal-PASS R7 ledger."""

    data = dict(payload) if isinstance(payload, Mapping) else {}
    checks = {
        "terminal_pass": data.get("status") == contract.r7.PIPELINE_TERMINAL_PASS_STATUS
        and data.get("passed") is True
...
def deferred_prerequisite_state() -> dict[str, Any]:
    """Expose the current source-only state without probing future artifacts."""
    ...
    return {
        ...
        "missing_runtime_sources": list(contract.DEFERRED_RUNTIME_SOURCE_PATHS),
...
def invoke_verifier_hooks(hooks: R7SemanticVerifierHooks) -> dict[str, Any]:
    ...
    for name, callback in zip(names, callbacks, strict=True):
        ...
        if not isinstance(value, Mapping):
            raise V12R7Q3PrerequisiteError(
                f"R7 semantic verifier returned a non-object: {name}"
            )
...
def bind_candidate(candidate_id: Any, candidate_module: Any) -> dict[str, Any]:
    """Bind one exact terminal-PASS R7 candidate once per process."""

    global R7_CANDIDATE_ID, R7_CANDIDATE_MODULE
    binding = validate_candidate_tree(candidate_id, candidate_module)
    if R7_CANDIDATE_ID is not None and (
        R7_CANDIDATE_ID != binding["candidate_id"]
        or R7_CANDIDATE_MODULE != binding["candidate_module"]
    ):
        raise V12R7Q3PrerequisiteError("R7-Q3 candidate binding is immutable")
    R7_CANDIDATE_ID = str(binding["candidate_id"])
    R7_CANDIDATE_MODULE = copy.deepcopy(binding["candidate_module"])
    return current_candidate_binding()


def current_candidate_binding() -> dict[str, Any]:
    if R7_CANDIDATE_ID is None or R7_CANDIDATE_MODULE is None:
        raise V12R7Q3PrerequisiteError(
            "R7-Q3 remains fail-closed: terminal R7 candidate is not bound"
        )
    return {
        "candidate_id": R7_CANDIDATE_ID,
        "candidate_module": copy.deepcopy(R7_CANDIDATE_MODULE),
    }


def clear_candidate_binding_for_tests() -> None:
    global R7_CANDIDATE_ID, R7_CANDIDATE_MODULE
    R7_CANDIDATE_ID = None
    R7_CANDIDATE_MODULE = None


__all__ = [
    "R7SemanticVerifierHooks",
    "V12R7Q3PrerequisiteError",
    "bind_candidate",
