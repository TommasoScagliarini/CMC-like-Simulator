"""Freeze and verify the V12R8 recovery protocol and one-shot lock.
...
the inherited V12R6 runtime, immutable V12R7 adjudication dependencies, and all
R8 production sources.  The execution lock additionally attests the exact
terminal R7 evidence and the current OpenSim/plugin runtime.
"""
...
class V12R8FreezeError(RuntimeError):
...
LOCAL_PRODUCTION_SOURCES = (
    ...
    "Trajectory Generator/baseline_MLP/validation/v12r8/h0_v12r8_recovery_contract.py",
    "Trajectory Generator/baseline_MLP/validation/v12r8/h0_v12r8_prefix_adjudicator.py",
    "Trajectory Generator/baseline_MLP/validation/v12r8/h0_v12r8_recovery_probe.py",
    "Trajectory Generator/baseline_MLP/validation/v12r8/h0_v12r8_recovery_fitter.py",
    "Trajectory Generator/baseline_MLP/validation/v12r8/freeze_h0_v12r8_recovery.py",
...
def resolve_relative(path: str | os.PathLike[str] | PurePosixPath) -> Path:
    ...
    if (
        ...
        raise V12R8FreezeError(f"non-canonical repository path: {raw!r}")
...
def _reject_link_ancestors(path: Path, *, include_leaf: bool) -> None:
    try:
        ...
        raise V12R8FreezeError(f"path escaped repository: {path}") from exc
    limit = len(parts) if include_leaf else max(0, len(parts) - 1)
    current = REPO_ROOT
    for part in parts[:limit]:
        current /= part
        if os.path.lexists(current) and _is_link_or_reparse(current):
            raise V12R8FreezeError(f"unsafe symlink/junction component: {current}")
...
def safe_repository_path(
    ...
    *,
    ...
    if type(include_leaf) is not bool:
        raise V12R8FreezeError("include_leaf must be strict bool")
...
def artifact_record(path: str | os.PathLike[str] | PurePosixPath) -> dict[str, Any]:
    ...
    if not _regular_file(target) or _is_link_or_reparse(target):
        raise V12R8FreezeError(f"unsafe or missing artifact: {target}")
...
def tree_record(path: str | os.PathLike[str] | PurePosixPath) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8FreezeError(f"unsafe or missing tree: {root}") from exc
    if not stat.S_ISDIR(root_status.st_mode) or _is_link_or_reparse(root):
        raise V12R8FreezeError(f"unsafe or missing tree: {root}")
    files: list[Path] = []
    for current_text, directory_names, file_names in os.walk(
        ...
        for name in directory_names:
            ...
            try:
                ...
                raise V12R8FreezeError(f"unsafe tree entry: {child}") from exc
            if not stat.S_ISDIR(child_status.st_mode) or _is_link_or_reparse(child):
                raise V12R8FreezeError(f"unsafe tree directory: {child}")
        for name in file_names:
            child = current / name
            if not _regular_file(child) or _is_link_or_reparse(child):
                raise V12R8FreezeError(f"unsafe tree file: {child}")
            files.append(child)
    if not files:
        raise V12R8FreezeError(f"empty artifact tree: {root}")
...
def _mapping(path: str | os.PathLike[str] | PurePosixPath) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8FreezeError(f"invalid strict JSON mapping: {target}") from exc
    if not isinstance(value, dict):
        raise V12R8FreezeError(f"expected JSON mapping: {target}")
    return value
...
def _locked_artifact(name: str) -> dict[str, Any]:
    ...
    if observed != exact:
        raise V12R8FreezeError(f"locked artifact drifted: {name}")
    return observed
...
def _coverage_reference_row_count() -> int:
    ...
    try:
        ...
        raise V12R8FreezeError("coverage reference corpus is unreadable") from exc
    ...
    if (
        ...
        raise V12R8FreezeError("coverage reference corpus rows drifted")
    return rows


def attest_locked_inputs() -> dict[str, Any]:
    """Recompute R6 fit lineage plus immutable terminal R7 evidence."""
    ...
    if (
        ...
        raise V12R8FreezeError("locked R6 candidate tree drifted")
    source_h0 = tree_record(contract.SOURCE_H0_MODULE_PATH)
    if not _strict_equal(source_h0, contract.LOCKED_SOURCE_H0_TREE):
        raise V12R8FreezeError("locked source H0 teacher tree drifted")
    ...
    if not all(semantic_checks.values()):
        ...
        raise V12R8FreezeError(f"locked input semantic audit failed: {failed}")
    r7_records = {
        name: artifact_record(expected["path"])
        for name, expected in contract.LOCKED_R7_EVIDENCE.items()
    }
    if any(
        not _strict_equal(r7_records[name], expected)
        for name, expected in contract.LOCKED_R7_EVIDENCE.items()
    ):
        raise V12R8FreezeError("terminal R7 evidence digest or size drifted")
    if artifact_record(contract.R7_CONTRACT_SOURCE_RECORD["path"]) != (
        contract.R7_CONTRACT_SOURCE_RECORD
    ):
        raise V12R8FreezeError("R7 contract source closure drifted")
    r7_adjudication = adjudicator.load_and_adjudicate_r7_plus()
    if (
        r7_adjudication.get("passed") is not True
        or r7_adjudication.get("offline_label_authorized") is not True
        or r7_adjudication.get("r7_probe_rerun") is not False
        or r7_adjudication.get("r7_artifacts_modified") is not False
    ):
        raise V12R8FreezeError("terminal R7 prefix is not safely labelable")
    r7_adjudication_attestation = {
        "status": r7_adjudication["status"],
        "passed": True,
        "artifact_records": copy.deepcopy(r7_adjudication["artifact_records"]),
        "target_contract_derivation": copy.deepcopy(
            r7_adjudication["target_contract_derivation"]
        ),
        "projected_fields": copy.deepcopy(
            r7_adjudication["normalization"]["projected_fields"]
        ),
        "nested_gate_passed_preserved": r7_adjudication["normalization"][
            "nested_gate_passed_preserved"
        ],
        "counterfactual_r7_gate_status": r7_adjudication["counterfactual_r7_gate"][
            "status"
        ],
    ...
    return {
        ...
        "r7_evidence_records": r7_records,
        "r7_contract_source": copy.deepcopy(contract.R7_CONTRACT_SOURCE_RECORD),
        "r7_plus_adjudication": r7_adjudication_attestation,
    }
...
def _runtime_attestation() -> dict[str, Any]:
    try:
        ...
        raise V12R8FreezeError("runtime/plugin attestation failed") from exc
    ...
    if (
        ...
        raise V12R8FreezeError("runtime/plugin stack is not ready")
    return value
...
def _strict_runtime(attestor: RuntimeAttestor | None) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8FreezeError("runtime attestor returned malformed evidence") from exc
    readiness = value.get("platform_plugin_readiness")
    if (
        value.get("inference_stack_ready") is not True
        or not isinstance(readiness, Mapping)
        ...
        raise V12R8FreezeError("runtime/plugin readiness is not PASS")
    return value
...
def expected_protocol_payload() -> dict[str, Any]:
    ...
    if self_check.get("passed") is not True:
        raise V12R8FreezeError("R8 contract self-check failed")
    return {
        ...
        "transitive_source_basis": (
            "V12R6_RUNTIME_CLOSURE_PLUS_IMMUTABLE_R7_ADJUDICATION_PLUS_R8"
        ),
        "one_shot": True,
...
def verify_protocol_freeze() -> dict[str, Any]:
    ...
    if not _strict_equal(observed, expected):
        raise V12R8FreezeError("R8 protocol freeze or source closure drifted")
    return observed
...
def publish_protocol_freeze() -> dict[str, Any]:
    ...
    if os.path.lexists(resolve_relative(contract.RUN_ROOT)):
        raise V12R8FreezeError("run root exists before protocol freeze")
    ...
    *, runtime_attestor: RuntimeAttestor | None = None
    ...
    return {
        ...
        "status": (
            ...
            if passed
            else "FAIL_H0_V12R8_RECOVERY_EXECUTION_LOCK"
        ),
        "passed": passed,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
...
def verify_execution_lock(
    *,
    ...
    if type(require_pristine) is not bool:
        raise V12R8FreezeError("require_pristine must be strict bool")
    ...
    if not all(value is True for value in identity_checks.values()):
        ...
        raise V12R8FreezeError(f"R8 execution lock drifted: {failed}")
    if require_pristine and os.path.lexists(resolve_relative(contract.RUN_ROOT)):
        raise V12R8FreezeError("R8 run root is no longer pristine")
    return lock
...
def publish_execution_lock(
    *, runtime_attestor: RuntimeAttestor | None = None
    ...
    if payload.get("passed") is not True:
        ...
        raise V12R8FreezeError(f"execution lock preconditions failed: {failed}")
