"""One-shot V12R8 recovery collection, fit and pure development runner.

The stage order is fixed by the execution-free contract: read-only adjudication
and offline labelling of the immutable R7 plus prefix, five new normalized R6
pure-policy observer collections, one actor-only fit, semantic five-file
candidate freeze, six pure candidate developments, and one final receipt.
"""
...
import freeze_h0_v12r8_recovery as freezer  # noqa: E402
...
class V12R8ExecutionError(RuntimeError):
    """Raised after the one-shot R8 lineage has failed terminally."""
...
class RunnerAdapters:
    ...
    run_adjudication: Callable[..., Mapping[str, Any]] = (
        probe.publish_r7_plus_adjudication
    )
    run_historical_labels: Callable[..., Mapping[str, Any]] = (
        probe.label_adjudicated_r7_plus
    )
...
def _path(value: str | PurePath | Path) -> Path:
    if isinstance(value, Path) and value.is_absolute():
        ...
        try:
            ...
            raise V12R8ExecutionError(f"path escaped repository: {target}") from exc
    ...
    try:
        ...
        raise V12R8ExecutionError(f"unsafe repository path: {raw!r}") from exc
...
def _record(path: str | PurePath | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R8ExecutionError(f"invalid artifact: {path}") from exc
...
def _tree(path: str | PurePath | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R8ExecutionError(f"invalid artifact tree: {path}") from exc
...
def _mapping(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8ExecutionError(f"invalid strict JSON: {target}") from exc
    if not isinstance(value, Mapping):
        raise V12R8ExecutionError(f"expected JSON object: {target}")
    return dict(value)
...
def _sequence(path: str | PurePath | Path) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise V12R8ExecutionError(f"expected JSON array: {target}")
    return value
...
def _write(
    ...
    try:
        ...
        raise V12R8ExecutionError(
            "R8 runner mutation escaped the exclusive R8 namespace"
        ) from exc
...
def _artifact_exact(value: Any) -> bool:
    ...
    except (KeyError, TypeError, V12R8ExecutionError):
        return False
...
def _increment(activity: dict[str, int], name: str, amount: int = 1) -> None:
    if name not in activity:
        raise V12R8ExecutionError(f"unknown activity counter: {name}")
    if type(amount) is not int or amount < 0:
        raise V12R8ExecutionError("activity increment must be non-negative int")
    current = activity[name]
    if type(current) is not int or current < 0:
        raise V12R8ExecutionError("activity counter drifted")
...
def _stage_claim_path(stage_id: str) -> Path:
    try:
        ...
        raise V12R8ExecutionError(f"unknown stage id: {stage_id}") from exc
    if "/" in stage_id or "\\" in stage_id or ".." in stage_id:
        raise V12R8ExecutionError("unsafe stage identifier")
...
def _expected_pipeline_claim() -> dict[str, Any]:
    return {
        ...
        "status": "CLAIMED_H0_V12R8_RECOVERY_PIPELINE_ONCE",
        "passed": True,
...
def verify_pipeline_claim() -> dict[str, Any]:
    ...
    if not _strict_equal(claim, _expected_pipeline_claim()):
        raise V12R8ExecutionError("pipeline claim semantic drifted")
    return claim


def _expected_worker_claim(stage_id: str) -> dict[str, Any]:
    if stage_id not in contract.STAGE_IDS:
        raise V12R8ExecutionError(f"unknown stage id: {stage_id}")
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "CLAIMED_H0_V12R8_STAGE_ONCE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_id": stage_id,
...
def verify_worker_claim(stage_id: str) -> dict[str, Any]:
    ...
    if not _strict_equal(claim, _expected_worker_claim(stage_id)):
        raise V12R8ExecutionError(f"worker claim semantic drifted: {stage_id}")
    return claim
...
def verify_claim_directory(stage_ids: Sequence[str]) -> dict[str, Any]:
    ...
    if len(expected_ids) != len(set(expected_ids)) or any(
        ...
        raise V12R8ExecutionError("claim directory stage set drifted")
    root = _claims_root()
    if not root.is_dir() or root.is_symlink():
        raise V12R8ExecutionError("claims root is absent or unsafe")
    ...
    if {entry.name for entry in entries} != expected_names or any(
        ...
        raise V12R8ExecutionError("claims directory contains missing or extra entries")
...
def _historical_label_root() -> Path:
    return (
        _path(contract.collection_case_root(contract.HISTORICAL_CASE_ID))
        / "observer_labels"
    )


def _run_r7_plus_adjudication(adapters: RunnerAdapters) -> dict[str, Any]:
    result = adapters.run_adjudication(
        destination=_path(contract.R7_PLUS_ADJUDICATION_PATH),
        artifact_root=REPO_ROOT,
    )
    verified = probe.verify_r7_plus_adjudication(
        destination=_path(contract.R7_PLUS_ADJUDICATION_PATH),
        artifact_root=REPO_ROOT,
    )
    if not _strict_equal(result, verified) or verified.get("passed") is not True:
        raise V12R8ExecutionError("R7 plus adjudication verifier disagrees")
    if any(
        verified.get(name) != 0 or type(verified.get(name)) is not int
        for name in ("environment_reset_calls", "environment_step_calls")
    ):
        raise V12R8ExecutionError("historical adjudication accessed the environment")
    stage_id = "adjudicate_r7_plus_prefix"
    stage_receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PASS_H0_V12R8_R7_PLUS_ADJUDICATION_STAGE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_id": stage_id,
        "case_id": contract.HISTORICAL_CASE_ID,
        "adjudication": _record(contract.R7_PLUS_ADJUDICATION_PATH),
        "r7_terminal_ledger": _record(contract.R7_TERMINAL_LEDGER_PATH),
        "r7_plus_trace": _record(contract.R7_PLUS_TRACE_PATH),
        "r7_plus_replay": _record(contract.R7_PLUS_REPLAY_PATH),
        "r7_plus_summary": _record(contract.R7_PLUS_SUMMARY_PATH),
        "r7_plus_gate": _record(contract.R7_PLUS_GATE_PATH),
        "pipeline_claim": _record(contract.CLAIM_PATH),
    ...
    _write(contract.R7_PLUS_ADJUDICATION_STAGE_RECEIPT_PATH, stage_receipt)
    return verify_r7_plus_adjudication_stage_receipt()


def verify_r7_plus_adjudication_stage_receipt() -> dict[str, Any]:
    semantic = probe.verify_r7_plus_adjudication(
        destination=_path(contract.R7_PLUS_ADJUDICATION_PATH),
        artifact_root=REPO_ROOT,
    )
    receipt = _mapping(contract.R7_PLUS_ADJUDICATION_STAGE_RECEIPT_PATH)
    stage_id = "adjudicate_r7_plus_prefix"
    verify_worker_claim(stage_id)
    expected_records = {
        "adjudication": contract.R7_PLUS_ADJUDICATION_PATH,
        "r7_terminal_ledger": contract.R7_TERMINAL_LEDGER_PATH,
        "r7_plus_trace": contract.R7_PLUS_TRACE_PATH,
        "r7_plus_replay": contract.R7_PLUS_REPLAY_PATH,
        "r7_plus_summary": contract.R7_PLUS_SUMMARY_PATH,
        "r7_plus_gate": contract.R7_PLUS_GATE_PATH,
        "pipeline_claim": contract.CLAIM_PATH,
        "worker_claim": _stage_claim_path(stage_id),
    }
    checks = {
        "schema": _strict_schema(receipt, R7_ADJUDICATION_STAGE_FIELDS),
        "semantic": semantic.get("passed") is True
        and semantic.get("offline_label_authorized") is True,
        "identity": receipt.get("status") == "PASS_H0_V12R8_R7_PLUS_ADJUDICATION_STAGE"
        ...
        and receipt.get("r7_probe_rerun") is False
        and receipt.get("r7_artifacts_modified") is False,
        "zero_activity": all(
    ...
    if not all(value is True for value in checks.values()):
        ...
        raise V12R8ExecutionError(f"adjudication stage receipt drifted: {failed}")
    return receipt


def _run_historical_label(
    activity: dict[str, int], adapters: RunnerAdapters
) -> dict[str, Any]:
    label_root = _historical_label_root()
    result = adapters.run_historical_labels(
        adjudication_destination=_path(contract.R7_PLUS_ADJUDICATION_PATH),
        label_destination=label_root,
        source_h0_path=_path(contract.SOURCE_H0_MODULE_PATH),
        artifact_root=REPO_ROOT,
    )
    if result.get("passed") is not True:
        raise V12R8ExecutionError("historical R7 plus offline labels failed")
    label_receipt = _verify_historical_label_artifacts()
    rows = label_receipt.get("labelled_row_count")
    if type(rows) is not int or rows != 179:
        raise V12R8ExecutionError("historical plus label row count drifted")
    stage_id = "label_r7_plus_prefix"
    stage_receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PASS_H0_V12R8_R7_PLUS_OFFLINE_LABEL_STAGE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_id": stage_id,
        ...
        "adjudication_stage_receipt": _record(
            contract.R7_PLUS_ADJUDICATION_STAGE_RECEIPT_PATH
        ),
    ...
    _write(contract.R7_PLUS_LABEL_STAGE_RECEIPT_PATH, stage_receipt)
    _increment(activity, "offline_teacher_label_calls", rows)
    _increment(activity, "collection_rounds_completed")
    return verify_historical_label_receipt()


def _verify_historical_label_artifacts() -> dict[str, Any]:
    adjudication_receipt = probe.verify_r7_plus_adjudication(
        destination=_path(contract.R7_PLUS_ADJUDICATION_PATH),
        artifact_root=REPO_ROOT,
    )
    label_root = _historical_label_root()
    receipt = _mapping(label_root / "receipt.json")
    summary = _mapping(label_root / "summary.json")
    gate = _mapping(label_root / "gate.json")
    checks = {
        "adjudication": adjudication_receipt.get("passed") is True
        and adjudication_receipt.get("offline_label_authorized") is True,
        "identity": receipt.get("passed") is True
        and receipt.get("protocol_id") == contract.PROTOCOL_ID
        and receipt.get("stage_id") == "label_r7_plus_prefix"
        and receipt.get("case_id") == contract.HISTORICAL_CASE_ID,
        "gate": gate == contract.label_gate(summary) and gate.get("passed") is True,
        "rows": receipt.get("labelled_row_count") == 179
        and receipt.get("same_state_teacher_label_count") == 179
        and receipt.get("teacher_query_count") == 179,
        "environment_free": all(
            type(receipt.get(name)) is int and receipt[name] == 0
            for name in (
                "environment_reset_calls",
                "environment_step_calls",
        ...
        "bindings": receipt.get("source_receipt")
        == _record(contract.R7_PLUS_ADJUDICATION_PATH)
        and receipt.get("source_replay") == _record(contract.R7_PLUS_REPLAY_PATH)
        and receipt.get("labels") == _record(label_root / "labels.npz")
        and receipt.get("summary") == _record(label_root / "summary.json")
        and receipt.get("gate") == _record(label_root / "gate.json"),
        "offline_label_inputs": receipt.get("teacher_h0_id") == contract.SOURCE_H0_ID
        and receipt.get("teacher_h0") == contract.LOCKED_SOURCE_H0_TREE
    ...
    if not all(value is True for value in checks.values()):
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R8ExecutionError(f"historical label receipt drifted: {failed}")
    return receipt


def verify_historical_label_receipt() -> dict[str, Any]:
    _verify_historical_label_artifacts()
    verify_r7_plus_adjudication_stage_receipt()
    semantic = probe.verify_observer_label_closure(contract.HISTORICAL_CASE_ID)
    label_root = _historical_label_root()
    receipt = _mapping(contract.R7_PLUS_LABEL_STAGE_RECEIPT_PATH)
    stage_id = "label_r7_plus_prefix"
    verify_worker_claim(stage_id)
    expected_records = {
        "adjudication_stage_receipt": (
    ...
    checks = {
        "schema": _strict_schema(receipt, HISTORICAL_LABEL_STAGE_FIELDS),
        "identity": receipt.get("status") == "PASS_H0_V12R8_R7_PLUS_OFFLINE_LABEL_STAGE"
        and receipt.get("passed") is True
        and receipt.get("protocol_id") == contract.PROTOCOL_ID
        and receipt.get("pipeline_id") == contract.PIPELINE_ID
        and receipt.get("stage_id") == stage_id
        ...
        "zero_environment_and_updates": all(
            type(receipt.get(name)) is int and receipt[name] == 0
            for name in (
                "environment_reset_calls",
                "environment_step_calls",
    ...
    if not all(value is True for value in checks.values()):
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R8ExecutionError(f"historical label stage receipt drifted: {failed}")
    return receipt
...
def _run_collection_label(
    ...
    if result.get("passed") is not True:
        raise V12R8ExecutionError(f"R8 collection probe failed: {case_id}")
    ...
    if labelled.get("passed") is not True:
        raise V12R8ExecutionError(f"R8 observer labels failed: {case_id}")
    ...
    if (
        ...
        raise V12R8ExecutionError(f"R8 observer label gate drifted: {case_id}")
    rows = label_summary.get("labelled_row_count")
    if type(rows) is not int or rows < contract.MINIMUM_RECOVERABLE_PREFIX_STEPS:
        raise V12R8ExecutionError("observer label row count drifted")
    _increment(activity, "offline_teacher_label_calls", rows)
    _increment(activity, "collection_rounds_completed")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PASS_H0_V12R8_COLLECTION_AND_LABEL_STAGE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_id": stage_id,
...
def verify_collection_label_receipt(case_id: str) -> dict[str, Any]:
    ...
    checks = {
        ...
        "identity": receipt.get("status") == "PASS_H0_V12R8_COLLECTION_AND_LABEL_STAGE"
        and receipt.get("passed") is True
        and receipt.get("protocol_id") == contract.PROTOCOL_ID
        and receipt.get("pipeline_id") == contract.PIPELINE_ID
        and receipt.get("stage_id") == stage_id
    ...
    if not all(value is True for value in checks.values()):
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R8ExecutionError(f"collection/label receipt drifted: {failed}")
    return receipt
...
def _run_fit(activity: dict[str, int], adapters: RunnerAdapters) -> dict[str, Any]:
    ...
    if receipt.get("passed") is not True:
        raise V12R8ExecutionError("R8 actor fit receipt is not PASS")
    verified = dict(adapters.verify_fit())
    if not _strict_equal(receipt, verified):
        raise V12R8ExecutionError("R8 actor fit verifier disagrees")
    return verified
...
def _run_candidate_freeze(adapters: RunnerAdapters) -> dict[str, Any]:
    ...
    if semantic.get("passed") is not True:
        raise V12R8ExecutionError("R8 candidate semantic freeze failed")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
    ...
    if fit_receipt.get("candidate_id") != identity:
        raise V12R8ExecutionError("fit/freeze candidate identity drifted")
...
def verify_candidate_freeze_receipt(
    *, fit_verifier: Callable[[], Mapping[str, Any]] | None = None
    ...
    if not all(value is True for value in checks.values()):
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R8ExecutionError(f"candidate freeze receipt drifted: {failed}")
    return receipt
...
def _physical_config() -> physical.PhysicalDevelopmentConfig:
    return physical.PhysicalDevelopmentConfig(
        protocol_id=contract.PROTOCOL_ID,
        start_status="STARTED_H0_V12R8_PURE_DEVELOPMENT",
        partial_status="PERSISTED_H0_V12R8_DEVELOPMENT_BEFORE_GATE",
        complete_status="COMPLETE_H0_V12R8_PURE_DEVELOPMENT",
        schema_version=contract.SCHEMA_VERSION,
        artifact_root=REPO_ROOT,
        progress_label="V12R8 pure development",
        progress_every=25,
    )
...
def _run_development(
    ...
    if not isinstance(raw_summary, Mapping):
        raise V12R8ExecutionError("development summary is malformed")
    try:
    ...
    except probe.adjudicator.V12R8AdjudicationError as exc:
        raise V12R8ExecutionError("development V26 normalization failed") from exc
    summary = {
    ...
    if not all(value is True for value in runtime_checks.values()):
        ...
        raise V12R8ExecutionError(f"R8 development runtime integrity failed: {failed}")
    gate = contract.development_gate(summary, case_id=case_id)
    _write(destination / "gate.json", gate)
    if gate.get("passed") is not True:
        raise V12R8ExecutionError(f"R8 development gate failed: {case_id}")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PASS_H0_V12R8_PURE_DEVELOPMENT_RECEIPT",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
...
def verify_development_receipt(
    ...
    *,
    ...
    try:
        ...
        raise V12R8ExecutionError("development normalization closure failed") from exc
    ...
    checks = {
        ...
        and receipt.get("status") == "PASS_H0_V12R8_PURE_DEVELOPMENT_RECEIPT"
        and receipt.get("passed") is True
        and receipt.get("protocol_id") == contract.PROTOCOL_ID
        and receipt.get("pipeline_id") == contract.PIPELINE_ID
    ...
    if not all(value is True for value in checks.values()):
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R8ExecutionError(f"development receipt drifted: {failed}")
    return receipt
...
def _run_final_development(
    ...
    if activity.get("collection_rounds_completed") != len(contract.COLLECTION_CASE_IDS):
        raise V12R8ExecutionError("collection round count drifted")
    if activity.get("actor_fit_stage_calls") != 1 or activity.get("actor_updates") != 1:
        raise V12R8ExecutionError("sole actor fit count drifted")
    if activity.get("development_rollouts_completed") != len(
        contract.DEVELOPMENT_CASE_IDS
    ):
        raise V12R8ExecutionError("development rollout count drifted")
    expected_activity = {
    ...
    if any(activity.get(name) != value for name, value in expected_activity.items()):
        raise V12R8ExecutionError("scoped environment reset accounting drifted")
    if (
        ...
        raise V12R8ExecutionError("scoped environment step accounting drifted")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
    ...
    *, fit_verifier: Callable[[], Mapping[str, Any]] | None = None
    ...
    for case_id in contract.DEVELOPMENT_CASE_IDS:
        ...
        if type(steps) is not int or not 1 <= steps <= contract.EXPECTED_STEPS:
            raise V12R8ExecutionError("development step count drifted")
    ...
    for case_id in contract.NEW_COLLECTION_CASE_IDS:
        ...
        if type(steps) is not int or not 1 <= steps <= contract.EXPECTED_STEPS:
            raise V12R8ExecutionError("collection step count drifted")
    ...
    if not all(value is True for value in checks.values()):
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R8ExecutionError(f"final development receipt drifted: {failed}")
    return receipt
    ...
    *,
    ...
    return {
        ...
        "status": (
            ...
            if passed
            else "FAIL_H0_V12R8_RECOVERY_PIPELINE_TERMINAL"
        ),
        "passed": passed,
        "terminal": True,
        "protocol_id": contract.PROTOCOL_ID,
        ...
        "collection_round_count": sum(
            item.get("stage_id") == "label_r7_plus_prefix"
        ...
        "next_stage": (
            "WAIT_SEPARATE_V12R8Q3_PROTOCOL" if passed else "STOP_TERMINAL_NO_RETRY"
        ),
    }
...
def _verify_completed_stage_semantics(
    ...
    *,
    ...
    verify_worker_claim(stage_id)
    if stage_id == "adjudicate_r7_plus_prefix":
        return verify_r7_plus_adjudication_stage_receipt()
    if stage_id == "label_r7_plus_prefix":
        return verify_historical_label_receipt()
    ...
    raise V12R8ExecutionError(f"unknown completed stage: {stage_id}")
...
def _prefix_activity_coherent(
    ...
    *,
    ...
    completed_labels = sum(
        stage == "label_r7_plus_prefix" or stage.startswith("collect_label__")
    ...
    attempted_label = bool(
        attempted_stage == "label_r7_plus_prefix"
    ...
    completed_label_rows = sum(
        int(_mapping(_stage_receipt_path(stage))["labelled_row_count"])
        for stage in completed_ids
        if stage == "label_r7_plus_prefix" or stage.startswith("collect_label__")
    )
    ...
    return bool(
        ...
        and in_closed_interval(
            ...
            completed_label_rows + contract.EXPECTED_STEPS * int(attempted_label),
        )
...
def verify_terminal_ledger(
    *, fit_verifier: Callable[[], Mapping[str, Any]] | None = None
    ...
    if set(ledger) != TERMINAL_LEDGER_FIELDS:
        raise V12R8ExecutionError("terminal ledger schema drifted")
    if type(ledger.get("passed")) is not bool:
        raise V12R8ExecutionError("terminal ledger passed flag drifted")
    ...
    if (
        ...
        raise V12R8ExecutionError("terminal completed-stage prefix drifted")
    ...
    for index, item in enumerate(completed):
        ...
        if (
            ...
            raise V12R8ExecutionError("terminal stage receipt binding drifted")
        _verify_completed_stage_semantics(expected_stage, fit_verifier=verify_fit_once)
        completed_ids.append(expected_stage)
    if not all(value is True for value in checks.values()):
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R8ExecutionError(f"terminal ledger drifted: {failed}")
    if passed:
        if (
            ...
            raise V12R8ExecutionError("terminal PASS closure drifted")
        ...
        if (
            ...
            raise V12R8ExecutionError("terminal FAIL forensic prefix drifted")
    ...
    if not _strict_equal(freezer.attest_locked_inputs(), locked_inputs_before):
        raise V12R8ExecutionError("locked inputs changed during terminal verification")
    return ledger


def _stage_receipt_path(stage_id: str) -> Path:
    if stage_id == "adjudicate_r7_plus_prefix":
        return _path(contract.R7_PLUS_ADJUDICATION_STAGE_RECEIPT_PATH)
    if stage_id == "label_r7_plus_prefix":
        return _path(contract.R7_PLUS_LABEL_STAGE_RECEIPT_PATH)
    if stage_id.startswith("collect_label__"):
    ...
    raise V12R8ExecutionError(f"unknown stage: {stage_id}")
...
def execute(*, adapters: RunnerAdapters | None = None) -> dict[str, Any]:
    ...
    if os.path.lexists(run_root):
        raise V12R8ExecutionError("R8 run root occupied; resume is forbidden")
    ...
    try:
        for stage_id in contract.STAGE_IDS:
            ...
            if stage_id == "adjudicate_r7_plus_prefix":
                _run_r7_plus_adjudication(selected)
            elif stage_id == "label_r7_plus_prefix":
                _run_historical_label(activity, selected)
    ...
    if not passed:
        ...
        raise V12R8ExecutionError(
            f"V12R8 stopped terminally at {attempted_stage}: {error}"
        ) from error
    return verified
...
def preflight() -> dict[str, Any]:
    ...
    if not label_paths_match:
        raise V12R8ExecutionError("fitter/probe observer label path contract drifted")
    return {
        "passed": True,
        "status": "READY_H0_V12R8_RECOVERY_ONE_SHOT",
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_order": list(contract.STAGE_IDS),
        ...
        "single_collection_round": True,
        "single_actor_fit": True,
        "historical_plus_rerun_authorized": False,
        "historical_collection_environment_calls": {
            "reset": 0,
            "step": 0,
        },
...
__all__ = [
    ...
    "verify_historical_label_receipt",
    "verify_r7_plus_adjudication_stage_receipt",
