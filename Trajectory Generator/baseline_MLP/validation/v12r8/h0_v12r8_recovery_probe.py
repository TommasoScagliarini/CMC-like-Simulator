"""Normalized R8 recovery probes and offline same-state H0 labels.

The historical critical-plus case is never executed here: it is admitted only
through :mod:`h0_v12r8_prefix_adjudicator` and labelled from the immutable R7
replay.  The other five preregistered cases use the frozen R6 behaviour policy.
Their physical summary is normalized from nested V26 evidence *before* the
collection gate is evaluated or published.
"""
...
PROBE_BEHAVIOR = "FROZEN_R6_PURE_POLICY_R8_OBSERVER_ONLY"
PROBE_COMPLETE_STATUS = "COMPLETE_H0_V12R8_R6_RECOVERY_PROBE"
LABEL_COMPLETE_STATUS = "COMPLETE_H0_V12R8_SAME_STATE_LABELS"
RECEIPT_STATUS = "PASS_H0_V12R8_RECOVERY_PROBE_CLOSURE"
ADJUDICATION_RECEIPT_STATUS = "PASS_H0_V12R8_R7_PLUS_ADJUDICATION_RECEIPT"
...
class V12R8RecoveryProbeError(RuntimeError):
    """Raised when an R8 probe or label stage cannot fail closed."""


@dataclass(frozen=True)
class RecoveryProbeConfig:
    """Frozen bindings for one of the five new R8 R6-behaviour probes."""
    ...
    progress_label: str = "V12R8 R6 normalized recovery probe"


def _validate_config(config: RecoveryProbeConfig) -> None:
    if not isinstance(config, RecoveryProbeConfig):
        raise V12R8RecoveryProbeError("config must be RecoveryProbeConfig")
    if config.case_id not in contract.NEW_COLLECTION_CASE_IDS:
        raise V12R8RecoveryProbeError(
            "R8 probe accepts only the five new cases; historical plus rerun is forbidden"
        )
    if config.protocol_id != contract.PROTOCOL_ID:
        raise V12R8RecoveryProbeError("protocol identity drifted")
    if config.schema_version != contract.SCHEMA_VERSION:
        raise V12R8RecoveryProbeError("schema identity drifted")
    if type(config.progress_every) is not int or config.progress_every < 0:
        raise V12R8RecoveryProbeError("progress_every must be non-negative")
    if not isinstance(config.progress_label, str) or not config.progress_label:
        raise V12R8RecoveryProbeError("progress_label must be non-empty")
    if (
        config.expected_candidate_tree_sha256
        != contract.LOCKED_INPUTS["r6_candidate"]["tree_sha256"]
        or config.expected_candidate_id != EXPECTED_CANDIDATE_ID
    ):
        raise V12R8RecoveryProbeError("frozen R6 candidate identity drifted")
    if (
        config.event_contract_id != contract.EVENT_CONTRACT_ID
        or config.target_contract_id != contract.TARGET_CONTRACT_ID
    ):
        raise V12R8RecoveryProbeError("V26 detector contract drifted")
    root = Path(config.artifact_root).expanduser().resolve()
    if not root.is_dir() or root.is_symlink():
        raise V12R8RecoveryProbeError("artifact_root must be a safe directory")
...
def _inside(path: str | Path, root: Path, *, label: str) -> Path:
    ...
    try:
        ...
        raise V12R8RecoveryProbeError(f"{label} escaped artifact_root") from exc
    return target
...
def _artifact(path: Path, root: Path) -> dict[str, Any]:
    try:
        ...
        raise V12R8RecoveryProbeError(f"invalid artifact: {path}") from exc
...
def _tree_artifact(path: Path, root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8RecoveryProbeError("teacher H0 tree is missing") from exc
    if not stat.S_ISDIR(root_status.st_mode) or stat.S_ISLNK(root_status.st_mode):
        raise V12R8RecoveryProbeError("teacher H0 tree is unsafe")
    files: list[Path] = []
    for current_text, directory_names, file_names in os.walk(
        ...
        for name in file_names:
            ...
            if not stat.S_ISREG(status.st_mode) or stat.S_ISLNK(status.st_mode):
                raise V12R8RecoveryProbeError("teacher H0 tree contains unsafe file")
            files.append(child)
    if not files:
        raise V12R8RecoveryProbeError("teacher H0 tree is empty")
    ...
    *,
    ...
    if root == canonical_root:
        ...
        if (
            ...
            raise V12R8RecoveryProbeError("locked source H0 teacher tree drifted")
    ...
    if coverage != expected_coverage:
        raise V12R8RecoveryProbeError("locked coverage reference corpus drifted")
    try:
    ...
    if (
        ...
        raise V12R8RecoveryProbeError("locked coverage reference rows drifted")
    return {
...
def _assert_array_mapping_byte_exact(
    ...
    *,
    ...
    if set(expected) != set(observed):
        raise V12R8RecoveryProbeError(f"{label} array schema drifted")
    for name in expected:
        ...
        if (
            ...
            raise V12R8RecoveryProbeError(f"{label} array bytes drifted: {name}")
...
def _strict_mapping(path: Path) -> dict[str, Any]:
    try:
        ...
        raise V12R8RecoveryProbeError(f"invalid strict JSON: {path}") from exc
    if not isinstance(value, Mapping):
        raise V12R8RecoveryProbeError(f"expected JSON object: {path}")
    return dict(value)
...
def _strict_sequence(path: Path) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise V12R8RecoveryProbeError(f"expected JSON array: {path}")
    return value
...
def _activity(callback: ActivityCallback | None, name: str, amount: int = 1) -> None:
    if type(amount) is not int or amount < 0:
        raise V12R8RecoveryProbeError("activity amount must be non-negative")
...
def pure_prefix_trace_audit(
    ...
    """Audit one closed R8 pure-policy prefix or complete episode."""
...
def recovery_prefix_gate(
    ...
    *,
    ...
    """Accept a normalized R8 physical prefix for labels, never autonomy."""

    _validate_config(config)
    if not isinstance(summary, Mapping):
        raise V12R8RecoveryProbeError("summary must be a mapping")
    ...
    return {
        ...
        "status": (
            "PASS_H0_V12R8_RECOVERABLE_R6_PREFIX"
            if passed
            else "FAIL_H0_V12R8_R6_PREFIX_INTEGRITY"
        ),
        "passed": passed,
...
def _default_tree_record(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise V12R8RecoveryProbeError("candidate escaped repository root") from exc
...
def run_recovery_probe(
    *,
    ...
    """Run one of exactly five new probes and normalize V26 before gating."""
    ...
    if artifact_root == REPO_ROOT:
        ...
        if root != expected_root:
            raise V12R8RecoveryProbeError(
                "canonical R8 probe destination drifted outside its case root"
            )
    if os.path.lexists(root):
        raise V12R8RecoveryProbeError(f"probe destination exists: {root}")
    candidate_path = (
        Path(
            module_path
    ...
    if not candidate_path.is_dir() or candidate_path.is_symlink():
        raise V12R8RecoveryProbeError("candidate checkpoint is missing or unsafe")
    record_tree = tree_recorder or _default_tree_record
    try:
        candidate_tree = dict(record_tree(candidate_path))
    except Exception as exc:
        raise V12R8RecoveryProbeError("candidate tree attestation failed") from exc
    if (
        ...
        raise V12R8RecoveryProbeError("candidate is not the frozen V12R6 actor")
    ...
    try:
        ...
        raise V12R8RecoveryProbeError("production rollout stack is not ready") from exc
    candidate = RLModule.from_checkpoint(candidate_path)
    eval_mode = getattr(candidate, "eval", None)
    if not callable(eval_mode):
        raise V12R8RecoveryProbeError("candidate lacks eval()")
    eval_mode()
    ...
    if innovations.shape != (contract.EXPECTED_STEPS, contract.EXPECTED_ACTION_DIM):
        raise V12R8RecoveryProbeError("frozen innovations shape drifted")
    if case["action_selection"] == "deterministic" and np_runtime.any(innovations != 0):
        raise V12R8RecoveryProbeError("deterministic innovations are not exact zero")
    built_config = builder(case)
    if not isinstance(built_config, Mapping):
        raise V12R8RecoveryProbeError("environment config builder was malformed")
    ...
    writer.start(
        {
            "schema_version": config.schema_version,
            "status": "STARTED_H0_V12R8_R6_RECOVERY_PROBE",
            "protocol_id": config.protocol_id,
            "stage_id": stage_id,
    ...
    try:
        ...
        if not isinstance(reset_info, Mapping):
            raise V12R8RecoveryProbeError("reset info is malformed")
        ...
        if not math.isfinite(body_weight_n) or body_weight_n <= 0.0:
            raise V12R8RecoveryProbeError("body weight is malformed")
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if not isinstance(info, Mapping):
                raise V12R8RecoveryProbeError("step info is malformed")
            ...
            if not isinstance(physical_step, Mapping):
                raise V12R8RecoveryProbeError("physical step audit is malformed")
        ...
        try:
            writer.publish_failure(
                end_reason="v12r8_recovery_probe_failed",
                error=exc,
                status="FAIL_H0_V12R8_RECOVERY_PROBE_RUNTIME",
            )
        except Exception:
            pass
        raise
    ...
    if audit is None or replay_recorder is None or not rows:
        raise V12R8RecoveryProbeError("probe audit/replay was not initialized")
    ...
    if not isinstance(raw_physical_summary, Mapping):
        raise V12R8RecoveryProbeError("physical summary is malformed")
    try:
        normalization = adjudicator.normalize_v26_prefix_summary(
            raw_physical_summary,
            frozen_target_contract_id=config.target_contract_id,
            attested_contract_source=contract.R7_CONTRACT_SOURCE_RECORD,
        )
    except adjudicator.V12R8AdjudicationError as exc:
        raise V12R8RecoveryProbeError("V26 summary normalization failed") from exc
    summary = {
    ...
    partial = {
        "schema_version": config.schema_version,
        "status": "PERSISTED_H0_V12R8_PROBE_BEFORE_GATE",
        "protocol_id": config.protocol_id,
        "stage_id": stage_id,
        "steps": len(rows),
...
def verify_probe_closure(
    ...
    *,
    ...
    """Re-read, hash-bind, and recompute one normalized R8 probe."""
    ...
    if persisted_gate != recomputed_gate or recomputed_gate.get("passed") is not True:
        raise V12R8RecoveryProbeError("recovery prefix gate is not closed PASS")
    expected_records = {
        "run_start": _artifact(root / "run_start.json", artifact_root),
    ...
    if any(receipt.get(name) != record for name, record in expected_records.items()):
        raise V12R8RecoveryProbeError("probe receipt artifact binding drifted")
    stage_id = f"collect_label__{config.case_id}"
    candidate_tree = copy.deepcopy(contract.LOCKED_INPUTS["r6_candidate"])
    summary_checks = {
        "schema_version": config.schema_version,
        "status": PROBE_COMPLETE_STATUS,
        "protocol_id": config.protocol_id,
    ...
    if type(summary.get("schema_version")) is not int or any(
        ...
        raise V12R8RecoveryProbeError("probe summary identity drifted")
    scalar_checks = {
        "schema_version": config.schema_version,
    ...
    if (
        ...
        raise V12R8RecoveryProbeError("probe receipt semantic binding drifted")
    if receipt.get("v26_summary_normalization") != summary.get(
        "v26_summary_normalization"
    ):
        raise V12R8RecoveryProbeError("normalization receipt binding drifted")
    if any(
        ...
        raise V12R8RecoveryProbeError("probe receipt zero counters drifted")
    if expected_replay is not None:
        if expected_replay.n_steps != replay.n_steps or any(
            ...
            raise V12R8RecoveryProbeError("probe replay changed across closure check")
    return {
        "passed": True,
        "summary": summary,
        "trace": trace,
...
def publish_r7_plus_adjudication(
    *, destination: str | Path, artifact_root: str | Path = REPO_ROOT
) -> dict[str, Any]:
    """Publish the R8 receipt for read-only R7 critical-prefix adjudication."""

    root = Path(artifact_root).expanduser().resolve()
    target = _inside(destination, root, label="adjudication destination")
    expected = REPO_ROOT.joinpath(*contract.R7_PLUS_ADJUDICATION_PATH.parts)
    if target != expected:
        raise V12R8RecoveryProbeError("adjudication destination is not canonical R8")
    result = adjudicator.load_and_adjudicate_r7_plus(artifact_root=root)
    if result.get("passed") is not True:
        raise V12R8RecoveryProbeError("R7 plus prefix adjudication failed")
    payload = {
        **copy.deepcopy(result),
        "status": ADJUDICATION_RECEIPT_STATUS,
    ...
    return verify_r7_plus_adjudication(destination=target, artifact_root=root)


def verify_r7_plus_adjudication(
    *, destination: str | Path, artifact_root: str | Path = REPO_ROOT
) -> dict[str, Any]:
    """Recompute and strictly compare the R8 historical-prefix receipt."""
    ...
    if observed != expected or observed.get("passed") is not True:
        raise V12R8RecoveryProbeError("R7 plus adjudication receipt drifted")
    return observed
...
def _persist_labels(
    *,
    ...
    if label_root.exists():
        raise V12R8RecoveryProbeError(f"label destination exists: {label_root}")
    ...
    if gate.get("passed") is not True:
        raise V12R8RecoveryProbeError("offline observer label gate failed")
    return {
        "passed": True,
...
def _check_offline_result(result: observer.ObserverLabelResult) -> None:
    if (
        ...
        raise V12R8RecoveryProbeError("offline same-state label invariant failed")
...
def label_recovery_probe(
    *,
    ...
    """Offline-label one newly collected, normalized R8 probe."""
    ...
    if offline_inputs_before != offline_inputs_after:
        raise V12R8RecoveryProbeError("offline label inputs changed during labeling")
...
def label_adjudicated_r7_plus(
    *,
    ...
    """Offline-label immutable R7 plus replay only after R8 adjudication."""
    ...
    expected_label_root = REPO_ROOT.joinpath(
        *contract.collection_case_root(contract.HISTORICAL_CASE_ID).parts,
        "observer_labels",
    )
    if label_root != expected_label_root:
        raise V12R8RecoveryProbeError(
            "historical labels must use canonical R8 namespace"
        )
    adjudication_receipt = verify_r7_plus_adjudication(
        destination=adjudication_path, artifact_root=root
    )
    if adjudication_receipt.get("offline_label_authorized") is not True:
        raise V12R8RecoveryProbeError("historical offline label is not authorized")
    ...
    def closure_validator(replay: observer.LoadedReplay) -> None:
        observed = verify_r7_plus_adjudication(
            destination=adjudication_path, artifact_root=root
        )
        evidence = adjudicator.load_r7_plus_evidence(artifact_root=root)
        expected = evidence["replay"]
        if (
            ...
            raise V12R8RecoveryProbeError("historical replay closure drifted")

    result = observer.label_closed_probe_in_memory(
        plus_root / "replay_boundaries.npz",
        probe_closure_validator=closure_validator,
        source_h0_path=source_h0_path,
    ...
    if offline_inputs_before != offline_inputs_after:
        raise V12R8RecoveryProbeError("offline label inputs changed during labeling")
    return _persist_labels(
        config_case_id=contract.HISTORICAL_CASE_ID,
        artifact_root=root,
        label_root=label_root,
        result=result,
        collection_integrity_passed=True,
        source_receipt=_artifact(adjudication_path, root),
        source_replay=_artifact(plus_root / "replay_boundaries.npz", root),
        offline_inputs=offline_inputs_before,
    ...
    try:
        with np.load(path, allow_pickle=False) as archive:
            if set(archive.files) != set(observer.LABEL_ARRAY_DTYPES):
                raise V12R8RecoveryProbeError("observer label NPZ schema drifted")
            arrays = {
                name: np.ascontiguousarray(archive[name].copy())
                for name in archive.files
            }
    except V12R8RecoveryProbeError:
        raise
    except Exception as exc:
        raise V12R8RecoveryProbeError("observer label NPZ is unreadable") from exc
    rows = len(arrays["observations"])
    try:
        observer.validate_observer_label_arrays(arrays, rows=rows)
    except Exception as exc:
        raise V12R8RecoveryProbeError("observer label NPZ validation failed") from exc
    return arrays
...
def _verify_label_stage_binding(
    *,
    ...
    historical = case_id == contract.HISTORICAL_CASE_ID
    stage_path = (
        artifact_root.joinpath(*contract.R7_PLUS_LABEL_STAGE_RECEIPT_PATH.parts)
        if historical
        else artifact_root.joinpath(
            *contract.collection_case_root(case_id).parts, "stage_receipt.json"
        )
    )
    stage = _strict_mapping(stage_path)
    expected_stage_id = (
        "label_r7_plus_prefix" if historical else f"collect_label__{case_id}"
    )
    ...
    if historical:
        expected_records["adjudication_stage_receipt"] = artifact_root.joinpath(
            *contract.R7_PLUS_ADJUDICATION_STAGE_RECEIPT_PATH.parts
        )
    else:
    ...
    common = (
        ...
        and (historical or stage.get("single_collection_round") is True)
    )
    if not common:
        raise V12R8RecoveryProbeError("observer label stage binding drifted")
...
def verify_observer_label_closure(
    ...
    *,
    ...
    if case_id not in contract.COLLECTION_CASE_IDS:
        raise V12R8RecoveryProbeError(f"unknown observer case: {case_id!r}")
    if type(require_stage_receipt) is not bool:
        raise V12R8RecoveryProbeError("require_stage_receipt must be strict bool")
    root = Path(artifact_root).expanduser().resolve()
    teacher_path = (
    ...
    historical = case_id == contract.HISTORICAL_CASE_ID
    if historical:
        source_receipt_path = root.joinpath(*contract.R7_PLUS_ADJUDICATION_PATH.parts)
        replay_path = root.joinpath(*contract.R7_PLUS_REPLAY_PATH.parts)
        ...
        def closure_validator(replay: observer.LoadedReplay) -> None:
            current = verify_r7_plus_adjudication(
                destination=source_receipt_path, artifact_root=root
            )
            evidence = adjudicator.load_r7_plus_evidence(artifact_root=root)
            expected = evidence["replay"]
    ...
    if observed_summary != expected_summary:
        raise V12R8RecoveryProbeError("observer label summary semantic drifted")
    ...
    if observed_gate != expected_gate or expected_gate.get("passed") is not True:
        raise V12R8RecoveryProbeError("observer label gate semantic drifted")
    ...
    if observed_receipt != expected_receipt:
        raise V12R8RecoveryProbeError("observer label receipt semantic drifted")
...
def run_probe_and_label(
    *,
    ...
    if probe.get("passed") is not True:
        raise V12R8RecoveryProbeError("probe prefix is not labelable")
...
__all__ = [
    ...
    "RECEIPT_STATUS",
    "RecoveryProbeConfig",
    "V12R8RecoveryProbeError",
    "attest_offline_label_inputs",
    "canonical_probe_case",
    "label_adjudicated_r7_plus",
    "label_recovery_probe",
    "publish_r7_plus_adjudication",
