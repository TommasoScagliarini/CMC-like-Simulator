"""Single actor-only recovery fitter for the V12R8 lineage.
...
``+0.20`` prefix, and the six V12R8 observer-label NPZ files.  The rows are
...
import h0_v12r8_recovery_probe as recovery_probe  # noqa: E402
import warm_start  # noqa: E402


class V12R8RecoveryFitError(RuntimeError):
    """Raised when an R8 corpus, fit, or publication invariant drifts."""
...
FIT_COMPLETE_STATUS = "COMPLETE_H0_V12R8_RECOVERY_FIT"
FIT_PASS_STATUS = "PASS_H0_V12R8_RECOVERY_FIT_RECEIPT"
CANDIDATE_BUILD_STATUS = "H0_V12R8_RECOVERY_CANDIDATE_BUILD_PASS"
...
def _resolve(path: str | PurePath | Path) -> Path:
    ...
    try:
        ...
        raise V12R8RecoveryFitError(f"path escapes repository root: {path}") from exc
    return resolved
...
def _record(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    if not _regular_file(resolved):
        raise V12R8RecoveryFitError(f"unsafe or missing artifact: {resolved}")
    return {
...
def _tree_record(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8RecoveryFitError(f"unsafe or missing tree: {root}") from exc
    if not stat.S_ISDIR(root_status.st_mode) or _is_link_or_reparse(root):
        raise V12R8RecoveryFitError(f"unsafe or missing tree: {root}")
    files: list[Path] = []
    for current_text, directory_names, file_names in os.walk(
        ...
        for name in directory_names:
            ...
            if _is_link_or_reparse(child) or not stat.S_ISDIR(os.lstat(child).st_mode):
                raise V12R8RecoveryFitError(f"unsafe tree directory: {child}")
        for name in file_names:
            child = current / name
            if not _regular_file(child) or _is_link_or_reparse(child):
                raise V12R8RecoveryFitError(f"unsafe tree file: {child}")
            files.append(child)
    if not files:
        raise V12R8RecoveryFitError(f"empty artifact tree: {root}")
...
def _mapping(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8RecoveryFitError(f"invalid strict JSON: {resolved}") from exc
    if not isinstance(payload, Mapping):
        raise V12R8RecoveryFitError(f"expected JSON object: {resolved}")
    return dict(payload)
...
def _read_npz_exact(
    ...
    _reject_link_components(resolved, include_leaf=True)
    if not _regular_file(resolved):
        raise V12R8RecoveryFitError(f"unsafe or missing NPZ: {resolved}")
    try:
    ...
    except V12R8RecoveryFitError:
        raise
    except Exception as exc:
        raise V12R8RecoveryFitError(f"cannot read exact NPZ: {resolved}") from exc
...
def _locked_artifact_exact(name: str, path: str | PurePath | Path) -> None:
    ...
    if _record(path) != {
        ...
        raise V12R8RecoveryFitError(f"locked input drifted: {name}")
...
def attest_locked_inputs() -> dict[str, Any]:
    ...
    if (
        ...
        raise V12R8RecoveryFitError("locked R6 candidate tree drifted")
    source_h0 = _tree_record(contract.SOURCE_H0_MODULE_PATH)
    if not _strict_equal(source_h0, contract.LOCKED_SOURCE_H0_TREE):
        raise V12R8RecoveryFitError("locked source H0 teacher tree drifted")
    return {
...
def _load_base_piece() -> dict[str, np.ndarray]:
    ...
    if (
        ...
        raise V12R8RecoveryFitError("locked R5 base corpus semantic drifted")
    return arrays
...
def _load_r4_piece(expected_features: np.ndarray) -> dict[str, np.ndarray]:
    ...
    if (
        ...
        raise V12R8RecoveryFitError("locked R4 failure labels drifted")
    arrays["episode_ids"] = np.repeat(
        np.asarray(["v12r8_source:r4_failed_plus"], dtype="U192"), rows
    )
    return arrays
...
def observer_labels_path(case_id: str) -> PurePosixPath:
    if case_id not in contract.COLLECTION_CASE_IDS:
        raise V12R8RecoveryFitError(f"unknown observer case: {case_id!r}")
...
def _load_observer_piece(
    ...
    *,
    ...
    if (
        ...
        raise V12R8RecoveryFitError(f"observer label NPZ drifted: {case_id}")
    arrays["episode_ids"] = np.repeat(
        np.asarray([f"v12r8_observer:{case_id}"], dtype="U192"), rows
    )
    return arrays
...
def compute_equal_stratum_weights(
    ...
    if strata.ndim != 1 or strata.dtype.kind != "U" or len(strata) == 0:
        raise V12R8RecoveryFitError("stratum ids must be a non-empty Unicode vector")
    expected = expected_stratum_ids()
    if set(strata.astype(str)) != set(expected):
        raise V12R8RecoveryFitError("stratum id set drifted")
    ...
    for stratum_id in expected:
        ...
        if len(selected) == 0:
            raise V12R8RecoveryFitError(f"empty stratum: {stratum_id}")
        ...
        if not math.isclose(mass, target, rel_tol=0.0, abs_tol=1.0e-9):
            raise V12R8RecoveryFitError(f"stratum mass failed closure: {stratum_id}")
    ...
    if (
        ...
        raise V12R8RecoveryFitError("equal-stratum weights failed closure")
...
def load_recovery_corpus() -> RecoveryCorpusBundle:
    ...
    if (
        ...
        raise V12R8RecoveryFitError("assembled recovery corpus failed closure")
    corpus = RecoveryFitCorpus(
...
def _assert_array_mapping_byte_exact(
    ...
    *,
    ...
    if set(expected) != set(observed):
        raise V12R8RecoveryFitError(f"{label} array schema drifted")
    for name in expected:
        ...
        if (
            ...
            raise V12R8RecoveryFitError(f"{label} array bytes drifted: {name}")
...
def verify_persisted_corpus(
    ...
    *,
    ...
    _assert_array_mapping_byte_exact(expected, observed, label="assembled R8 corpus")
    return {
        "passed": True,
        "corpus": _record(path),
...
def adamw_learning_rate(epoch: int) -> float:
    ...
    if type(epoch) is not int or not 1 <= epoch <= epochs:
        raise V12R8RecoveryFitError(f"AdamW epoch outside 1..{epochs}: {epoch!r}")
    if epoch <= 1000:
        return 3.0e-4
    if epoch <= 1700:
        return 1.0e-4
...
def validate_source_r6_state(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if set(state) != set(shapes):
        raise V12R8RecoveryFitError("R6 actor state key set drifted")
    ...
    for name in shapes:
        ...
        if array.dtype != np.dtype(np.float32):
            raise V12R8RecoveryFitError(f"R6 actor tensor dtype drifted: {name}")
        arrays[name] = array
    if any(arrays[name].shape != shape for name, shape in shapes.items()):
        raise V12R8RecoveryFitError("R6 actor state shape drifted")
    aliases = (
    ...
    if (
        ...
        raise V12R8RecoveryFitError("R6 actor state semantic drifted")
    ...
    if not np.allclose(
        ...
        raise V12R8RecoveryFitError("R6 actor logstd/sigma drifted")
    return {
        "hidden_dims": [512, 512],
...
def _load_source_module_and_state() -> tuple[Any, dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise V12R8RecoveryFitError("R6 source module topology drifted")
    ...
    if (
        ...
        raise V12R8RecoveryFitError("R6 source actor manifest drifted")
...
def _new_normalized_model(
    ...
    if not _positive_zero(
        ...
        raise V12R8RecoveryFitError("normalized model re-enabled clock columns")
    return model
...
def _fold_normalization_into_state(
    ...
    if not _positive_zero(normalized_w0[:, contract.DISABLED_CLOCK_COLUMNS]):
        raise V12R8RecoveryFitError("trained clock columns are not positive zero")
    ...
    if not _positive_zero(raw_w0[:, contract.DISABLED_CLOCK_COLUMNS]):
        raise V12R8RecoveryFitError("folded clock columns are not positive zero")
...
def fit_recovery_full_mean_in_memory(
    *,
    ...
    """Run the sole fixed AdamW(2000)+LBFGS(300/600) R8 fit."""
    ...
    if (
        ...
        raise V12R8RecoveryFitError("R8 weighted fit arrays are malformed")
    ...
    try:
        ...
        for epoch in range(1, 2001):
            ...
            if not torch.isfinite(loss):
                raise V12R8RecoveryFitError(f"non-finite AdamW loss at epoch {epoch}")
            loss.backward()
        ...
        if last_lbfgs_loss is None:
            raise V12R8RecoveryFitError("LBFGS never evaluated the objective")
        v11._milestone(
            history,
            stage="lbfgs_final",
            index=closure_calls,
        ...
        if not preservation["passed"]:
            raise V12R8RecoveryFitError("mean-only preservation audit failed")
        ...
        raise V12R8RecoveryFitError("fixed R8 recovery fit failed") from exc
    finally:
...
def _metric_pair(
    ...
    if selected.ndim != 1 or len(selected) == 0:
        raise V12R8RecoveryFitError("metric slice is empty or malformed")
...
def recompute_fit_metric_payload(
    ...
    if predictions.shape != corpus.actions.shape or not np.all(
        ...
        raise V12R8RecoveryFitError("recomputed candidate predictions are malformed")
    base_case_metrics = {
    ...
    if reset_error.size == 0:
        raise V12R8RecoveryFitError("recomputed reset metric slice is empty")
...
def _assert_fit_metric_payload(
    ...
    *,
    ...
    if drifted:
        raise V12R8RecoveryFitError(f"{label} metrics drifted: {drifted}")
...
def _candidate_actor_manifest(
    *, state: Mapping[str, Any], feature_names: Sequence[str], module_state: Path
    ...
    return {
        "schema_version": 1,
        "status": "H0_V12R8_RECOVERY_ACTOR_FEATURE_CONTRACT",
...
def _save_candidate_exact(
    *,
    ...
    if not all(
        ...
        raise V12R8RecoveryFitError("candidate core save/reload drifted")
    ...
    if set(actor_manifest) != ACTOR_MANIFEST_FIELDS:
        raise V12R8RecoveryFitError("actor manifest schema drifted")
    ...
    if set(build_manifest) != BUILD_MANIFEST_FIELDS:
        raise V12R8RecoveryFitError("candidate build manifest schema drifted")
    forensic.write_json_exclusive(
    ...
    if observed != EXPECTED_CANDIDATE_FILES:
        raise V12R8RecoveryFitError(f"candidate file set drifted: {sorted(observed)}")
    ...
    if not all(
        ...
        raise V12R8RecoveryFitError("candidate final save/reload drifted")
...
def run_fit_stage(
    *,
    ...
    """Execute and exclusively publish the sole R8 recovery actor fit."""

    destination = _resolve(contract.FIT_ROOT)
    if os.path.lexists(destination):
        raise V12R8RecoveryFitError("R8 fit destination exists/no-clobber")
    ...
    if tuple(source_manifest["actor_feature_names"]) != tuple(
        ...
        raise V12R8RecoveryFitError("source/corpus actor feature order drifted")
    ...
    if len(base_v8) != 3000:
        raise V12R8RecoveryFitError("frozen normalization base row count drifted")
    ...
    if source_tree_before != _tree_record(contract.R6_CANDIDATE_MODULE_PATH):
        raise V12R8RecoveryFitError("R6 source candidate changed during fit")
    ...
    corpus_manifest = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "H0_V12R8_RECOVERY_CORPUS_PASS",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "fit_contract_id": contract.FIT_CONTRACT_ID,
    ...
    if not _bytes_equal(metric_payload["predictions"], result.predictions):
        raise V12R8RecoveryFitError("fit result predictions drifted from saved state")
    ...
    report = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "COMPLETE_H0_V12R8_RECOVERY_ADAPTATION_REPORT",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "fit_contract_id": contract.FIT_CONTRACT_ID,
    ...
    if gate.get("passed") is not True:
        raise V12R8RecoveryFitError("R8 offline fit gate failed")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
    ...
    if set(receipt) != FIT_RECEIPT_FIELDS:
        raise V12R8RecoveryFitError("fit receipt schema drifted")
    forensic.write_json_exclusive(receipt_path, receipt)
    if not _strict_equal(attest_locked_inputs(), locked_inputs_before):
        raise V12R8RecoveryFitError("locked inputs changed during fit")
    return receipt
...
def verify_fit_stage() -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R8RecoveryFitError("adaptation history is not strict JSON") from exc
    if not isinstance(history, list) or not history:
        raise V12R8RecoveryFitError("adaptation history is empty or malformed")
    ...
    if not _strict_equal(corpus_manifest, expected_corpus_manifest):
        raise V12R8RecoveryFitError("corpus manifest semantic closure drifted")

    module = _tree_record(contract.CANDIDATE_MODULE_PATH)
    if {row["path"] for row in module["files"]} != EXPECTED_CANDIDATE_FILES or module[
        "file_count"
    ] != 5:
        raise V12R8RecoveryFitError("candidate five-file tree drifted")
    ...
    if (
        ...
        raise V12R8RecoveryFitError("candidate manifest schema drifted")
    _source_module, source_state, source_manifest = _load_source_module_and_state()
    from ray.rllib.core.rl_module.rl_module import RLModule
    ...
    if preservation.get("passed") is not True:
        raise V12R8RecoveryFitError("candidate mean-only preservation closure failed")
    ...
    if not isinstance(normalization_audit, Mapping) or not isinstance(
        ...
        raise V12R8RecoveryFitError("fit audit payload is malformed")
    ...
    report_static_valid = (
        ...
        and report.get("status") == "COMPLETE_H0_V12R8_RECOVERY_ADAPTATION_REPORT"
    ...
    if not all(checks.values()):
        ...
        raise V12R8RecoveryFitError(f"R8 fit verification failed: {failed}")
    if not _strict_equal(attest_locked_inputs(), locked_inputs_before):
        raise V12R8RecoveryFitError("locked inputs changed during fit verification")
    return receipt


__all__ = [
    "V12R8RecoveryFitError",
