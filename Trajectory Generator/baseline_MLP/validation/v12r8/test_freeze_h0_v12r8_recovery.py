"""Focused source-only tests for the V12R8 protocol freezer."""
...
def test_canonical_path_parser_rejects_escape_and_cross_platform_aliases(
    ...
    for unsafe in (
        ...
        with pytest.raises(freezer.V12R8FreezeError, match="non-canonical"):
...
def test_artifact_and_tree_records_reject_symlink_entries(
    ...
    with pytest.raises(freezer.V12R8FreezeError, match="symlink|unsafe"):
        freezer.artifact_record("safe/linked.bin")
    with pytest.raises(freezer.V12R8FreezeError, match="unsafe tree"):
...
def test_locked_r6_inputs_and_complete_source_closure_are_exact() -> None:
    ...
    assert attestation["r7_plus_adjudication"]["passed"] is True
    assert attestation["r7_plus_adjudication"]["offline_label_authorized"] is True
    assert attestation["r7_plus_adjudication"]["r7_probe_rerun"] is False
    assert attestation["r7_plus_adjudication"]["r7_artifacts_modified"] is False
    assert attestation["r7_evidence_records"] == contract.LOCKED_R7_EVIDENCE
    assert (
...
def test_runtime_and_verifier_boolean_contracts_are_type_strict() -> None:
    ...
    for drifted in (
        ...
        with pytest.raises(freezer.V12R8FreezeError, match="readiness"):
            freezer._strict_runtime(lambda drifted=drifted: drifted)

    with pytest.raises(freezer.V12R8FreezeError, match="strict bool"):
