"""Focused source-only orchestration tests for the V12R8 one-shot runner."""
...
import run_h0_v12r8_recovery as runner  # noqa: E402
...
def test_activity_and_absolute_path_guards_are_strict(
    ...
    for amount in (True, -1):
        with pytest.raises(runner.V12R8ExecutionError, match="non-negative int"):
            runner._increment(activity, "environment_step_calls", amount)
    with pytest.raises(runner.V12R8ExecutionError, match="escaped repository"):
        runner._path(tmp_path.parent / "outside.json")
    with pytest.raises(runner.V12R8ExecutionError, match="unsafe repository path"):
...
def test_failed_first_stage_terminalizes_once_and_never_retries(
    ...
    with pytest.raises(runner.V12R8ExecutionError, match="stopped terminally"):
        runner.execute()
    assert calls == 1
...
def test_terminal_fail_rejects_mutated_attempted_stage(
    ...
    with pytest.raises(runner.V12R8ExecutionError, match="forensic prefix"):
...
def test_terminal_fail_dispatches_semantic_verifier_for_forged_completed_stage(
    ...
    def reject_semantic(_case_id: str) -> dict[str, Any]:
        raise runner.V12R8ExecutionError("synthetic completed-stage semantic drift")

    monkeypatch.setattr(runner, "verify_collection_label_receipt", reject_semantic)
    with pytest.raises(runner.V12R8ExecutionError, match="semantic drift"):
        runner.verify_terminal_ledger()
...
def test_terminal_verifier_rejects_locked_input_drift_at_exit(
    ...
    with pytest.raises(runner.V12R8ExecutionError, match="locked inputs changed"):
        runner.verify_terminal_ledger()
    assert calls == 2
...
def test_terminal_fail_rejects_control_and_candidate_mutations(
    ...
    ledger_path.unlink()
    runner.forensic.write_json_exclusive(ledger_path, ledger)
    with pytest.raises(runner.V12R8ExecutionError):
        runner.verify_terminal_ledger()
...
def test_terminal_fail_rejects_extra_schema_field(
    ...
    ledger_path.unlink()
    runner.forensic.write_json_exclusive(ledger_path, ledger)
    with pytest.raises(runner.V12R8ExecutionError, match="schema drifted"):
        runner.verify_terminal_ledger()
...
def test_terminal_rejects_rehashed_semantically_forged_pipeline_claim(
    ...
    ledger_path.unlink()
    runner.forensic.write_json_exclusive(ledger_path, ledger)
    with pytest.raises(runner.V12R8ExecutionError, match="pipeline claim semantic"):
        runner.verify_terminal_ledger()
...
def test_attempted_worker_claim_and_claim_directory_are_semantic_exact(
    ...
    with pytest.raises(runner.V12R8ExecutionError, match="worker claim semantic"):
        runner.verify_terminal_ledger()
    ...
    with pytest.raises(runner.V12R8ExecutionError, match="claims directory"):
        runner.verify_terminal_ledger()
...
def test_stage_order_and_scoped_activity_distinguish_historical_and_live_env() -> None:
    ...
    activity = runner._zero_activity()
    runner._increment_scoped(activity, "new_collection", "environment_reset_calls", 5)
    ...
    assert activity["new_collection_environment_reset_calls"] == 5
    assert activity["development_environment_reset_calls"] == 6
    assert activity["environment_reset_calls"] == 11
...
def test_write_surface_rejects_every_non_r8_namespace(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(runner, "REPO_ROOT", tmp_path)
    monkeypatch.setattr(runner.freezer, "REPO_ROOT", tmp_path)
    monkeypatch.setattr(contract, "ROOT", PurePosixPath("validation/v12r8"))
    with pytest.raises(runner.V12R8ExecutionError, match="exclusive R8 namespace"):
        runner._write(
