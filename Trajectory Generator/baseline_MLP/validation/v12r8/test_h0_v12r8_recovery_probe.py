...
def _closed_fixture(
    ...
    normalization = probe.adjudicator.normalize_v26_prefix_summary(
        ...
        attested_contract_source=probe.contract.R7_CONTRACT_SOURCE_RECORD,
    )
    summary = {
...
def test_probe_rejects_historical_plus_rerun(tmp_path: Path) -> None:
    with pytest.raises(probe.V12R8RecoveryProbeError, match="rerun is forbidden"):
...
def test_tampered_normalization_receipt_blocks_label_loader(tmp_path: Path) -> None:
    ...
    with pytest.raises(probe.V12R8RecoveryProbeError, match="normalization"):
...
def test_probe_closure_rejects_rehashed_summary_identity_drift(tmp_path: Path) -> None:
    ...
    with pytest.raises(probe.V12R8RecoveryProbeError, match="summary identity"):
        probe.verify_probe_closure(root, config=config)


def test_historical_label_cannot_reach_h0_without_adjudication(
    ...
    label_root = tmp_path.joinpath(
        *probe.contract.collection_case_root(probe.contract.HISTORICAL_CASE_ID).parts,
        "observer_labels",
    )
    with pytest.raises(probe.V12R8RecoveryProbeError, match="not authorized"):
