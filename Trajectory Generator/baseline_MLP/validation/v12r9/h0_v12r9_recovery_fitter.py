"""Single actor-only recovery fitter for the V12R9 lineage.
...
``+0.20`` prefix, and the six V12R9 observer-label NPZ files.  The rows are
...
import h0_v12r9_recovery_probe as recovery_probe  # noqa: E402
import warm_start  # noqa: E402


class V12R9RecoveryFitError(RuntimeError):
    """Raised when an R9 corpus, fit, or publication invariant drifts."""
...
FIT_COMPLETE_STATUS = "COMPLETE_H0_V12R9_RECOVERY_FIT"
FIT_PASS_STATUS = "PASS_H0_V12R9_RECOVERY_FIT_RECEIPT"
CANDIDATE_BUILD_STATUS = "H0_V12R9_RECOVERY_CANDIDATE_BUILD_PASS"
...
def _resolve(path: str | PurePath | Path) -> Path:
    ...
    try:
        ...
        raise V12R9RecoveryFitError(f"path escapes repository root: {path}") from exc
    return resolved
...
def _record(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    if not _regular_file(resolved):
        raise V12R9RecoveryFitError(f"unsafe or missing artifact: {resolved}")
    return {
...
def _tree_record(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R9RecoveryFitError(f"unsafe or missing tree: {root}") from exc
    if not stat.S_ISDIR(root_status.st_mode) or _is_link_or_reparse(root):
        raise V12R9RecoveryFitError(f"unsafe or missing tree: {root}")
    files: list[Path] = []
    for current_text, directory_names, file_names in os.walk(
        ...
        for name in directory_names:
            ...
            if _is_link_or_reparse(child) or not stat.S_ISDIR(os.lstat(child).st_mode):
                raise V12R9RecoveryFitError(f"unsafe tree directory: {child}")
        for name in file_names:
            child = current / name
            if not _regular_file(child) or _is_link_or_reparse(child):
                raise V12R9RecoveryFitError(f"unsafe tree file: {child}")
            files.append(child)
    if not files:
        raise V12R9RecoveryFitError(f"empty artifact tree: {root}")
...
def _mapping(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R9RecoveryFitError(f"invalid strict JSON: {resolved}") from exc
    if not isinstance(payload, Mapping):
        raise V12R9RecoveryFitError(f"expected JSON object: {resolved}")
    return dict(payload)
...
def _read_npz_exact(
    ...
    _reject_link_components(resolved, include_leaf=True)
    if not _regular_file(resolved):
        raise V12R9RecoveryFitError(f"unsafe or missing NPZ: {resolved}")
    try:
    ...
    except V12R9RecoveryFitError:
        raise
    except Exception as exc:
        raise V12R9RecoveryFitError(f"cannot read exact NPZ: {resolved}") from exc
...
def _locked_artifact_exact(name: str, path: str | PurePath | Path) -> None:
    ...
    if _record(path) != {
        ...
        raise V12R9RecoveryFitError(f"locked input drifted: {name}")
...
def attest_locked_inputs() -> dict[str, Any]:
    ...
    _locked_artifact_exact(
        "coverage_reference_corpus", contract.COVERAGE_REFERENCE_CORPUS_PATH
    )
    source_tree = _tree_record(contract.R6_CANDIDATE_MODULE_PATH)
    expected_tree = contract.LOCKED_INPUTS["r6_candidate"]
    if (
        not _strict_equal(source_tree, expected_tree)
        or {row["path"] for row in source_tree["files"]} != EXPECTED_SOURCE_FILES
    ):
        raise V12R9RecoveryFitError("locked R6 candidate tree drifted")
    source_h0 = _tree_record(contract.SOURCE_H0_MODULE_PATH)
    if not _strict_equal(source_h0, contract.LOCKED_SOURCE_H0_TREE):
        raise V12R9RecoveryFitError("locked source H0 teacher tree drifted")
    return {
        ...
        "coverage_reference_corpus": _record(contract.COVERAGE_REFERENCE_CORPUS_PATH),
    }
...
def _load_base_piece() -> dict[str, np.ndarray]:
    ...
    if (
        ...
        raise V12R9RecoveryFitError("locked R5 base corpus semantic drifted")
    return arrays
...
def _load_r4_piece(expected_features: np.ndarray) -> dict[str, np.ndarray]:
    ...
    if (
        ...
        raise V12R9RecoveryFitError("locked R4 failure labels drifted")
    arrays["episode_ids"] = np.repeat(
        np.asarray(["v12r9_source:r4_failed_plus"], dtype="U192"), rows
    )
    return arrays
...
def observer_labels_path(case_id: str) -> PurePosixPath:
    if case_id not in contract.COLLECTION_CASE_IDS:
        raise V12R9RecoveryFitError(f"unknown observer case: {case_id!r}")
...
def _load_observer_piece(
    ...
    *,
    ...
    if (
        ...
        raise V12R9RecoveryFitError(f"observer label NPZ drifted: {case_id}")
    arrays["episode_ids"] = np.repeat(
        np.asarray([f"v12r9_observer:{case_id}"], dtype="U192"), rows
    )
    return arrays
...
def compute_equal_stratum_weights(
    ...
    if strata.ndim != 1 or strata.dtype.kind != "U" or len(strata) == 0:
        raise V12R9RecoveryFitError("stratum ids must be a non-empty Unicode vector")
    expected = expected_stratum_ids()
    if set(strata.astype(str)) != set(expected):
        raise V12R9RecoveryFitError("stratum id set drifted")
    ...
    for stratum_id in expected:
        ...
        if len(selected) == 0:
            raise V12R9RecoveryFitError(f"empty stratum: {stratum_id}")
        ...
        if not math.isclose(mass, target, rel_tol=0.0, abs_tol=1.0e-9):
            raise V12R9RecoveryFitError(f"stratum mass failed closure: {stratum_id}")
    ...
    if (
        ...
        raise V12R9RecoveryFitError("equal-stratum weights failed closure")
...
def verify_observer_label_closures() -> dict[str, dict[str, Any]]:
    """Recompute all six offline labels from their closed source replays."""

    closures: dict[str, dict[str, Any]] = {}
    for case_id in contract.COLLECTION_CASE_IDS:
        try:
            closures[case_id] = recovery_probe.verify_observer_label_closure(case_id)
        except Exception as exc:
            raise V12R9RecoveryFitError(
                f"observer label semantic closure failed: {case_id}"
            ) from exc
    return closures


def load_recovery_corpus() -> RecoveryCorpusBundle:
    """Load eight NPZ inputs and bind their exact thirteen-stratum assembly."""

    locked = attest_locked_inputs()
    label_closures = verify_observer_label_closures()
    ...
    if (
        ...
        raise V12R9RecoveryFitError("assembled recovery corpus failed closure")
    corpus = RecoveryFitCorpus(
        ...
        source_records={
            ...
            "observer_label_semantic_closures": copy.deepcopy(label_closures),
        },
        probe_label_bindings=tuple(
            copy.deepcopy(label_closures[case_id])
            for case_id in contract.COLLECTION_CASE_IDS
        ),
...
def assembled_corpus_arrays(bundle: RecoveryCorpusBundle) -> dict[str, np.ndarray]:
    arrays = {
        name: np.ascontiguousarray(value)
        for name, value in bundle.corpus.arrays().items()
    }
    arrays["stratum_ids"] = np.ascontiguousarray(bundle.stratum_ids, dtype="U96")
    return arrays
...
def _assert_array_mapping_byte_exact(
    ...
    *,
    ...
    if set(expected) != set(observed):
        raise V12R9RecoveryFitError(f"{label} array schema drifted")
    for name in expected:
        ...
        if (
            ...
            raise V12R9RecoveryFitError(f"{label} array bytes drifted: {name}")


def verify_persisted_corpus(
    bundle: RecoveryCorpusBundle,
    *,
    path: str | PurePath | Path = contract.CORPUS_PATH,
) -> dict[str, Any]:
    """Compare all corpus arrays and the explicit 13-stratum mapping bytewise."""

    expected = assembled_corpus_arrays(bundle)
    observed = _read_npz_exact(path, set(expected))
    _assert_array_mapping_byte_exact(expected, observed, label="assembled R9 corpus")
    return {
        "passed": True,
        "corpus": _record(path),
        "array_names": sorted(expected),
        "row_count": int(len(bundle.corpus.observations)),
        "stratum_count": len(set(bundle.stratum_ids.astype(str))),
        "stratum_ids_sha256": v10s_fit.array_sha256(bundle.stratum_ids),
    }
...
def adamw_learning_rate(epoch: int) -> float:
    ...
    if type(epoch) is not int or not 1 <= epoch <= epochs:
        raise V12R9RecoveryFitError(f"AdamW epoch outside 1..{epochs}: {epoch!r}")
    if epoch <= 1000:
        return 3.0e-4
    if epoch <= 1700:
        return 1.0e-4
...
def validate_source_r6_state(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if set(state) != set(shapes):
        raise V12R9RecoveryFitError("R6 actor state key set drifted")
    ...
    for name in shapes:
        ...
        if array.dtype != np.dtype(np.float32):
            raise V12R9RecoveryFitError(f"R6 actor tensor dtype drifted: {name}")
        arrays[name] = array
    if any(arrays[name].shape != shape for name, shape in shapes.items()):
        raise V12R9RecoveryFitError("R6 actor state shape drifted")
    aliases = (
    ...
    if (
        ...
        raise V12R9RecoveryFitError("R6 actor state semantic drifted")
    ...
    if not np.allclose(
        ...
        raise V12R9RecoveryFitError("R6 actor logstd/sigma drifted")
    return {
        "hidden_dims": [512, 512],
...
def _load_source_module_and_state() -> tuple[Any, dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise V12R9RecoveryFitError("R6 source module topology drifted")
    ...
    if (
        ...
        raise V12R9RecoveryFitError("R6 source actor manifest drifted")
...
def _new_normalized_model(
    ...
    if not _positive_zero(
        ...
        raise V12R9RecoveryFitError("normalized model re-enabled clock columns")
    return model
...
def _fold_normalization_into_state(
    ...
    if not _positive_zero(normalized_w0[:, contract.DISABLED_CLOCK_COLUMNS]):
        raise V12R9RecoveryFitError("trained clock columns are not positive zero")
    ...
    if not _positive_zero(raw_w0[:, contract.DISABLED_CLOCK_COLUMNS]):
        raise V12R9RecoveryFitError("folded clock columns are not positive zero")
...
def fit_recovery_full_mean_in_memory(
    *,
    ...
    """Run the sole fixed AdamW(2000)+LBFGS(300/600) R9 fit."""
    ...
    if (
        ...
        raise V12R9RecoveryFitError("R9 weighted fit arrays are malformed")
    ...
    try:
        ...
        for epoch in range(1, 2001):
            ...
            if not torch.isfinite(loss):
                raise V12R9RecoveryFitError(f"non-finite AdamW loss at epoch {epoch}")
            loss.backward()
        ...
        if last_lbfgs_loss is None:
            raise V12R9RecoveryFitError("LBFGS never evaluated the objective")
        v11._milestone(
            history,
            stage="lbfgs_final",
            index=closure_calls,
        ...
        if not preservation["passed"]:
            raise V12R9RecoveryFitError("mean-only preservation audit failed")
        ...
        raise V12R9RecoveryFitError("fixed R9 recovery fit failed") from exc
    finally:
...
def _metric_pair(
    ...
    if selected.ndim != 1 or len(selected) == 0:
        raise V12R9RecoveryFitError("metric slice is empty or malformed")
...
def _runtime_state_logits_exact(
    module: Any,
    candidate_state: Mapping[str, Any],
    observations: np.ndarray,
) -> np.ndarray:
    """Prove that the reloaded RLModule executes the audited state tensors."""

    values = np.ascontiguousarray(observations, dtype=np.float32)
    runtime_logits = r6_builder._logits(module, values)  # noqa: SLF001
    state_logits = np.ascontiguousarray(
        v11._state_logits(candidate_state, values), dtype=np.float32
    )
    if (
        runtime_logits.dtype != state_logits.dtype
        or runtime_logits.shape != state_logits.shape
        or runtime_logits.tobytes(order="C") != state_logits.tobytes(order="C")
    ):
        raise V12R9RecoveryFitError(
            "reloaded RLModule logits differ from candidate state tensors"
        )
    return runtime_logits
...
def recompute_fit_metric_payload(
    ...
    """Run offline candidate inference and rebuild every persisted fit metric."""

    corpus = bundle.corpus
    predictions = np.ascontiguousarray(
        v11._state_logits(candidate_state, corpus.observations)[
            :, : contract.EXPECTED_ACTION_DIM
        ],
        dtype=np.float32,
    )
    if predictions.shape != corpus.actions.shape or not np.all(
        np.isfinite(predictions)
    ):
        raise V12R9RecoveryFitError("recomputed candidate predictions are malformed")
    base_case_metrics = {
    ...
    if reset_error.size == 0:
        raise V12R9RecoveryFitError("recomputed reset metric slice is empty")
    ...
    return {
        ...
        "r4_failed_plus_metrics": _metric_pair(
            predictions, corpus.actions, bundle.r4_indices
        ),
...
def _assert_fit_metric_payload(
    observed: Mapping[str, Any],
    recomputed: Mapping[str, Any],
    *,
    include_worst_row: bool,
    label: str,
) -> None:
    fields = FIT_METRIC_FIELDS if include_worst_row else FIT_METRIC_FIELDS[:-1]
    drifted = [
        name
        for name in fields
    ...
    if drifted:
        raise V12R9RecoveryFitError(f"{label} metrics drifted: {drifted}")


def recompute_preservation_audit(
    source_state: Mapping[str, Any],
    candidate_state: Mapping[str, Any],
    observations: np.ndarray,
) -> dict[str, Any]:
    audit = full_mean_update_audit(source_state, candidate_state)
    source_logits = v11._state_logits(source_state, observations)
    candidate_logits = v11._state_logits(candidate_state, observations)
    audit = {
        **audit,
        "logstd_outputs_byte_exact": bool(
            source_logits[:, 2:].tobytes(order="C")
            == candidate_logits[:, 2:].tobytes(order="C")
        ),
    }
    audit["passed"] = bool(audit["passed"] and audit["logstd_outputs_byte_exact"])
    return audit
...
def _candidate_actor_manifest(
    *, state: Mapping[str, Any], feature_names: Sequence[str], module_state: Path
    ...
    return {
        "schema_version": 1,
        "status": "H0_V12R9_RECOVERY_ACTOR_FEATURE_CONTRACT",
...
def _save_candidate_exact(
    *,
    ...
    if not all(
        ...
        raise V12R9RecoveryFitError("candidate core save/reload drifted")
    ...
    if set(actor_manifest) != ACTOR_MANIFEST_FIELDS:
        raise V12R9RecoveryFitError("actor manifest schema drifted")
    ...
    if set(build_manifest) != BUILD_MANIFEST_FIELDS:
        raise V12R9RecoveryFitError("candidate build manifest schema drifted")
    forensic.write_json_exclusive(
    ...
    if observed != EXPECTED_CANDIDATE_FILES:
        raise V12R9RecoveryFitError(f"candidate file set drifted: {sorted(observed)}")
    ...
    if not all(
        ...
        raise V12R9RecoveryFitError("candidate final save/reload drifted")
...
def run_fit_stage(
    *,
    ...
    """Execute and exclusively publish the sole R9 recovery actor fit."""

    destination = _resolve(contract.FIT_ROOT)
    if os.path.lexists(destination):
        raise V12R9RecoveryFitError("R9 fit destination exists/no-clobber")
    ...
    locked_inputs_before = attest_locked_inputs()
    ...
    if tuple(source_manifest["actor_feature_names"]) != tuple(
        ...
        raise V12R9RecoveryFitError("source/corpus actor feature order drifted")
    ...
    if len(base_v8) != 3000:
        raise V12R9RecoveryFitError("frozen normalization base row count drifted")
    ...
    if source_tree_before != _tree_record(contract.R6_CANDIDATE_MODULE_PATH):
        raise V12R9RecoveryFitError("R6 source candidate changed during fit")
    destination.mkdir(parents=True, exist_ok=False)
    corpus_path = destination / "corpus.npz"
    v10s_fit._write_npz_exclusive(corpus_path, assembled_corpus_arrays(bundle))
    corpus_record = _record(corpus_path)
    corpus_manifest = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "H0_V12R9_RECOVERY_CORPUS_PASS",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "fit_contract_id": contract.FIT_CONTRACT_ID,
    ...
    metric_payload = recompute_fit_metric_payload(bundle, result.candidate_state)
    if not _bytes_equal(metric_payload["predictions"], result.predictions):
        raise V12R9RecoveryFitError("fit result predictions drifted from saved state")
    base_case_metrics = metric_payload["per_case_metrics"]
    observer_metrics = metric_payload["observer_case_metrics"]
    global_metrics = metric_payload["global_metrics"]
    reset_max_abs_error = metric_payload["reset_max_abs_error"]
    r4_metrics = metric_payload["r4_failed_plus_metrics"]
    observer_plus_late = metric_payload["observer_plus_late_metrics"]
    ...
    report = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "COMPLETE_H0_V12R9_RECOVERY_ADAPTATION_REPORT",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "fit_contract_id": contract.FIT_CONTRACT_ID,
    ...
    summary = {
        ...
        "candidate_module": module_record,
        "worst_row": copy.deepcopy(metric_payload["worst_row"]),
        "corpus": corpus_record,
    ...
    if gate.get("passed") is not True:
        raise V12R9RecoveryFitError("R9 offline fit gate failed")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
    ...
    if set(receipt) != FIT_RECEIPT_FIELDS:
        raise V12R9RecoveryFitError("fit receipt schema drifted")
    forensic.write_json_exclusive(receipt_path, receipt)
    if not _strict_equal(attest_locked_inputs(), locked_inputs_before):
        raise V12R9RecoveryFitError("locked inputs changed during fit")
    return receipt


def verify_fit_stage() -> dict[str, Any]:
    """Reassemble inputs and recompute inference, metrics, invariants and gate."""
    ...
    report = _mapping(root / ADAPTATION_REPORT_NAME)
    corpus_manifest = _mapping(root / CORPUS_MANIFEST_NAME)
    history_path = root / ADAPTATION_HISTORY_NAME
    try:
        history = forensic.strict_json_load(history_path)
    except Exception as exc:
        raise V12R9RecoveryFitError("adaptation history is not strict JSON") from exc
    if not isinstance(history, list) or not history:
        raise V12R9RecoveryFitError("adaptation history is empty or malformed")

    # The verifier deliberately rebuilds all eight NPZ inputs (base, R4, six
    # observer labels) and their thirteen-stratum mapping before trusting any
    # persisted fit output.
    bundle = load_recovery_corpus()
    corpus = bundle.corpus
    corpus_verification = verify_persisted_corpus(bundle)
    expected_corpus_manifest = {
        ...
        "stratum_audit": copy.deepcopy(corpus.audit["weight_audit"]),
        "source_records": copy.deepcopy(corpus.source_records),
        "corpus": _record(contract.CORPUS_PATH),
    }
    if not _strict_equal(corpus_manifest, expected_corpus_manifest):
        raise V12R9RecoveryFitError("corpus manifest semantic closure drifted")

    module = _tree_record(contract.CANDIDATE_MODULE_PATH)
    if {row["path"] for row in module["files"]} != EXPECTED_CANDIDATE_FILES or module[
        "file_count"
    ] != 5:
        raise V12R9RecoveryFitError("candidate five-file tree drifted")
    ...
    if (
        ...
        raise V12R9RecoveryFitError("candidate manifest schema drifted")
    _source_module, source_state, source_manifest = _load_source_module_and_state()
    from ray.rllib.core.rl_module.rl_module import RLModule
    ...
    runtime_logits = _runtime_state_logits_exact(
        candidate, candidate_state, corpus.observations
    )
    preservation = recompute_preservation_audit(
        source_state, candidate_state, corpus.observations
    )
    if preservation.get("passed") is not True:
        raise V12R9RecoveryFitError("candidate mean-only preservation closure failed")
    metrics = recompute_fit_metric_payload(bundle, candidate_state)
    if metrics["predictions"].tobytes(order="C") != np.ascontiguousarray(
        runtime_logits[:, : contract.EXPECTED_ACTION_DIM], dtype=np.float32
    ).tobytes(order="C"):
        raise V12R9RecoveryFitError(
            "recomputed metrics do not use reloaded RLModule predictions"
        )
    _assert_fit_metric_payload(
        summary, metrics, include_worst_row=True, label="fit summary"
    )
    _assert_fit_metric_payload(
        report, metrics, include_worst_row=False, label="adaptation report"
    )

    base_rows = int(contract.LOCKED_INPUTS["base_corpus"]["rows"])
    base_v8 = np.flatnonzero(
    ...
    optimizer_audit = report.get("optimizer_audit")
    if not isinstance(normalization_audit, Mapping) or not isinstance(
        optimizer_audit, Mapping
    ):
        raise V12R9RecoveryFitError("fit audit payload is malformed")
    expected_actor_manifest = _candidate_actor_manifest(
        state=candidate_state,
        feature_names=corpus.actor_feature_names.astype(str).tolist(),
        module_state=_resolve(contract.CANDIDATE_MODULE_PATH) / "module_state.pkl",
    )
    expected_build_manifest = {
        ...
        "save_reload_exact": True,
        "actor_digest": expected_actor_manifest["actor_digest"],
        "module_state_sha256": expected_actor_manifest["module_state_sha256"],
        "actor_feature_manifest": ACTOR_FEATURE_MANIFEST_NAME,
        "corpus": _record(contract.CORPUS_PATH),
    }
    ...
    expected_records = {
        ...
        "adaptation_history": history_path,
        "actor_feature_manifest": actor_manifest_path,
        "candidate_build_manifest": build_manifest_path,
    ...
    optimizer_static = {
        "fit_contract_id": contract.FIT_CONTRACT_ID,
        "seed": int(contract.FIT["optimizer"]["seed"]),
        "full_batch": True,
        "sample_count": int(len(corpus.observations)),
        "explicit_sample_weights": True,
        "sample_weight_dtype": "float64",
        "sample_weights_sha256": v10s_fit.array_sha256(
            corpus.normalized_sample_weights
        ),
        "total_sample_mass": float(
            math.fsum(float(value) for value in corpus.normalized_sample_weights)
        ),
        "adamw_epochs": 2000,
        "adamw_learning_rates": list(contract.FIT["optimizer"]["adamw_learning_rates"]),
        "adamw_boundaries": list(contract.FIT["optimizer"]["adamw_boundaries"]),
        "adamw_weight_decay": float(contract.FIT["optimizer"]["weight_decay"]),
        "gradient_clip_norm": 10.0,
        "lbfgs_lr": 0.7,
        "lbfgs_max_iter": 300,
        "lbfgs_max_eval": 600,
    ...
    optimizer_static_valid = all(
        _strict_equal(optimizer_audit.get(name), value)
        for name, value in optimizer_static.items()
    )
    closure_calls = optimizer_audit.get("lbfgs_closure_calls")
    history_valid = (
        type(closure_calls) is int
        and 1 <= closure_calls <= 600
        and summary.get("lbfgs_closure_calls") == closure_calls
        and isinstance(history[-1], Mapping)
        and history[-1].get("stage") == "lbfgs_final"
        and history[-1].get("index") == closure_calls
    )
    summary_static = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": FIT_COMPLETE_STATUS,
        ...
        "corpus": _record(contract.CORPUS_PATH),
        "corpus_manifest": _record(root / CORPUS_MANIFEST_NAME),
        "adaptation_report": _record(root / ADAPTATION_REPORT_NAME),
        "adaptation_history": _record(history_path),
    ...
    summary_static_valid = all(
        ...
        for name, value in summary_static.items()
    )
    report_static_valid = (
        report.get("schema_version") == contract.SCHEMA_VERSION
        and report.get("status") == "COMPLETE_H0_V12R9_RECOVERY_ADAPTATION_REPORT"
        and report.get("passed") is True
        and report.get("protocol_id") == contract.PROTOCOL_ID
        and report.get("fit_contract_id") == contract.FIT_CONTRACT_ID
        and report.get("training_samples") == len(corpus.observations)
        and _strict_equal(report.get("stratum_audit"), corpus.audit["weight_audit"])
        and _strict_equal(report.get("preservation_audit"), preservation)
        and _strict_equal(report.get("candidate_module"), module)
        and _strict_equal(report.get("actor_feature_manifest"), actor_manifest)
        and _strict_equal(report.get("candidate_build_manifest"), build_manifest)
        and _strict_equal(report.get("source_records"), corpus.source_records)
    )
    checks = {
        ...
        "corpus_byte_exact": corpus_verification.get("passed") is True
        and corpus_verification.get("stratum_count") == 13,
        "actor_manifest": _strict_equal(actor_manifest, expected_actor_manifest)
        ...
        "build_manifest": _strict_equal(build_manifest, expected_build_manifest),
        "mean_only": preservation["passed"] is True,
        "normalization": normalization_audit.get("normalization")
        == normalization.record()
        and normalization_audit.get("normalization_folded_into_first_layer") is True
        and normalization_audit.get("runtime_normalization_wrapper") is False
        and normalization_audit.get("normalized_clock_columns_bit_zero") is True
        and normalization_audit.get("folded_clock_columns_bit_zero") is True
        and normalization_audit.get("fold_equivalence_passed") is True,
        "optimizer": optimizer_static_valid and history_valid,
        "summary_static": summary_static_valid,
        "report_static": report_static_valid,
    ...
    if not all(checks.values()):
        ...
        raise V12R9RecoveryFitError(f"R9 fit verification failed: {failed}")
    if not _strict_equal(attest_locked_inputs(), locked_inputs_before):
        raise V12R9RecoveryFitError("locked inputs changed during fit verification")
    return receipt


__all__ = [
    "V12R9RecoveryFitError",
    "RecoveryCorpusBundle",
    "adamw_learning_rate",
    "assembled_corpus_arrays",
    ...
    "recompute_preservation_audit",
    "run_fit_stage",
    "validate_source_r6_state",
    "verify_fit_stage",
    "verify_observer_label_closures",
