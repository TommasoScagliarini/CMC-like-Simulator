"""Focused source-only tests for the V12R9 protocol freezer."""
...
def test_canonical_path_parser_rejects_escape_and_cross_platform_aliases(
    ...
    for unsafe in (
        ...
        with pytest.raises(freezer.V12R9FreezeError, match="non-canonical"):
...
def test_artifact_and_tree_records_reject_symlink_entries(
    ...
    with pytest.raises(freezer.V12R9FreezeError, match="symlink|unsafe"):
        freezer.artifact_record("safe/linked.bin")
    with pytest.raises(freezer.V12R9FreezeError, match="unsafe tree"):
...
def test_locked_r6_inputs_and_complete_source_closure_are_exact() -> None:
    ...
    assert attestation["r8_minus_adjudication"]["passed"] is True
    assert attestation["r8_minus_adjudication"]["offline_label_authorized"] is True
    assert attestation["r8_minus_adjudication"]["r8_probe_rerun"] is False
    assert attestation["r8_minus_adjudication"]["r8_artifacts_modified"] is False
    assert attestation["r8_evidence_records"] == contract.LOCKED_R8_EVIDENCE
    assert attestation["r8_plus_label_import"]["passed"] is True
    assert attestation["r8_plus_label_import"]["semantic_and_byte_exact_closed"] is True
    assert attestation["r8_plus_label_import"]["labels_copied"] is False
    assert attestation["r6_candidate"] == contract.LOCKED_INPUTS["r6_candidate"]
    assert attestation["source_h0_teacher_id"] == contract.SOURCE_H0_ID
    assert attestation["source_h0_teacher"] == contract.LOCKED_SOURCE_H0_TREE
    assert attestation["coverage_reference_corpus"] == {
...
def test_runtime_and_verifier_boolean_contracts_are_type_strict() -> None:
    ...
    for drifted in (
        ...
        with pytest.raises(freezer.V12R9FreezeError, match="readiness"):
            freezer._strict_runtime(lambda drifted=drifted: drifted)

    with pytest.raises(freezer.V12R9FreezeError, match="strict bool"):
