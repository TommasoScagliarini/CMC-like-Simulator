"""Deterministic recovery-weighted full-mean fitter for V12R2.
...
class V12R2RecoveryWeightedFitError(RuntimeError):
    """Raised when V12R2 fitting evidence is incomplete or inconsistent."""


# Compatibility spelling used by inherited orchestration code.
V10SFitError = V12R2RecoveryWeightedFitError
...
def _mapping(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R2RecoveryWeightedFitError(
            f"invalid strict JSON object: {resolved}"
        ) from exc
    if not isinstance(value, Mapping):
        raise V12R2RecoveryWeightedFitError(f"expected strict JSON object: {resolved}")
    return dict(value)
...
def _record(path: str | PurePath | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R2RecoveryWeightedFitError(f"cannot record artifact: {path}") from exc
...
def _read_npz_exact(
    ...
    except V12R2RecoveryWeightedFitError:
        raise
    except Exception as exc:
        raise V12R2RecoveryWeightedFitError(
...
def recovery_ramp_weight(previous_penetration_m: float) -> float:
    ...
    try:
        ...
        raise V12R2RecoveryWeightedFitError("penetration is not numeric") from exc
...
def frozen_base_normalization(base_observations: Any) -> FrozenNormalization:
    try:
        ...
        raise V12R2RecoveryWeightedFitError("base normalization failed") from exc
...
def normalized_observations(
    ...
    try:
        ...
        raise V12R2RecoveryWeightedFitError("observation normalization failed") from exc
...
def _load_seed_arrays() -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError("V11 P3 seed arrays drifted")
...
def load_coverage_reference() -> CoverageReference:
    ...
    try:
        ...
        raise V12R2RecoveryWeightedFitError("scipy cKDTree is unavailable") from exc
...
def evaluate_observer_coverage(observations: Any) -> dict[str, Any]:
    ...
    if audit_k != 64 or audit_k >= len(reference64):
        raise V12R2RecoveryWeightedFitError("coverage candidate audit k drifted")
...
def observer_episode_weights(
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError("pure episode weight metadata drifted")
    raw = np.asarray(
        [
            max(
...
def _shielded_episode_weights(reset_mask: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError("shielded episode reset layout drifted")
...
def _episode_id(family: str, stage_or_round: str, case_id: str) -> str:
    if not family or not stage_or_round or not case_id:
        raise V12R2RecoveryWeightedFitError("empty episode identity component")
...
def _load_observer_piece(
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError(f"observer label receipt drifted: {stage}")
    ...
    if type(rows) is not int or not 1 <= rows <= contract.EXPECTED_STEPS:
        raise V12R2RecoveryWeightedFitError("observer label row count drifted")
    observations = arrays["observations"]
    actions = arrays["actions"]
    reset = arrays["reset_mask"]
    if (
        ...
        raise V12R2RecoveryWeightedFitError(f"observer label arrays drifted: {stage}")
...
def _load_shielded_piece(
    ...
    episode_ids = np.repeat(
        np.asarray(
            [_episode_id("v12r2_shielded", f"r{round_index}", case_id)],
            dtype="U192",
        ),
        rows,
    )
...
def _validate_piece(piece: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError("fit corpus piece shape drifted")
...
def expected_corpus_component_order(stage: str) -> tuple[tuple[Any, ...], ...]:
    ...
    if stage not in contract.FIT_STAGES:
        raise V12R2RecoveryWeightedFitError(f"unknown fit stage: {stage!r}")
    ...
    for prior_index in range(contract.FIT_STAGES.index(stage)):
        ...
        order.extend(
            ("v12r2_shielded", round_index, case_id)
...
def build_fit_corpus_audit(
    *,
    ...
    if any(type(value) is not int or value < 0 for value in values):
        raise V12R2RecoveryWeightedFitError("fit corpus audit counts are invalid")
    if type(all_finite) is not bool:
        raise V12R2RecoveryWeightedFitError("fit corpus finite flag is invalid")
    return {
...
def load_fit_corpus(
    *, stage: str, pipeline_claim_path: str | PurePath | Path
    ...
    if _resolve(pipeline_claim_path) != _resolve(contract.PIPELINE_CLAIM_PATH):
        raise V12R2RecoveryWeightedFitError("pipeline claim path is not canonical")
    ...
    dagger_rows = sum(
        len(piece["observations"])
        for piece in pieces
        if piece["family"] == "v12r2_shielded"
    )
    if (
    ...
    if any(
        ...
        raise V12R2RecoveryWeightedFitError("actor feature names drifted")
...
def fit_recovery_weighted_full_mean_in_memory(
    *,
    ...
    """Run the sole fixed AdamW+LBFGS design with explicit V12R2 weights."""
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError("weighted fit corpus arrays are malformed")
    ...
    try:
        ...
        if not preservation["passed"]:
            raise V12R2RecoveryWeightedFitError("mean-only preservation audit failed")
        ...
        raise V12R2RecoveryWeightedFitError("fixed weighted fit failed") from exc
    finally:
...
def _load_source_module_and_state() -> tuple[Any, dict[str, Any]]:
    try:
        ...
        raise V12R2RecoveryWeightedFitError("cannot load immutable H0 module") from exc
...
def _save_candidate_exact(
    *, source_module: Any, candidate_state: Mapping[str, Any], destination: Path
    ...
    try:
        ...
        raise V12R2RecoveryWeightedFitError("candidate save/reload failed") from exc
...
def _source_tree_checked() -> dict[str, Any]:
    ...
    if record.get("tree_sha256") != contract.SOURCE_H0_TREE_SHA256:
        raise V12R2RecoveryWeightedFitError("source H0 tree hash drifted")
    return record
...
def run_design_audit_in_memory() -> dict[str, Any]:
    ...
    if source_before != _source_tree_checked():
        raise V12R2RecoveryWeightedFitError("source H0 changed during design audit")
...
def run_fit_stage(
    *,
    ...
    """Run and publish one canonical, no-clobber V12R2 fit stage."""
    ...
    if design_path != _resolve(contract.DESIGN_AUDIT_RECEIPT_PATH):
        raise V12R2RecoveryWeightedFitError("design audit path is not canonical")
    if lock_path != _resolve(contract.EXECUTION_LOCK_PATH):
        raise V12R2RecoveryWeightedFitError("execution lock path is not canonical")
    ...
    if _resolve(worker_claim_path) != _resolve(
        ...
        raise V12R2RecoveryWeightedFitError("fit worker claim path is not canonical")
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError("design audit/execution lock is not PASS")
    ...
    if source_before != _source_tree_checked():
        raise V12R2RecoveryWeightedFitError("source H0 changed during fit")
    module_record = _tree_record(module_path)
    if module_record["path"] != contract.MODULE_PATHS[stage].as_posix():
        raise V12R2RecoveryWeightedFitError("candidate module path drifted")
...
def verify_fit_stage(stage: str) -> dict[str, Any]:
    ...
    if observed_gate != expected_gate or expected_gate.get("passed") is not True:
        raise V12R2RecoveryWeightedFitError(f"persisted fit gate drifted: {stage}")
    receipt = _mapping(receipt_path)
    expected_keys = {
        "schema_version",
        "status",
    ...
    if (
        ...
        raise V12R2RecoveryWeightedFitError(f"persisted fit receipt drifted: {stage}")
    return receipt
