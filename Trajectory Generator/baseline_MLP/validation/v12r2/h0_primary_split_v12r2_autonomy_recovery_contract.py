# ruff: noqa: F821
"""Additive V12R2 correction of the frozen V12 autonomy/recovery contract.

V12R2 preserves the complete V12 scientific design and immutable V12 evidence,
keeps the terminal V12R1 attempt as immutable failure evidence, and closes
three fail-closed specification gaps:
...
* inherited V12 functions retain their original code objects and stable wire
  keys; protocol identities and paths are versioned only through explicit
...
gate consumes the V12R2 identifiers and paths without mutating the V12 module.
...
def _rebind_function(value: types.FunctionType) -> types.FunctionType:
    """Rebind a V12 function to R2 globals without rewriting its bytecode.
...
SCHEMA_VERSION = 122
REVISION = "2026-08-09"
PROTOCOL_ID = "AB06_H0_PRIMARY_SPLIT_V12R2_V26_AUTONOMY_RECOVERY"
PIPELINE_ID = "H0_V12R2_V26_RECOVERY_WEIGHTED_PURE_PROBE_DAGGER"
FIT_CONTRACT_ID = "h0_primary_split_v12r2_recovery_weighted_full_mean_v1"
DESIGN_AUDIT_ID = "H0_V12R2_P0_RECOVERY_WEIGHTED_FEASIBILITY_V1"
AUTHORITY_TEXT = (
    "Fai tutti i passaggi che ti servono per arrivare ad essere training ready. "
    "Non hai bisogno di autorizzazioni mie, considera già tutto autorizzato."
)
TRAINING_READINESS_SCOPE = "V12R2_TRAINING_READINESS_ONLY"
EXECUTION_AUTHORITY_SCOPE = "V12R2_ONE_SHOT_EXECUTION_READY"

V12R2_VALIDATION_ROOT = PurePosixPath(
    "Trajectory Generator/baseline_MLP/validation/v12r2"
)
V12_VALIDATION_ROOT = V12R2_VALIDATION_ROOT
RUN_ROOT = V12R2_VALIDATION_ROOT / ("h0_v12r2_run_20260809")
...
PROTOCOL_FREEZE_PATH = V12R2_VALIDATION_ROOT / (
    "h0_primary_split_v12r2_autonomy_recovery_protocol_freeze.json"
)
EXECUTION_LOCK_PATH = V12R2_VALIDATION_ROOT / (
    "h0_primary_split_v12r2_autonomy_recovery_execution_lock.json"
)
DESIGN_AUDIT_RECEIPT_PATH = V12R2_VALIDATION_ROOT / (
...
# checkpoint, corpus, or partial fit below may be promoted into V12R2.
...
PROBE_REPLAY_SCHEMA = {
    "schema_id": "H0_V12R2_OBSERVER_REPLAY_BOUNDARIES_V2",
...
P0_REPRODUCTION_TOLERANCE = {
    ...
    "reference": "SOLE_V12R2_DESIGN_AUDIT_P0_IN_MEMORY_FIT",
...
def _r2_status(value: str) -> str:
    return value.replace("V12", "V12R2")
...
DESIGN_AUDIT_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V12R2_DESIGN_AUDIT"
DESIGN_AUDIT_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V12R2_DESIGN_AUDIT"
EXECUTION_LOCK_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V12R2_EXECUTION_LOCK"
EXECUTION_LOCK_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V12R2_EXECUTION_LOCK"


# Rebind every inherited pure function after all V12R2 globals are defined.
for _name, _value in vars(_v12).items():
...
_NEXT_STAGE_RENAMES = {
    "STOP_V12_FIT_FAILURE": "STOP_V12R2_FIT_FAILURE",
    "STOP_V12_PROBE_INTEGRITY_FAILURE": "STOP_V12R2_PROBE_INTEGRITY_FAILURE",
    "STOP_V12_TERMINAL": "STOP_V12R2_TERMINAL",
    "STOP_V12_COLLECTION_INTEGRITY_FAILURE": (
        "STOP_V12R2_COLLECTION_INTEGRITY_FAILURE"
    ),
    "STOP_V12_LABEL_INTEGRITY_FAILURE": "STOP_V12R2_LABEL_INTEGRITY_FAILURE",
    "STOP_V12_FREEZE_FAILURE": "STOP_V12R2_FREEZE_FAILURE",
}
...
def replay_schema_gate(value: Any) -> dict[str, Any]:
    """Validate the exact self-contained V12R2 replay declaration."""
...
def fit_gate(summary: Mapping[str, Any], *, stage: str) -> dict[str, Any]:
    ...
    result["next_stage"] = (
        f"PURE_PROBE_{stage.upper()}" if passed else "STOP_V12R2_FIT_FAILURE"
    )
    return result
...
def fit_contract_self_check() -> dict[str, Any]:
    ...
    for index, stage in enumerate(FIT_STAGES):
        ...
        stage_checks = {
            ...
            "legacy_v12_keys_preserved": (
                ...
                and not any("v12r2" in key for key in expected)
            ),
    ...
    return {
        "schema_version": SCHEMA_VERSION,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V12R2_FIT_CONTRACT_SELF_CHECK"
            if passed
            else "FAIL_H0_PRIMARY_SPLIT_V12R2_FIT_CONTRACT_SELF_CHECK"
        ),
        "passed": passed,
        "protocol_id": PROTOCOL_ID,
...
def declared_mutation_paths() -> dict[str, PurePosixPath]:
    """Return the exhaustive V12R2 write manifest, including failure paths."""
