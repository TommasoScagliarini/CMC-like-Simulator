"""Execution-surface tests for the additive, one-shot V12R2 pipeline.

These tests exercise orchestration only.  They never load a policy checkpoint,
fit an actor, reset an environment, or create a canonical V12R2 artifact.
"""
...
for _root in (REPO_ROOT, LOCAL_VALIDATION_ROOT, V12R2_ROOT):
    if str(_root) not in sys.path:
        sys.path.insert(0, str(_root))

import h0_primary_split_v12r2_autonomy_recovery_contract as contract  # noqa: E402
import run_h0_primary_split_v12r2_autonomy_recovery as runner  # noqa: E402
import run_h0_primary_split_v12r2_design_audit as audit_runner  # noqa: E402
...
def test_execute_pipeline_once_publishes_one_terminal_failure_and_stops(
    ...
    def fail_on_third(stage: str) -> dict[str, bool]:
        ...
        if stage == failed_stage:
            raise runner.V12R2ExecutionError("synthetic terminal failure")
        return {"passed": True}

    monkeypatch.setattr(runner, "_run_stage", fail_on_third)
    with pytest.raises(runner.V12R2ExecutionError, match="synthetic terminal"):
...
def test_pipeline_claim_publication_failure_writes_zero_activity_ledger(
    ...
    ledger = writes[0][1]
    assert ledger["status"] == ("FAIL_H0_PRIMARY_SPLIT_V12R2_PIPELINE_CLAIM_TERMINAL")
...
def test_run_root_claim_is_atomic_and_no_clobber(
    ...
    with pytest.raises(runner.V12R2ExecutionError, match="already claimed"):
        runner._claim_run_root()
