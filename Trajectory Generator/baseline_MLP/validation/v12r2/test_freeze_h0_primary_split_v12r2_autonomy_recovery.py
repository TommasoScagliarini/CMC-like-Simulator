...
import freeze_h0_primary_split_v12r2_autonomy_recovery as freeze
...
def test_sources_bind_every_r2_design_file_and_nested_attributes() -> None:
    ...
    for name in ("contract", "freeze", "contract_tests", "freeze_tests"):
        assert "v12r2" in str(records[name]["path"])
...
def test_nested_attributes_cover_r2_text_and_binary_artifacts() -> None:
    path = freeze.resolve_relative(
        freeze.contract.SOURCE_RELATIVE_PATHS["line_endings_r2"]
    )
    ...
    for extension in (
        ...
        assert f"h0_v12r2_run_*/**/*.{extension} -text" in attributes
...
def test_build_passes_without_opening_execution_scope(tmp_path: Path) -> None:
    ...
    assert payload["schema_version"] == 122
    ...
    assert payload["next_stage"] == ("REQUIRE_SEPARATE_V12R2_NO_CLOBBER_EXECUTION_LOCK")
...
def test_noncanonical_prepare_is_exact_no_clobber_and_tamper_evident(
    ...
    with pytest.raises(freeze.V12R2ProtocolFreezeError, match="drifted"):
        freeze.verify_protocol_freeze(
            input_path=destination,
            enforce_canonical_destination=False,
        )
    with pytest.raises(freeze.V12R2ProtocolFreezeError, match="exists/no-clobber"):
...
def test_canonical_outputs_are_in_a_valid_readiness_phase() -> None:
    ...
    assert not os.path.lexists(freeze._lexical_absolute(freeze.contract.RUN_ROOT))
