# ruff: noqa: F821
"""Build and verify the additive, non-executable V12R2 protocol freeze.

The command re-audits the immutable V12/V11 evidence and may publish exactly
one no-clobber V12R2 protocol JSON.  It cannot fit an actor, load H0 for probe
inference, label a probe, reset/step an environment, or mint an execution
claim.  A distinct execution lock must pass the pure V12R2 lock gate first.
"""
...
try:
    from validation import h0_primary_split_v12r2_autonomy_recovery_contract as contract
except (ImportError, ModuleNotFoundError):
...
class V12R2ProtocolFreezeError(RuntimeError):
...
V12ProtocolFreezeError = V12R2ProtocolFreezeError
...
def _v12r1_terminal_failure_gate() -> dict[str, Any]:
    ...
    return {
        ...
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V12R2_R1_TERMINAL_LINEAGE"
            if all(checks.values())
            else "FAIL_H0_PRIMARY_SPLIT_V12R2_R1_TERMINAL_LINEAGE"
        ),
        "passed": all(checks.values()),
...
def _assemble_protocol_freeze(occupancy: Mapping[str, bool]) -> dict[str, Any]:
    ...
    if set(occupancy) != expected_occupancy_keys or not all(
        ...
        raise V12R2ProtocolFreezeError("occupancy evidence schema drifted")
    ...
    return {
        ...
        "next_stage": "REQUIRE_SEPARATE_V12R2_NO_CLOBBER_EXECUTION_LOCK",
    }
...
def verify_protocol_freeze(
    *,
    ...
    if enforce_canonical_destination and destination != canonical:
        raise V12R2ProtocolFreezeError(f"non-canonical protocol freeze: {destination}")
    snapshot = artifact_snapshot(destination)
    if snapshot.get("kind") != "regular" or _symlink_ancestors(destination):
        raise V12R2ProtocolFreezeError(
            "V12R2 protocol freeze is not a regular safe path"
        )
    if enforce_canonical_destination:
        ...
        if invalid_order:
            raise V12R2ProtocolFreezeError(
                "V12R2 later occupancy is out of readiness order"
            )
    ...
    try:
        ...
        raise V12R2ProtocolFreezeError(
            "V12R2 protocol freeze is not strict JSON"
        ) from exc
    if payload != expected or destination.read_bytes() != forensic.canonical_json_bytes(
        expected
    ):
        raise V12R2ProtocolFreezeError("V12R2 protocol freeze drifted")
    return dict(payload)
