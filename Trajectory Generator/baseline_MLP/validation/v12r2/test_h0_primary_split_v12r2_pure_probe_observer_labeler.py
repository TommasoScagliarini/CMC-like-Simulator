...
import h0_primary_split_v12r2_pure_probe_observer_labeler as labeler  # noqa: E402
...
def test_missing_journal_array_and_tampered_delivery_are_rejected(
    ...
    with pytest.raises(labeler.V12R2ObserverLabelError, match="schema drifted"):
    ...
    with pytest.raises(
        labeler.V12R2ObserverLabelError,
...
def test_config_hash_and_noncanonical_config_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(labeler.V12R2ObserverLabelError, match="SHA-256 mismatch"):
    ...
    with pytest.raises(labeler.V12R2ObserverLabelError, match="noncanonical"):
        labeler.load_probe_replay_strict(path)
...
def test_object_dtype_cannot_cross_no_pickle_loader(tmp_path: Path) -> None:
    ...
    with pytest.raises(labeler.V12R2ObserverLabelError, match="pickle-free"):
        labeler.load_probe_replay_strict(path)
...
def test_failed_closure_prevents_fsm_replay_and_h0_load(tmp_path: Path) -> None:
    ...
    with pytest.raises(labeler.V12R2ObserverLabelError, match="not validly closed"):
...
def test_replay_npz_is_no_clobber(tmp_path: Path) -> None:
    path = tmp_path / "replay.npz"
    labeler.write_replay_npz_exclusive(path, _replay_arrays())
    with pytest.raises(labeler.V12R2ObserverLabelError, match="clobber"):
        labeler.write_replay_npz_exclusive(path, _replay_arrays())
