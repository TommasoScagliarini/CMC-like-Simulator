"""Deterministic recovery-weighted full-mean fitter for V12R1.
...
class V12R1RecoveryWeightedFitError(RuntimeError):
    """Raised when V12R1 fitting evidence is incomplete or inconsistent."""


# Compatibility spelling used by inherited orchestration code.
V10SFitError = V12R1RecoveryWeightedFitError
...
def _mapping(path: str | PurePath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R1RecoveryWeightedFitError(
            f"invalid strict JSON object: {resolved}"
        ) from exc
    if not isinstance(value, Mapping):
        raise V12R1RecoveryWeightedFitError(
            f"expected strict JSON object: {resolved}"
        )
    return dict(value)
...
def _record(path: str | PurePath | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R1RecoveryWeightedFitError(f"cannot record artifact: {path}") from exc
...
def _read_npz_exact(
    ...
    except V12R1RecoveryWeightedFitError:
        raise
    except Exception as exc:
        raise V12R1RecoveryWeightedFitError(f"cannot read safe NPZ: {resolved}") from exc
...
def recovery_ramp_weight(previous_penetration_m: float) -> float:
    ...
    try:
        ...
        raise V12R1RecoveryWeightedFitError("penetration is not numeric") from exc
...
def _normalize_episode_exact(raw_weights: Sequence[float]) -> np.ndarray:
    ...
    scaled[-1] = EPISODE_TARGET_MASS - math.fsum(
        float(value) for value in scaled[:-1]
    )
    for _ in range(4):
...
def frozen_base_normalization(base_observations: Any) -> FrozenNormalization:
    try:
        ...
        raise V12R1RecoveryWeightedFitError("base normalization failed") from exc
...
def normalized_observations(
    ...
    try:
        ...
        raise V12R1RecoveryWeightedFitError("observation normalization failed") from exc
...
def _load_seed_arrays() -> tuple[dict[str, np.ndarray], dict[str, Any]]:
    ...
    if (
        ...
        raise V12R1RecoveryWeightedFitError("V11 P3 seed arrays drifted")
...
def load_coverage_reference() -> CoverageReference:
    ...
    try:
        ...
        raise V12R1RecoveryWeightedFitError("scipy cKDTree is unavailable") from exc

    arrays, artifact = _load_seed_arrays()
    observations = np.ascontiguousarray(arrays["observations"], dtype=np.float32)
    base = np.ascontiguousarray(observations[:BASE_NORMALIZATION_ROWS], dtype=np.float32)
    ...
    def select(candidates_matrix: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        for row_index, candidate_row in enumerate(candidates_matrix):
            ...
            delta = (
                candidate_features.astype(np.float64)
                - features[row_index].astype(np.float64)
            )
            distances = np.sqrt(
                np.sum(np.square(delta), axis=1, dtype=np.float64)
                / features.shape[1]
            )
            ...
            ties = [
                ...
                for candidate, distance in zip(
                    alternatives, distances, strict=True
                )
    ...
    checks = {
        ...
        == tie_spec["maximum_minimum_distance_tie_count"],
...
def evaluate_observer_coverage(observations: Any) -> dict[str, Any]:
    ...
    audit_k = int(
        contract.COVERAGE_WEIGHTING["new_row_query"][
            "global_minimum_candidate_audit_k"
        ]
    )
    if audit_k != 64 or audit_k >= len(reference64):
        raise V12R1RecoveryWeightedFitError("coverage candidate audit k drifted")
    ...
    return {
        ...
        "maximum_certificate_threshold_sum": float(
            np.max(thresholds, initial=0.0)
        ),
        ...
        "audit": {
            ...
            "minimum_certificate_margin_sum": float(
                np.min(margins, initial=math.inf)
            ),
            "new_row_query": copy.deepcopy(
                contract.COVERAGE_WEIGHTING["new_row_query"]
            ),
...
def observer_episode_weights(
    ...
    if (
        ...
        raise V12R1RecoveryWeightedFitError("pure episode weight metadata drifted")
    raw = np.asarray(
        [
            max(
...
def _shielded_episode_weights(reset_mask: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    reset = np.ascontiguousarray(reset_mask, dtype=np.bool_)
    if reset.ndim != 1 or len(reset) < 1 or int(np.count_nonzero(reset)) != 1 or not reset[0]:
        raise V12R1RecoveryWeightedFitError("shielded episode reset layout drifted")
...
def _episode_id(family: str, stage_or_round: str, case_id: str) -> str:
    if not family or not stage_or_round or not case_id:
        raise V12R1RecoveryWeightedFitError("empty episode identity component")
...
def _load_observer_piece(stage: str, pipeline_claim: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        ...
        raise V12R1RecoveryWeightedFitError(
            f"observer label receipt drifted: {stage}"
        )
    ...
    if type(rows) is not int or not 1 <= rows <= contract.EXPECTED_STEPS:
        raise V12R1RecoveryWeightedFitError("observer label row count drifted")
    observations = arrays["observations"]
    actions = arrays["actions"]
    reset = arrays["reset_mask"]
    if (
        ...
        raise V12R1RecoveryWeightedFitError(
            f"observer label arrays drifted: {stage}"
        )
    ...
    mismatches = [
        name for name, expected in exact_arrays.items() if not _bytes_equal(arrays[name], expected)
    ]
    if mismatches:
        raise V12R1RecoveryWeightedFitError(
    ...
    expected_origins = np.asarray(
        [
            f"pure_observer:{stage}:{case_id}:{step}"
...
def _load_shielded_piece(
    ...
    episode_ids = np.repeat(
        np.asarray(
            [_episode_id("v12r1_shielded", f"r{round_index}", case_id)],
            dtype="U192",
        ),
        rows,
    )
...
def _validate_piece(piece: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise V12R1RecoveryWeightedFitError("fit corpus piece shape drifted")
...
def expected_corpus_component_order(stage: str) -> tuple[tuple[Any, ...], ...]:
    ...
    if stage not in contract.FIT_STAGES:
        raise V12R1RecoveryWeightedFitError(f"unknown fit stage: {stage!r}")
    ...
    for prior_index in range(contract.FIT_STAGES.index(stage)):
        ...
        order.extend(
            ("v12r1_shielded", round_index, case_id)
...
def load_fit_corpus(
    *, stage: str, pipeline_claim_path: str | PurePath | Path
    ...
    if _resolve(pipeline_claim_path) != _resolve(contract.PIPELINE_CLAIM_PATH):
        raise V12R1RecoveryWeightedFitError("pipeline claim path is not canonical")
    ...
    pure_rows = sum(len(piece["observations"]) for piece in pieces if piece["family"] == "pure_observer")
    dagger_rows = sum(len(piece["observations"]) for piece in pieces if piece["family"] == "v12r1_shielded")
    if (
        len(observations) != expected["sample_count"]
        or int(np.count_nonzero(reset)) != expected["reset_row_count"]
        or len(episode_order)
        != 12 + 2 * stage_index + stage_index
    ...
    if any(
        ...
        raise V12R1RecoveryWeightedFitError("actor feature names drifted")
    audit = {
        "v11_seed_sample_count": 6000,
    ...
    return RecoveryFitCorpus(
        ...
        source_records={
            ...
            "shielded_collections": [
                piece["source_record"]
                for piece in pieces
                if piece["family"] == "v12r1_shielded"
            ],
        },
...
def fit_recovery_weighted_full_mean_in_memory(
    *,
    ...
    """Run the sole fixed AdamW+LBFGS design with explicit V12R1 weights."""
    ...
    try:
        ...
        raise V12R1RecoveryWeightedFitError("source H0 state validation failed") from exc
    raw = np.ascontiguousarray(observations, dtype=np.float32)
    ...
    if (
        ...
        raise V12R1RecoveryWeightedFitError("weighted fit corpus arrays are malformed")
    ...
    try:
        ...
        for epoch in range(1, 3001):
            ...
            if epoch in milestones:
                v11._milestone(
                    history, stage="adamw", index=epoch, loss=loss, lr=rate
                )
        ...
        if not preservation["passed"]:
            raise V12R1RecoveryWeightedFitError(
                "mean-only preservation audit failed"
            )
        ...
        optimizer_audit = {
            ...
            "adamw_epochs": 3000,
            "adamw_schedule": copy.deepcopy(contract.FIT["adamw"]["learning_rate_schedule"]),
        ...
        raise V12R1RecoveryWeightedFitError("fixed weighted fit failed") from exc
    finally:
...
def _load_source_module_and_state() -> tuple[Any, dict[str, Any]]:
    try:
        ...
        raise V12R1RecoveryWeightedFitError("cannot load immutable H0 module") from exc
...
def _save_candidate_exact(
    *, source_module: Any, candidate_state: Mapping[str, Any], destination: Path
    ...
    try:
        ...
        raise V12R1RecoveryWeightedFitError("candidate save/reload failed") from exc
...
def _source_tree_checked() -> dict[str, Any]:
    ...
    if record.get("tree_sha256") != contract.SOURCE_H0_TREE_SHA256:
        raise V12R1RecoveryWeightedFitError("source H0 tree hash drifted")
    return record
...
def run_design_audit_in_memory() -> dict[str, Any]:
    ...
    if source_before != _source_tree_checked():
        raise V12R1RecoveryWeightedFitError("source H0 changed during design audit")
...
def run_fit_stage(
    *,
    ...
    """Run and publish one canonical, no-clobber V12R1 fit stage."""
    ...
    if design_path != _resolve(contract.DESIGN_AUDIT_RECEIPT_PATH):
        raise V12R1RecoveryWeightedFitError("design audit path is not canonical")
    if lock_path != _resolve(contract.EXECUTION_LOCK_PATH):
        raise V12R1RecoveryWeightedFitError("execution lock path is not canonical")
    ...
    if _resolve(worker_claim_path) != _resolve(contract.worker_claim_path(f"fit_{stage}")):
        raise V12R1RecoveryWeightedFitError("fit worker claim path is not canonical")
    design_payload = _mapping(design_path)
    lock_payload = _mapping(lock_path)
    if design_payload.get("passed") is not True or lock_payload.get("passed") is not True:
        raise V12R1RecoveryWeightedFitError("design audit/execution lock is not PASS")
    ...
    if source_before != _source_tree_checked():
        raise V12R1RecoveryWeightedFitError("source H0 changed during fit")
    module_record = _tree_record(module_path)
    if module_record["path"] != contract.MODULE_PATHS[stage].as_posix():
        raise V12R1RecoveryWeightedFitError("candidate module path drifted")
    ...
    summary = {
        ...
        "teacher_evidence_receipt": copy.deepcopy(
        ...
        * EPISODE_TARGET_MASS,
        ...
        "corpus_loss_reduction": contract.RECOVERY_WEIGHTING[
            "corpus_loss_reduction"
        ],
...
def verify_fit_stage(stage: str) -> dict[str, Any]:
    ...
    if observed_gate != expected_gate or expected_gate.get("passed") is not True:
        raise V12R1RecoveryWeightedFitError(
            f"persisted fit gate drifted: {stage}"
        )
    receipt = _mapping(receipt_path)
    expected_keys = {
        "schema_version",
    ...
    if (
        ...
        raise V12R1RecoveryWeightedFitError(
            f"persisted fit receipt drifted: {stage}"
        )
    return receipt
