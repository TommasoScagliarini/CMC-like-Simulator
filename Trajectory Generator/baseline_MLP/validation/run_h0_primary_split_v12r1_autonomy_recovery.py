"""Execute the frozen V12R1 autonomy-recovery pipeline exactly once.

This module is the execution overlay for the additive V12R1 protocol.  The
...
import h0_primary_split_v12r1_autonomy_recovery_contract as contract  # noqa: E402
import h0_primary_split_v12r1_pure_probe_observer_labeler as label_engine  # noqa: E402
import h0_primary_split_v12r1_recovery_weighted_fitter as fit_engine  # noqa: E402
import run_h0_primary_split_v12r1_design_audit as audit_engine  # noqa: E402


class V12R1ExecutionError(RuntimeError):
    """Raised when V12R1 cannot continue without violating the freeze."""
...
def _activity_increment(name: str, amount: int = 1) -> None:
    ...
    if type(amount) is not int or amount < 0:
        raise V12R1ExecutionError("stage activity increment is malformed")
...
def resolve_relative(path: str | os.PathLike[str] | PurePosixPath) -> Path:
    ...
    if (
        ...
        raise V12R1ExecutionError(f"non-canonical repository path: {raw!r}")
...
def _mapping(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise V12R1ExecutionError(f"expected strict JSON object: {path}")
    return dict(value)
...
def _sequence(path: str | Path) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise V12R1ExecutionError(f"expected strict JSON array: {path}")
    return value
...
def _tree_record(path: str | Path) -> dict[str, Any]:
    ...
    if not root.is_dir():
        raise V12R1ExecutionError(f"artifact tree is missing: {root}")
    files = sorted(item for item in root.rglob("*") if item.is_file())
    if not files or any(item.is_symlink() for item in files):
        raise V12R1ExecutionError(f"artifact tree is empty or symlinked: {root}")
...
def _inherited_runtime_evidence_audit() -> dict[str, Any]:
    ...
    if not isinstance(frozen_sources, Mapping) or not isinstance(
        ...
        raise V12R1ExecutionError("V11 execution lock evidence is malformed")
    ...
    checks = {
        "v11_lock_artifact_exact": lock_record
...
def _runtime_record() -> dict[str, Any]:
    ...
    if (
        ...
        raise V12R1ExecutionError("execution inference stack identity drifted")
    return {
...
def _query_mean_std(module: Any, actor: Any, *, np: Any, torch: Any) -> tuple[Any, Any]:
    with _legacy_contract_bound():
        return legacy_runner._query_mean_std(
            module, actor, np=np, torch=torch
        )
...
def _frozen_innovations(
    case_id: str, *, action_selection: str, np: Any
) -> Any:
    with _legacy_contract_bound():
...
def _assert_execution_authority() -> None:
    if EXECUTION_AUTHORITY != {
        ...
        raise V12R1ExecutionError("execution authority schema/identity drifted")
...
def _design_audit_gate(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if dict(payload) != verified:
        raise V12R1ExecutionError("design audit receipt/current bindings drifted")
    gate_function = getattr(contract, "design_audit_gate", None)
    if not callable(gate_function):
        raise V12R1ExecutionError("R1 contract lacks design_audit_gate")
    ...
    if gate.get("passed") is not True:
        ...
        raise V12R1ExecutionError(f"design audit is not PASS: {failed}")
    return gate
...
def build_execution_lock(*, require_unoccupied: bool = True) -> dict[str, Any]:
    ...
    if not PROTOCOL_FREEZE_PATH.is_file():
        raise V12R1ExecutionError("canonical R1 protocol freeze is missing")
    protocol = _mapping(PROTOCOL_FREEZE_PATH)
    if protocol.get("passed") is not True:
        raise V12R1ExecutionError("R1 protocol freeze is not PASS")
    ...
    checks = {
        ...
        == tuple(contract.stage_descriptor(stage) and stage for stage in contract.STAGE_IDS),
    ...
    payload = {
        ...
        "status": getattr(
            ...
            "PASS_H0_PRIMARY_SPLIT_V12R1_EXECUTION_LOCK",
        )
        if passed
        else getattr(
            contract,
            "EXECUTION_LOCK_FAIL_STATUS",
            "FAIL_H0_PRIMARY_SPLIT_V12R1_EXECUTION_LOCK",
        ),
        "passed": passed,
        "protocol_id": contract.PROTOCOL_ID,
        ...
        "next_stage": "EXECUTE_V12R1_ONCE" if passed else "STOP",
    }
    ...
    if require_unoccupied and (not passed or payload.get("passed") is not True):
        ...
        raise V12R1ExecutionError(f"execution lock preflight failed: {failed}")
    return payload


def prepare_execution_lock() -> dict[str, Any]:
    if os.path.lexists(LOCK_PATH):
        raise V12R1ExecutionError("execution lock exists/no-clobber")
...
def verify_execution_lock(*, require_run_root_absent: bool = False) -> dict[str, Any]:
    ...
    if (
        ...
        raise V12R1ExecutionError("protocol freeze content drifted")
    audit = _mapping(DESIGN_AUDIT_PATH)
    audit_gate = _design_audit_gate(audit)
    coverage = fit_engine.coverage_reference_audit()
    source_h0 = _tree_record(SOURCE_H0_MODULE)
    ...
    if not all(value is True for value in checks.values()):
        failed = [name for name, value in checks.items() if value is not True]
        raise V12R1ExecutionError(f"execution lock bindings failed: {failed}")
    expected = {
        "schema_version": contract.SCHEMA_VERSION,
        ...
        "protected_trials_opened": [],
        "reserve_trials_opened": [],
        "next_stage": "EXECUTE_V12R1_ONCE",
    }
    expected["contract_gate"] = dict(contract.execution_lock_gate(expected))
    if (
        expected["contract_gate"].get("passed") is not True
        or observed != expected
    ):
        raise V12R1ExecutionError("execution lock exact payload drifted")
    if require_run_root_absent and os.path.lexists(RUN_ROOT):
        raise V12R1ExecutionError("V12R1 run root already claimed")
    return observed


def _token_sha256(token: str) -> str:
    if not isinstance(token, str) or len(token) < 32:
        raise V12R1ExecutionError("execution token is malformed")
...
def _verify_pipeline_claim() -> dict[str, Any]:
    ...
    if not isinstance(token_sha256, str) or len(token_sha256) != 64:
        raise V12R1ExecutionError("pipeline claim token digest is malformed")
    expected = _pipeline_claim_payload(token_sha256)
    if claim != expected:
        raise V12R1ExecutionError("pipeline claim/current lock binding drifted")
    return claim
...
def _verify_worker_claim(stage_id: str) -> dict[str, Any]:
    ...
    if claim != expected:
        raise V12R1ExecutionError(f"worker claim chain drifted: {stage_id}")
    return claim
...
def _pipeline_claim_payload(token_sha256: str) -> dict[str, Any]:
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "CLAIM_H0_PRIMARY_SPLIT_V12R1_PIPELINE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_order": list(contract.STAGE_IDS),
...
def _worker_claim_payload(
    *, stage_id: str, token_sha256: str, previous_receipts: Sequence[Mapping[str, Any]]
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "CLAIM_H0_PRIMARY_SPLIT_V12R1_WORKER",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "stage_id": stage_id,
...
def _write_worker_claim(stage_id: str, token_sha256: str) -> Path:
    ...
    if os.path.lexists(claim_path) or os.path.lexists(_stage_receipt_path(stage_id)):
        raise V12R1ExecutionError(f"stage already consumed: {stage_id}")
...
def _load_rollout_stack() -> tuple[Any, Any, Any, Any, Any, Any, Any]:
    ...
    if (
        ...
        raise V12R1ExecutionError("inference stack identity drifted")
...
def _verify_stage_receipt(stage_id: str) -> dict[str, Any]:
    ...
    if kind == "probe":
        ...
        if gate != expected or gate.get("integrity_passed") is not True:
            raise V12R1ExecutionError(f"probe closure drifted: {stage}")
        ...
        current_bindings = (
            ...
            and summary.get("partial_summary")
            == _record(root / "partial_summary.json")
            and summary.get("replay_payload")
            == _record(root / "replay_boundaries.npz")
            and summary.get("worker_claim") == _record(_claim_path(stage_id))
        )
        if receipt != expected_receipt or not current_bindings:
            raise V12R1ExecutionError(f"probe receipt/binding drifted: {stage}")
        return receipt
    if kind == "label":
    ...
    if kind == "collection":
        ...
        if gates != {"data_gate": data_gate, "latch_dependence_gate": latch_gate}:
            raise V12R1ExecutionError(f"collection gate closure drifted: {stage_id}")
        if data_gate.get("passed") is not True:
            raise V12R1ExecutionError(f"collection data gate failed: {stage_id}")
        import numpy as np
        ...
        trace_corpus_exact = (
            ...
            and np.array_equal(
                ...
                np.asarray(
                    [index == 0 for index in range(rows)], dtype=np.bool_
                ),
            )
            ...
            == {f"v12r1_collect_r{descriptor['round_index']}"}
            and set(label_arrays["origins"].astype(str).tolist())
            == {"V12R1_SHIELDED_SAME_STATE"}
        )
        ...
        if receipt != expected_receipt or not current_bindings:
            raise V12R1ExecutionError(
                f"collection receipt/binding drifted: {stage_id}"
            )
        return receipt
    if kind == "freeze":
        summary = _mapping(CANDIDATE_FREEZE_PATH.with_name("candidate_freeze_summary.json"))
        gate = _mapping(CANDIDATE_FREEZE_PATH.with_name("candidate_freeze_gate.json"))
        if gate != contract.candidate_freeze_gate(summary) or gate.get("passed") is not True:
            raise V12R1ExecutionError("candidate freeze closure drifted")
                ...
                expected_collection_receipts.append(
                    {
                        ...
                        "latch_independence_passed": gates[
        ...
        current_bindings = (
            ...
            and summary.get("collection_receipts")
            == expected_collection_receipts
        )
        if receipt != expected_receipt or not current_bindings:
            raise V12R1ExecutionError("candidate freeze receipt/binding drifted")
        return receipt
    if kind == "final":
        root = resolve_relative(descriptor["case"]["destination"])
        summary = _mapping(root / "summary.json")
        gate = _mapping(root / "gate.json")
        if gate != contract.final_rollout_gate(summary) or gate.get("passed") is not True:
            raise V12R1ExecutionError(f"final closure drifted: {stage_id}")
        module = _tree_record(resolve_relative(contract.MODULE_PATHS["p3"]))
        ...
        if receipt != expected_receipt or not current_bindings:
            raise V12R1ExecutionError(f"final receipt/binding drifted: {stage_id}")
        return receipt
    ...
    if gate != contract.final_development_gate(summary) or gate.get("passed") is not True:
        raise V12R1ExecutionError("final aggregate closure drifted")
    ...
    if receipt != expected_receipt or not current_bindings:
        raise V12R1ExecutionError("final aggregate receipt/binding drifted")
    return receipt
    ...
    *,
    ...
    for name, record in expected.items():
        if persisted.get(name) != record:
            raise V12R1ExecutionError(
                f"prospective rollout record drifted: {name}"
            )
...
def _run_probe(stage: str) -> dict[str, Any]:
    ...
    if os.path.lexists(destination):
        raise V12R1ExecutionError(f"probe destination exists: {stage}")
    ...
    writer.start(
        {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "STARTED_H0_PRIMARY_SPLIT_V12R1_PURE_PROBE",
            "protocol_id": contract.PROTOCOL_ID,
            "pipeline_id": contract.PIPELINE_ID,
            "stage_id": stage_id,
            "fit_stage": stage,
            ...
            "fit_receipt": _record(
                resolve_relative(contract.FIT_RECEIPT_PATHS[stage])
            ),
    ...
    try:
        ...
        if not math.isfinite(body_weight_n) or body_weight_n <= 0.0:
            raise V12R1ExecutionError("body weight is malformed")
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if not isinstance(info, Mapping):
                raise V12R1ExecutionError("probe info is malformed")
        ...
        try:
            writer.publish_failure(
                end_reason="v12r1_pure_probe_failed",
                error=exc,
    ...
    if audit is None or replay_recorder is None:
        raise V12R1ExecutionError("probe audit/replay was not initialized")
    ...
    if replay.n_steps != len(rows) or replay.boundary_count != len(rows) + 1:
        raise V12R1ExecutionError("probe replay row/boundary count drifted")
    partial = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PERSISTED_H0_PRIMARY_SPLIT_V12R1_PROBE_BEFORE_GATE",
        "protocol_id": contract.PROTOCOL_ID,
        "stage_id": stage_id,
        "fit_stage": stage,
        "steps": len(rows),
    ...
    if gate.get("integrity_passed") is not True:
        raise V12R1ExecutionError(f"probe integrity failed: {stage}")
    if stage == "p3" and gate.get("passed") is not True:
        raise V12R1ExecutionError("P3 autonomy probe failed terminally")
    return receipt
...
def _run_label(stage: str) -> dict[str, Any]:
    ...
    if result.get("passed") is not True:
        raise V12R1ExecutionError(f"observer label gate failed: {stage}")
    _activity_increment(
...
def _run_collection(round_index: int, case_id: str) -> dict[str, Any]:
    ...
    destination = resolve_relative(case["destination"])
    if os.path.lexists(destination):
        raise V12R1ExecutionError(f"collection destination exists: {stage_id}")
    fit_stage = str(case["candidate_fit_stage"])
    ...
    writer.start(
        {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "STARTED_H0_PRIMARY_SPLIT_V12R1_SHIELDED_COLLECTION",
            "protocol_id": contract.PROTOCOL_ID,
            "pipeline_id": contract.PIPELINE_ID,
            "stage_id": stage_id,
            "round_index": round_index,
    ...
    try:
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if candidate_std.tobytes() != teacher_std.tobytes():
                raise V12R1ExecutionError("candidate/teacher logstd mismatch")
            ...
            if (
                ...
                raise V12R1ExecutionError("blend/latch/noise semantics drifted")
            applied = np.ascontiguousarray(
            ...
            if not isinstance(info, Mapping):
                raise V12R1ExecutionError("collection info is malformed")
            ...
            if step == 1 or step % 25 == 0:
                ...
                eta = elapsed / step * (contract.EXPECTED_STEPS - step)
                print(
                    f"[V12R1 collect r{round_index}/{case_id}] "
        ...
        try:
            writer.publish_failure(
                end_reason="v12r1_shielded_collection_failed",
                error=exc,
        ...
        env.close()
    if audit is None:
        raise V12R1ExecutionError("collection audit was not initialized")
    ...
    v10s_fit._write_npz_exclusive(
        label_path,
        {
            ...
            "tranche_ids": np.asarray(
                [f"v12r1_collect_r{round_index}"] * sample_count,
                dtype="U96",
            ),
            "origins": np.asarray(
                ["V12R1_SHIELDED_SAME_STATE"] * sample_count, dtype="U160"
            ),
        },
    )
    partial = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PERSISTED_H0_PRIMARY_SPLIT_V12R1_COLLECTION_BEFORE_GATE",
        "protocol_id": contract.PROTOCOL_ID,
        "stage_id": stage_id,
        "round_index": round_index,
        "case_id": case_id,
    ...
    data_gate = dict(
        contract.collection_data_gate(summary, round_index=round_index)
    )
    latch_gate = dict(
        contract.latch_dependence_gate(
            summary, collection_data_passed=data_gate.get("passed") is True
    ...
    if data_gate.get("passed") is not True:
        writer.publish_failure(
            end_reason="v12r1_collection_data_gate_failed",
            error="V12R1 collection integrity/physical gate failed",
            status=contract.COLLECTION_FAIL_STATUS,
...
def _run_candidate_freeze() -> dict[str, Any]:
    ...
    stage_id = "freeze_p3"
    summary_path = CANDIDATE_FREEZE_PATH.with_name(
    ...
    for path in (CANDIDATE_FREEZE_PATH, summary_path, gate_path):
        if os.path.lexists(path):
            raise V12R1ExecutionError(f"candidate freeze output exists: {path}")
    ...
    for fit_stage in contract.FIT_STAGES:
        ...
        probe_receipt = _verify_stage_receipt(f"probe_{fit_stage}")
    ...
    summary = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "COMPLETE_H0_PRIMARY_SPLIT_V12R1_CANDIDATE_FREEZE_UNGATED",
        "protocol_id": contract.PROTOCOL_ID,
        ...
        "design_audit_passed": _design_audit_gate(
            _mapping(DESIGN_AUDIT_PATH)
        ).get("passed")
        is True,
        "execution_lock": _record(LOCK_PATH),
    ...
    if gate.get("passed") is not True:
        raise V12R1ExecutionError("candidate freeze gate failed")
    receipt = {
        **summary,
        "status": contract.CANDIDATE_FREEZE_PASS_STATUS,
        "passed": True,
...
def _run_final(case_id: str) -> dict[str, Any]:
    ...
    destination = resolve_relative(case["destination"])
    if os.path.lexists(destination):
        raise V12R1ExecutionError(f"final destination exists: {case_id}")
    ...
    if freeze.get("candidate_id") != candidate_id:
        raise V12R1ExecutionError("candidate freeze identity drifted")
    candidate = RLModule.from_checkpoint(candidate_module_path)
    candidate.eval()
    innovations = _frozen_innovations(
        case_id,
    ...
    writer.start(
        {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "STARTED_H0_PRIMARY_SPLIT_V12R1_FINAL_ROLLOUT",
            "protocol_id": contract.PROTOCOL_ID,
            "pipeline_id": contract.PIPELINE_ID,
            "stage_id": stage_id,
            "case": copy.deepcopy(case),
    ...
    try:
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if not isinstance(info, Mapping):
                raise V12R1ExecutionError("final rollout info is malformed")
            physical = _consume_physical_step(
                audit,
                step=step,
                info=info,
        ...
        try:
            writer.publish_failure(
                end_reason="v12r1_final_rollout_failed",
                error=exc,
        ...
        env.close()
    if audit is None:
        raise V12R1ExecutionError("final audit was not initialized")
    partial = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PERSISTED_H0_PRIMARY_SPLIT_V12R1_FINAL_BEFORE_GATE",
        "protocol_id": contract.PROTOCOL_ID,
        "stage_id": stage_id,
        "case_id": case_id,
        "steps": len(rows),
    ...
    if gate.get("passed") is not True:
        writer.publish_failure(
            end_reason="v12r1_final_gate_failed",
            error="V12R1 final rollout physical/dependency gate failed",
            status=contract.FINAL_ROLLOUT_FAIL_STATUS,
            details={"gate": _record(writer.gate_path)},
        )
        raise V12R1ExecutionError(f"final rollout gate failed: {case_id}")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": contract.FINAL_ROLLOUT_PASS_STATUS,
        "passed": True,
...
def _run_finalize_development() -> dict[str, Any]:
    ...
    for path in (receipt_path, summary_path, gate_path):
        if os.path.lexists(path):
            raise V12R1ExecutionError(f"final aggregate output exists: {path}")
    freeze = _candidate_freeze_receipt()
    module = _tree_record(resolve_relative(contract.MODULE_PATHS["p3"]))
    candidate_id = contract.candidate_id("p3", module["tree_sha256"])
    ...
    forensic.write_json_exclusive(gate_path, gate)
    if gate.get("passed") is not True:
        raise V12R1ExecutionError("final development aggregate gate failed")
    receipt = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": contract.FINAL_DEVELOPMENT_PASS_STATUS,
        "passed": True,
...
def _run_fit(stage: str) -> dict[str, Any]:
    ...
    if result.get("passed") is not True:
        raise V12R1ExecutionError(f"fit gate failed: {stage}")
...
def _run_stage(stage_id: str) -> dict[str, Any]:
    ...
    raise V12R1ExecutionError(f"unknown stage kind: {kind!r}")
...
def _pipeline_activity_gate(
    ...
    checks = {
        ...
        "four_observer_label_stages": totals[
            "offline_teacher_label_stage_calls"
        ]
        == 4,
        "positive_exact_rollout_steps": totals["environment_step_calls"] > 0,
        "positive_exact_offline_labels": totals[
            "offline_teacher_label_calls_confirmed"
        ]
        > 0,
    }
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V12R1_PIPELINE_ACTIVITY"
            if all(checks.values())
            else "FAIL_H0_PRIMARY_SPLIT_V12R1_PIPELINE_ACTIVITY"
        ),
        "passed": all(checks.values()),
        "protocol_id": contract.PROTOCOL_ID,
        "checks": checks,
...
def _ledger_payload(
    *,
    ...
    completed_rollout_count = sum(
        int(contract.stage_descriptor(stage)["kind"] in {"probe", "collection", "final"})
        for stage in completed_stages
    )
    ...
    if passed and activity_gate.get("passed") is not True:
        raise V12R1ExecutionError("pipeline PASS activity accounting drifted")
...
def _claim_run_root() -> None:
    """Atomically claim the exact V12R1 run directory before its first file."""

    if RUN_ROOT.resolve().parent != resolve_relative(contract.RUN_ROOT.parent):
        raise V12R1ExecutionError("run-root parent drifted")
    for ancestor in (RUN_ROOT.parent, *RUN_ROOT.parent.parents):
        ...
        if os.path.lexists(ancestor) and ancestor.is_symlink():
            raise V12R1ExecutionError(f"run-root ancestor is a symlink: {ancestor}")
    RUN_ROOT.parent.mkdir(parents=True, exist_ok=True)
    try:
        RUN_ROOT.mkdir(exist_ok=False)
    except FileExistsError as exc:
        raise V12R1ExecutionError("V12R1 run root already claimed") from exc
...
def _publish_terminal_stage_failure(
    ...
    try:
        ...
        artifact = writer.publish_failure(
            end_reason="v12r1_stage_failed_terminally",
            error=error,
            status=status,
...
def _preclaim_failure_ledger(error: BaseException) -> dict[str, Any]:
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "FAIL_H0_PRIMARY_SPLIT_V12R1_PIPELINE_CLAIM_TERMINAL",
        "passed": False,
        "protocol_id": contract.PROTOCOL_ID,
        "pipeline_id": contract.PIPELINE_ID,
        "execution_lock": _record(LOCK_PATH),
...
def execute_pipeline_once() -> dict[str, Any]:
    """Claim and execute the exact 26-stage V12R1 order once."""
    ...
    if os.path.lexists(PIPELINE_CLAIM_PATH) or os.path.lexists(PIPELINE_LEDGER_PATH):
        raise V12R1ExecutionError("pipeline is already claimed or terminal")
    ...
    try:
        for stage_id in contract.STAGE_IDS:
            ...
            _verify_worker_claim(stage_id)
            print(
                f"[V12R1 pipeline] start {len(completed) + 1:02d}/"
            ...
            attempted = None
            print(
                f"[V12R1 pipeline] pass  {len(completed):02d}/"
                f"{len(contract.STAGE_IDS):02d} {stage_id}",
                flush=True,
            )
        if tuple(completed) != tuple(contract.STAGE_IDS):
            raise V12R1ExecutionError("pipeline completed-stage order drifted")
        ledger = _ledger_payload(
            status="PASS_H0_PRIMARY_SPLIT_V12R1_PIPELINE",
            passed=True,
        ...
        failure = _ledger_payload(
            status="FAIL_H0_PRIMARY_SPLIT_V12R1_PIPELINE_TERMINAL",
            passed=False,
            completed_stages=completed,
...
def _parser() -> argparse.ArgumentParser:
    ...
    action.add_argument(
        "--execute-once",
        action="store_true",
        help="claim and execute the frozen V12R1 pipeline once",
    )
    return parser
