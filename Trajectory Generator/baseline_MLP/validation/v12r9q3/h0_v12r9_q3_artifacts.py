"""Portable fail-closed artifact predicates for the V12R9-Q3 scaffold.
...
class V12R9Q3ArtifactError(ValueError):
...
def tree_digest(rows: Sequence[Mapping[str, Any]]) -> str:
    """Compute the canonical tree digest used by the R6/R9 actor artifacts."""
...
def validate_exact_tree(
    ...
    *,
    ...
    """Return a normalized exact tree or raise :class:`V12R9Q3ArtifactError`."""
    ...
    if (
        ...
        raise V12R9Q3ArtifactError("artifact tree schema or root path is invalid")
    ...
    for row in rows:
        ...
        if (
            ...
            raise V12R9Q3ArtifactError("artifact tree contains a malformed file")
        normalized.append(current)

    names = tuple(row["path"] for row in normalized)
    if names != expected_names or len(set(names)) != len(names):
        raise V12R9Q3ArtifactError("artifact tree file manifest is not exact")
    if tree_digest(normalized) != tree["tree_sha256"]:
        raise V12R9Q3ArtifactError("artifact tree digest does not bind its files")
    return {
        "path": path,
