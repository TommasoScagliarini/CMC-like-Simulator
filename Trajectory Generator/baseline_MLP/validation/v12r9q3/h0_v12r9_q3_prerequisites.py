"""Deferred R9 binding and semantic prerequisite verifier hooks for Q3.
...
try:
    from . import h0_v12r9_q3_artifacts as artifacts
    from . import h0_v12r9_q3_qualification_contract as contract
except ImportError:  # Direct test execution with this directory on sys.path.
    import h0_v12r9_q3_artifacts as artifacts
    import h0_v12r9_q3_qualification_contract as contract


class V12R9Q3PrerequisiteError(RuntimeError):
    """Raised unless the exact terminal-PASS R9 lineage is verified."""


@dataclass(frozen=True)
class R9SemanticVerifierHooks:
    """The only five live R9 verifier entry points accepted by this scaffold."""
...
R9_CANDIDATE_ID: str | None = None
R9_CANDIDATE_MODULE: dict[str, Any] | None = None
...
def validate_candidate_tree(candidate_id: Any, candidate_module: Any) -> dict[str, Any]:
    """Validate the exact five-file standard R9 actor tree and identity."""
    ...
    expected_id = contract.r9.candidate_id(module["tree_sha256"])
    if candidate_id != expected_id:
        raise artifacts.V12R9Q3ArtifactError(
            "R9 candidate identity does not bind the exact five-file tree"
        )
...
def validate_actor_feature_manifest(
    ...
    *,
    ...
    """Validate the R9 standard 35->512->512->2 actor feature contract."""
    ...
    if not (record_ok and semantic_ok):
        raise artifacts.V12R9Q3ArtifactError(
            "R9 actor feature manifest is not the exact standard W512 contract"
        )
...
def candidate_freeze_semantics(
    ...
    *,
    ...
    """Pure semantic verifier for the R9 candidate-freeze receipt."""
    ...
    try:
        ...
        manifest = None
    checks = {
        "status": data.get("status") == contract.r9.CANDIDATE_FREEZE_PASS_STATUS
        and data.get("passed") is True,
        "identity": data.get("protocol_id") == contract.r9.PROTOCOL_ID
        and data.get("pipeline_id") == contract.r9.PIPELINE_ID
        ...
        and data.get("stage_id") == "freeze_recovery_actor",
...
def final_development_semantics(
    ...
    *,
    ...
    """Pure semantic verifier for the six-case R9 final-development receipt."""

    data = dict(payload) if isinstance(payload, Mapping) else {}
    rows = data.get("case_gates")
    rows_ok = isinstance(rows, list) and len(rows) == len(
        contract.r9.DEVELOPMENT_CASE_IDS
    )
    if rows_ok:
        for row, case_id in zip(rows, contract.r9.DEVELOPMENT_CASE_IDS, strict=True):
            ...
            if (
                ...
                break
    checks = {
        "status": data.get("status") == contract.r9.DEVELOPMENT_PASS_STATUS
        and data.get("passed") is True,
        "identity": data.get("protocol_id") == contract.r9.PROTOCOL_ID
        and data.get("pipeline_id") == contract.r9.PIPELINE_ID
        ...
        and _one_int(data.get("actor_updates"))
        and data.get("development_rollout_count") == 6
        ...
        and _zero_int(data.get("critic_updates"))
        and _zero_int(data.get("ppo_updates")),
        "physical_accounting": data.get("stage_id") == "finalize_development"
        and data.get("historical_prefix_environment_reset_calls") == 0
        and data.get("historical_prefix_environment_step_calls") == 0
        and data.get("new_collection_environment_reset_calls")
        == len(contract.r9.NEW_COLLECTION_CASE_IDS)
        and data.get("development_environment_reset_calls")
        == len(contract.r9.DEVELOPMENT_CASE_IDS)
        and data.get("environment_reset_calls")
        == len(contract.r9.NEW_COLLECTION_CASE_IDS)
        + len(contract.r9.DEVELOPMENT_CASE_IDS)
        and _positive_int(data.get("new_collection_environment_step_calls"))
        and _positive_int(data.get("development_environment_step_calls"))
        and data.get("environment_step_calls")
        == data.get("new_collection_environment_step_calls")
        + data.get("development_environment_step_calls"),
        "qualification_closed": data.get("q3_paths_opened") == []
        and data.get("runtime_promoted") is False
        and data.get("qualification_executed") is False
        ...
        and data.get("alpha_sweep_authorized") is False,
    }
    passed = all(checks.values())
    return {
...
def terminal_ledger_semantics(
    ...
    *,
    ...
    """Pure semantic verifier for the immutable terminal-PASS R9 ledger."""

    data = dict(payload) if isinstance(payload, Mapping) else {}
    completed = data.get("completed_stages")
    completed_ok = (
        isinstance(completed, list)
        and len(completed) == len(contract.r9.STAGE_IDS)
        and data.get("completed_stage_count") == len(contract.r9.STAGE_IDS)
    )
    if completed_ok:
        completed_ok = all(
            isinstance(row, Mapping) and row.get("stage_id") == stage_id
            for row, stage_id in zip(completed, contract.r9.STAGE_IDS, strict=True)
        )
    observation = data.get("candidate_module_observation")
    checks = {
        "terminal_pass": data.get("status") == contract.r9.PIPELINE_TERMINAL_PASS_STATUS
        and data.get("passed") is True
        ...
        and data.get("candidate_id") == binding.get("candidate_id")
        and data.get("candidate_module") == binding.get("candidate_module"),
        "full_candidate_observation": isinstance(observation, Mapping)
        and observation.get("state") == "VALID_NONEMPTY_TREE"
        and observation.get("tree") == binding.get("candidate_module"),
        ...
        and _zero_int(data.get("critic_updates"))
        and _zero_int(data.get("ppo_updates")),
        "complete_stage_order": data.get("stage_order") == list(contract.r9.STAGE_IDS)
        and completed_ok
        and data.get("attempted_stage") is None
        and data.get("error") is None,
        "physical_accounting": data.get("development_count")
        == len(contract.r9.DEVELOPMENT_CASE_IDS)
        and data.get("historical_prefix_environment_reset_calls") == 0
        and data.get("historical_prefix_environment_step_calls") == 0
        and data.get("new_collection_environment_reset_calls")
        ...
        and data.get("resume_authorized") is False
        and data.get("alpha_sweep_authorized") is False
        and data.get("next_stage") == "WAIT_SEPARATE_V12R9Q3_PROTOCOL",
    }
    passed = all(checks.values())
    return {
...
def invoke_verifier_hooks(hooks: R9SemanticVerifierHooks) -> dict[str, Any]:
    ...
    for name, callback in zip(names, callbacks, strict=True):
        ...
        if not isinstance(value, Mapping):
            raise V12R9Q3PrerequisiteError(
                f"R9 semantic verifier returned a non-object: {name}"
            )
...
def bind_candidate(candidate_id: Any, candidate_module: Any) -> dict[str, Any]:
    """Bind one exact terminal-PASS R9 candidate once per process."""

    global R9_CANDIDATE_ID, R9_CANDIDATE_MODULE
    binding = validate_candidate_tree(candidate_id, candidate_module)
    if R9_CANDIDATE_ID is not None and (
        R9_CANDIDATE_ID != binding["candidate_id"]
        or R9_CANDIDATE_MODULE != binding["candidate_module"]
    ):
        raise V12R9Q3PrerequisiteError("R9-Q3 candidate binding is immutable")
    R9_CANDIDATE_ID = str(binding["candidate_id"])
    R9_CANDIDATE_MODULE = copy.deepcopy(binding["candidate_module"])
    return current_candidate_binding()


def current_candidate_binding() -> dict[str, Any]:
    if R9_CANDIDATE_ID is None or R9_CANDIDATE_MODULE is None:
        raise V12R9Q3PrerequisiteError(
            "R9-Q3 remains fail-closed: terminal R9 candidate is not bound"
        )
    return {
        "candidate_id": R9_CANDIDATE_ID,
        "candidate_module": copy.deepcopy(R9_CANDIDATE_MODULE),
    }


def clear_candidate_binding_for_tests() -> None:
    global R9_CANDIDATE_ID, R9_CANDIDATE_MODULE
    R9_CANDIDATE_ID = None
    R9_CANDIDATE_MODULE = None


__all__ = [
    "R9SemanticVerifierHooks",
    "V12R9Q3PrerequisiteError",
    "bind_candidate",
