...
class V12R9Q3NoiseError(RuntimeError):
    """Raised before an invalid or premature tape operation."""


NOISE_TAPES_PASS_STATUS = "PASS_H0_V12R9_Q3_QUALIFICATION_NOISE_TAPES"
...
def build_tapes() -> dict[str, dict[str, Any]]:
    ...
    if hashes != contract.EXPECTED_TAPE_ARRAY_SHA256:
        raise V12R9Q3NoiseError("Q3 tape ABI/hash drifted")
    if not all(
        ...
        raise V12R9Q3NoiseError("Q3 tape array contract drifted")
    return tapes


def _write_npz_exclusive(path: Path, *, array: Any, seed: int | None) -> None:
    if os.path.lexists(path):
        raise V12R9Q3NoiseError(f"refusing to clobber: {path}")
    ...
    try:
        with np.load(path, allow_pickle=False) as archive:
            ...
            if set(archive.files) != expected:
                raise V12R9Q3NoiseError(f"tape keys drifted: {path}")
            if seed is not None:
                ...
                if (
                    ...
                    raise V12R9Q3NoiseError(f"tape seed drifted: {path}")
            stored = archive["standard_normal"]
            if stored.dtype != np.float32 or stored.shape != TAPE_SHAPE:
                raise V12R9Q3NoiseError(f"tape array ABI drifted: {path}")
            return np.ascontiguousarray(stored)
    except V12R9Q3NoiseError:
        raise
    except BaseException as exc:
        raise V12R9Q3NoiseError(f"cannot read tape: {path}") from exc
...
def _strict_mapping(path: Path) -> dict[str, Any]:
    try:
        ...
        raise V12R9Q3NoiseError(f"cannot read strict JSON: {path}") from exc
    if not isinstance(value, Mapping):
        raise V12R9Q3NoiseError(f"expected JSON object: {path}")
    result = dict(value)
    if path.read_bytes() != forensic.canonical_json_bytes(result):
        raise V12R9Q3NoiseError(f"JSON is not canonical: {path}")
    return result


def _validated_snapshot(provided: Mapping[str, Any] | None) -> dict[str, Any]:
    snapshot = (
        freezer.live_r9_prerequisite_snapshot(require_q3_unopened=False)
        if provided is None
        else copy.deepcopy(dict(provided))
    )
    gate = snapshot.get("gate")
    if not isinstance(gate, Mapping) or gate.get("passed") is not True:
        raise V12R9Q3NoiseError("R9 prerequisite snapshot is not terminal PASS")
    try:
        prerequisites.bind_candidate(
            gate.get("candidate_id"), gate.get("candidate_module")
        )
    except BaseException as exc:
        raise V12R9Q3NoiseError("R9 candidate snapshot is invalid") from exc
    return snapshot
...
def prepare(
    *,
    ...
    if enforce_canonical_destination and root != canonical:
        raise V12R9Q3NoiseError(f"non-canonical noise destination: {root}")
    if os.path.lexists(root):
        raise V12R9Q3NoiseError(f"refusing to clobber: {root}")
    ...
    if (
        ...
        raise V12R9Q3NoiseError("protocol/lock records are not canonical")
    definitions = build_tapes()
...
def verify_manifest(
    *,
    ...
    if enforce_canonical_destination and root != canonical:
        raise V12R9Q3NoiseError(f"non-canonical noise root: {root}")
    ...
    if (
        ...
        raise V12R9Q3NoiseError("noise root file set drifted")
    ...
    if (
        ...
        raise V12R9Q3NoiseError("noise manifest semantics drifted")
    ...
    for filename, expected_hash in contract.EXPECTED_TAPE_ARRAY_SHA256.items():
        ...
        if (
            ...
            raise V12R9Q3NoiseError(f"noise tape closure drifted: {filename}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.build_only:
        result = {
            "status": "PASS_H0_V12R9_Q3_TAPES_BUILD_ONLY",
