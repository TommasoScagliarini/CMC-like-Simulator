"""Freeze V12R9-Q3 only after five live semantic R9 verifiers pass.

Importing this module is inert.  Publication is explicit, exclusive, and
fail-closed; the canonical Q3 paths must all be absent when the live R9
...
class V12R9Q3FreezeError(RuntimeError):
    """Raised before any unsafe, premature, or drifting Q3 freeze."""


PROTOCOL_FREEZE_PASS_STATUS = "PASS_H0_V12R9_Q3_QUALIFICATION_PROTOCOL_FREEZE"
EXECUTION_LOCK_PASS_STATUS = "PASS_H0_V12R9_Q3_QUALIFICATION_EXECUTION_LOCK"
...
def resolve_relative(value: str | PurePosixPath) -> Path:
    ...
    if not artifacts.canonical_relative_path(raw):
        raise V12R9Q3FreezeError(f"non-canonical repository path: {raw!r}")
...
def _record(value: str | PurePosixPath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R9Q3FreezeError(f"unsafe or missing artifact: {path}") from exc
...
def _mapping(path: str | PurePosixPath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R9Q3FreezeError(f"strict JSON could not be read: {source}") from exc
    if not isinstance(value, Mapping):
        raise V12R9Q3FreezeError(f"expected JSON object: {source}")
    result = dict(value)
    if source.read_bytes() != forensic.canonical_json_bytes(result):
        raise V12R9Q3FreezeError(f"JSON is not canonical: {source}")
    return result
...
def official_r9_hooks() -> prerequisites.R9SemanticVerifierHooks:
    """Load the five public R9 verifiers lazily in the RLlib environment."""

    try:
        freezer = importlib.import_module("freeze_h0_v12r9_recovery")
        runner = importlib.import_module("run_h0_v12r9_recovery")
    except BaseException as exc:
        raise V12R9Q3FreezeError("R9 verifier modules are unavailable") from exc
    return prerequisites.R9SemanticVerifierHooks(
...
def live_r9_prerequisite_snapshot(
    *,
    hooks: prerequisites.R9SemanticVerifierHooks | None = None,
    ...
    if occupied:
        raise V12R9Q3FreezeError(f"Q3 namespace is already occupied: {occupied}")
    selected = official_r9_hooks() if hooks is None else hooks
    try:
        payloads = prerequisites.invoke_verifier_hooks(selected)
    except BaseException as exc:
        raise V12R9Q3FreezeError("five live R9 verifiers did not converge") from exc
    ...
    if mismatched:
        raise V12R9Q3FreezeError(
            f"R9 verifier payload/file binding drifted: {mismatched}"
        )
    ...
    if gate.get("passed") is not True:
        ...
        raise V12R9Q3FreezeError(f"semantic R9 prerequisite gate failed: {failed}")
    return {
        "payloads": payloads,
        "records": records,
...
def source_closure() -> dict[str, dict[str, Any]]:
    ...
    if gate.get("passed") is not True:
        ...
        raise V12R9Q3FreezeError(f"Q3 source closure failed: {failed}")
    return records
...
def source_closure_readiness() -> dict[str, Any]:
    ...
    if missing:
        return {
            "status": "DEFERRED_H0_V12R9_Q3_SOURCE_CLOSURE",
            "passed": False,
    ...
    except V12R9Q3FreezeError as exc:
        return {
            "status": "FAIL_H0_V12R9_Q3_SOURCE_CLOSURE",
            "passed": False,
            "missing_sources": [],
            "error": str(exc),
            "records": {},
    ...
    return {
        "status": "PASS_H0_V12R9_Q3_SOURCE_CLOSURE",
        "passed": True,
        "missing_sources": [],
        "records": records,
    }
...
def build_protocol_payload(
    *,
    ...
    if not isinstance(gate, Mapping) or gate.get("passed") is not True:
        raise V12R9Q3FreezeError("protocol requires a semantic terminal-PASS R9 gate")
    ...
    if closure_gate.get("passed") is not True:
        raise V12R9Q3FreezeError("protocol source closure is incomplete")
    return {
        ...
        "candidate_binding_state": "BOUND_FROM_R9_TERMINAL_PASS",
        ...
        "r9_prerequisite_gate": copy.deepcopy(dict(gate)),
        "r9_prerequisite_records": copy.deepcopy(
...
def protocol_payload_gate(payload: Any) -> dict[str, Any]:
    ...
    checks = {
        ...
        and data.get("candidate_binding_state") == "BOUND_FROM_R9_TERMINAL_PASS"
        and data.get("candidate_selection_rule") == contract.CANDIDATE_SELECTION_RULE,
        "prerequisite": isinstance(data.get("r9_prerequisite_gate"), Mapping)
        and data["r9_prerequisite_gate"].get("passed") is True,
...
def build_execution_lock_payload(
    *,
    ...
    if protocol_payload_gate(protocol_payload).get("passed") is not True:
        raise V12R9Q3FreezeError("execution lock requires an exact protocol payload")
    expected_occupancy = {
    ...
    if dict(occupancy) != expected_occupancy:
        raise V12R9Q3FreezeError("execution lock occupancy is not pristine")
...
def freeze_protocol_and_lock() -> dict[str, Any]:
    """Publish both canonical Q3 locks exactly once after R9 terminal PASS."""

    if _q3_occupied():
        raise V12R9Q3FreezeError(f"Q3 freeze paths already occupied: {_q3_occupied()}")
    snapshot = live_r9_prerequisite_snapshot(require_q3_unopened=True)
    ...
    if protocol_gate.get("passed") is not True:
        raise V12R9Q3FreezeError("Q3 protocol pure gate failed")
...
def verify_protocol_freeze() -> dict[str, Any]:
    ...
    if protocol_payload_gate(payload).get("passed") is not True:
        raise V12R9Q3FreezeError("Q3 protocol freeze drifted")
    if payload.get("source_closure") != source_closure():
        raise V12R9Q3FreezeError("Q3 protocol source bytes drifted")
    return payload
...
def verify_execution_lock() -> dict[str, Any]:
    ...
    if not all(checks.values()):
        ...
        raise V12R9Q3FreezeError(f"Q3 execution lock drifted: {failed}")
...
def deferred_preflight() -> dict[str, Any]:
    ...
    return {
        "status": "DEFERRED_H0_V12R9_Q3_PROTOCOL_FREEZE",
        "passed": False,
        "lineage_state": contract.LINEAGE_STATE,
        "r9_prerequisites": state,
...
def live_preflight() -> dict[str, Any]:
    """Verify five live R9 receipts and the closed source set without writing."""
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PASS_H0_V12R9_Q3_LIVE_FREEZE_PREFLIGHT",
        "passed": True,
        "candidate_id": gate["candidate_id"],
        "candidate_module": copy.deepcopy(gate["candidate_module"]),
