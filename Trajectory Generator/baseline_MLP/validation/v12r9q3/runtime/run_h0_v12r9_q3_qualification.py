"""Run the candidate-bound V12R9-Q3 qualification exactly once.

The runtime is deliberately inert on import.  ``execute_qualification`` first
re-runs the five official R9 semantic verifiers, verifies the immutable Q3
...
import freeze_h0_v12r9_q3_qualification_protocol as freezer  # noqa: E402
import h0_v12r9_q3_physical_rollout as physical_runtime  # noqa: E402
import prepare_h0_v12r9_q3_noise_tapes as noise  # noqa: E402


class V12R9Q3QualificationExecutionError(RuntimeError):
    """Raised when the immutable one-shot Q3 pipeline cannot continue."""


PIPELINE_CLAIM_STATUS = "CLAIM_H0_V12R9_Q3_QUALIFICATION_PIPELINE"
ROLLOUT_STARTED_STATUS = "STARTED_H0_V12R9_Q3_QUALIFICATION_ROLLOUT"
ROLLOUT_PERSISTED_STATUS = "PERSISTED_H0_V12R9_Q3_BEFORE_GATE"
STAGE_FAILURE_STATUS = "FAIL_H0_V12R9_Q3_QUALIFICATION_STAGE"
PIPELINE_TERMINAL_FAIL_STATUS = "FAIL_H0_V12R9_Q3_PIPELINE_TERMINAL"
...
def _strict_mapping(value: str | PurePosixPath | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R9Q3QualificationExecutionError(
            f"cannot load strict JSON: {path}"
        ) from exc
    if not isinstance(payload, Mapping):
        raise V12R9Q3QualificationExecutionError(f"expected JSON object: {path}")
    result = dict(payload)
    if path.read_bytes() != forensic.canonical_json_bytes(result):
        raise V12R9Q3QualificationExecutionError(f"JSON is not canonical: {path}")
    return result
...
def _tree_record(value: str | PurePosixPath) -> dict[str, Any]:
    ...
    if not root.is_dir() or root.is_symlink():
        raise V12R9Q3QualificationExecutionError(f"unsafe artifact tree: {root}")
    ...
    if not files:
        raise V12R9Q3QualificationExecutionError(f"artifact tree is empty: {root}")
    rows = []
    for item in files:
        record = _record(item)
        rows.append(
...
def _stage_root(stage_id: str) -> Path:
    ...
    if prefix != "rollout":
        raise V12R9Q3QualificationExecutionError(f"invalid Q3 stage: {stage_id}")
...
def _runtime_inputs(
    ...
    if not isinstance(gate, Mapping):
        raise V12R9Q3QualificationExecutionError("live R9 gate is malformed")
    if (
        ...
        raise V12R9Q3QualificationExecutionError(
            "live R9 candidate no longer matches the Q3 lock"
        )
    candidate = _tree_record(contract.CANDIDATE_MODULE_PATH)
    if candidate != lock.get("candidate_module"):
        raise V12R9Q3QualificationExecutionError("R9 candidate tree bytes drifted")
    ...
    if (
        ...
        raise V12R9Q3QualificationExecutionError("source H0 tree bytes drifted")
    closure = lock.get("source_closure")
    if not isinstance(closure, Mapping):
        raise V12R9Q3QualificationExecutionError("Q3 lock source closure is malformed")
    result = {
...
def build_execution_preflight(
    *, require_run_root_absent: bool = True
    ...
    snapshot = freezer.live_r9_prerequisite_snapshot(require_q3_unopened=False)
    ...
    checks = {
        ...
        "five_live_r9_verifiers": snapshot.get("gate", {}).get("passed") is True
    ...
    return {
        ...
        "status": "PASS_H0_V12R9_Q3_EXECUTION_PREFLIGHT",
        "passed": True,
        "checks": checks,
...
def _claim_pipeline(preflight: Mapping[str, Any]) -> str:
    ...
    if os.path.lexists(run_root):
        raise V12R9Q3QualificationExecutionError("Q3 run root exists/no retry")
...
def _verify_pipeline_claim() -> dict[str, Any]:
    ...
    if not artifacts.is_sha256(token_hash):
        raise V12R9Q3QualificationExecutionError("pipeline token hash drifted")
    preflight = build_execution_preflight(require_run_root_absent=False)
    expected = _pipeline_claim_payload(preflight, token_hash)
    if claim != expected:
        raise V12R9Q3QualificationExecutionError("pipeline claim drifted")
    return claim
...
def _run_rollout(
    ...
    *,
    ...
    if os.path.lexists(root):
        raise V12R9Q3QualificationExecutionError(f"stage exists/no retry: {stage_id}")
...
def verify_rollout_receipt(role: str, case_id: str) -> dict[str, Any]:
    ...
    if role == contract.CANDIDATE_ROLE:
        ...
        if pair != recalculated or pair.get("passed") is not True:
            raise V12R9Q3QualificationExecutionError(f"pair gate drifted: {case_id}")
...
def _run_aggregate() -> dict[str, Any]:
    ...
    if os.path.lexists(root):
        raise V12R9Q3QualificationExecutionError("aggregate root exists/no retry")
    for role in contract.ROLE_ORDER:
        for case_id in contract.CASE_IDS:
    ...
    try:
        ...
        if gate.get("passed") is not True:
            raise V12R9Q3QualificationExecutionError("aggregate six-of-six gate failed")
        forensic.write_json_exclusive(
...
def verify_final_receipt() -> dict[str, Any]:
    ...
    if (
        ...
        raise V12R9Q3QualificationExecutionError("Q3 aggregate receipt closure drifted")
    return receipt
...
def deferred_preflight() -> dict[str, Any]:
    """Return the source-only state without invoking R9 or publishing Q3."""

    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "DEFERRED_H0_V12R9_Q3_RUNTIME",
        "passed": False,
        "lineage_state": contract.LINEAGE_STATE,
        "candidate_binding_state": contract.CANDIDATE_BINDING_STATE,
        "stage_order": list(STAGE_IDS),
        "five_live_r9_verifiers_required": True,
...
__all__ = [
    ...
    "STAGE_IDS",
    "V12R9Q3QualificationExecutionError",
