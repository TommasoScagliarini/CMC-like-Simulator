"""Environment-free tests for the canonical deferred V12R9-Q3 runtime."""
...
import h0_v12r9_q3_physical_rollout as physical  # noqa: E402
...
import run_h0_v12r9_q3_qualification as runner  # noqa: E402
...
def _candidate() -> tuple[str, dict[str, Any]]:
    ...
    return contract.r9.candidate_id(module["tree_sha256"]), module
...
def test_imports_are_inert_and_deferred() -> None:
    ...
    assert state["lineage_state"] == "DEFERRED_UNTIL_R9_TERMINAL_PASS"
...
import h0_v12r9_q3_physical_rollout
import run_h0_v12r9_q3_qualification
...
def test_official_hook_adapter_uses_the_five_r9_entrypoints(
    ...
    def fake_import(name: str) -> Any:
        return r9_freezer if name == "freeze_h0_v12r9_recovery" else r9_runner

    monkeypatch.setattr(freezer.importlib, "import_module", fake_import)
    hooks = freezer.official_r9_hooks()
...
def test_actual_r9_endpoint_surface_is_importable_without_invocation() -> None:
    hooks = freezer.official_r9_hooks()
    callbacks = (
...
def test_current_source_closure_is_complete_and_hash_bound() -> None:
    ...
    assert readiness["status"] == "PASS_H0_V12R9_Q3_SOURCE_CLOSURE"
...
def test_tape_prepare_is_no_clobber_and_tamper_evident(tmp_path: Path) -> None:
    ...
    with pytest.raises(noise.V12R9Q3NoiseError, match="clobber"):
        noise.prepare(
            output_root=root,
            prerequisite_snapshot=snapshot,
            protocol_record=_record(contract.PROTOCOL_FREEZE_PATH.as_posix()),
    ...
    with pytest.raises(noise.V12R9Q3NoiseError, match="closure drifted"):
        noise.verify_manifest(noise_root=root, enforce_canonical_destination=False)
...
def test_comparison_metric_flattening_is_exact_and_finite() -> None:
    ...
    with pytest.raises(physical.V12R9Q3PhysicalRolloutError, match="missing"):
...
def test_r9_q3_cli_reports_deferred_without_publication(
    capsys: pytest.CaptureFixture[str],
) -> None:
    assert runner.main(["--deferred-preflight"]) == 0
    assert capsys.readouterr().out.strip() == "DEFERRED_H0_V12R9_Q3_RUNTIME"


def test_live_mutation_paths_remain_deferred_until_r9_terminal_artifacts_exist() -> (
    None
):
    assert freezer.deferred_preflight()["passed"] is False
    assert runner.deferred_preflight()["five_live_r9_verifiers_required"] is True
