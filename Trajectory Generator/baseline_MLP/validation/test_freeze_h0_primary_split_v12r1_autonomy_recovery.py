...
import freeze_h0_primary_split_v12r1_autonomy_recovery as freeze
...
def test_sources_bind_every_r1_design_file_and_nested_attributes() -> None:
    ...
    for name in ("contract", "freeze", "contract_tests", "freeze_tests"):
        assert "v12r1" in str(records[name]["path"])
...
def test_nested_attributes_cover_r1_text_and_binary_artifacts() -> None:
    path = freeze.resolve_relative(
        freeze.contract.SOURCE_RELATIVE_PATHS["line_endings_r1"]
    )
    attributes = path.read_text(encoding="utf-8")
    assert "h0_primary_split_v12r1_*.py text eol=lf" in attributes
    assert "h0_v12r1_run_*/**/*.json text eol=lf" in attributes
    for extension in (
        "npz",
        "npy",
        "pkl",
        ...
        assert f"h0_v12r1_run_*/**/*.{extension} -text" in attributes
...
def test_authority_is_design_only_and_rejects_execution_flag(
    ...
    assert all(gate["checks"].values())
    monkeypatch.setitem(
        freeze.contract.AUTHORITY, "environment_reset_authorized", True
    )
...
def test_build_passes_without_opening_execution_scope(tmp_path: Path) -> None:
    ...
    assert payload["schema_version"] == 121
    ...
    assert payload["next_stage"] == (
        "REQUIRE_SEPARATE_V12R1_NO_CLOBBER_EXECUTION_LOCK"
    )
...
def test_build_adversarially_rejects_missing_event_or_config_field(
    ...
    broken = copy.deepcopy(freeze.contract.PROBE_REPLAY_SCHEMA)
    del broken["left_event_journal_arrays"][
...
def test_noncanonical_prepare_is_exact_no_clobber_and_tamper_evident(
    ...
    assert freeze.verify_protocol_freeze(
    ...
    with pytest.raises(freeze.V12R1ProtocolFreezeError, match="drifted"):
        freeze.verify_protocol_freeze(
            input_path=destination,
            enforce_canonical_destination=False,
        )
    with pytest.raises(freeze.V12R1ProtocolFreezeError, match="exists/no-clobber"):
...
def test_canonical_outputs_remain_absent_until_root_review() -> None:
    for path in (
        freeze.contract.PROTOCOL_FREEZE_PATH,
        freeze.contract.EXECUTION_LOCK_PATH,
        freeze.contract.DESIGN_AUDIT_RECEIPT_PATH,
        freeze.contract.RUN_ROOT,
    ):
        assert not os.path.lexists(freeze._lexical_absolute(path))
