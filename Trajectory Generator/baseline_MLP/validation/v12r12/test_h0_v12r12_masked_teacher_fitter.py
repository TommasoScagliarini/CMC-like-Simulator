...
import run_h0_v12r12_masked_teacher_fit as runner  # noqa: E402
...
def test_high_fidelity_gates_match_review_contract() -> None:
    assert fitter.GLOBAL_RMSE_LIMIT == 2.5e-4
    assert fitter.GLOBAL_MAX_ABS_LIMIT == 3.0e-3
    assert fitter.PER_CASE_ACTION_RMSE_LIMIT == 5.0e-4
    assert fitter.PER_TRAJECTORY_ACTION_RMSE_LIMIT == 5.0e-4
    assert fitter.RESET_MAX_ABS_LIMIT == 1.0e-5
    assert fitter.TRANSITION_MAX_ABS_LIMIT == 2.0e-3
    assert fitter.TEMPORAL_FIRST_DIFFERENCE_MAX_ABS_LIMIT == 3.0e-3
...
@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("global.rmse", np.nextafter(2.5e-4, np.inf)),
        ("global.max_abs_error", np.nextafter(3.0e-3, np.inf)),
        ("worst_per_case_or_action_rmse", np.nextafter(5.0e-4, np.inf)),
        ("worst_per_trajectory_or_action_rmse", np.nextafter(5.0e-4, np.inf)),
        ("reset_max_abs_error", np.nextafter(1.0e-5, np.inf)),
        ("transition_window_max_abs_error", np.nextafter(2.0e-3, np.inf)),
        (
            "temporal_first_difference_max_abs_error",
            np.nextafter(3.0e-3, np.inf),
        ),
...
def test_real_save_reload_and_fresh_critic_transplant_without_fit(
    ...
    with pytest.raises(fitter.V12R12MaskedTeacherFitError, match="no-clobber"):
...
def test_p1_journals_independently_reconstruct_supervision_and_transitions(
    ...
    fitter._write_json_exclusive(  # noqa: SLF001
        ...
        {
            "passed": True,
            "status": "PASS_H0_V12R12_P1_CANDIDATE_EXPOSED_COLLECTION",
            "trajectory_id": trajectory_id,
    ...
    with pytest.raises(fitter.V12R12MaskedTeacherFitError, match="artifact record"):
...
def test_p1_npz_rejects_each_reconstructed_journal_field_drift(
    ...
    with pytest.raises(
        fitter.V12R12MaskedTeacherFitError,
...
def test_governance_freeze_and_lock_are_exact_and_no_clobber(
    ...
    with pytest.raises(fitter.V12R12MaskedTeacherFitError, match="freeze"):
...
def test_execute_is_blocked_before_loading_without_frozen_governance(
    ...
    with pytest.raises(fitter.V12R12MaskedTeacherFitError, match="strict JSON"):
...
def test_p1_real_builders_roundtrip_through_p2_loader(
    ...
    import h0_v12r12_tape_dagger as p1_contract  # noqa: PLC0415
    import run_h0_v12r12_tape_dagger as p1_runner  # noqa: PLC0415
    ...
    for plan_row in plan_rows:
        ...
        fitter._write_json_exclusive(  # noqa: SLF001
            root / "summary.json",
            {
                ...
                "source_snapshot_after": snapshot,
                "steps": rows_per_case,
                "sample_count": rows_per_case,
                "persisted_label_count": rows_per_case,
                "actor_updates": 0,
                "critic_updates": 0,
                "ppo_updates": 0,
                "candidate_update_count_before": 0,
    ...
    assert loaded.observations.shape == (p1_rows, 35)
    assert loaded.actions.tobytes() == np.tile(p0_actions, (len(alphas), 1)).tobytes()
