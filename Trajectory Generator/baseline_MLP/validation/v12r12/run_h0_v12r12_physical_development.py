"""Prepare, execute, or verify the one-shot V12R12 P3 physical protocol.
...
import h0_v12r12_physical_development as contract  # noqa: E402


PREPARE_ACKNOWLEDGEMENT = "V12R12_P3_FREEZE_AFTER_P2_PASS"
EXECUTE_ACKNOWLEDGEMENT = "V12R12_P3_ONE_SHOT_NO_RETRY"
...
def _mapping(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise contract.V12R12PhysicalDevelopmentError(f"expected JSON mapping: {path}")
    return value
...
def _sequence(path: Path) -> list[Any]:
    ...
    if not isinstance(value, list):
        raise contract.V12R12PhysicalDevelopmentError(f"expected JSON array: {path}")
    return value
...
def _source_record(relative: PurePosixPath) -> dict[str, Any]:
    ...
    if relative.is_absolute() or ".." in relative.parts or raw != str(relative):
        raise contract.V12R12PhysicalDevelopmentError(
...
def build_source_closure() -> dict[str, Any]:
    """Build the complete inherited runtime plus V12R12 source closure."""
...
def prepare_lock(
    ...
    return {
        "passed": True,
        "status": "PASS_H0_V12R12_P3_GOVERNANCE_PREPARED",
        "protocol_freeze": freeze_record,
...
def _physical_config(physical: Any, paths: contract.DevelopmentPaths) -> Any:
    return physical.PhysicalDevelopmentConfig(
        ...
        progress_label="V12R12 P3 pure development",
        progress_every=25,
    )
...
def _run_case(
    *,
    ...
    if not isinstance(trace, Sequence) or isinstance(trace, (str, bytes)):
        raise contract.V12R12PhysicalDevelopmentError("physical trace is malformed")
    if not isinstance(summary, Mapping):
        raise contract.V12R12PhysicalDevelopmentError("physical summary is malformed")
...
def _finalize(
    *,
    ...
    summary = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "COMPLETE_H0_V12R12_P3_PURE_PHYSICAL_DEVELOPMENT",
        "protocol_id": contract.PROTOCOL_ID,
        "development_id": contract.DEVELOPMENT_ID,
        "candidate_id": binding["p2"]["candidate_id"],
        "candidate_module": binding["p2"]["candidate_module"],
...
def execute(
    ...
    *,
    ...
    claim = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "CLAIMED_H0_V12R12_P3_PURE_PIPELINE_ONCE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "development_id": contract.DEVELOPMENT_ID,
        "design_sha256": contract.protocol_design_sha256(),
    ...
    candidate_freeze = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PASS_H0_V12R12_P3_P2_CANDIDATE_FREEZE",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "development_id": contract.DEVELOPMENT_ID,
        "candidate_id": binding["p2"]["candidate_id"],
    ...
    if not passed:
        ...
        raise contract.V12R12PhysicalDevelopmentError(
            f"V12R12 P3 stopped terminally at {attempted_case}: {error}"
        ) from error
    return verified
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except contract.V12R12PhysicalDevelopmentError as exc:
        print(
            json.dumps(
                {
                    "status": "FAIL_H0_V12R12_P3_PHYSICAL_CLI",
                    "passed": False,
