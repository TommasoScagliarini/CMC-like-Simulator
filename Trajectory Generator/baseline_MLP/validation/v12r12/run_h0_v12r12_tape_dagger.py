#!/usr/bin/env python3
"""Prepare or execute the V12R12 P1 tape-reference collection exactly once.
...
import h0_v12r12_tape_dagger as contract  # noqa: E402


class V12R12TapeDaggerExecutionError(RuntimeError):
...
def _mapping(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise V12R12TapeDaggerExecutionError(f"expected JSON object: {path}")
    return dict(value)
...
def _write_npz_exclusive(path: Path, arrays: Mapping[str, Any]) -> Path:
    ...
    try:
        ...
        raise V12R12TapeDaggerExecutionError("NPZ path escaped repository") from exc
    destination.parent.mkdir(parents=True, exist_ok=True)
    flags = os.O_CREAT | os.O_EXCL | os.O_WRONLY | getattr(os, "O_BINARY", 0)
    try:
        descriptor = os.open(destination, flags, 0o600)
    except FileExistsError as exc:
        raise V12R12TapeDaggerExecutionError(
...
def verify_p0_candidate(
    *,
    ...
    if enforce_canonical:
        ...
        if paths != expected:
            raise V12R12TapeDaggerExecutionError("non-canonical P0 input path")
    ...
    checks = {
        ...
        "gate_pass": gate.get("status") == "PASS_H0_V12R12_MASKED_SAFE_TEACHER_FIT"
    ...
    if not all(checks.values()):
        ...
        raise V12R12TapeDaggerExecutionError(f"P0 candidate closure failed: {failed}")
    return {
...
def build_protocol_freeze() -> dict[str, Any]:
    ...
    source_paths = {
        "contract": REVISION_ROOT / "h0_v12r12_tape_dagger.py",
        "runner": Path(__file__).resolve(),
        "tests": REVISION_ROOT / "test_h0_v12r12_tape_dagger.py",
    }
...
def write_protocol_freeze() -> dict[str, Any]:
    if os.path.lexists(contract.P1_PROTOCOL_FREEZE):
        raise V12R12TapeDaggerExecutionError("protocol freeze exists/no-clobber")
...
def write_execution_lock() -> dict[str, Any]:
    ...
    if os.path.lexists(contract.P1_EXECUTION_LOCK):
        raise V12R12TapeDaggerExecutionError("execution lock exists/no-clobber")
    if os.path.lexists(contract.P1_ROOT):
        raise V12R12TapeDaggerExecutionError("P1 output exists before lock")
    prepared = prepare_p1(
...
def prepare_p1(
    *,
    ...
    if require_unoccupied and not passed:
        failed = sorted(name for name, value in checks.items() if not value)
        raise V12R12TapeDaggerExecutionError(f"P1 preflight failed: {failed}")
    payload = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            "PASS_H0_V12R12_P1_CANDIDATE_EXPOSED_PREFLIGHT"
            if passed
            else "FAIL_H0_V12R12_P1_CANDIDATE_EXPOSED_PREFLIGHT"
        ),
        "passed": passed,
        "protocol_id": contract.PROTOCOL_ID,
        "stage_id": contract.STAGE_ID,
...
def _load_production_context(candidate_module: Path) -> ExecutionContext:
    ...
    try:
        ...
        raise V12R12TapeDaggerExecutionError("R6/V26 stack is not ready") from exc
    if runtime_np is not np:
        raise V12R12TapeDaggerExecutionError("runtime NumPy identity drifted")
    module = RLModule.from_checkpoint(candidate_module)
    if not callable(getattr(module, "eval", None)):
        raise V12R12TapeDaggerExecutionError("P0 module lacks eval()")
    module.eval()
...
def _finite_float(value: Any, *, label: str) -> float:
    if (
        ...
        raise V12R12TapeDaggerExecutionError(f"{label} must be finite")
    return float(value)
...
def _phase_state(runtime: Any, info: Mapping[str, Any]) -> str:
    try:
        ...
        raise V12R12TapeDaggerExecutionError("active V26 phase is malformed") from exc
...
def _publish_case_failure(
    ...
    try:
        writer.publish_failure(
            end_reason="v12r12_p1_collection_failed",
            error=exc,
...
def run_collection_case(
    *,
    ...
    if dict(plan_row) not in [dict(row) for row in contract.collection_plan()]:
        raise V12R12TapeDaggerExecutionError("collection plan row is not canonical")
    ...
    try:
        ...
        if not isinstance(built, Mapping):
            raise V12R12TapeDaggerExecutionError("environment config is malformed")
        ...
        if not isinstance(reset_info, Mapping):
            raise V12R12TapeDaggerExecutionError("reset info is malformed")
        ...
        for index in range(contract.EXPECTED_STEPS):
            ...
            if actor.shape != (contract.EXPECTED_ACTOR_FEATURES,):
                raise V12R12TapeDaggerExecutionError("V26 actor view is malformed")
            ...
            if not isinstance(info, Mapping):
                raise V12R12TapeDaggerExecutionError("step info is malformed")
            ...
            if not isinstance(physical, Mapping):
                raise V12R12TapeDaggerExecutionError("physical payload is malformed")
            ...
            if step == 1 or step % 25 == 0 or step == contract.EXPECTED_STEPS:
                ...
                print(
                    f"[V12R12 P1 {plan_row['trajectory_id']}] {step:3d}/500 "
            ...
            if terminated or truncated:
                break
        if audit is None:
            raise V12R12TapeDaggerExecutionError("physical audit was not initialized")
        ...
        if not isinstance(physical_summary, Mapping):
            raise V12R12TapeDaggerExecutionError("physical summary is malformed")
        summary = {
        ...
        partial = {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "H0_V12R12_P1_COLLECTION_PERSISTED_BEFORE_GATE",
            "protocol_id": contract.PROTOCOL_ID,
            "trajectory_id": plan_row["trajectory_id"],
            "steps": len(rows),
        ...
        if isinstance(exc, V12R12TapeDaggerExecutionError):
            raise
        raise V12R12TapeDaggerExecutionError(
...
def _transition_mask(
    ...
    if len(rows) != contract.EXPECTED_STEPS:
        raise V12R12TapeDaggerExecutionError("transition trace is incomplete")
    event_steps: list[int] = []
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("step") != index or not isinstance(accepted, list):
            raise V12R12TapeDaggerExecutionError("transition provenance drifted")
        if accepted:
...
def build_p1_corpus(
    ...
    *,
    ...
    if len(case_receipts) != len(plan):
        raise V12R12TapeDaggerExecutionError("P1 receipt count is not 18")
    ...
    for plan_row, receipt in zip(plan, case_receipts, strict=True):
        ...
        if (
            ...
            raise V12R12TapeDaggerExecutionError("case receipt order/binding drifted")
        ...
        if not isinstance(value, list) or len(value) != contract.EXPECTED_STEPS:
            raise V12R12TapeDaggerExecutionError("case trace is incomplete")
        rows = [dict(row) for row in value if isinstance(row, Mapping)]
        if len(rows) != contract.EXPECTED_STEPS:
            raise V12R12TapeDaggerExecutionError("case trace row schema drifted")
    ...
    if any(arrays[name].shape != shape for name, shape in expected_shapes.items()):
        raise V12R12TapeDaggerExecutionError("P1 corpus shape drifted")
    ...
    if observed_trajectories != expected_trajectories:
        raise V12R12TapeDaggerExecutionError("P1 trajectory order drifted")
    hashes = {name: contract.array_sha256(value) for name, value in arrays.items()}
    array_keys = sorted(arrays)
    manifest = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "PASS_H0_V12R12_P1_CANDIDATE_EXPOSED_CORPUS_MANIFEST",
        "passed": True,
        "protocol_id": contract.PROTOCOL_ID,
        "source_candidate_id": prepared.source_candidate_id,
        "source_p0_candidate": copy.deepcopy(prepared.source_candidate_tree),
...
def execute_p1(
    *,
    ...
    if os.path.lexists(contract.P1_ROOT):
        raise V12R12TapeDaggerExecutionError("P1 output root is occupied")
    completed: list[dict[str, Any]] = []
    forensic.write_json_exclusive(
        contract.P1_ROOT / "run_start.json",
        {
            "schema_version": contract.SCHEMA_VERSION,
            "status": "H0_V12R12_P1_CANDIDATE_EXPOSED_PIPELINE_STARTED",
            "protocol_id": contract.PROTOCOL_ID,
            "source_candidate_id": prepared.source_candidate_id,
            "source_p0_candidate": copy.deepcopy(prepared.source_candidate_tree),
    ...
    try:
        ...
        if isinstance(exc, V12R12TapeDaggerExecutionError):
            raise
        raise V12R12TapeDaggerExecutionError("P1 pipeline failed") from exc


EXECUTION_ACKNOWLEDGEMENT = "V12R12_P1_ONE_SHOT_18_ROLLOUTS_9000_STEPS"
GOVERNANCE_ACKNOWLEDGEMENT = "V12R12_P1_FREEZE_REVIEWED_SOURCES"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        V12R12TapeDaggerExecutionError,
        contract.V12R12TapeDaggerError,
        forensic.ForensicRolloutError,
    ) as exc:
        print(
            json.dumps(
                {
                    "status": "FAIL_H0_V12R12_P1_TAPE_DAGGER_CLI",
                    "passed": False,
