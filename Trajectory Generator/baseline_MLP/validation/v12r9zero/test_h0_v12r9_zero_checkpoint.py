"""Source/component tests for the deferred V12R9 checkpoint-zero scaffold."""
...
R9_ROOT = HERE.parent / "v12r9"
Q3_ROOT = HERE.parent / "v12r9q3"
for root in (HERE, BASELINE_ROOT, R9_ROOT, Q3_ROOT):
    if str(root) not in sys.path:
        sys.path.insert(0, str(root))

import freeze_h0_v12r9_zero_checkpoint as freezer  # noqa: E402
import h0_v12r9_q3_qualification_contract as q3_source  # noqa: E402
import h0_v12r9_recovery_contract as r9_source  # noqa: E402
import h0_v12r9_zero_checkpoint_contract as contract  # noqa: E402
import h0_v12r9_zero_checkpoint_gates as gates  # noqa: E402
import run_h0_v12r9_zero_checkpoint as runner  # noqa: E402
...
def _r9_terminal(module: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": contract.r9.SCHEMA_VERSION,
        ...
        "error": None,
        "candidate_freeze": _artifact(contract.r9.CANDIDATE_FREEZE_PATH.as_posix()),
        "final_development_receipt": _artifact(
            contract.r9.FINAL_DEVELOPMENT_PATH.as_posix()
        ),
...
def _upstream(module: dict[str, Any]) -> dict[str, Any]:
    r9 = _r9_terminal(module)
    q3 = _q3_terminal(module)
    return {
        "v12r9_terminal": r9,
        "v12r9_q3_terminal": q3,
        "semantic_attestations": {
            "v12r9_terminal": _attestation(contract.R9_TERMINAL_ENDPOINT, r9),
            "v12r9_q3_terminal": _attestation(contract.Q3_TERMINAL_ENDPOINT, q3),
        },
    }
...
def _runtime_audit(world: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "status": "COMPLETE_H0_V12R9_ZERO_CHECKPOINT_AUDIT",
...
def test_contract_binds_exact_stable_r9_and_q3_endpoints() -> None:
    assert contract.SCHEMA_VERSION == 1294
    assert contract.R9_TERMINAL_ENDPOINT["protocol_id"] == r9_source.PROTOCOL_ID
    assert contract.R9_TERMINAL_ENDPOINT["path"] == r9_source.LEDGER_PATH.as_posix()
    assert contract.R9_TERMINAL_ENDPOINT["required_status"] == (
        r9_source.PIPELINE_TERMINAL_PASS_STATUS
    )
    assert (
        contract.R9_TERMINAL_ENDPOINT["required_next_stage"]
        == "WAIT_SEPARATE_V12R9Q3_PROTOCOL"
    )
    ...
    assert (
        contract.Q3_TERMINAL_ENDPOINT["required_next_stage"]
        == q3_source.NEXT_STAGE_AFTER_Q3_PASS
    )
    assert contract.CANDIDATE_MODULE_PATH == r9_source.CANDIDATE_MODULE_PATH
...
def test_q3_binding_is_the_independent_minus_plus_030_seed130_133_matrix() -> None:
    assert q3_source.CASE_IDS == (
    ...
    assert q3_source.STOCHASTIC_SEEDS == (130, 131, 132, 133)
    assert set(q3_source.CASE_IDS).isdisjoint(r9_source.COLLECTION_CASE_IDS)
    assert set(q3_source.CASE_IDS).isdisjoint(r9_source.DEVELOPMENT_CASE_IDS)

    zero_runtime_sources = (
        contract.SOURCE_RELATIVE_PATHS["contract"],
        contract.SOURCE_RELATIVE_PATHS["gates"],
        contract.SOURCE_RELATIVE_PATHS["freezer"],
        contract.SOURCE_RELATIVE_PATHS["runner"],
    )
    repo_root = HERE.parents[3]
    for relative in zero_runtime_sources:
        source = repo_root.joinpath(*Path(relative).parts).read_text(encoding="utf-8")
...
def test_freeze_execute_and_handoff_reject_non_repository_root_cwd(
    ...
    with pytest.raises(runner.ZeroCheckpointDeferredError, match="repository_root"):
...
def test_upstream_gate_requires_live_semantic_attestations_and_same_candidate() -> None:
    ...
    cross_candidate["v12r9_q3_terminal"]["candidate_id"] = (
    ...
    q3_nonzero["v12r9_q3_terminal"]["actor_updates"] = 1
...
def test_verify_lock_rejects_semantic_upstream_mutation(
    ...
    mutated["v12r9_q3_terminal"]["candidate_id"] = contract.candidate_id_for_tree(
...
def test_runtime_audit_requires_byte_exact_fresh_state_and_only_positive_0025(
    ...
    cwd_drift = copy.deepcopy(audit)
    cwd_drift["working_directory"]["observed_working_directory"] = "baseline_MLP"
...
def test_terminal_receipt_and_handoff_are_non_authorizing_resume_only(
    ...
    assert receipt["required_working_directory"] == "repository_root"
    ...
    wrong_cwd_receipt = copy.deepcopy(receipt)
    wrong_cwd_receipt["required_working_directory"] = "baseline_MLP"
    assert gates.terminal_receipt_gate(wrong_cwd_receipt)["passed"] is False
