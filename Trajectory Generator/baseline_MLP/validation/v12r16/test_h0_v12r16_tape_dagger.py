"""Source-only tests for the V12R16 P1 tape-reference collector."""
...
import run_h0_v12r16_tape_dagger as runner  # noqa: E402
...
def test_support_boundary_is_closed_and_nextafter_is_fallback() -> None:
    ...
    with pytest.raises(contract.V12R16TapeDaggerError):
...
def test_npz_writer_is_exclusive(
    ...
    with pytest.raises(runner.V12R16TapeDaggerExecutionError, match="no-clobber"):
        runner._write_npz_exclusive(destination, arrays)  # noqa: SLF001
...
def test_execute_cli_requires_exact_one_shot_acknowledgement(
    ...
    assert error["status"] == "FAIL_H0_V12R16_P1_TAPE_DAGGER_CLI"
...
def test_execute_rejects_before_context_loader_when_lock_preflight_fails(
    ...
    def fail_preflight(**_kwargs: object) -> runner.PreparedP1:
        raise runner.V12R16TapeDaggerExecutionError("execution lock stale")
    ...
    with pytest.raises(
        runner.V12R16TapeDaggerExecutionError, match="execution lock stale"
    ):
...
def test_manifest_public_schema_names_are_frozen_in_source() -> None:
    ...
    assert contract.P1_LEDGER_STATUS == ("PASS_H0_V12R16_P1_CANDIDATE_EXPOSED_PIPELINE")
...
def test_collection_gate_rejects_incoherent_truncation() -> None:
    ...
    missing_stop = _truncated_summary(plan_row, steps=202)
    missing_stop["safety_stop_count"] = 0
    gate = contract.collection_gate(missing_stop, plan_row=plan_row)
    assert gate["passed"] is False
    assert gate["checks"]["safety_stop_semantics"] is False

    spurious_stop = _passing_summary(plan_row)
    spurious_stop["safety_stop_count"] = 1
    gate = contract.collection_gate(spurious_stop, plan_row=plan_row)
    assert gate["passed"] is False
    assert gate["checks"]["safety_stop_semantics"] is False


def test_collection_gate_recovers_the_real_v12r14_truncated_summary() -> None:
    """Byte-real evidence closure (the fixture-fidelity lesson, twice paid):
    the V12R16 gate must recover the ACTUAL truncated summary produced by the
    V12R14 one-shot on alpha 0.50/seed 127 — the exact artifact whose spurious
    rejection burned that lineage."""

    summary_path = (
        REVISION_ROOT.parent
        / "v12r14"
        / "h0_v12r14_run_20260816"
        / "p1_candidate_exposed"
        / "collections"
        / "alpha_0p50"
    ...
    if not summary_path.is_file():
        pytest.skip("historical V12R14 truncated summary not present")
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    summary["schema_version"] = contract.SCHEMA_VERSION
    summary["status"] = contract.COLLECTION_COMPLETE_STATUS
    summary["protocol_id"] = contract.PROTOCOL_ID
    plan_row = next(
        dict(row)
        for row in contract.collection_plan()
        if row["trajectory_id"] == "alpha_0p50__stochastic_nominal_seed_127"
    )
    gate = contract.collection_gate(summary, plan_row=plan_row)
    failed = sorted(name for name, passed in gate["checks"].items() if passed is False)
