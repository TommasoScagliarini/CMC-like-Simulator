"""Fail-closed V12R16 P3 pure-policy physical-development contract.
...
DEVELOPMENT_ID = "H0_V12R16_P3_PURE_V26_W256_DEVELOPMENT_V1"
...
P2_RECEIPT_STATUS = "PASS_H0_V12R16_P2_MASKED_CANDIDATE_EXPOSED_FIT_RECEIPT"
P2_GATE_STATUS = "PASS_H0_V12R16_P2_MASKED_CANDIDATE_EXPOSED_FIT"
P2_SUMMARY_STATUS = "COMPLETE_H0_V12R16_P2_MASKED_CANDIDATE_EXPOSED_FIT"
...
START_STATUS = "STARTED_H0_V12R16_P3_PURE_PHYSICAL_CASE"
PARTIAL_STATUS = "PERSISTED_H0_V12R16_P3_PURE_CASE_BEFORE_GATE"
COMPLETE_STATUS = "COMPLETE_H0_V12R16_P3_PURE_PHYSICAL_CASE"
EARLY_COMPLETE_STATUS = "COMPLETE_H0_V12R16_P3_PURE_CASE_EARLY_TERMINATION"
CASE_PASS_STATUS = "PASS_H0_V12R16_P3_PURE_PHYSICAL_CASE"
CASE_FAIL_STATUS = "FAIL_H0_V12R16_P3_PURE_PHYSICAL_CASE_TERMINAL"
AGGREGATE_PASS_STATUS = "PASS_H0_V12R16_P3_PURE_PHYSICAL_DEVELOPMENT"
PIPELINE_PASS_STATUS = "PASS_H0_V12R16_P3_PURE_PIPELINE_TERMINAL"
PIPELINE_FAIL_STATUS = "FAIL_H0_V12R16_P3_PURE_PIPELINE_TERMINAL"
FREEZE_PASS_STATUS = "PASS_H0_V12R16_P3_PROTOCOL_FREEZE"
LOCK_PASS_STATUS = "PASS_H0_V12R16_P3_EXECUTION_LOCK"
...
OWN_PRODUCTION_SOURCES = (
    PurePosixPath(
        ...
        "h0_v12r16_physical_development.py"
    ),
    PurePosixPath(
        "Trajectory Generator/baseline_MLP/validation/v12r16/"
        "run_h0_v12r16_physical_development.py"
    ),
)
GOVERNANCE_SOURCE_PATHS = (
    PurePosixPath(
        "Trajectory Generator/baseline_MLP/validation/v12r16/"
        "test_h0_v12r16_physical_development.py"
    ),
)
LINEAGE_SOURCE_PATHS = (
    PurePosixPath(
        "Trajectory Generator/baseline_MLP/validation/v12r16/"
        "h0_v12r16_masked_teacher_fitter.py"
    ),
    PurePosixPath(
        "Trajectory Generator/baseline_MLP/validation/v12r16/"
        "run_h0_v12r16_masked_teacher_fit.py"
    ),
    PurePosixPath(
        "Trajectory Generator/baseline_MLP/validation/v12r16/h0_v12r16_tape_dagger.py"
    ),
    PurePosixPath(
        "Trajectory Generator/baseline_MLP/validation/v12r16/"
        "run_h0_v12r16_tape_dagger.py"
    ),
)
...
class V12R16PhysicalDevelopmentError(RuntimeError):
...
class DevelopmentPaths:
    ...
    def production(cls) -> "DevelopmentPaths":
        ...
        return cls(
            ...
            protocol_freeze=(REVISION_ROOT / "h0_v12r16_p3_protocol_freeze.json"),
            execution_lock=(REVISION_ROOT / "h0_v12r16_p3_execution_lock.json"),
            p3_root=p3_root,
        )
...
def canonical_json_bytes(value: Any) -> bytes:
    ...
    try:
        ...
        raise V12R16PhysicalDevelopmentError("value is not strict JSON") from exc
...
def strict_json_load(path: Path) -> Any:
    try:
        ...
        raise V12R16PhysicalDevelopmentError(f"invalid strict JSON: {path}") from exc
...
def write_json_exclusive(path: Path, value: Any) -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R16PhysicalDevelopmentError(
...
def sha256_file(path: Path) -> str:
    if not _safe_regular_file(path):
        raise V12R16PhysicalDevelopmentError(f"unsafe or missing file: {path}")
...
def artifact_record(path: Path, *, artifact_root: Path) -> dict[str, Any]:
    ...
    if not _safe_regular_file(target):
        raise V12R16PhysicalDevelopmentError(f"artifact is not regular: {target}")
    return {
...
def tree_record(path: Path, *, artifact_root: Path) -> dict[str, Any]:
    ...
    if not root.is_dir() or _is_link_or_reparse(root):
        raise V12R16PhysicalDevelopmentError(f"unsafe or missing tree: {root}")
    ...
    if not files:
        raise V12R16PhysicalDevelopmentError(f"tree is empty: {root}")
    digest = hashlib.sha256()
...
def validate_artifact_record(record: Mapping[str, Any], *, artifact_root: Path) -> None:
    if set(record) != {"path", "sha256", "size_bytes"}:
        raise V12R16PhysicalDevelopmentError("artifact record schema drifted")
    raw = record.get("path")
    if not isinstance(raw, str):
        raise V12R16PhysicalDevelopmentError("artifact record path is malformed")
    pure = PurePosixPath(raw)
    if pure.is_absolute() or ".." in pure.parts or pure.as_posix() != raw:
        raise V12R16PhysicalDevelopmentError("artifact record path is non-canonical")
    observed = artifact_record(
        artifact_root.joinpath(*pure.parts), artifact_root=artifact_root
    )
    if observed != dict(record):
        raise V12R16PhysicalDevelopmentError(f"artifact record drifted: {raw}")


def validate_tree_record(record: Mapping[str, Any], *, artifact_root: Path) -> None:
    if set(record) != {"path", "tree_sha256", "file_count", "files"}:
        raise V12R16PhysicalDevelopmentError("tree record schema drifted")
    raw = record.get("path")
    if not isinstance(raw, str):
        raise V12R16PhysicalDevelopmentError("tree record path is malformed")
    pure = PurePosixPath(raw)
    if pure.is_absolute() or ".." in pure.parts or pure.as_posix() != raw:
        raise V12R16PhysicalDevelopmentError("tree record path is non-canonical")
    observed = tree_record(
        artifact_root.joinpath(*pure.parts), artifact_root=artifact_root
    )
    if observed != dict(record):
        raise V12R16PhysicalDevelopmentError(f"tree record drifted: {raw}")


def canonical_case(case_id: str) -> dict[str, Any]:
    matches = [case for case in DEVELOPMENT_CASES if case["case_id"] == case_id]
    if len(matches) != 1:
        raise V12R16PhysicalDevelopmentError(f"unknown P3 case: {case_id!r}")
...
def describe_protocol(paths: DevelopmentPaths = PRODUCTION_PATHS) -> dict[str, Any]:
    ...
    return {
        "passed": True,
        "status": "DESCRIBED_H0_V12R16_P3_PURE_PHYSICAL_DEVELOPMENT",
...
def _mapping(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise V12R16PhysicalDevelopmentError(f"expected JSON mapping: {path}")
    return value
...
def verify_p2(paths: DevelopmentPaths = PRODUCTION_PATHS) -> dict[str, Any]:
    ...
    if (
        ...
        raise V12R16PhysicalDevelopmentError("canonical P2 candidate path drifted")
    ...
    if not isinstance(artifacts, Mapping) or set(artifacts) != {
        ...
        raise V12R16PhysicalDevelopmentError("P2 receipt artifacts are malformed")
    for value, label in (
    ...
    for key, path in (("gate", paths.p2_gate), ("summary", paths.p2_summary)):
        ...
        if not isinstance(record, Mapping) or dict(record) != expected:
            raise V12R16PhysicalDevelopmentError(f"P2 {key} binding drifted")
    corpus_record = artifacts.get("corpus")
    if not isinstance(corpus_record, Mapping):
        raise V12R16PhysicalDevelopmentError("P2 corpus binding is malformed")
    ...
    if not isinstance(candidate_id, str) or summary.get("candidate_id") != candidate_id:
        raise V12R16PhysicalDevelopmentError("P2 candidate identity drifted")
    return {
        "passed": True,
...
def verify_execution_lock(
    ...
    *,
    ...
    if not isinstance(closure, Mapping) or not closure:
        raise V12R16PhysicalDevelopmentError("execution source closure is malformed")
    required = {
        path.as_posix()
        for path in (
            *REQUIRED_TRANSITIVE_RUNTIME_SOURCES,
    ...
    for relative, expected in sorted(closure.items()):
        if not isinstance(relative, str) or not isinstance(expected, Mapping):
            raise V12R16PhysicalDevelopmentError("source closure entry is malformed")
        ...
        if observed != dict(expected):
            raise V12R16PhysicalDevelopmentError(f"locked source drifted: {relative}")
    ...
    if (
        ...
        raise V12R16PhysicalDevelopmentError("source closure digest drifted")
    return {
        "passed": True,
...
def preflight(paths: DevelopmentPaths = PRODUCTION_PATHS) -> dict[str, Any]:
    ...
    return {
        "passed": True,
        "status": "READY_H0_V12R16_P3_PURE_PHYSICAL_ONE_SHOT",
        "protocol_id": PROTOCOL_ID,
        "development_id": DEVELOPMENT_ID,
        "design_sha256": protocol_design_sha256(),
        "p2": p2,
...
__all__ = [
    ...
    "START_STATUS",
    "TARGET_CONTRACT_ID",
    "V12R16PhysicalDevelopmentError",
    "aggregate_gate",
    "artifact_record",
    "canonical_case",
