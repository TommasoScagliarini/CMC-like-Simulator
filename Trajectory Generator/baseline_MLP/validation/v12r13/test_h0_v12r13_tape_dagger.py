"""Source-only tests for the V12R13 P1 tape-reference collector."""
...
import run_h0_v12r13_tape_dagger as runner  # noqa: E402
...
def test_support_boundary_is_closed_and_nextafter_is_fallback() -> None:
    ...
    with pytest.raises(contract.V12R13TapeDaggerError):
...
def test_npz_writer_is_exclusive(
    ...
    with pytest.raises(runner.V12R13TapeDaggerExecutionError, match="no-clobber"):
        runner._write_npz_exclusive(destination, arrays)  # noqa: SLF001
...
def test_execute_cli_requires_exact_one_shot_acknowledgement(
    ...
    assert error["status"] == "FAIL_H0_V12R13_P1_TAPE_DAGGER_CLI"
...
def test_execute_rejects_before_context_loader_when_lock_preflight_fails(
    ...
    def fail_preflight(**_kwargs: object) -> runner.PreparedP1:
        raise runner.V12R13TapeDaggerExecutionError("execution lock stale")
    ...
    with pytest.raises(
        runner.V12R13TapeDaggerExecutionError, match="execution lock stale"
    ):
...
def test_manifest_public_schema_names_are_frozen_in_source() -> None:
    ...
    assert contract.P1_LEDGER_STATUS == ("PASS_H0_V12R13_P1_CANDIDATE_EXPOSED_PIPELINE")
