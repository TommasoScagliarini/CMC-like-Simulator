"""Semantic, execution-free tests for the V12R13 P3 physical runner."""
...
import run_h0_v12r13_physical_development as runner  # noqa: E402
...
def _paths(tmp_path: Path) -> contract.DevelopmentPaths:
    revision = tmp_path / "Trajectory Generator/baseline_MLP/validation/v12r13"
    run_root = revision / "h0_v12r13_run_20260816"
    p2 = run_root / "p2_fit"
    return contract.DevelopmentPaths(
        ...
        protocol_freeze=revision / "h0_v12r13_p3_protocol_freeze.json",
        execution_lock=revision / "h0_v12r13_p3_execution_lock.json",
...
def test_p2_adapter_accepts_only_canonical_p2_w256(tmp_path: Path) -> None:
    ...
    with pytest.raises(contract.V12R13PhysicalDevelopmentError, match="P2 candidate"):
...
@pytest.mark.parametrize(
    ...
    (
        ("receipt", "status", "PASS_H0_V12R13_MASKED_SAFE_TEACHER_FIT_RECEIPT"),
        ("gate", "fit_stage", "p0"),
...
def test_p2_adapter_rejects_status_stage_and_collector_aliases(
    ...
    with pytest.raises(
        contract.V12R13PhysicalDevelopmentError,
...
def test_execute_closes_first_failed_prefix_without_loading_environment(
    ...
    with pytest.raises(
        contract.V12R13PhysicalDevelopmentError, match="stopped terminally"
    ):
...
def test_preflight_reports_missing_p2_without_writes(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    with pytest.raises(
        contract.V12R13PhysicalDevelopmentError, match="P2 prerequisite"
    ):
