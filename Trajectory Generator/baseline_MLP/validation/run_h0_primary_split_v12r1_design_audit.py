"""Run or verify the sole V12R1 in-memory P0 numerical design audit.

``--prepare`` first verifies the immutable V12R1 protocol freeze, binds all
five execution sources, then calls the frozen fitter exactly once.  The fit is
...
R1_VALIDATION_ROOT = Path(__file__).resolve().parent
for _root in (
    REPO_ROOT,
    VALIDATION_ROOT,
    TRAJECTORY_ROOT,
...
import freeze_h0_primary_split_v12r1_autonomy_recovery as protocol_freezer  # noqa: E402
import h0_primary_split_v12r1_autonomy_recovery_contract as contract  # noqa: E402
import h0_primary_split_v12r1_recovery_weighted_fitter as fitter  # noqa: E402


class V12R1DesignAuditError(RuntimeError):
...
def resolve_relative(path: str | os.PathLike[str] | PurePosixPath) -> Path:
    ...
    if (
        ...
        raise V12R1DesignAuditError(f"non-canonical repository path: {raw!r}")
...
def _execution_source_records() -> dict[str, dict[str, Any]]:
    ...
    if (
        ...
        raise V12R1DesignAuditError("execution source declaration drifted")
    try:
        return {name: _record(paths[name]) for name in expected_names}
    except Exception as exc:
        raise V12R1DesignAuditError(
            "all five execution sources must exist before the audit"
        ) from exc
...
def _runtime_record() -> dict[str, str]:
    ...
    for name in ("numpy", "scipy", "torch"):
        ...
        if not isinstance(value, str) or not value:
            raise V12R1DesignAuditError(f"runtime version unavailable: {name}")
        versions[name] = value
    return versions
...
def _verify_protocol_freeze() -> dict[str, Any]:
    try:
        ...
        raise V12R1DesignAuditError(
            "canonical V12R1 protocol freeze is missing or drifted"
        ) from exc
    if (
        ...
        raise V12R1DesignAuditError("protocol freeze is not design-only PASS")
    return dict(payload)
...
def _verify_protocol_freeze_snapshot() -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R1DesignAuditError(
            "canonical V12R1 protocol freeze is unreadable"
        ) from exc
    if (
        observed != expected
        or PROTOCOL_FREEZE_PATH.read_bytes()
        != forensic.canonical_json_bytes(expected)
    ):
        raise V12R1DesignAuditError(
            "V12R1 protocol freeze/current design sources drifted"
        )
...
def build_design_audit() -> dict[str, Any]:
    ...
    if v12_record_before != expected_v12:
        raise V12R1DesignAuditError("immutable V12 parent freeze drifted")
    ...
    if not isinstance(raw, Mapping):
        raise V12R1DesignAuditError("in-memory fitter returned a non-object")
    ...
    if occupancy_after != {name: True for name in occupancy_after}:
        raise V12R1DesignAuditError("in-memory audit opened execution scope")
    if os.path.lexists(RECEIPT_PATH):
        raise V12R1DesignAuditError("audit fitter unexpectedly created the receipt")
    if sources_after != sources_before:
        raise V12R1DesignAuditError("execution source changed during audit")
    if runtime_after != runtime_before:
        raise V12R1DesignAuditError("runtime identity changed during audit")
    if _record(contract.PROTOCOL_FREEZE_PATH) != protocol_record_before:
        raise V12R1DesignAuditError("V12R1 protocol freeze changed during audit")
    if _record(contract.V12_PROTOCOL_FREEZE_PATH) != v12_record_before:
        raise V12R1DesignAuditError("V12 protocol freeze changed during audit")
    ...
    if gate.get("passed") is not True:
        ...
        raise V12R1DesignAuditError(f"design audit contract gate failed: {failed}")
    payload["contract_gate"] = copy.deepcopy(gate)
    if contract.design_audit_gate(payload) != gate:
        raise V12R1DesignAuditError("design audit gate is not payload-stable")
    return payload
...
def _write_json_exclusive(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    if destination != RECEIPT_PATH:
        raise V12R1DesignAuditError(f"non-canonical audit receipt: {destination}")
    if _symlink_ancestors(destination):
        raise V12R1DesignAuditError("audit receipt has a symlink ancestor")
    try:
        parent_status = os.lstat(destination.parent)
    except FileNotFoundError as exc:
        raise V12R1DesignAuditError("audit receipt parent is missing") from exc
    if not stat.S_ISDIR(parent_status.st_mode):
        raise V12R1DesignAuditError("audit receipt parent is not a directory")
    if os.path.lexists(destination):
        raise V12R1DesignAuditError("audit receipt exists/no-clobber")
    ...
    try:
        ...
        raise V12R1DesignAuditError("audit receipt exists/no-clobber") from exc
    try:
...
def verify_design_audit() -> dict[str, Any]:
    ...
    try:
        ...
        raise V12R1DesignAuditError("canonical design audit receipt is missing") from exc
    if not stat.S_ISREG(status.st_mode) or _symlink_ancestors(RECEIPT_PATH):
        raise V12R1DesignAuditError("design audit receipt is not a safe regular file")
    try:
        payload = forensic.strict_json_load(RECEIPT_PATH)
    except Exception as exc:
        raise V12R1DesignAuditError("design audit receipt is not strict JSON") from exc
    if not isinstance(payload, Mapping):
        raise V12R1DesignAuditError("design audit receipt is not an object")
    observed = dict(payload)
    if RECEIPT_PATH.read_bytes() != forensic.canonical_json_bytes(observed):
        raise V12R1DesignAuditError("design audit receipt bytes are not canonical")

    gate = contract.design_audit_gate(observed)
    if gate.get("passed") is not True or observed.get("contract_gate") != gate:
        raise V12R1DesignAuditError("design audit contract gate drifted")
    current_bindings = {
    ...
    if drifted:
        raise V12R1DesignAuditError(
            f"design audit current bindings drifted: {drifted}"
        )
    return observed
