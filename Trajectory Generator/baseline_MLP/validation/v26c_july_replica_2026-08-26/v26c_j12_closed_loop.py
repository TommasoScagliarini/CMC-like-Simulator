"""V26C J12 - CLOSED-LOOP QUALIFICATION of the J11 actor.
...
THE ACTOR IS J11, AND ONLY J11
    j11_runs/j11_multistart_fit_v26c_2026-08-27_r1/rl_module.
    NOT J8, NOT J4, NOT a July checkpoint. The J11 leaf is pinned file by file and is consumed
...
LINEAGE
    August V26 imitation -> J2 35D -> J11 -> THIS qualification. July is METHODOLOGY AND EVIDENCE
...
WHAT THIS IS NOT
    ...
    plugin are untouched, as are every J0-J11 artefact and the production configuration.

HARDENING CARRIED FORWARD FROM J10R1 AND J11
...
class J12Error(RuntimeError):
    pass


STAGE = "V26C_J12_CLOSED_LOOP_QUALIFICATION"
PREREG = HERE / "v26c_j12_prereg_closed_loop_qualification.json"
PIN_PREREG = "5f0ed000b62c7c64ceaca8eca0d83d36e863619bf528a285aa7d596d63120bbe"
...
# ------------------------------------------------------------------ the ONLY actor --------------
# The J11 leaf, pinned FILE BY FILE. Eight files, not six: J11 also commits its aggregate dataset
# and its own commit_verification.json.
J11_LEAF = HERE / "j11_runs" / "j11_multistart_fit_v26c_2026-08-27_r1"
J11_MODULE_DIR = J11_LEAF / "rl_module"
PIN_J11: dict[str, str] = {
    "commit_verification.json":
        "1d24eed05dd04c187c014181c89a0611a648e5601cacff5e1ae0221adcfb8643",
    "history.json":
        "bbfcc0c64e8d8c9019d8bb8a7bfd485228c707b7b3229649cce1d946dcc96d14",
    "rl_module/actor_feature_manifest.json":
        "9cc0276c22c9cd41d75a12ad1e09e6ce1931cf8a27eafc3ab0741cd5937f6b28",
    ...
    "rl_module/module_state.pkl":
        "19bf8a43804774c06c24db30626138856bb07acf7002419606d8c1bb887f6b73",
...
PIN_J11_MODULE_STATE = PIN_J11["rl_module/module_state.pkl"]
PIN_J11_ACTOR_DIGEST = "f3c8fcb1b2ed28ed5fcb96440af8b5b9805e030125667fae3759a5c1447f1633"
...
# cycles; J10R1 collected the missing rows and J11 fitted on them. Counting WAIT_HS here makes
...
J12_COMMON_GATE: dict[str, Any] = {k: v for k, v in J1.J1_GATE.items()
                                  if k != "max_penetration_m_max"}
J12_KINEMATIC_GATE: dict[str, Any] = dict(J3.J3_KINEMATIC_GATE)   # UNCHANGED
J12_GATE_SOURCE = ("v26c_j1_collect.J1_GATE for every non-penetration criterion, "
...
PREFLIGHT_SENTINEL = HERE / "_j12_preflight_sentinel_never_created"
...
def _leaf_rel(q: Path, root: Path) -> str:
    ...
    commit. That provenance defect is corrected here, as it was in J10R1 and J11.
    """
    try:
        rel = Path(q).relative_to(root)
    except ValueError:
        raise J12Error(f"{q} is not under the staging root {root}") from None
    if ".." in rel.parts:
        raise J12Error(f"the relative path for {q} under {root} escapes the leaf: {rel}")
    return str(rel).replace(os.sep, "/")


def _resolve_inside(leaf: Path, rel: str) -> Path:
    if not rel or Path(rel).is_absolute() or ".." in Path(rel).parts:
        raise J12Error(f"recorded path is not leaf-relative: {rel!r}")
    q = leaf / rel
    if q.is_symlink():
        raise J12Error(f"recorded path is a symlink, which is never committed here: {rel}")
    try:
        q.resolve().relative_to(leaf.resolve())
    except ValueError:
        raise J12Error(f"recorded path escapes the committed leaf: {rel}") from None
    return q
...
def verify_committed_leaf(leaf: Path, *, expected_receipt_sha: str | None = None) -> dict[str, Any]:
    ...
    Carried forward from J10R1 and J11. J9R1 recorded its receipt's hash after the rename and
    ...
    if receipt_bytes is None:
        return {"schema": "v26c_j12_commit_verification.1", "stage": STAGE, "pass": False,
    ...
    return {
        "schema": "v26c_j12_commit_verification.1", "stage": STAGE,
...
def verify_prereg() -> dict[str, Any]:
    ...
    if not PREREG.is_file():
        raise J12Error("the J12 preregistration is missing")
    digest = _sha_file(PREREG)
    if PIN_PREREG != "PENDING" and digest != PIN_PREREG:
        raise J12Error(f"the J12 preregistration changed: {digest} != {PIN_PREREG}")
    data = json.loads(PREREG.read_text())
    if data.get("stage_proposed") != STAGE:
        raise J12Error(f"the preregistration proposes {data.get('stage_proposed')!r}, not {STAGE}")

    local: dict[str, str] = {}
    for rel, pin in data["pinned_artefacts_sha256"].items():
        got = _sha_file(HERE / rel)
        if got != pin:
            raise J12Error(f"the pinned artefact {rel} changed: {got} != {pin}")
        local[rel] = got
    repo: dict[str, str] = {}
    ...
    if [c["id"] for c in declared] != [c["id"] for c in MATRIX]:
        raise J12Error("the preregistered matrix order and the runner's disagree")
    for want, got in zip(MATRIX, declared):
        if (str(got["mode"]), int(got["seed"]), float(got["offset_s"])) != \
            ...
            raise J12Error(f"cell {want['id']} differs from the preregistration")
    if int(data["matrix"]["steps_per_cell"]) != EXPECTED_STEPS \
            or int(data["matrix"]["cells_count"]) != len(MATRIX):
        raise J12Error("the preregistration and the runner disagree on steps or cell count")
    if data["matrix"]["behavioural_fail_fast"] is not False:
        raise J12Error("the preregistration must declare behavioural_fail_fast false")
    if int(data["matrix"]["noise_hold_steps"]) != NOISE_HOLD_STEPS \
            or float(data["matrix"]["sigma"]) != SIGMA:
        raise J12Error("the preregistration and the runner disagree on the noise contract")

    # the actor identity
    if data["actor"]["stage"] != "V26C_J11_MULTISTART_FIT" \
            or data["actor"]["is_j8"] is not False or data["actor"]["is_july"] is not False:
        raise J12Error("the preregistration must name J11, and neither J8 nor July, as the actor")
    ...
    if "valid_cycle_count" in data["diagnostics_not_binding"]:
        raise J12Error("valid_cycles is a BINDING check; listing its measure among the "
...
def verify_scientific_equivalence() -> dict[str, Any]:
    ...
    if digest != PIN_J9R1_MODULE:
        raise J12Error(f"the J9R1 module changed: {digest} != {PIN_J9R1_MODULE}")
    if path.parent != HERE:
        raise J12Error(f"the imported J9R1 module is not the one beside this stage: {path}")
    ...
    def same(name: str, mine: Any, theirs: Any) -> None:
        ...
        if a != b:
            raise J12Error(f"{name} differs from the frozen J9R1 source: {a!r} != {b!r}")
        compared[name] = a

    same("COMMON_GATE", J12_COMMON_GATE, J9R1.J9_COMMON_GATE)
    same("KINEMATIC_GATE", J12_KINEMATIC_GATE, J9R1.J9_KINEMATIC_GATE)
    ...
    if len(MATRIX) != len(J9R1.MATRIX):
        raise J12Error("the matrix length differs from the frozen J9R1 source")
    cells = []
    for mine, theirs in zip(MATRIX, J9R1.MATRIX):
        for field in ("id", "mode", "seed", "offset_s"):
            if mine[field] != theirs[field]:
                raise J12Error(f"matrix cell {mine['id']}: {field} differs from J9R1 "
    ...
    if J12_COMMON_GATE != {k: v for k, v in J1.J1_GATE.items() if k != "max_penetration_m_max"}:
        raise J12Error("the common gate is no longer J1_GATE minus the soft penetration bar")
    if J12_KINEMATIC_GATE != dict(J3.J3_KINEMATIC_GATE):
        raise J12Error("the kinematic gate is no longer J3_KINEMATIC_GATE")
    ...
    return {
        ...
        "actor_before": "J8 (j8_runs/j8_recovery_fit_v26c_2026-08-26_r1)",
        "actor_now": "J11 (j11_runs/j11_multistart_fit_v26c_2026-08-27_r1)",
...
def verify_actor() -> dict[str, Any]:
    """The exact EIGHT-file J11 leaf, the 35D mask contract, and the actor's own frozen sigma.

    Eight, not the six of the J8 leaf J9R1 verified: J11 also commits its aggregate dataset and
    its own commit_verification.json.
    """
    if not J11_LEAF.is_dir():
        raise J12Error(f"the J11 leaf is missing: {J11_LEAF}")
    present = sorted(str(p.relative_to(J11_LEAF)).replace(os.sep, "/")
                     for p in J11_LEAF.rglob("*") if p.is_file())
    if present != sorted(PIN_J11):
        raise J12Error(f"the J11 leaf holds {present}, expected exactly {sorted(PIN_J11)}")
    # A J11 leaf is VALID EVIDENCE only while its own commit verification says so. An
    # unverified or invalidated leaf is not a dataset and its actor is not loadable here.
    marker = J11_LEAF / TECHNICAL_INVALID_NAME
    if marker.exists():
        raise J12Error(f"the J11 leaf is marked {TECHNICAL_INVALID_NAME}; its actor may not be "
                       f"loaded, whatever its receipt says")
    cv_path = J11_LEAF / COMMIT_VERIFICATION_NAME
    if not cv_path.is_file():
        raise J12Error(f"the J11 leaf carries no {COMMIT_VERIFICATION_NAME}: it is not valid "
                       f"evidence and its actor may not be loaded")
    cv = json.loads(cv_path.read_text())
    if cv.get("pass") is not True:
        raise J12Error(f"the J11 commit verification does not pass: {cv.get('pass')!r}")
    for stale in (J11_LEAF.parent.glob(".lock_*"), J11_LEAF.parent.glob(".staging_*")):
        leftovers = sorted(q.name for q in stale)
        if leftovers:
            raise J12Error(f"the J11 run directory still holds {leftovers}; the commit did not "
                           f"complete cleanly and the leaf is not trustworthy")

    checked: dict[str, str] = {}
    for rel, pin in PIN_J11.items():
        got = _sha_file(J11_LEAF / rel)
        if got != pin:
            raise J12Error(f"the J11 artefact {rel} changed: {got} != {pin}")
        checked[rel] = got

    with (J11_MODULE_DIR / "module_state.pkl").open("rb") as fh:
    ...
    if not inputs:
        raise J12Error(f"the J11 state holds no {ACTOR_WIDTH}D input layer")
    ...
    for key in inputs:
        ...
        if zero != clock:
            raise J12Error(f"the J11 input layer {key} has zero columns {zero}; expected exactly "
        ...
        if dead:
            raise J12Error(f"the J11 input layer {key} has dead controller columns {dead}")
    ...
    if widths != [ACTOR_WIDTH]:
        raise J12Error(f"the J11 input layers are {widths}D; expected {ACTOR_WIDTH}D")
    ...
    if not bool(np.all(w[dim:] == 0.0)):
        raise J12Error("the actor's log-std output weights are not zero, so sigma is not constant")
    ...
    if not bool(np.all(np.isfinite(sigmas))) \
        ...
        raise J12Error(f"the actor carries sigma {sigmas.tolist()}, expected {SIGMA} within "
                      f"{SIGMA_TOLERANCE}. This stage REFUSES to edit a log-std head.")

    receipt = json.loads((J11_LEAF / "v26c_j11_multistart_fit_receipt.json").read_text())
    sources = receipt["aggregate"]["sources"]
    lineage = {
        "parent_module_state_sha256": receipt["parent"]["module_state_sha256"],
        ...
        "j11_verdict": receipt["verdict"],
    }
    if lineage["parent_module_state_sha256"] != PIN_J2_PARENT:
        raise J12Error("the J11 receipt records a different J2 parent")
    if lineage["j7_dataset_sha256"] != PIN_J7_DATASET:
        raise J12Error("the J11 receipt records a different J7 dataset")
    if lineage["cell_B_dataset_sha256"] != PIN_J10R1_CELL_B \
            or lineage["cell_C_dataset_sha256"] != PIN_J10R1_CELL_C:
        raise J12Error("the J11 receipt records different J10R1 teacher cells")
    if int(lineage["aggregate_rows"]) != 24713:
        raise J12Error(f"the J11 receipt records {lineage['aggregate_rows']} rows, not 24713")
    if lineage["j11_verdict"] != "PASS":
        raise J12Error(f"the J11 receipt verdict is {lineage['j11_verdict']!r}, not PASS")
    if receipt["parent"].get("is_j8") is not False or receipt["parent"].get("is_july") is not False:
        raise J12Error("the J11 receipt does not declare its parent free of J8 and July")

    # the manifest must describe THIS module, not its parent - the J8 defect J11 corrected
    manifest = json.loads((J11_MODULE_DIR / "actor_feature_manifest.json").read_text())
    if manifest.get("module_state_sha256") != PIN_J11_MODULE_STATE:
        raise J12Error(f"the J11 manifest names module {manifest.get('module_state_sha256')}, "
                       f"not the module beside it ({PIN_J11_MODULE_STATE})")
    if manifest.get("actor_digest") != PIN_J11_ACTOR_DIGEST:
        raise J12Error("the J11 manifest carries an unexpected actor_digest")
    if len(manifest.get("actor_feature_names", ())) != ACTOR_WIDTH:
        raise J12Error("the J11 manifest does not declare 35 actor features")
    if manifest.get("deployable") is not False:
        raise J12Error("the J11 manifest claims deployability, which no stage has demonstrated")
    return {
        "leaf": _rel(J11_LEAF), "module": _rel(J11_MODULE_DIR), "artefacts_sha256": checked,
        ...
        "leaf_validity": {"commit_verification_pass": True, "technical_invalid_marker": False,
                          ...
                          "rule": "a J11 leaf is valid evidence only while its own "
...
def unit_correction() -> dict[str, Any]:
    ...
    if got != J9_PREREG_SHA:
        raise J12Error(f"the original J9 preregistration changed: {got} != {J9_PREREG_SHA}")
...
def base_env_config(output_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if output_root is None:
        raise J12Error("base_env_config requires a REAL output root. Passing None is exactly the "
    ...
    if float(env["episode_start_offset_s"]) != OFFSET_NOMINAL:
        raise J12Error(f"the pinned config's episode_start_offset_s is "
    ...
    if env.get("record_outputs") is not True:
        raise J12Error(f"record_outputs is {env.get('record_outputs')!r}, expected exactly True. "
    ...
    if env.get("save_outputs_on_close") is not True:
        raise J12Error(f"save_outputs_on_close is {env.get('save_outputs_on_close')!r}, expected "
                      "exactly True. Without it osim_trj_cmc_like.close() never calls "
                      "runner.save_results().")
    if not env.get("output_prefix"):
        raise J12Error("output_prefix is absent from the base env config; the J1 builder sets it "
                      "only alongside a real output_dir")
    if not env.get("output_dir"):
        raise J12Error("output_dir is absent from the base env config")
    return env, cfg
...
def cell_env_config(base: Mapping[str, Any], cfg: Mapping[str, Any], offset_s: float,
    ...
    if float(offset_s) not in [float(v) for v in FROZEN_OFFSETS]:
        raise J12Error(f"{offset_s} is not one of the three frozen offsets {FROZEN_OFFSETS}")
    env = dict(base)
    ...
    if forbidden:
        raise J12Error(f"the cell config mutates {forbidden}; ONLY episode_start_offset_s (and the "
                      f"per-cell output_dir) may differ from the verified base")
    if float(env["episode_start_offset_s"]) != float(offset_s):
        raise J12Error("the cell config does not carry the requested offset")
    added = sorted(set(env) - set(base))
    removed = sorted(set(base) - set(env))
    if removed or sorted(set(added) - {"output_dir"}):
        raise J12Error(f"the cell config adds {added} and removes {removed} relative to the "
    ...
    if differing_stable:
        raise J12Error(f"these stable fields differ from the verified base: {differing_stable}. "
...
def evaluate_cell_gate(summary: Mapping[str, Any], knee: np.ndarray, ankle: np.ndarray,
    ...
    checks = {
        "steps": summary["steps"] == J12_COMMON_GATE["steps_required"],
        "end_reason": summary["end_reason"] == J12_COMMON_GATE["end_reason"],
        "valid_cycles": summary["valid_cycle_count"] >= J12_COMMON_GATE["valid_cycles_min"],
        "phase_timeout_stance":
            summary["phase_timeout_stance"] <= J12_COMMON_GATE["phase_timeout_stance_max"],
        "phase_timeout_swing":
            summary["phase_timeout_swing"] <= J12_COMMON_GATE["phase_timeout_swing_max"],
        "morphology_causal_contract_failure":
            summary["morphology_causal_contract_failure"]
            <= J12_COMMON_GATE["morphology_causal_contract_failure_max"],
        "hs_cancelled_count":
            summary["hs_cancelled_count"] <= J12_COMMON_GATE["hs_cancelled_count_max"],
        "resync_count": summary["resync_count"] <= J12_COMMON_GATE["resync_count_max"],
...
def preflight() -> dict[str, Any]:
    ...
    if contract["sha256"] != PIN_CONTRACT:
        raise J12Error(f"the penetration contract changed: {contract['sha256']} != {PIN_CONTRACT}")
    ...
    if lock_path.exists() or lock_path.is_symlink():
        blockers.append(f"a J12 lock is already held or was left behind: {lock_path}. This stage "
    ...
    return {
        ...
        "gate_specification": {
            "binding_common": dict(J12_COMMON_GATE),
        ...
        "multistart_disambiguation": {
            "training_data": (
                ...
                "teacher cells, 500 unique rows each tiled eight times, for 24713 in total. "
            ...
            "corrected_from_j9r1": (
                ...
                "asserted something FALSE about the J11 actor.")},
        "would_write": {"leaf": _rel(leaf), "relative_leaf": RELATIVE_LEAF,
                        "leaf_exists": bool(leaf.exists()),
                        "per_cell": ["j12_cell_<ID>_trace.json", "j12_cell_<ID>_kinematics.npz",
                                     "j12_cell_<ID>_penetration.npz",
                                     "j12_cell_<ID>_sim_outputs/ (exactly 19 regular files)"],
...
def _refuse_symlink(path: Path, root: Path) -> None:
    ...
    while True:
        if current.is_symlink():
            raise J12Error(f"refusing a symlinked path component: {current}")
...
def validate_stage(token: str | None) -> str:
    if token != STAGE:
        raise J12Error(f"--authorized-stage must be exactly {STAGE!r}, got {token!r}")
    return token


def validate_out(out_arg: str | None) -> Path:
    if out_arg is None:
        raise J12Error("--run requires --out, naming the authorised leaf exactly")
    leaf = authorized_leaf()
    got = Path(out_arg).expanduser()
    if got.is_symlink():
        raise J12Error(f"refusing a symlinked --out: {got}")
    if got.resolve(strict=False) != leaf.resolve(strict=False):
        raise J12Error(f"--out is {got}, which is not the authorised leaf {leaf}")
    root = Path(OUTPUT_ROOT_OVERRIDE) if OUTPUT_ROOT_OVERRIDE is not None else HERE
    _refuse_symlink(leaf.parent, root)
    if leaf.exists() or leaf.is_symlink():
        raise J12Error(f"the authorised leaf already exists; this stage is no-clobber and "
...
def run_cell(cell: Mapping[str, Any], *, stack: Any, base_env: Mapping[str, Any],
    ...
    seed = int(cell["seed"])
    mode = str(cell["mode"])
    sim_out = staging / f"j12_cell_{cid}_sim_outputs"
    env_config, mutation = cell_env_config(base_env, cfg, float(cell["offset_s"]), sim_out)

    stack.seed(seed)
    module = stack.load_module(J11_MODULE_DIR)
    ...
    if module_class != EXPECTED_MODULE_CLASS:
        raise J12Error(f"cell {cid}: the loaded module is a {module_class}, not a "
    ...
    if loaded_keys != EXPECTED_STATE_KEYS:
        raise J12Error(f"cell {cid}: the loaded module holds {loaded_keys}, expected "
    ...
    if mismatched:
        raise J12Error(f"cell {cid}: the loaded weights differ from the pinned module_state.pkl "
                       f"on {mismatched}")
    ...
    try:
        ...
        if WAIT_HS_FEATURE not in feature_names:
            raise J12Error(f"cell {cid}: the runtime actor exposes no {WAIT_HS_FEATURE}")
        ...
        if live_duration is not None:
            if not isinstance(live_duration, (int, float)) \
                ...
                raise J12Error(f"cell {cid}: env_cfg.episode_duration is {live_duration!r}, not a "
                              "finite number")
        ...
        if (live_duration is None) != (declared_duration is None):
            raise J12Error(f"cell {cid}: the live episode_duration is {live_duration!r} while the "
                          f"config declares {declared_duration!r}")
        if live_duration is not None \
                and abs(live_duration - float(declared_duration)) > 1e-12:
            raise J12Error(f"cell {cid}: the live episode_duration {live_duration} and the declared "
        ...
        if mode == "stochastic_held":
            if getattr(stack, "held_stochastic_action", None) is None:
                raise J12Error("the stack exposes no held_stochastic_action helper")
        ...
        for step in range(1, EXPECTED_STEPS + 1):
            ...
            if mode == "deterministic":
                ...
                if dev != 0.0:
                    raise J12Error(f"cell {cid} step {step}: the deterministic action differs from "
                ...
                if getattr(stack, "reference_action", None) is not None:
                    ...
                    if not (np.array_equal(raw, r_a) and np.array_equal(mean, r_m)
                        ...
                        raise J12Error(f"cell {cid} step {step}: the deterministic action does not "
                ...
                if not np.array_equal(np.asarray(d_m, dtype=np.float32), mean):
                    raise J12Error(f"cell {cid} step {step}: forward_exploration and "
                                  "forward_inference disagree on the policy mean")
                if not np.array_equal(np.asarray(d_s, dtype=np.float32), std):
                    raise J12Error(f"cell {cid} step {step}: the two forward paths disagree on the "
                                  "std")
                if float(np.max(np.abs(std.astype(np.float64) - SIGMA))) > SIGMA_TOLERANCE:
                    raise J12Error(f"cell {cid} step {step}: the effective std is {std.tolist()}, "
                                  f"expected {SIGMA}")
                if not np.array_equal(applied_noise, (std * unit).astype(np.float32)):
                    raise J12Error(f"cell {cid} step {step}: applied noise is not std * unit_noise")
                parity_steps += 1
            ...
            if "time" not in info:
                raise J12Error(f"cell {cid} step {step}: info exposes no 'time'")
            terms = J1._jsonable(info.get("reward_terms", {}), "reward_terms")
            if "grf_penetration_m" not in terms:
                raise J12Error(f"cell {cid} step {step}: reward_terms carries no "
                              "grf_penetration_m")
        ...
        # chained J12Error, but KeyboardInterrupt and SystemExit are NOT exceptions of the run -
        ...
        try:
            ...
            raise J12Error(f"cell {cid}: the environment failed to close cleanly, so the cell is "
    ...
    penetration = penetration_report(pen_arr, contract, f"J12 cell {cid}")
    ...
    # J9R1 accepted any non-empty directory; J10R1 hardened this and J12 inherits the hardening.
    if not sim_out.is_dir():
        raise J12Error(f"cell {cid}: the sim_outputs directory is missing at {sim_out}")
    ...
    if non_files:
        raise J12Error(f"cell {cid}: sim_outputs holds non-regular entries {non_files}")
...
def run_matrix(out_arg: str | None, stage_token: str | None, *, stack: Any = None,
    ...
    lock_path = leaf.parent / LOCK_NAME
    if staging.exists() or staging.is_symlink():
        raise J12Error(f"a stale staging directory is in the way: {staging}")
    ...
    try:
        ...
        try:
            ...
            raise J12Error(f"the J12 lock already exists: {lock_path}. This stage fails closed and "
        ...
        for flag in ("record_outputs", "save_outputs_on_close"):
            if base_env.get(flag) is not True:
                raise J12Error(f"the base env config about to be used has {flag}="
        ...
        if not base_env.get("output_prefix") or not base_env.get("output_dir"):
            raise J12Error("the base env config about to be used carries no output_prefix or no "
        ...
        if actor_after["artefacts_sha256"] != actor_before["artefacts_sha256"]:
            raise J12Error("the J11 actor changed during the matrix")
        ...
        receipt = {
            "schema": "v26c_j12_closed_loop_receipt.1", "stage": STAGE,
        ...
        if leaf.exists() or leaf.is_symlink():
            raise J12Error(f"the leaf appeared while staging; refusing to clobber: {leaf}")
        os.rename(staging, leaf)
        staging_created = None

        marker = leaf / TECHNICAL_INVALID_NAME
        if not marker.is_file():
            raise J12Error(f"the committed leaf does not carry {TECHNICAL_INVALID_NAME}: "
        ...
        try:
            ...
            verification = {"schema": "v26c_j12_commit_verification.1", "stage": STAGE,
        ...
        try:
            ...
            raise J12Error(f"the verification could not be recorded in {leaf}: "
        ...
        try:
            ...
            raise J12Error(f"the verification passed but the marker could not be removed from "
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="V26C J12 closed-loop qualification of the J11 actor")
    ...
    if a.out is not None:
        raise J12Error("--out is meaningless without --run; the preflight writes nothing")
    r = preflight()
    print(json.dumps(r, indent=2, default=str))
