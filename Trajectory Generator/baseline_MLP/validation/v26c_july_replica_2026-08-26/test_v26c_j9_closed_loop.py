"""Fail-closed tests for the V26C J9 closed-loop qualification.
...
import v26c_j9_closed_loop as J9  # noqa: E402
...
class FakeEnv:
    ...
    def __init__(self, cfg):
        ...
        (out / "sim_outputs_marker.txt").write_text("synthetic", encoding="utf-8")
        FakeEnv.built.append({"offset": self.offset, "output_dir": str(out)})
...
def main() -> int:
    src = (HERE / "v26c_j9_closed_loop.py").read_text()
    tree = ast.parse(src)
    ids = {t.string for t in tokenize.generate_tokens(io.StringIO(src).readline)
           if t.type == tokenize.NAME}
    prereg = json.loads(J9.PREREG.read_text())
    before_all = snapshot(HERE)

    # ---------------------------------------------------------------- preregistration ----------
    check(prereg["kind"] == "ADDITIVE, IMMUTABLE, PREREGISTRATION"
          and prereg["stage_authorised"] == "V26C-J9-CLOSED-LOOP-QUALIFICATION" == J9.STAGE,
          "the prereg is additive, immutable and names the J9 stage")
    check(J9._sha_file(J9.PREREG) == J9.PIN_PREREG
          == "e6a6888a473d5ef76b1f009037f7825995fd62b35cc51749dfe41ad7f718c44a",
          "and is pinned by exact hash")
    check(len(prereg["pinned_artefacts_sha256"]) == 15
          and all(J9._sha_file(HERE / r) == h
                  for r, h in prereg["pinned_artefacts_sha256"].items()),
          "all 15 local pins verify on disk")
    check(len(prereg["pinned_repo_artefacts_sha256"]) == 5
    ...
    for rel, why in (("Trajectory Generator/osim_trj_cmc_like.py", "the environment itself"),
        ...
        check(rel in prereg["pinned_repo_artefacts_sha256"]
              and prereg["pinned_repo_artefacts_sha256"][rel] == J9._sha_file(J9.REPO / rel),
              f"{why} is pinned and verifies on disk")
    check(prereg["pinned_repo_artefacts_sha256"]["Trajectory Generator/osim_trj_cmc_like.py"]
    ...
    base, cfg = J9.base_env_config(output_dir=None)
    ...
    for off in (J9.OFFSET_NOMINAL, J9.OFFSET_MINUS, J9.OFFSET_PLUS):
        ...
        others = [k for k in base if k != "episode_start_offset_s" and base[k] != env.get(k)]
        check(others == [], f"MEASURED: every other field is identical ({others})")
        check(mut["base_verified_by_j1_before_mutation"] is True
              and mut["second_j1_verify_on_mutated_config"] is False
              and mut["fields_identical_to_verified_base"] == len(base) - 1,
    ...
    check(J9.base_env_config(output_dir=None)[0]["episode_start_offset_s"] == J9.OFFSET_NOMINAL,
    ...
    for rel in ("v26c_j1_collect.py", "v26c_j3_closed_loop.py"):
        check(prereg["pinned_artefacts_sha256"].get(rel) == J9._sha_file(HERE / rel),
    ...
    check(set(prereg["imported_frozen_runners"]) >= {"v26c_j1_collect.py",
          ...
          "and the prereg records why they are pinned")
    e = None
    saved_pin = J9.PIN_PREREG
    try:
        J9.PIN_PREREG = "0" * 64
        e = expect(J9.verify_prereg, J9.J9Error, "a changed preregistration")
    finally:
        J9.PIN_PREREG = saved_pin
    check("preregistration changed" in str(e), "verify_prereg is fail-closed on its own hash")
    ...
    check(leaf == HERE / "j9_runs" / "j9_closed_loop_v26c_2026-08-26_r1",
          "the authorised leaf is the architect's")
    ...
    for bad_out in (str(HERE), str(HERE / "j9_runs"), str(HERE / "j8_runs")):
    ...
    check(snapshot(HERE) == before_all and not (HERE / "j9_runs").exists(),
          "MEASURED: every refusal wrote nothing; j9_runs does not exist")
    ...
    try:
        ...
        check(target == tmp / "j9_runs" / "j9_closed_loop_v26c_2026-08-26_r1",
        ...
        lock.unlink()
        sentinel = tmp / "j9_runs" / "concurrent.txt"
    ...
    check(prereg["matrix"]["behavioural_fail_fast"] is False
    ...
    check(imported == {"__future__", "argparse", "hashlib", "json", "os", "pickle", "shutil",
                       "sys", "pathlib", "typing", "numpy", "v26c_j1_collect",
    ...
    # ---------------------------------------------------------------- nothing disturbed ---------
    check(not (HERE / "j9_runs").exists(),
          "MEASURED: this suite never created the authorised j9_runs")
    check(snapshot(HERE) == before_all,
