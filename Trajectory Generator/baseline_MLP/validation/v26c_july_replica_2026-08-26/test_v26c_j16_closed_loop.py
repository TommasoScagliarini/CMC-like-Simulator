#!/usr/bin/env python
"""Fail-closed tests for the V26C J16 closed-loop qualification of the J15R1 actor.
...
root. The authorised J16 leaf is never created.
...
import v26c_j16_closed_loop as J16  # noqa: E402   the runner under test
import v26c_j12_closed_loop as J12  # noqa: E402   the PREDECESSOR, for identity proofs
import builtins, dis, inspect       # noqa: E402
...
# ---------------------------------------------------------------- pinned truth ------------------
MANIFEST = json.loads((J16.J15R1_MODULE_DIR / "actor_feature_manifest.json").read_text())
NAMES = tuple(str(n) for n in MANIFEST["actor_feature_names"])
WAIT_IDX = NAMES.index("phase_fsm_wait_hs")
with (J16.J15R1_MODULE_DIR / "module_state.pkl").open("rb") as _fh:
...
class FakeEnv:
    ...
    def __init__(self, cfg):
        ...
        self.start = J16.expected_reset_time(11.99, 21.0, cfg.get("episode_duration"),
...
STD = np.full(2, J16.SIGMA, dtype=np.float32)
...
def fake_deterministic_action(module, obs, shape, torch_mod=None):
    ...
    would only prove that torch works. What this suite must prove is J16's own logic - the matrix,
...
def main() -> int:
    src = (HERE / "v26c_j16_closed_loop.py").read_text()
    tree = ast.parse(src)
    ids = {t.string for t in tokenize.generate_tokens(io.StringIO(src).readline)
           if t.type == tokenize.NAME}
    prereg = json.loads(J16.PREREG.read_text())
    before_all = snapshot(HERE)
    leaf_exists_at_start = J16.authorized_leaf().exists()

    # ---------------------------------------------------------------- preregistration ----------
    check(prereg["kind"] == "ADDITIVE, IMMUTABLE, PREREGISTRATION"
          and prereg["stage_proposed"] == "V26C_J16_J15R1_CLOSED_LOOP_REQUALIFICATION" == J16.STAGE,
          "the prereg is additive, immutable and proposes the J16 stage")
    ...
    check(J16._sha_file(J16.PREREG) == J16.PIN_PREREG
          == "150f49b1bd2865d9224d43336d50098c2fd610b4b5d121dd1ce737213a0864aa",
          "and is pinned by exact hash")
    check(all(J16._sha_file(HERE / r) == h
              for r, h in prereg["pinned_artefacts_sha256"].items()),
          f"all {len(prereg['pinned_artefacts_sha256'])} local pins verify on disk")
    check(all(J16._sha_file(J16.REPO / r) == h
    ...
    # ---------------------------------------------------------------- lineage ------------------
    check(prereg["actor"]["stage"] == "V26C_J15_FRESH_35D_POST_MISMATCH_REFIT"
          and prereg["actor"]["is_j8"] is False and prereg["actor"]["is_july"] is False
          and prereg["actor"]["is_j11"] is False,
          "the actor is J15R1: not J8, not July, and NOT J11")
    check(prereg["actor"]["j11_is_the_actor_that_FAILED_j12"]
          == J16.PIN_J11_FAILED_ACTOR != prereg["actor"]["module_state_sha256"],
          "and the prereg names the J11 actor that FAILED this same matrix, as a DIFFERENT hash")
    check("j15_runs" in str(J16.J15R1_LEAF) and "j8_runs" not in str(J16.J15R1_LEAF)
          and "j11_runs" not in str(J16.J15R1_LEAF),
          "the runner points at the J15R1 leaf, never at J11's")
    j11_state = HERE / "j11_runs" / "j11_multistart_fit_v26c_2026-08-27_r1" / "rl_module" \
        / "module_state.pkl"
    check(J16._sha_file(j11_state) == J16.PIN_J11_FAILED_ACTOR
          != J16.PIN_J15R1_MODULE_STATE,
          "MEASURED: the J11 actor exists, is pinned as negative evidence, and is NOT the actor "
          "under test")
    j8_state = HERE / "j8_runs" / "j8_recovery_fit_v26c_2026-08-26_r1" / "rl_module" \
        / "module_state.pkl"
    check(J16._sha_file(j8_state) != J16.PIN_J15R1_MODULE_STATE,
          "MEASURED: the J8 module is NOT this stage's actor")
    check(len(J16.PIN_J15R1) == 8,
          "the J15R1 leaf is pinned file by file - EIGHT files")
    check(sorted(J16.PIN_J15R1) == sorted(
              str(p.relative_to(J16.J15R1_LEAF)).replace(os.sep, "/")
              for p in J16.J15R1_LEAF.rglob("*") if p.is_file()),
          "MEASURED: and the pin table is exactly the leaf's file set")

    # ---------------------------------------------------------------- the actor ----------------
    actor = J16.verify_actor()
    ...
    check(actor["leaf_validity"]["commit_verification_pass"] is True
          ...
          "the J15R1 leaf is consumed only because its own commit verification PASSED")
    ...
    check(abs(actor["sigma"]["max_abs_deviation"]) <= J16.SIGMA_TOLERANCE
          and actor["sigma"]["logstd_head_edited"] is False,
          f"sigma is the actor's own, within {J16.SIGMA_TOLERANCE} "
          f"(deviation {actor['sigma']['max_abs_deviation']:.3g})")
    check(actor["lineage"]["parent_module_state_sha256"] == J16.PIN_J2_PARENT
          and actor["lineage"]["j7_dataset_sha256"] == J16.PIN_J7_DATASET
          and actor["lineage"]["cell_B_dataset_sha256"] == J16.PIN_J10R1_CELL_B
          and actor["lineage"]["cell_C_dataset_sha256"] == J16.PIN_J10R1_CELL_C
          and actor["lineage"]["j14_increment_sha256"] == J16.PIN_J14_DATASET
          and actor["lineage"]["aggregate_rows"] == 25567
          and actor["lineage"]["j15r1_verdict"] == "PASS",
          "the J15R1 receipt's lineage is read from ITS schema and every hash matches")
    check(actor["lineage"]["j11_was_loaded"] is False
          and actor["lineage"]["j11_module_state_sha256"] == J16.PIN_J11_FAILED_ACTOR,
          "and it declares that the J11 actor - the one that FAILED J12 - was never loaded")
    check(actor["lineage"]["aggregate_rows"] == 25567 != 24713,
          "MEASURED: 25567 rows, not J11's 24713 - the aggregate gained the 854 J14 rows")
    check(actor["manifest"]["module_state_sha256"] == J16.PIN_J15R1_MODULE_STATE
          and actor["manifest"]["regenerated_not_copied"] is True
          and actor["manifest"]["deployable"] is False,
          "the J15R1 manifest describes the module beside it - the J8 defect is not inherited")
    j8_man = json.loads((HERE / "j8_runs" / "j8_recovery_fit_v26c_2026-08-26_r1" / "rl_module"
                         / "actor_feature_manifest.json").read_text())
    check(j8_man["module_state_sha256"] != J16._sha_file(j8_state),
          "MEASURED: J8's own manifest names a DIFFERENT module - the defect J15R1 corrected")

    # ---------------------------------------------------------------- the science ---------------
    eq = J16.verify_scientific_equivalence()
    check(eq["thresholds_invented_here"] == 0, "no threshold is invented here")
    check(eq["source_sha256"] == J16.PIN_J9R1_MODULE, "the J9R1 source is pinned by hash")
    check(J16.J16_COMMON_GATE == J9R1.J9_COMMON_GATE
    ...
    check(J16.J16_COMMON_GATE == {"steps_required": 500, "end_reason": "episode_time_limit",
    ...
    check("max_penetration_m_max" not in J16.J16_COMMON_GATE,
          "the 0.020 bar is DROPPED: the sole binding penetration bar is 0.028")
    check(J16.J16_KINEMATIC_GATE == dict(J3.J3_KINEMATIC_GATE)
    ...
    check((J16.SIGMA, J16.SIGMA_TOLERANCE, J16.NOISE_HOLD_STEPS, J16.EXPECTED_STEPS,
           J16.RESET_TIME_TOLERANCE_S) == (0.005, 1e-6, 1, 500, 1e-9),
    ...
    got = [(c["id"], c["mode"], c["seed"], c["offset_s"]) for c in J16.MATRIX]
    ...
    check(len(J16.MATRIX) == 6 and prereg["matrix"]["cells_count"] == 6
    ...
    try:
        ...
        expect(J16.verify_scientific_equivalence, J16.J16Error,
               "a drifted J9R1 threshold stops this stage")
    finally:
        J9R1.J9_COMMON_GATE = real_gate
    check(J16.verify_scientific_equivalence()["thresholds_invented_here"] == 0, "restored")

    # ---------------------------------------------------------------- penetration ---------------
    contract = PC.load_contract()
    check(contract["sha256"] == J16.PIN_CONTRACT, "the penetration contract is pinned")
    ...
    check(J16.WAIT_HS_IS_BINDING is False and prereg["wait_hs"]["binding"] is False,
          "WAIT_HS is DIAGNOSTIC: attaching a threshold would be a NEW gate")
    check(J16.WAIT_HS_FEATURE == "phase_fsm_wait_hs" and WAIT_IDX == 17,
    ...
    check("valid_cycle_count" not in J16.DIAGNOSTIC_NOT_BINDING,
    ...
    check("six-file leaf" not in src and "EIGHT-file J15R1 leaf" in src,
          "verify_actor's docstring says EIGHT-file J15R1 leaf, not six")
    check("J9 lock" not in src and src.count("J16 lock") >= 2,
          "both lock messages say J16 lock: the preflight blocker and the commit refusal")
    check("j9_cell_" not in src and "j16_cell_<ID>_trace.json" in src,
          "the preflight's would_write names j16_cell_<ID>_* artefacts")
    check("the J8 manifest holds" not in src and "the J15R1 manifest holds" in src,
          "the feature-count error names the J15R1 manifest")
    check("the J8 actor changed" not in src and "the J15R1 actor changed" in src,
          "the post-matrix drift error names the J15R1 actor")
    check("chained J9Error" not in src and "chained J16Error" in src,
          "the close-failure comment names J16Error, the exception actually raised")
    ...
    check("J16" in desc and "J9" not in desc,
          f"the argparse description names J16, not J9 ({desc!r})")
    ...
    # J11 is HISTORY here: it is the actor that failed this same matrix, and the runner must say
    # so rather than quietly forget it.
    for j11_ref in ("NOT J11 - J11 is the actor that FAILED J12",
                    "PIN_J11_FAILED_ACTOR",
                    "which \"\n                        \"FAILED this same matrix 4/6 in J12"):
        check(j11_ref in src, f"the J11 history is retained: {j11_ref[:44]!r}")
    check("j15_runs/j15_fresh_refit_v26c_2026-08-27_r1" in src
          and "j11_runs/j11_multistart_fit_v26c_2026-08-27_r1" in src,
          "both leaves are named: the actor under test, and the one that failed")
    check(J16.J9_PREREG_SHA and J16.J9_RUNNER_SHA and J16.J9_TEST_SHA
          and J16.J9_AUTHORIZATION_SHA and J16.J9_FAILURE_RECORD_SHA,
          "the five superseded J9 artefacts are still pinned as immutable evidence")

    # ---------------------------------------------------------------- the env-mutation rule ----
    pol = J16.ENV_MUTATION_POLICY
    ...
    md = J16.preflight()["multistart_disambiguation"]
    check("PRESENT" in md["training_data"] and "25567" in md["training_data"]
          and "854" in md["training_data"],
          "the multistart TRAINING data is declared PRESENT: the J15R1 actor was fitted on it")
    ...
    check("FALSE about the J15R1 actor" in md["corrected_from_j9r1"],
          "the J9R1 sentence, true of J8 and false of J15R1, is explicitly corrected")
    ...
    check(not J16.PREFLIGHT_SENTINEL.exists()
    ...
    # ---------------------------------------------------------------- destination guards -------
    expect(lambda: J16.validate_stage(None), J16.J16Error, "a missing stage token is refused")
    for bad in ("V26C_J9R1_CLOSED_LOOP", "V26C_J16_CLOSED_LOOP_QUALIFICATION ", ""):
        expect(lambda b=bad: J16.validate_stage(b), J16.J16Error,
               f"the stage token {bad!r} is refused")
    J16.validate_stage(J16.STAGE)
    check(True, "and the exact token is accepted")
    expect(lambda: J16.validate_out(None), J16.J16Error, "--out is required")
    expect(lambda: J16.validate_out("/tmp/elsewhere"), J16.J16Error,
    ...
    tmp = Path(tempfile.mkdtemp(prefix="v26c_j16_matrix_"))
    saved_root = J16.OUTPUT_ROOT_OVERRIDE
    real_det = J3.deterministic_action
    try:
        J3.deterministic_action = fake_deterministic_action
        J16.OUTPUT_ROOT_OVERRIDE = tmp
        target = tmp / "j16_runs" / "j16_closed_loop_v26c_2026-08-27_r1"
        lock = target.parent / J16.LOCK_NAME
        FakeEnv.wait_hs_steps = 12

        # ---- the silent-load failure is caught
        FakeStack.module_cls = BareRLModule
        FakeEnv.built.clear()
        e = expect(lambda: J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(),
                                          progress=False),
                   J16.J16Error, "a weightless module is REFUSED")
        ...
        try:
            e = expect(lambda: J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(),
                                              progress=False),
                       J16.J16Error, "an empty get_state() is REFUSED even from the right class")
        finally:
        ...
        try:
            e = expect(lambda: J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(),
                                              progress=False),
                       J16.J16Error, "weights that differ from the pinned module_state are REFUSED")
        ...
        for bad_n in (18, 20):
            ...
            e = expect(lambda: J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(),
                                              progress=False),
                       J16.J16Error, f"sim_outputs with {bad_n} files is REFUSED")
        ...
        held = json.dumps({"stage": J16.STAGE, "pid": 999999})
        lock.write_text(held, encoding="utf-8")
        e = expect(lambda: J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(),
                                          progress=False),
                   J16.J16Error, "a held lock is fail-closed")
        check(lock.read_text() == held, "MEASURED: the foreign lock was left untouched")
        check(J16.preflight()["verdict"] == "BLOCKED",
              "and the read-only preflight reports it as a blocker")
        lock.unlink()

        # ---- the real matrix
        FakeEnv.built.clear(); FakeEnv.closed.clear(); FakeEnv.saved.clear()
        res = J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(), progress=False)
        rc = json.loads((target / J16.RECEIPT_NAME).read_text())
        ...
        check(J16.STAGING_NAME not in (target / J16.RECEIPT_NAME).read_text(),
              "no staging path appears anywhere in the receipt")

        # ---- the commit is verified
        cv = json.loads((target / J16.COMMIT_VERIFICATION_NAME).read_text())
        ...
        check(not (target / J16.TECHNICAL_INVALID_NAME).exists(),
              "the born-invalid marker is GONE after a passing verification")
        for rel, sha in rc["committed_files_sha256"].items():
            check((target / rel).is_file() and J16._sha_file(target / rel) == sha,
        ...
        check(J16.verify_committed_leaf(target)["pass"] is False,
              "MEASURED: a tampered committed file is caught")
        victim.write_bytes(keep)
        check(J16.verify_committed_leaf(target)["pass"] is True, "and restoring it passes again")
        check(J16.verify_committed_leaf(target, expected_receipt_sha="0" * 64)["pass"] is False,
              "MEASURED: a receipt whose bytes differ from the staged bytes FAILS")
        for bad in ("/etc/passwd", "../outside", "", "a/../../b"):
            expect(lambda b=bad: J16._resolve_inside(target, b), J16.J16Error,
        ...
        try:
            res2 = J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(), progress=False)
        finally:
            FakeEnv.bad_cycles = False
        rc2 = json.loads((target / J16.RECEIPT_NAME).read_text())
        ...
        check((target / J16.COMMIT_VERIFICATION_NAME).is_file()
              and json.loads((target / J16.COMMIT_VERIFICATION_NAME).read_text())["pass"] is True,
        ...
        shutil.rmtree(target)
        sentinel = tmp / "j16_runs" / "concurrent.txt"
        sentinel.write_text("other", encoding="utf-8")
        FakeEnv.close_raises = True
        try:
            expect(lambda: J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(),
                                          progress=False),
        ...
        check(not target.exists() and not lock.exists()
              and not (target.parent / J16.STAGING_NAME).exists(),
        ...
        FakeEnv.built.clear()
        real_verify = J16.verify_committed_leaf
        J16.verify_committed_leaf = lambda leaf, **k: {"pass": False, "paths_missing": ["x"],
                                                       "hash_mismatches": []}
        try:
            expect(lambda: J16.run_matrix(str(target), J16.STAGE, stack=FakeStack(),
                                          progress=False),
                   J16.J16Error, "a FAILED post-commit verification RAISES")
        finally:
            J16.verify_committed_leaf = real_verify
        check(target.is_dir() and (target / J16.RECEIPT_NAME).is_file(),
              "MEASURED: the leaf is PRESERVED - a corrupted commit is evidence about the commit")
        check((target / J16.TECHNICAL_INVALID_NAME).is_file(),
        ...
        J16.OUTPUT_ROOT_OVERRIDE = saved_root
    ...
    # ============ J16-SPECIFIC: the delta versus J12, proved rather than asserted ============
    # The architect's contract is that the ONLY scientific variable is the actor. Prose cannot
    # establish that. These checks compare the two runners as PROGRAMS.

    def code_sig(code):
        """Bytecode, non-code constants, varnames and arity - recursively through nested code.
        Code objects are compared by this signature, never by identity, which never holds
        across two modules."""
        return (code.co_code,
                tuple(code_sig(c) if hasattr(c, "co_code") else c for c in code.co_consts),
                code.co_varnames, code.co_argcount)

    SCIENTIFIC_PATH = ("evaluate_cell_gate", "cell_verdict", "penetration_report",
                       "expected_reset_time", "base_env_config", "cell_env_config",
                       "unit_correction")
    for name in SCIENTIFIC_PATH:
        a, b = getattr(J12, name), getattr(J16, name)
        check(code_sig(a.__code__) == code_sig(b.__code__),
              f"BYTECODE IDENTITY: {name} is the SAME PROGRAM as J12's - every threshold, "
              f"comparison and environment setting on the scientific path is untouched")

    def consts(code, out=None):
        out = [] if out is None else out
        for c in code.co_consts:
            consts(c, out) if hasattr(c, "co_code") else out.append(c)
        return out

    nums = lambda fn: [c for c in consts(fn.__code__)
                       if isinstance(c, (int, float)) and not isinstance(c, bool)]
    changed_numeric = [n for n in SCIENTIFIC_PATH
                       if nums(getattr(J12, n)) != nums(getattr(J16, n))]
    check(not changed_numeric,
          f"and not one NUMERIC constant differs on that path ({changed_numeric})")

    shared = sorted(n for n, f in vars(J12).items()
                    if inspect.isfunction(f) and f.__module__ == J12.__name__
                    and inspect.isfunction(getattr(J16, n, None)))
    numeric_diff = {n: (nums(getattr(J12, n)), nums(getattr(J16, n))) for n in shared
                    if nums(getattr(J12, n)) != nums(getattr(J16, n))}
    check(set(numeric_diff) == {"verify_actor"},
          f"ACROSS THE WHOLE RUNNER exactly ONE function changes a number: {sorted(numeric_diff)}")
    a_nums, b_nums = numeric_diff["verify_actor"]
    check(sorted(set(a_nums) ^ set(b_nums)) == [24713, 25567],
          f"and that one change is 24713 -> 25567, the J15R1 aggregate row count "
          f"({sorted(set(a_nums) ^ set(b_nums))})")

    for c in ("MATRIX", "OFFSET_NOMINAL", "OFFSET_MINUS", "OFFSET_PLUS", "FROZEN_OFFSETS",
              "SIGMA", "SIGMA_TOLERANCE", "NOISE_HOLD_STEPS", "EXPECTED_STEPS",
              "EXPECTED_SIM_OUTPUT_FILES", "ACTOR_WIDTH", "CLOCK_COLUMNS", "CONTROLLER_COLUMNS",
              "RESET_TIME_TOLERANCE_S", "DIAGNOSTIC_NOT_BINDING", "EXPECTED_STATE_KEYS",
              "EXPECTED_MODULE_CLASS", "WAIT_HS_IS_BINDING", "PIN_J2_PARENT", "PIN_J7_DATASET",
              "PIN_J10R1_CELL_B", "PIN_J10R1_CELL_C", "PIN_J9R1_MODULE", "PIN_CONTRACT",
              "PIN_RUNTIME_CONFIG_SHA", "RESET_SEMANTICS", "ENV_MUTATION_POLICY"):
        check(getattr(J12, c) == getattr(J16, c),
              f"the frozen constant {c} is identical to J12's")
    check(J12.J12_COMMON_GATE == J16.J16_COMMON_GATE
          and J12.J12_KINEMATIC_GATE == J16.J16_KINEMATIC_GATE,
          "and both gates are identical to J12's, value for value")

    # the actor IS the variable, and it is the only one
    check(J16.PIN_J15R1_MODULE_STATE != J12.PIN_J11_MODULE_STATE
          and J16.PIN_J15R1_MODULE_STATE == J16.PIN_J15R1["rl_module/module_state.pkl"],
          "MEASURED: the actor differs from J12's, and the two pins agree with each other")
    check(J16.PIN_J11_FAILED_ACTOR == J12.PIN_J11_MODULE_STATE,
          "and J16 pins J12's actor by its exact hash, as negative evidence")

    # ---- namespace closure: the J15 defect must not recur -------------------------------------
    def unresolved(fn):
        ...
        return sorted(n for n in out
                      if not hasattr(J16, n) and not hasattr(builtins, n))
    fns = [(n, f) for n, f in vars(J16).items()
           if inspect.isfunction(f) and f.__module__ == J16.__name__]
    bad = {n: unresolved(f) for n, f in fns if unresolved(f)}
    check(not bad, f"NAMESPACE CLOSURE: every global read by every module-level function of this "
                   f"runner resolves. Unresolved: {bad}")
    for name in ("run_matrix", "run_cell", "preflight", "verify_actor", "main"):
        check(not unresolved(getattr(J16, name)),
              f"and {name} in particular has zero unresolved globals")

    # ---- seed sealing: a NEW construct, not carried over from J12 ------------------------------
    seeds = sorted({c["seed"] for c in J16.MATRIX})
    check(seeds == [123, 124, 125], f"MEASURED: the matrix reads seeds {seeds} and no other")
    sealed_lits = {n.value for n in nodes if isinstance(n, ast.Constant)
                   and isinstance(n.value, int) and n.value in (126, 127, 128)}
    check(not sealed_lits,
          f"MEASURED: seeds 126, 127 and 128 appear as NO literal in this runner ({sealed_lits})")
    check(prereg["seeds"]["sealed_never_read"] == [126, 127, 128]
          and prereg["seeds"]["read_by_this_stage"] == [123, 124, 125],
          "and the preregistration declares the reserve SEALED - a construct J12 did not carry, "
          "introduced here deliberately rather than inherited")

    # ---- no production mutation ---------------------------------------------------------------
    prod = J16.REPO / "Trajectory Generator" / "baseline_MLP"
    for rel, pin in prereg["pinned_repo_artefacts_sha256"].items():
        check(J16._sha_file(J16.REPO / rel) == pin,
              f"the production source {rel.split('/')[-1]} is unchanged")

    # ---------------------------------------------------------------- nothing disturbed --------
    check(not (HERE / "j16_runs").exists() if not leaf_exists_at_start else True,
          "MEASURED: this suite never created the authorised j16_runs")
