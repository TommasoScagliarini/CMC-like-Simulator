"""V26C J20 R3 - the zero-iteration restore audit that closes gate G9.

A MECHANICAL COPY of v26c_j20_restore_audit_r2.py, the R2 runner, which stays
byte-identical at e5776700f9e12c8632d239898b673eba0f0cab6e08905001a9778bc12b75e81c
because the R2 architect GO pins it and the R2 receipt names it. Nothing about
R1 or R2 is edited, reused or overwritten; both leaves are pinned file by file
and re-hashed before and after this stage's child.
...
self-guard fired at 600 s. algo.restore_from_path was never reached. rev4's
environment correction fixed that and this file carries it unchanged.

R2 reached the restore for the first time and failed closed at 7/13. Its gate
R9 reported eight differences, all inside param_groups[0]: five Python booleans
against zero-dimensional tensors, and betas[0], betas[1] and eps against their
float32 images. Every moment tensor agreed byte for byte. Those eight are what
happens when the checkpoint is read BEFORE RLlib's conversion and the live
optimizer AFTER it: TorchLearner._set_optimizer_state calls
convert_to_torch_tensor(state_dict["state"], device=self._device), and that
function routes every non-None leaf through numpy into torch and downcasts every
floating tensor except float16 to float32.

rev6 changed ONE thing about the audit: R9 now compares both sides under the
SAME post-conversion semantics, by passing each through RLlib's own
convert_to_torch_tensor and then comparing dtype, shape and the sha256 of the
C-order bytes of every leaf. No tolerance is introduced anywhere. rev6 also
requires that the five gates downstream of R9 be EVALUATED only when the live
restore completed, because in R2 they read fields an aborted run never wrote.

rev7 is the governing amendment, and it closes a defect in rev6's own
implementation. _reapply_optimizer_learning_rate overwrites param_groups lr with
the stage's configured value on the line before the capture, so the canonical
tree compares 1e-4 against 1e-4 whatever the restore actually loaded. rev6
recorded the pre-overwrite value but required nothing of it, so a wrong restored
learning rate passed 13 of 13 - measured, not theorised. rev7 makes the wrapper
compute and ENFORCE that observation before the verdict is written, and makes R9
require it together with the canonical fields that prove the comparison was
performed. Nothing else moves.

Seven functions differ from the R2 runner and no others: live_restore_completed
(new), preflight, go_pin_targets, expected_pin_hashes, check_entry_evidence,
evaluate_gates and run_execution. restore_command, hermetic_restore,
verify_commit, validate_go, child_environment and launch_once are all
byte-identical, so the four command operations, the hermetic second opinion, the
commit protocol, the APPROVED-only GO rule and the single environment correction
carry over unchanged. The test suite proves that mechanically, function by
function.

rev7 adds no new function: it lands inside evaluate_gates, which already differs
from R2's, so the seven-function delta below is unchanged.
...
R9), rev4 (which records R1's observed cause and the single environment
correction), rev5 (which corrects the semantics and strengthens the smoke test)
and rev6 (which canonicalises R9's comparison and disciplines the downstream
gates). All seven are pinned by the GO and re-hashed here; precedence is
rev6 > rev5 > rev4 > rev3 > rev2 > rev1 > base. The gate count is still 13 and
only R9's comparison semantics moved.

R3 is the THIRD CORRECTIVE ATTEMPT of this audit - a third attempt after R1 and
R2 failed, never an automatic retry. It has its own stage identifier,
V26C_J20_RESTORE_AUDIT_R3, its own preregistration, its own GO, its own
validation-only wrapper and its own leaf, and it runs exactly once, only after
BOTH an explicit user authorisation and an APPROVED architect GO.
...
STAGE = "V26C_J20_RESTORE_AUDIT_R3"
...
PREREG_REV6_NAME = "v26c_j20_prereg_restore_audit_rev6.json"
PIN_PREREG_REV6 = "6575f64ccadf30e53dfc5a1d7af2966541efe6911ad37550adf1e6a2346c6615"
PREREG_REV7_NAME = "v26c_j20_prereg_restore_audit_rev7.json"
PIN_PREREG_REV7 = "32238dafb13b25e90428538302d197c421acd69cf67f8884ab4622b3c27c38d8"
PREREG_PRECEDENCE = ("rev7", "rev6", "rev5", "rev4", "rev3", "rev2", "rev1",
                     "base")
GO_REQUIRED_STATUS = "APPROVED"
RUNNER_NAME = "v26c_j20_restore_audit_r3.py"
TEST_NAME = "test_v26c_j20_restore_audit_r3.py"
# The R2 runner, its suite, its GO and every file of its leaf: pinned so R3
# cannot run if any of them moved, and so R2 stays immutable evidence.
R2_RUNNER_NAME = "v26c_j20_restore_audit_r2.py"
PIN_R2_RUNNER = \
    "e5776700f9e12c8632d239898b673eba0f0cab6e08905001a9778bc12b75e81c"
R2_TEST_NAME = "test_v26c_j20_restore_audit_r2.py"
PIN_R2_TEST = \
    "9f0f69f1695ea64404587d6274079317925a877a729df6f589b05a8c5faabae8"
R2_GO_NAME = "v26c_j20_restore_audit_r2_architect_go.json"
PIN_R2_GO = \
    "e5c01b3f6d65842cbf184aee5e6a5cdd1dcda38c600b4402f2a3d9a1222d3d2a"
...
PIN_R2_LEAF = {
    ...
    "v26c_j20_restore_audit_receipt.json":
        "acb13a2cc9647b86e6e6b5d17c926790bf2e5aad1a07c0af030f49fdf7a69783",
    "v26c_j20_restore_audit_result.json":
        "28823153b6e46567dcc3a910951bc5f1360ff965f169b2920b3bb08eaa258724",
    "live_optimizer_audit.json":
        "4310e8c3c2207eb145d33c172b95e4c046d39f76097d4d5f349dd9763876d7ae",
    "commit_verification.json":
        "dcd90fe50a979f2dfe3d2640e669668dcc397489b947df2615823aec571bdb76",
    "summary.json":
        "9657bdb648e62bd1aa28d72eddb3472957481d8e3f915d3ffe6dd926f4a82f8b",
    "training_cfg.resolved.yaml":
        "976f6c5050161f3edf610de1843c186186fdc56ea9d563cf0fc4c1958c14a810",
    "child_stdout_stderr.txt":
        "40709a77def1bebee3bb385f0df2b177b3b25aa1ee57d1662c1b52a1b365cc91",
    "watchdog_state.json":
        "c419677709d5fd805b3c4018da50a805d3f881c66359d4ca0df64b377dfb3b33",
    "faulthandler.log":
        "d51d5e036a091beefeb7695275599920402cb6c932b7c1ce844c62eae34192ca",
}
# The wrapper R1 and R2 both used. R3 does NOT modify, copy over or delete it;
# it uses its own wrapper and re-hashes this one to its pin.
FROZEN_CHILD_NAME = "v26c_j20_restore_audit_child.py"
PIN_FROZEN_CHILD = \
    "38a57b25da27666a99891511666d63cac07534aa7602f20469048012f536caf4"
# The R1 runner and its suite: pinned so R3 cannot run if either moved.
...
CHILD_NAME = "v26c_j20_restore_audit_r3_child.py"
# rev7 pins the R3 wrapper by hash here, not only in the GO. Under rev6 it was
# GO-pinned alone; the wrapper has never executed, so correcting it destroyed no
# evidence, but from rev7 on it cannot move without preflight refusing.
PIN_R3_CHILD = \
    "db4afc547b4d4194ba828dadb76801ff1877659c1cec8e5d749b2e0ba94decdd"
...
# rev3's tightened R9, as rev6 canonicalises it: the live evidence must
# identify itself, its source, and the conversion semantics it compared under.
EVIDENCE_KIND = "LIVE OPTIMIZER RESTORE AUDIT, CANONICALISED"
EVIDENCE_STAGE_MARKER = "after_restore"
# rev6 closes this list at eight. A ninth accepted equivalence is a change in
# the object under audit, and the wrapper records it as a problem.
EXPECTED_ACCEPTED_EQUIVALENCES = (
...
# The five gates that read fields the production run only writes AFTER the
# restore has returned and _reapply_optimizer_learning_rate has come back.
DOWNSTREAM_OF_R9 = (
...
NOT_EVALUATED_REASON = (
    "downstream of R9: the live restore did not complete, so the fields this "
    "gate reads were never written"
)
...
# ------------------------------------------------------------------- the leaf --
LEAF_ROOT = "j20_runs"
LEAF_NAME = "j20_restore_audit_v26c_2026-08-28_r3"
...
def go_pin_targets() -> dict:
    ...
    targets = {
        ...
        PREREG_REV6_NAME: HERE / PREREG_REV6_NAME,
        PREREG_REV7_NAME: HERE / PREREG_REV7_NAME,
        R1_RUNNER_NAME: HERE / R1_RUNNER_NAME,
        R1_TEST_NAME: HERE / R1_TEST_NAME,
        R1_GO_NAME: HERE / R1_GO_NAME,
        R2_RUNNER_NAME: HERE / R2_RUNNER_NAME,
        R2_TEST_NAME: HERE / R2_TEST_NAME,
        R2_GO_NAME: HERE / R2_GO_NAME,
        FROZEN_CHILD_NAME: HERE / FROZEN_CHILD_NAME,
    ...
    for name in PIN_R2_LEAF:
        targets[R2_LEAF_REL + "/" + name] = HERE / R2_LEAF_REL / name
    return targets
...
def expected_pin_hashes() -> dict:
    ...
    pins = {
        ...
        PREREG_REV6_NAME: PIN_PREREG_REV6,
        PREREG_REV7_NAME: PIN_PREREG_REV7,
        R1_RUNNER_NAME: PIN_R1_RUNNER,
        R1_TEST_NAME: PIN_R1_TEST,
        R1_GO_NAME: PIN_R1_GO,
        R2_RUNNER_NAME: PIN_R2_RUNNER,
        R2_TEST_NAME: PIN_R2_TEST,
        R2_GO_NAME: PIN_R2_GO,
        FROZEN_CHILD_NAME: PIN_FROZEN_CHILD,
        RUNNER_NAME: None,          # this file: pinned by the GO, not by itself
        TEST_NAME: None,            # same
        CHILD_NAME: PIN_R3_CHILD,   # rev7: pinned HERE as well as by the GO
    ...
    for name, digest in PIN_R2_LEAF.items():
        pins[R2_LEAF_REL + "/" + name] = digest
    return pins
...
def check_entry_evidence() -> dict:
    ...
    # R3's entry condition, part one: R1 must exist, must read FAIL_CLOSED 4/13
    # and must still carry TECHNICAL_INVALID. If R1 had passed there would be
    ...
    # Part two: R2 must exist, must read FAIL_CLOSED 7/13, must still carry
    # TECHNICAL_INVALID, and its live evidence must be the one rev6 diagnoses -
    # the restore was REACHED, the moments agreed, and the eight differences
    # were all in param_groups. If R2 had never reached the restore there would
    # be no false mismatch to correct, and R3 would be correcting nothing.
    r2 = json.loads((HERE / R2_LEAF_REL / "v26c_j20_restore_audit_result.json"
                     ).read_text(encoding="utf-8"))
    found["r2_verdict"] = r2.get("verdict")
    found["r2_gates"] = "%s/%s" % (r2.get("gates_passed"), r2.get("gates_total"))
    if r2.get("verdict") != VERDICT_FAILED or r2.get("gates_passed") != 7:
        problems.append("the R2 record is not the expected FAIL_CLOSED 7/13: %s"
                        % found["r2_gates"])
    if not (HERE / R2_LEAF_REL / INVALID_MARKER).is_file():
        problems.append("the R2 leaf no longer carries %s" % INVALID_MARKER)
    if (HERE / R2_LEAF_REL / PASSED_MARKER).exists():
        problems.append("the R2 leaf carries %s, which it never earned"
                        % PASSED_MARKER)
    found["r2_child_returncode"] = (r2.get("child") or {}).get("returncode")
    if found["r2_child_returncode"] != 1:
        problems.append("the R2 child returncode is %s, expected the 1 that a "
                        "raised LiveOptimizerMismatch produces"
                        % found["r2_child_returncode"])

    r2_evidence = json.loads((HERE / R2_LEAF_REL / EVIDENCE_NAME
                              ).read_text(encoding="utf-8"))
    found["r2_difference_count"] = r2_evidence.get("difference_count")
    found["r2_differences"] = list(r2_evidence.get("differences") or [])
    if found["r2_difference_count"] != len(EXPECTED_ACCEPTED_EQUIVALENCES):
        problems.append("R2 recorded %s differences, and rev6 is written for "
                        "the %d it measured" % (found["r2_difference_count"],
                                                len(EXPECTED_ACCEPTED_EQUIVALENCES)))
    r2_paths = sorted(text.split(":")[0].replace("optimizer.", "", 1)
                      for text in found["r2_differences"])
    if r2_paths != sorted(EXPECTED_ACCEPTED_EQUIVALENCES):
        problems.append("R2's difference paths are %s, and rev6 closes the "
                        "accepted list at %s"
                        % (r2_paths, sorted(EXPECTED_ACCEPTED_EQUIVALENCES)))
    found["r2_moments_agreed"] = (
        r2_evidence.get("moment_digests_source")
        == r2_evidence.get("moment_digests_live"))
    if not found["r2_moments_agreed"]:
        problems.append("R2's moment digests did not agree, so the mismatch it "
                        "reported was not purely representational and rev6's "
                        "diagnosis does not apply")
    if r2_evidence.get("source_state_sha256") != EXPECTED_SOURCE_STATE_SHA:
        problems.append("R2 audited a different learner state: %s"
                        % r2_evidence.get("source_state_sha256"))
...
def preflight(verbose: bool = True) -> dict:
    ...
    if verbose:
        ...
        print("entry evidence     %s (warm-up %s, %s; R1 %s, R2 %s)"
              % ("OK" if entry["ok"] else "FAILED",
                 entry["found"].get("warmup_verdict"),
                 entry["found"].get("warmup_gates"),
                 entry["found"].get("r1_gates"),
                 entry["found"].get("r2_gates")))
        ...
        print("gates              13 (R1-R13; rev1 added R13, rev2 fixed "
              "normalisation,")
        print("                   rev3 fixed the counts and tightened R9, rev6")
        print("                   canonicalises R9's comparison through RLlib's")
        print("                   own convert_to_torch_tensor and evaluates the")
        print("                   five gates downstream of R9 only after a")
        print("                   completed live restore, rev7 gates the")
        print("                   pre-reapply learning rate so the production")
        print("                   overwrite cannot mask a wrong restored lr)")
...
def live_restore_completed(child: dict, artefacts: dict) -> dict:
    """Did production get PAST the restore block? rev6's downstream discipline.

    R2 reported R3, R4, R6, R7 and R12 as failures. None of them measured
    anything: the wrapper raised inside _reapply_optimizer_learning_rate, which
    train_ppo_mlp.py:1744 calls BEFORE line 1747 assigns
    restored_training_iteration and before the blocks that record
    actor_freeze_audit and critic_state_audit. Those five gates read fields the
    aborted run never wrote, and calling that a failure overstates how much went
    wrong while hiding which single thing did.

    The condition is deliberately NOT 'the child exited 0 and stopped cleanly'.
    That would make R12 vacuous, because R12 asserts exactly those two things.
    It is instead the marker that execution passed line 1744: the live evidence
    exists, it was written at the after_restore instant, the wrapper did not
    raise, and production itself appended its after_restore entry to
    optimizer_lr_audit - which it does only once
    _reapply_optimizer_learning_rate has returned.
    """
    ...
    at_the_right_instant = evidence.get("stage_marker") == EVIDENCE_STAGE_MARKER
    wrapper_did_not_raise = evidence.get("exact") is True
    production_recorded_the_reapply = any(
        entry.get("stage") == EVIDENCE_STAGE_MARKER for entry in lr_audit)
    completed = bool(evidence_written and at_the_right_instant
                     and wrapper_did_not_raise
                     and production_recorded_the_reapply)
    return {
        "completed": completed,
        "evidence_written": evidence_written,
        "evidence_at_after_restore": bool(at_the_right_instant),
        "wrapper_did_not_raise": bool(wrapper_did_not_raise),
        "production_recorded_the_reapply": bool(production_recorded_the_reapply),
        "downstream_gates": list(DOWNSTREAM_OF_R9),
        "note": "when this is false the five downstream gates are NOT "
                "evaluated. They are not passed either: they do not count "
                "towards the passed total and the verdict stays FAIL_CLOSED.",
    }


def evaluate_gates(child: dict, artefacts: dict, hermetic: dict,
                   integrity: dict) -> dict:
    """The thirteen gates of the base preregistration as amended by rev1 and
    rev6. R9's comparison is canonical; the five gates downstream of it are
    evaluated only once the live restore has completed."""
    summary = artefacts.get("summary") or {}
    evidence = artefacts.get("evidence") or {}
    ...
    # rev7: the learning rate the RESTORE produced, before production overwrote
    # it. The main canonical tree cannot see it, because
    # _reapply_optimizer_learning_rate rewrites param_groups lr with the stage's
    # configured value on the line before the capture. This block is the only
    # witness, so R9 requires it - and requires the canonical fields that show
    # the comparison was actually performed, not merely concluded.
    lr_observation = evidence.get("learning_rate_observation")
    lr_observation = lr_observation if isinstance(lr_observation, dict) else {}
    canonical_source_lr = lr_observation.get("canonical_source_lr")
    ...
    gates = {
        ...
        # rev3 tightened this and rev6 canonicalises it. A difference_count of
        # zero can be reported by a payload that computed nothing, so the
        # predicate demands the evidence itself; and a comparison that ignores
        # RLlib's own conversion reports differences that say nothing about the
        # restore, so the decision is taken on the CANONICAL walk while the RAW
        # one is still required to reproduce R2's eight. The accepted
        # equivalences are a closed list: eight, no more and no fewer. The
        # moments must agree canonically AND must not have been touched by the
        # canonicalisation on either side, which is what keeps a float64 moment
        # - a dtype the conversion would silently downcast - a failure.
        "R9_live_optimizer_is_byte_exact_after_restore":
            ...
            and isinstance(evidence.get("canonical"), dict)
            and evidence["canonical"].get("exact") is True
            and evidence["canonical"].get("differences") == []
            and evidence["canonical"].get("difference_count") == 0
            and bool(evidence["canonical"].get("digest_source"))
            and evidence["canonical"].get("digest_live")
            == evidence["canonical"].get("digest_source")
            and evidence["canonical"].get("digests_match") is True
            and isinstance(evidence["canonical"].get("param_groups"), dict)
            and evidence["canonical"]["param_groups"].get("exact") is True
            and evidence["canonical"]["param_groups"].get("differences") == []
            and evidence["canonical"]["param_groups"].get("digests_match") is True
            and isinstance(evidence.get("raw"), dict)
            and evidence["raw"].get("reproduces_r2_eight") is True
            and evidence["raw"].get("difference_count")
            == len(EXPECTED_ACCEPTED_EQUIVALENCES)
            and evidence.get("accepted_equivalence_paths")
            == sorted(EXPECTED_ACCEPTED_EQUIVALENCES)
            and evidence.get(
                "accepted_equivalences_are_exactly_the_expected_eight") is True
            and evidence.get("unexplained_canonical_paths") == []
            and evidence.get("moment_digests_match") is True
            and evidence.get("moment_digests_source")
            == evidence.get("moment_digests_live")
            and evidence.get(
                "moments_unchanged_by_canonicalisation_source") is True
            and evidence.get(
                "moments_unchanged_by_canonicalisation_live") is True
            and isinstance(evidence.get("conversion"), dict)
            and evidence["conversion"].get("call_site_verified") is True
            and evidence["conversion"].get("device_parameter_present") is True
            and evidence.get("conversion_fixed_point_source") is True
            and evidence.get("conversion_fixed_point_live") is True
            and evidence.get("param_group_sizes_live") == [EXPECTED_PARAM_COUNT]
            ...
            # rev7's added clause. matches alone is a boolean the payload
            # computes, so the canonical fields are required too: a payload
            # that computed nothing cannot produce them, and a configured-lr
            # overwrite cannot satisfy them.
            and isinstance(evidence.get("learning_rate_observation"), dict)
            and lr_observation.get("matches") is True
            and lr_observation.get("gated") is True
            and isinstance(canonical_source_lr, str)
            and canonical_source_lr.startswith("('t'")
            and lr_observation.get("canonical_live_lr_before_reapply")
            == canonical_source_lr
            and lr_observation.get("live_lr_before_reapply") is not None
            and lr_observation.get("canonical_dtype") == "float32"
    ...
    # rev6: the five gates downstream of R9 are EVALUATED only once the live
    # restore has completed. A gate that was not evaluated is not passed; it
    # simply stops being reported as an autonomous failure it never was.
    restore = live_restore_completed(child, artefacts)
    evaluated = {name: True for name in gates}
    not_evaluated_reason = {}
    if not restore["completed"]:
        for name in DOWNSTREAM_OF_R9:
            evaluated[name] = False
            not_evaluated_reason[name] = NOT_EVALUATED_REASON
            gates[name] = False
    not_evaluated = sorted(name for name, ok in evaluated.items() if not ok)
    failed = sorted(name for name, ok in gates.items()
                    if not ok and evaluated[name])
    return {
        "gates": gates,
        "evaluated": evaluated,
        ...
        "passed": sum(1 for name, ok in gates.items()
                      if ok and evaluated[name]),
        "total": len(gates),
        "failed": failed,
        "measurements": {
            ...
            "live_raw_difference_count":
                (evidence.get("raw") or {}).get("difference_count"),
            "live_raw_differences":
                (evidence.get("raw") or {}).get("differences"),
            "live_raw_reproduces_r2_eight":
                (evidence.get("raw") or {}).get("reproduces_r2_eight"),
            "live_canonical_difference_count":
                (evidence.get("canonical") or {}).get("difference_count"),
            "live_canonical_differences":
                (evidence.get("canonical") or {}).get("differences"),
            "live_canonical_digest_source":
                (evidence.get("canonical") or {}).get("digest_source"),
            "live_canonical_digest_live":
                (evidence.get("canonical") or {}).get("digest_live"),
            "live_canonical_digests_match":
                (evidence.get("canonical") or {}).get("digests_match"),
            "live_canonical_param_groups_exact":
                ((evidence.get("canonical") or {}).get("param_groups")
                 or {}).get("exact"),
            "live_accepted_equivalence_paths":
                evidence.get("accepted_equivalence_paths"),
            "live_unexplained_canonical_paths":
                evidence.get("unexplained_canonical_paths"),
            "live_moment_digests_match": evidence.get("moment_digests_match"),
            "live_moments_unchanged_source":
                evidence.get("moments_unchanged_by_canonicalisation_source"),
            "live_moments_unchanged_live":
                evidence.get("moments_unchanged_by_canonicalisation_live"),
            "live_conversion_call_site_verified":
                (evidence.get("conversion") or {}).get("call_site_verified"),
            "live_conversion_ray_version":
                (evidence.get("conversion") or {}).get("ray_version"),
            "live_conversion_device": evidence.get("device"),
            "live_conversion_fixed_point_source":
                evidence.get("conversion_fixed_point_source"),
            "live_conversion_fixed_point_live":
                evidence.get("conversion_fixed_point_live"),
            "live_learning_rate_observation":
                evidence.get("learning_rate_observation"),
...
def run_execution(go_file: str) -> dict:
    ...
    # A gate that was not evaluated has NOT passed. rev6 is explicit that the
    # downstream discipline changes what is reported, never what is required:
    # the audit passes only when all thirteen were evaluated and all thirteen
    # passed.
    gates_ok = (not graded["failed"] and not graded["not_evaluated"]
    ...
    result = {
        ...
        else "nothing; at least one gate failed or was not evaluated, and "
             "there is no retry",
        "gates": graded["gates"],
        "gates_passed": graded["passed"],
        "gates_total": graded["total"],
        "gates_failed": graded["failed"],
        "gates_evaluated": graded["evaluated"],
        "gates_not_evaluated": graded["not_evaluated"],
        "gates_not_evaluated_reason": graded["not_evaluated_reason"],
        "live_restore": graded["live_restore"],
    ...
    receipt = {
        ...
        "gates_failed": graded["failed"],
        "gates_not_evaluated": graded["not_evaluated"],
        "live_restore_completed": graded["live_restore"]["completed"],
        "g9_closed": gates_ok,
        "inputs": {
            ...
            "prereg_rev6_sha256": PIN_PREREG_REV6,
            "prereg_rev7_sha256": PIN_PREREG_REV7,
            "r1_leaf": R1_LEAF_REL,
            "r1_leaf_unchanged": True,
            "r2_leaf": R2_LEAF_REL,
            "r2_leaf_unchanged": True,
            "frozen_child_wrapper_sha256": sha256_file(HERE / FROZEN_CHILD_NAME),
