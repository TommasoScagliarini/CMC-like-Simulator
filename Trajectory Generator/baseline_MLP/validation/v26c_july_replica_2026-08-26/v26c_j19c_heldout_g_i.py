"""V26C J19C - CLOSED-LOOP QUALIFICATION of the J19A actor.
...
DERIVED FROM J16
    ...
    v26c_j19c_derive_from_j16.py from a closed, pinned substitution map. J16 itself is
...
class J19CError(RuntimeError):
    pass


STAGE = "V26C_J19C_J19A_HELDOUT_G_I"
PREREG = HERE / "v26c_j19c_prereg_heldout_g_i.json"
PIN_PREREG = "54a1e6fc1f469ba169c80cc851be3fb1ab551e399553ed26c58aa1f101349e2d"
...
NOISE_HOLD_STEPS = 1
EXPECTED_STEPS = 500
AGGREGATE_RULE = "PASS iff 3/3 behavioural PASS AND 3/3 telemetry-valid"
...
J19C_COMMON_GATE: dict[str, Any] = {k: v for k, v in J1.J1_GATE.items()
                                  if k != "max_penetration_m_max"}
J19C_KINEMATIC_GATE: dict[str, Any] = dict(J3.J3_KINEMATIC_GATE)   # UNCHANGED
J19C_GATE_SOURCE = ("v26c_j1_collect.J1_GATE for every non-penetration criterion, "
...
PREFLIGHT_SENTINEL = HERE / "_j19c_preflight_sentinel_never_created"
...
def _leaf_rel(q: Path, root: Path) -> str:
    ...
    try:
        ...
        raise J19CError(f"{q} is not under the staging root {root}") from None
    if ".." in rel.parts:
        raise J19CError(f"the relative path for {q} under {root} escapes the leaf: {rel}")
    return str(rel).replace(os.sep, "/")


def _resolve_inside(leaf: Path, rel: str) -> Path:
    if not rel or Path(rel).is_absolute() or ".." in Path(rel).parts:
        raise J19CError(f"recorded path is not leaf-relative: {rel!r}")
    q = leaf / rel
    if q.is_symlink():
        raise J19CError(f"recorded path is a symlink, which is never committed here: {rel}")
    try:
        q.resolve().relative_to(leaf.resolve())
    except ValueError:
        raise J19CError(f"recorded path escapes the committed leaf: {rel}") from None
    return q
...
def verify_committed_leaf(leaf: Path, *, expected_receipt_sha: str | None = None) -> dict[str, Any]:
    ...
    if receipt_bytes is None:
        return {"schema": "v26c_j19c_commit_verification.1", "stage": STAGE, "pass": False,
    ...
    return {
        "schema": "v26c_j19c_commit_verification.1", "stage": STAGE,
...
def resolve_feature_names() -> dict[str, Any]:
    ...
    for rel, pin in PIN_FEATURE_NAME_SOURCES.items():
        ...
        got = _sha_file(path)
        if got != pin:
            raise J19CError(f"the pinned feature-name source {rel} changed: {got} != {pin}")
        if rel.endswith(".npz"):
    ...
    if len(distinct) != 1:
        raise J19CError(f"the three pinned feature-name sources disagree: "
                        f"{ {k: len(v) for k, v in resolved.items()} }")
    names = distinct.pop()
    if len(names) != ACTOR_WIDTH:
        raise J19CError(f"the pinned sources hold {len(names)} names, not {ACTOR_WIDTH}")
...
def verify_actor() -> dict[str, Any]:
    ...
    if not J19A_LEAF.is_dir():
        raise J19CError(f"the J19A leaf is missing: {J19A_LEAF}")
    present = sorted(str(p.relative_to(J19A_LEAF)).replace(os.sep, "/")
                     for p in J19A_LEAF.rglob("*") if p.is_file())
    if present != sorted(PIN_J19A):
        raise J19CError(f"the J19A leaf holds {present}, expected exactly {sorted(PIN_J19A)}")
    marker = J19A_LEAF / TECHNICAL_INVALID_NAME
    if marker.exists():
        raise J19CError(f"the J19A leaf is marked {TECHNICAL_INVALID_NAME}; its actor may not "
                        f"be loaded, whatever its receipt says")
    cv_path = J19A_LEAF / COMMIT_VERIFICATION_NAME
    if not cv_path.is_file():
        raise J19CError(f"the J19A leaf carries no {COMMIT_VERIFICATION_NAME}: it is not valid "
    ...
    if cv.get("ok") is not True or cv.get("problems"):
        raise J19CError(f"the J19A commit verification does not pass: ok={cv.get('ok')!r} "
    ...
    for stale in (J19A_LEAF.parent.glob(".lock_*"), J19A_LEAF.parent.glob(".staging_*")):
        ...
        if leftovers:
            raise J19CError(f"the J19A run directory still holds {leftovers}; the commit did "
    ...
    for rel, pin in PIN_J19A.items():
        ...
        if got != pin:
            raise J19CError(f"the J19A artefact {rel} changed: {got} != {pin}")
        checked[rel] = got
    ...
    if not inputs:
        raise J19CError(f"the J19A state holds no {ACTOR_WIDTH}D input layer")
    ...
    for key in inputs:
        ...
        if zero != clock:
            raise J19CError(f"the J19A input layer {key} has zero columns {zero}; expected "
        ...
        if dead:
            raise J19CError(f"the J19A input layer {key} has dead controller columns {dead}")
    ...
    if widths != [ACTOR_WIDTH]:
        raise J19CError(f"the J19A input layers are {widths}D; expected {ACTOR_WIDTH}D")
    ...
    if not bool(np.all(w[dim:] == 0.0)):
        raise J19CError("the actor's log-std output weights are not zero, so sigma is not "
                        "constant")
    ...
    if not bool(np.all(np.isfinite(sigmas))) \
        ...
        raise J19CError(f"the actor carries sigma {sigmas.tolist()}, expected {SIGMA} within "
    ...
    if lineage["j19a_verdict"] != "PASS":
        raise J19CError(f"the J19A receipt verdict is {lineage['j19a_verdict']!r}, not PASS")
    if lineage["actors_persisted"] != 1:
        raise J19CError("the J19A receipt does not record exactly one persisted actor")
    if lineage["rollout_performed"] is not False:
        raise J19CError("the J19A receipt claims a rollout, which that stage never ran")
    for flag in ("reproducibility_ok", "exact_fields_ok", "eligibility_ok"):
        if lineage[flag] is not True:
            raise J19CError(f"the J19A receipt does not declare {flag} true")
    if lineage["parent_j8_sha256"] != PIN_J19A_PARENT_J8:
        raise J19CError("the J19A receipt records a different J8 parent")
    ...
    if manifest.get("module_state_sha256") != PIN_J19A_MODULE_STATE:
        raise J19CError(f"the J19A manifest names module {manifest.get('module_state_sha256')}, "
                        f"not the module beside it ({PIN_J19A_MODULE_STATE})")
    if manifest.get("actor_digest") != PIN_J19A_ACTOR_DIGEST:
        raise J19CError("the J19A manifest carries an unexpected actor_digest")
    if manifest.get("actor_digest") == manifest.get("source_actor_digest"):
        raise J19CError("the J19A manifest declares the actor identical to its parent")
    if int(manifest.get("observation_width", 0)) != ACTOR_WIDTH:
        raise J19CError(f"the J19A manifest does not declare {ACTOR_WIDTH} observation width")
    ...
    if "actor_feature_names" in manifest or "deployable" in manifest:
        raise J19CError("the J19A manifest schema changed; this stage was derived against the "
...
def unit_correction() -> dict[str, Any]:
    ...
    if got != J9_PREREG_SHA:
        raise J19CError(f"the original J9 preregistration changed: {got} != {J9_PREREG_SHA}")
...
def base_env_config(output_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if output_root is None:
        raise J19CError("base_env_config requires a REAL output root. Passing None is exactly the "
    ...
    if float(env["episode_start_offset_s"]) != OFFSET_NOMINAL:
        raise J19CError(f"the pinned config's episode_start_offset_s is "
    ...
    if env.get("record_outputs") is not True:
        raise J19CError(f"record_outputs is {env.get('record_outputs')!r}, expected exactly True. "
    ...
    if env.get("save_outputs_on_close") is not True:
        raise J19CError(f"save_outputs_on_close is {env.get('save_outputs_on_close')!r}, expected "
                      "exactly True. Without it osim_trj_cmc_like.close() never calls "
                      "runner.save_results().")
    if not env.get("output_prefix"):
        raise J19CError("output_prefix is absent from the base env config; the J1 builder sets it "
                      "only alongside a real output_dir")
    if not env.get("output_dir"):
        raise J19CError("output_dir is absent from the base env config")
    return env, cfg
...
def cell_env_config(base: Mapping[str, Any], cfg: Mapping[str, Any], offset_s: float,
    ...
    if float(offset_s) not in [float(v) for v in FROZEN_OFFSETS]:
        raise J19CError(f"{offset_s} is not one of the three frozen offsets {FROZEN_OFFSETS}")
    env = dict(base)
    ...
    if forbidden:
        raise J19CError(f"the cell config mutates {forbidden}; ONLY episode_start_offset_s (and the "
                      f"per-cell output_dir) may differ from the verified base")
    if float(env["episode_start_offset_s"]) != float(offset_s):
        raise J19CError("the cell config does not carry the requested offset")
    added = sorted(set(env) - set(base))
    removed = sorted(set(base) - set(env))
    if removed or sorted(set(added) - {"output_dir"}):
        raise J19CError(f"the cell config adds {added} and removes {removed} relative to the "
    ...
    if differing_stable:
        raise J19CError(f"these stable fields differ from the verified base: {differing_stable}. "
...
def evaluate_cell_gate(summary: Mapping[str, Any], knee: np.ndarray, ankle: np.ndarray,
    ...
    checks = {
        "steps": summary["steps"] == J19C_COMMON_GATE["steps_required"],
        "end_reason": summary["end_reason"] == J19C_COMMON_GATE["end_reason"],
        "valid_cycles": summary["valid_cycle_count"] >= J19C_COMMON_GATE["valid_cycles_min"],
        "phase_timeout_stance":
            summary["phase_timeout_stance"] <= J19C_COMMON_GATE["phase_timeout_stance_max"],
        "phase_timeout_swing":
            summary["phase_timeout_swing"] <= J19C_COMMON_GATE["phase_timeout_swing_max"],
        "morphology_causal_contract_failure":
            summary["morphology_causal_contract_failure"]
            <= J19C_COMMON_GATE["morphology_causal_contract_failure_max"],
        "hs_cancelled_count":
            summary["hs_cancelled_count"] <= J19C_COMMON_GATE["hs_cancelled_count_max"],
        "resync_count": summary["resync_count"] <= J19C_COMMON_GATE["resync_count_max"],
...
def preflight() -> dict[str, Any]:
    ...
    if contract["sha256"] != PIN_CONTRACT:
        raise J19CError(f"the penetration contract changed: {contract['sha256']} != {PIN_CONTRACT}")
    ...
    if lock_path.exists() or lock_path.is_symlink():
        blockers.append(f"a J19C lock is already held or was left behind: {lock_path}. This stage "
    ...
    return {
        ...
        "gate_specification": {
            "binding_common": dict(J19C_COMMON_GATE),
        ...
        "would_write": {"leaf": _rel(leaf), "relative_leaf": RELATIVE_LEAF,
                        ...
                        "per_cell": ["j19c_cell_<ID>_trace.json", "j19c_cell_<ID>_kinematics.npz",
                                     "j19c_cell_<ID>_penetration.npz",
                                     "j19c_cell_<ID>_sim_outputs/ (exactly 19 regular files)"],
...
def _refuse_symlink(path: Path, root: Path) -> None:
    ...
    while True:
        if current.is_symlink():
            raise J19CError(f"refusing a symlinked path component: {current}")
...
def validate_stage(token: str | None) -> str:
    if token != STAGE:
        raise J19CError(f"--authorized-stage must be exactly {STAGE!r}, got {token!r}")
    return token


def validate_out(out_arg: str | None) -> Path:
    if out_arg is None:
        raise J19CError("--run requires --out, naming the authorised leaf exactly")
    leaf = authorized_leaf()
    got = Path(out_arg).expanduser()
    if got.is_symlink():
        raise J19CError(f"refusing a symlinked --out: {got}")
    if got.resolve(strict=False) != leaf.resolve(strict=False):
        raise J19CError(f"--out is {got}, which is not the authorised leaf {leaf}")
    root = Path(OUTPUT_ROOT_OVERRIDE) if OUTPUT_ROOT_OVERRIDE is not None else HERE
    _refuse_symlink(leaf.parent, root)
    if leaf.exists() or leaf.is_symlink():
        raise J19CError(f"the authorised leaf already exists; this stage is no-clobber and "
...
def run_cell(cell: Mapping[str, Any], *, stack: Any, base_env: Mapping[str, Any],
    ...
    seed = int(cell["seed"])
    mode = str(cell["mode"])
    sim_out = staging / f"j19c_cell_{cid}_sim_outputs"
    ...
    if module_class != EXPECTED_MODULE_CLASS:
        raise J19CError(f"cell {cid}: the loaded module is a {module_class}, not a "
    ...
    if loaded_keys != EXPECTED_STATE_KEYS:
        raise J19CError(f"cell {cid}: the loaded module holds {loaded_keys}, expected "
    ...
    if mismatched:
        raise J19CError(f"cell {cid}: the loaded weights differ from the pinned module_state.pkl "
                       f"on {mismatched}")
    ...
    try:
        ...
        if WAIT_HS_FEATURE not in feature_names:
            raise J19CError(f"cell {cid}: the runtime actor exposes no {WAIT_HS_FEATURE}")
        ...
        if live_duration is not None:
            if not isinstance(live_duration, (int, float)) \
                ...
                raise J19CError(f"cell {cid}: env_cfg.episode_duration is {live_duration!r}, not a "
                              "finite number")
        ...
        if (live_duration is None) != (declared_duration is None):
            raise J19CError(f"cell {cid}: the live episode_duration is {live_duration!r} while the "
                          f"config declares {declared_duration!r}")
        if live_duration is not None \
                and abs(live_duration - float(declared_duration)) > 1e-12:
            raise J19CError(f"cell {cid}: the live episode_duration {live_duration} and the declared "
        ...
        if mode == "stochastic_held":
            if getattr(stack, "held_stochastic_action", None) is None:
                raise J19CError("the stack exposes no held_stochastic_action helper")
        ...
        for step in range(1, EXPECTED_STEPS + 1):
            ...
            if mode == "deterministic":
                ...
                if dev != 0.0:
                    raise J19CError(f"cell {cid} step {step}: the deterministic action differs from "
                ...
                if getattr(stack, "reference_action", None) is not None:
                    ...
                    if not (np.array_equal(raw, r_a) and np.array_equal(mean, r_m)
                        ...
                        raise J19CError(f"cell {cid} step {step}: the deterministic action does not "
                ...
                if not np.array_equal(np.asarray(d_m, dtype=np.float32), mean):
                    raise J19CError(f"cell {cid} step {step}: forward_exploration and "
                                  "forward_inference disagree on the policy mean")
                if not np.array_equal(np.asarray(d_s, dtype=np.float32), std):
                    raise J19CError(f"cell {cid} step {step}: the two forward paths disagree on the "
                                  "std")
                if float(np.max(np.abs(std.astype(np.float64) - SIGMA))) > SIGMA_TOLERANCE:
                    raise J19CError(f"cell {cid} step {step}: the effective std is {std.tolist()}, "
                                  f"expected {SIGMA}")
                if not np.array_equal(applied_noise, (std * unit).astype(np.float32)):
                    raise J19CError(f"cell {cid} step {step}: applied noise is not std * unit_noise")
                parity_steps += 1
            ...
            if "time" not in info:
                raise J19CError(f"cell {cid} step {step}: info exposes no 'time'")
            terms = J1._jsonable(info.get("reward_terms", {}), "reward_terms")
            if "grf_penetration_m" not in terms:
                raise J19CError(f"cell {cid} step {step}: reward_terms carries no "
                              "grf_penetration_m")
        ...
        # chained J19CError, but KeyboardInterrupt and SystemExit are NOT exceptions of the run -
        ...
        try:
            ...
            raise J19CError(f"cell {cid}: the environment failed to close cleanly, so the cell is "
    ...
    penetration = penetration_report(pen_arr, contract, f"J19C cell {cid}")
    ...
    # J9R1 accepted any non-empty directory; J10R1 hardened this and J19C inherits the hardening.
    if not sim_out.is_dir():
        raise J19CError(f"cell {cid}: the sim_outputs directory is missing at {sim_out}")
    ...
    if non_files:
        raise J19CError(f"cell {cid}: sim_outputs holds non-regular entries {non_files}")
...
def run_matrix(out_arg: str | None, stage_token: str | None, *, stack: Any = None,
    ...
    if OUTPUT_ROOT_OVERRIDE is not None and stack is None:
        raise J19CError(f"OUTPUT_ROOT_OVERRIDE is set to {OUTPUT_ROOT_OVERRIDE}. It is permitted "
                      f"only for synthetic, isolated tests; the authorised matrix refuses it.")
    validate_stage(stage_token)
    leaf = validate_out(out_arg)
    pre = preflight()
    if pre["blockers"]:
        raise J19CError(f"preflight BLOCKED: {pre['blockers']}")
    ...
    if len(expected_features) != ACTOR_WIDTH:
        raise J19CError(f"the pinned feature-name sources hold {len(expected_features)} names")
    ...
    lock_path = leaf.parent / LOCK_NAME
    if staging.exists() or staging.is_symlink():
        raise J19CError(f"a stale staging directory is in the way: {staging}")
    ...
    try:
        ...
        try:
            ...
            raise J19CError(f"the J19C lock already exists: {lock_path}. This stage fails closed and "
        ...
        for flag in ("record_outputs", "save_outputs_on_close"):
            if base_env.get(flag) is not True:
                raise J19CError(f"the base env config about to be used has {flag}="
        ...
        if not base_env.get("output_prefix") or not base_env.get("output_dir"):
            raise J19CError("the base env config about to be used carries no output_prefix or no "
        ...
        if actor_after["artefacts_sha256"] != actor_before["artefacts_sha256"]:
            raise J19CError("the J19A actor changed during the matrix")
        ...
        receipt = {
            "schema": "v26c_j19c_closed_loop_receipt.1", "stage": STAGE,
        ...
        if leaf.exists() or leaf.is_symlink():
            raise J19CError(f"the leaf appeared while staging; refusing to clobber: {leaf}")
        os.rename(staging, leaf)
        staging_created = None

        marker = leaf / TECHNICAL_INVALID_NAME
        if not marker.is_file():
            raise J19CError(f"the committed leaf does not carry {TECHNICAL_INVALID_NAME}: "
        ...
        try:
            ...
            verification = {"schema": "v26c_j19c_commit_verification.1", "stage": STAGE,
        ...
        try:
            ...
            raise J19CError(f"the verification could not be recorded in {leaf}: "
        ...
        try:
            ...
            raise J19CError(f"the verification passed but the marker could not be removed from "
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        description="V26C J19C closed-loop qualification of the J19A actor")
    ...
    if a.out is not None:
        raise J19CError("--out is meaningless without --run; the preflight writes nothing")
    r = preflight()
    print(json.dumps(r, indent=2, default=str))
