...
STAGE = "V26C-J9-CLOSED-LOOP-QUALIFICATION"
PREREG = HERE / "v26c_j9_prereg_closed_loop.json"
PIN_PREREG = "e6a6888a473d5ef76b1f009037f7825995fd62b35cc51749dfe41ad7f718c44a"
...
def verify_prereg() -> dict[str, Any]:
    if not PREREG.is_file():
        raise J9Error("the J9 preregistration is missing")
    digest = _sha_file(PREREG)
    if PIN_PREREG != "PENDING" and digest != PIN_PREREG:
        raise J9Error(f"the J9 preregistration changed: {digest} != {PIN_PREREG}")
    data = json.loads(PREREG.read_text())
    if data.get("stage_authorised") != STAGE:
        raise J9Error(f"the preregistration names {data.get('stage_authorised')!r}, not {STAGE}")
    local: dict[str, str] = {}
    ...
    if [c["id"] for c in declared] != [c["id"] for c in MATRIX]:
        raise J9Error("the preregistered matrix order and the runner's disagree")
    for want, got in zip(MATRIX, declared):
        if (got["mode"], got["seed"], got["episode_start_offset_s"]) != \
                (want["mode"], want["seed"], want["offset_s"]):
            raise J9Error(f"cell {want['id']} differs between the preregistration and the runner")
    if data["matrix"]["behavioural_fail_fast"] is not False:
        raise J9Error("the preregistration must declare behavioural_fail_fast false")
    if data["matrix"]["offset_unit"] != "SECONDS of episode start time":
        raise J9Error("the preregistration must declare the offset unit as SECONDS")
...
def unit_correction() -> dict[str, Any]:
    ...
    data = json.loads(PREREG.read_text())["offset_unit_correction"]
...
def base_env_config(output_dir: Path | None = None) -> tuple[dict[str, Any], dict[str, Any]]:
    """The pinned config through the HARDENED J1 builder, verified before anything mutates it."""
...
def cell_env_config(base: Mapping[str, Any], cfg: Mapping[str, Any], offset_s: float,
    ...
    identical = [k for k in base if k != "episode_start_offset_s" and base[k] == env.get(k)]
    if len(identical) != len(base) - 1:
        raise J9Error("a field outside the declared mutation differs from the verified base")
    return env, {"mutated_keys": differing, "allowed_superset": allowed,
                 ...
                 "unit": OFFSET_UNIT,
                 "fields_identical_to_verified_base": len(identical),
...
def preflight() -> dict[str, Any]:
    ...
    env, cfg = base_env_config(output_dir=None)
    planned = []
    for cell in MATRIX:
...
def run_matrix(out_arg: str | None, stage_token: str | None, *, stack: Any = None,
    ...
    contract = PC.load_contract()
    base_env, cfg = base_env_config(output_dir=None)
