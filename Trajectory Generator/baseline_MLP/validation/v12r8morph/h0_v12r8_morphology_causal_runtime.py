"""Additive strict-delay runtime for the V12R8 morphology stage.

The R8 lineage freezes ``experimental_morphology_corridor.py`` byte-for-byte,
...
RUNTIME_ID = "h0_v12r8_morph_strict_terminal_delay_v1"
SITE_MARKER_ENV = "H0_V12R8_MORPH_CAUSAL_RUNTIME"
_TIME_EPS = 1.0e-12
...
class StrictDelayCausalMorphologyBuffer(_ORIGINAL_BUFFER):
    ...
    def update(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if not early:
            return result
        retained = tuple(
            item for item in result.resolved_samples if item not in early
        )
...
def install() -> dict[str, Any]:
    ...
    return {
        "runtime_id": RUNTIME_ID,
        "corridor_installed": (
            corridor.CausalDelayedMorphologyBuffer
            is StrictDelayCausalMorphologyBuffer
        ),
        "reward_module_loaded": reward is not None,
        "reward_installed": (
            reward is None
            or reward.CausalDelayedMorphologyBuffer
            is StrictDelayCausalMorphologyBuffer
        ),
    }
