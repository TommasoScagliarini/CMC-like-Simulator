"""Pure fail-closed gates for the latent-live V12R8 morphology A/B."""
...
_BINDING_KEYS = frozenset(
    {"candidate_id", "candidate_module", "actor_digest", "checkpoint", "resolved_config"}
)
...
def upstream_terminal_gate(payload: Any) -> dict[str, Any]:
    ...
    r8_result = _mapping(data.get("v12r8_terminal"))
    q3_result = _mapping(data.get("v12r8_q3_terminal"))
    zero_result = _mapping(data.get("v12r8_zero_terminal"))
    attestations = _mapping(data.get("semantic_attestations"))
    r8_module = _mapping(r8_result.get("candidate_module"))
    ...
    r8_endpoint = contract.R8_TERMINAL_ENDPOINT
    q3_endpoint = contract.Q3_TERMINAL_ENDPOINT
    zero_endpoint = contract.ZERO_TERMINAL_ENDPOINT
    checks = {
        "semantic_r8": _semantic_attestation_valid(
            attestations.get("v12r8_terminal"),
            endpoint=r8_endpoint,
            result=r8_result,
        ),
        "semantic_q3": _semantic_attestation_valid(
            attestations.get("v12r8_q3_terminal"),
            endpoint=q3_endpoint,
            result=q3_result,
        ),
        "semantic_zero": _semantic_attestation_valid(
            attestations.get("v12r8_zero_terminal"),
            endpoint=zero_endpoint,
            result=zero_result,
        ),
        "r8_pass": r8_result.get("schema_version") == r8_endpoint["schema_version"]
        and r8_result.get("status") == r8_endpoint["required_status"]
        and r8_result.get("passed") is True
        and r8_result.get("terminal") is True
        and r8_result.get("protocol_id") == r8_endpoint["protocol_id"]
        and r8_result.get("pipeline_id") == r8_endpoint["pipeline_id"]
        and r8_result.get("error") is None
        and r8_result.get("actor_updates") == 1
        and r8_result.get("critic_updates") == 0
        and r8_result.get("ppo_updates") == 0
        and r8_result.get("runtime_promoted") is False
        and r8_result.get("qualification_executed") is False
        and candidate_module_valid(r8_module),
        ...
        "same_candidate": r8_result.get("candidate_id")
        == q3_result.get("candidate_id")
        == zero_binding.get("candidate_id")
        and _strict_equal(r8_module, q3_module)
        and _strict_equal(r8_module, zero_module),
    ...
    return {
        ...
        "status": (
            "PASS_H0_V12R8_MORPH_UPSTREAMS"
            if passed
            else "FAIL_H0_V12R8_MORPH_UPSTREAMS"
        ),
        "passed": passed,
        "checks": checks,
        "binding": binding,
...
def q3_control_capability_gate(value: Any) -> dict[str, Any]:
    ...
    checks = {
        "identity": data.get("status") == "INSPECTED_H0_V12R8_Q3_CONTROL_CAPABILITY"
        ...
        ) if set(trace_artifacts) == set(contract.CASE_IDS) else False,
        "observed_abi": all(
            set(_mapping(observed.get(case_id)))
            == set(contract.Q3_REQUIRED_REUSE_STREAMS)
            and all(
                type(flag) is bool
                for flag in _mapping(observed.get(case_id)).values()
            )
        ...
        and all(
            type(missing.get(case_id)) is list
            and missing[case_id] == exact_missing
            for case_id in contract.CASE_IDS
        ),
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            "PASS_H0_V12R8_MORPH_Q3_CAPABILITY_FALLBACK"
            if passed
            else "FAIL_H0_V12R8_MORPH_Q3_CAPABILITY_FALLBACK"
        ),
        "passed": passed,
        "checks": checks,
...
def detector_audit_gate(value: Any) -> dict[str, Any]:
    data = _mapping(value)
    checks = {
        "config": _strict_equal(data.get("runtime_config"), contract.V26_RUNTIME_CONFIG),
...
def reward_ledger_gate(rows: Any, *, role: str) -> dict[str, Any]:
    ...
    for index, raw in enumerate(values):
        ...
        if loss < 0.0:
            ...
            continue
        expected_term = (
            0.0 if role == contract.CONTROL_ROLE else 0.0025 * loss
        )
...
def causal_ledger_gate(rows: Any, *, reward_rows: Any) -> dict[str, Any]:
    ...
    for index, raw in enumerate(values):
        ...
        if (
            ...
            or any(not _nonnegative_int(diagnostics.get(name)) for name in counter_names)
        ...
        for raw_sample in samples:
            ...
            delivery_latency = float(
                sample["anchor_delivered_time_s"]
                - sample["anchor_confirmed_time_s"]
            )
            if (
        ...
        if index >= len(rewards):
            ...
            if type(reward.get("morphology_loss")) is not float or not _float_bits_equal(
...
def arm_gate(
    ...
    *,
    ...
    checks = {
        ...
        "detector": detector_audit_gate(data.get("detector_audit"))["passed"]
        is True,
        ...
        is True,
        "activity": data.get("local_execution_index") in range(
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": contract.ARM_PASS_STATUS if passed else "FAIL_H0_V12R8_MORPH_ARM",
        "passed": passed,
        "checks": checks,
        "reward": reward,
        "causal": causal,
    ...
    *,
    ...
    for control_row, positive_row in zip(
        ...
        if not (
            ...
            and _float_bits_equal(left.get("actual_reward"), left.get("recomputed_reward"))
            and _float_bits_equal(
    ...
    checks = {
        ...
        "control_before_positive": _finite(
            control.get("completed_unix_s"), minimum=0.0
        )
        is not None
        ...
        "same_binding_condition": _strict_equal(control.get("binding"), expected_binding)
        ...
        and _strict_equal(control.get("detector_audit"), positive.get("detector_audit")),
    ...
    return {
        ...
        "effect": {
            ...
            "total_morphology_penalty": positive_reward[
                "total_morphology_penalty"
            ],
...
def aggregate_gate(pair_results: Any, *, expected_binding: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": (
            contract.AGGREGATE_PASS_STATUS
            if passed
        ...
        "terminal": True,
        "protocol_id": contract.PROTOCOL_ID,
        "binding": copy_mapping(expected_binding) if binding_valid(expected_binding) else None,
        "checks": checks,
        "pair_results": values,
        "effect": {
            "nonzero_effect_sample_count": nonzero,
