"""Build or publish the no-clobber V12R4-Q2 qualification design freeze.
...
class V12R4Q2QualificationDesignFreezeError(RuntimeError):
    """Raised when the additive Q2 design cannot remain fail closed."""


LOCKED_ACCESS_STATUS = "LOCKED_PENDING_R4_TERMINAL_PASS_AND_CANDIDATE_BINDING"
NEXT_STAGE = "WAIT_R4_TERMINAL_PASS_THEN_FREEZE_Q2_PROTOCOL"
FREEZE_KIND = "PRE_R4_DEFERRED_CANDIDATE_INDEPENDENT_Q2_DESIGN"
    ...
    for index, part in enumerate(parts):
        ...
        if not os.path.lexists(current):
            if allow_missing_leaf or index < len(parts) - 1:
                continue
            raise V12R4Q2QualificationDesignFreezeError(
...
def _historical_exclusion_gate() -> dict[str, Any]:
    ...
    return {
        ...
        "checks": checks,
        "records": {"q1_design_freeze": q1_record, "p1s_terminal_ledger": p1s_record},
    }
...
def _design_snapshot() -> dict[str, Any]:
    return {
        ...
        "post_pass_next_stage": contract.NEXT_STAGE_AFTER_Q2_PASS,
    }
...
def _design_gate(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "r4_prerequisite_interface_exact": prerequisite_rows
        ...
        "six_reused_unopened_conditions_exact": q2_signatures
        == _EXPECTED_HOLDOUT_SIGNATURES
        and q2_signatures == v6_signatures
        and q2_signatures == q1_signatures,
        "new_q2_tape_namespace_exact": all(
...
def _locked_access() -> dict[str, Any]:
    return {
        ...
        "candidate_binding_deferred_until_r4_terminal_pass": True,
...
def _occupancy_snapshot() -> dict[str, bool]:
    ...
    r4_paths = {
        "r4_protocol_freeze_absent": contract.R4_PROTOCOL_FREEZE_PATH,
        "r4_execution_lock_absent": contract.R4_EXECUTION_LOCK_PATH,
        "r4_candidate_freeze_receipt_absent": (
            contract.R4_CANDIDATE_FREEZE_RECEIPT_PATH
        ),
        "r4_final_development_receipt_absent": (
            contract.R4_FINAL_DEVELOPMENT_RECEIPT_PATH
        ),
        "r4_pipeline_ledger_absent": contract.R4_PIPELINE_LEDGER_PATH,
    }
    paths = {
        "design_freeze_unoccupied": contract.QUALIFICATION_DESIGN_FREEZE_PATH,
        "q2_protocol_freeze_absent": contract.PROTOCOL_FREEZE_PATH,
        "q2_execution_lock_absent": contract.EXECUTION_LOCK_PATH,
        "q2_noise_tapes_absent": contract.NOISE_ROOT,
        "q2_run_root_absent": contract.RUN_ROOT,
        **q1_paths,
        **r4_paths,
    }
    return {
...
def _assemble_design_freeze(occupancy: Mapping[str, bool]) -> dict[str, Any]:
    ...
    if set(occupancy) != expected_occupancy or not all(
        ...
        raise V12R4Q2QualificationDesignFreezeError("occupancy schema drifted")
    ...
    checks = {
        ...
        "historical_q1_p1s_excluded": historical["passed"] is True,
        ...
        "qualification_locked_pending_r4": access["status"] == LOCKED_ACCESS_STATUS
...
def build_design_freeze() -> dict[str, Any]:
    """Build the Q2 design in memory without publishing anything."""
    ...
    expected = _assemble_design_freeze(
        {
            ...
            "q2_protocol_freeze_absent": True,
            "q2_execution_lock_absent": True,
            "q2_noise_tapes_absent": True,
            "q2_run_root_absent": True,
            "q1_protocol_freeze_absent": True,
            "q1_execution_lock_absent": True,
            "q1_noise_root_absent": True,
            "q1_run_root_absent": True,
            "r4_protocol_freeze_absent": True,
            "r4_execution_lock_absent": True,
            "r4_candidate_freeze_receipt_absent": True,
            "r4_final_development_receipt_absent": True,
            "r4_pipeline_ledger_absent": True,
        }
    )
    if payload != expected or destination.read_bytes() != forensic.canonical_json_bytes(
        expected
    ):
        raise V12R4Q2QualificationDesignFreezeError("design freeze drifted")
    return dict(payload)
...
__all__ = [
    ...
    "NEXT_STAGE",
    "REFERENCE_TAPE_ABI_ACTIVITY",
    "V12R4Q2QualificationDesignFreezeError",
    "ZERO_DESIGN_ACTIVITY",
    "build_design_freeze",
