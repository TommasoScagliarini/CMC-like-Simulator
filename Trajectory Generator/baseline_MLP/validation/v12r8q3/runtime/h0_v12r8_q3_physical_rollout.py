"""Candidate-bound V12R8-Q3 physical rollout adapter.
...
injects only the V12R8 contract, the newly verified Q3 tape loader, and the
...
class V12R8Q3PhysicalRolloutError(RuntimeError):
...
def _load_mature_collector() -> Any:
    ...
    spec = importlib.util.spec_from_file_location(
        "_v12r8q3_mature_v12r5q3_physical_collector", _LEGACY_COLLECTOR_PATH
    )
    if spec is None or spec.loader is None:
        raise V12R8Q3PhysicalRolloutError("mature Q3 collector could not be loaded")
    module = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(module)
    except BaseException as exc:
        raise V12R8Q3PhysicalRolloutError("mature Q3 collector import failed") from exc
    finally:
        if inserted:
...
def build_candidate_env_config(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise V12R8Q3PhysicalRolloutError("base environment builder was malformed")
    ...
    if resolved_candidate_config(result) != contract.CANDIDATE_RESOLVED_ENV_CONFIG:
        raise V12R8Q3PhysicalRolloutError("resolved V26/morphology config drifted")
    return result
        ...
        if isinstance(info, Mapping):
            ...
            if isinstance(terms, Mapping):
                ...
                if actual_bytes != candidate_bytes or candidate_bytes != disabled_bytes:
                    raise V12R8Q3PhysicalRolloutError(
...
def flatten_comparison_metrics(summary: Mapping[str, Any]) -> dict[str, float]:
    ...
    if set(values) != {row[0] for row in contract.NONINFERIORITY_TOLERANCES}:
        raise V12R8Q3PhysicalRolloutError("comparison metric set drifted")
    if not all(math.isfinite(value) and value >= 0.0 for value in values.values()):
        raise V12R8Q3PhysicalRolloutError("comparison metric is non-finite")
    return values
...
def collect_physical_rollout(
    *,
    ...
    if role not in contract.ROLE_ORDER:
        raise V12R8Q3PhysicalRolloutError(f"unknown role: {role}")
    canonical = contract.canonical_case(str(case.get("case_id")))
    if dict(case) != canonical:
        raise V12R8Q3PhysicalRolloutError("physical case drifted from Q3 contract")
    collector = (
    ...
    if len(recorder_box) != 1:
        raise V12R8Q3PhysicalRolloutError("physical environment count drifted")
    ...
    if not isinstance(rows, list) or not isinstance(summary, Mapping):
        raise V12R8Q3PhysicalRolloutError("mature collector result is malformed")
