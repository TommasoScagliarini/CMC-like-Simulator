"""Self-contained pure-probe replay and observer-only H0 labelling for V12R1.
...
REPLAY_SCHEMA_ID = "H0_V12R1_OBSERVER_REPLAY_BOUNDARIES_V2"
LABEL_CORPUS_SCHEMA_ID = "H0_V12R1_OBSERVER_LABEL_CORPUS_V1"
ACTOR_FEATURE_COUNT = 35
ACTION_DIM = 2
MUTABLE_ACTOR_COLUMNS = tuple(range(10, 25))
IMMUTABLE_ACTOR_COLUMNS = tuple(
    index for index in range(ACTOR_FEATURE_COUNT)
    if index not in MUTABLE_ACTOR_COLUMNS
)
...
REPLAY_ARRAY_NAMES = frozenset(
    (*BOUNDARY_ARRAY_DTYPES, *STEP_ARRAY_DTYPES, *CONFIG_ARRAY_DTYPES,
     *SCALAR_ARRAY_DTYPES, *EVENT_ARRAY_DTYPES)
)
...
class V12R1ObserverLabelError(RuntimeError):
...
def _contract_module() -> Any:
    return importlib.import_module(
        "h0_primary_split_v12r1_autonomy_recovery_contract"
    )


def _finite(value: Any, *, label: str, nonnegative: bool = False) -> float:
    if isinstance(value, (bool, np.bool_)):
        raise V12R1ObserverLabelError(f"{label} must be numeric")
    try:
        result = float(value)
    except (TypeError, ValueError, OverflowError) as exc:
        raise V12R1ObserverLabelError(f"{label} must be numeric") from exc
    if not math.isfinite(result):
        raise V12R1ObserverLabelError(f"{label} must be finite")
    if nonnegative and result < 0.0:
        raise V12R1ObserverLabelError(f"{label} must be nonnegative")
    return result


def _native_bool(value: Any, *, label: str) -> bool:
    if not isinstance(value, (bool, np.bool_)):
        raise V12R1ObserverLabelError(f"{label} must be boolean")
    return bool(value)


def _utf8_array(value: str, *, label: str) -> np.ndarray:
    if not isinstance(value, str) or not value:
        raise V12R1ObserverLabelError(f"{label} must be nonempty text")
...
def _decode_utf8(array: np.ndarray, *, label: str, ascii_only: bool = False) -> str:
    if array.dtype != np.dtype(np.uint8) or array.ndim != 1 or array.size == 0:
        raise V12R1ObserverLabelError(f"{label} must be a nonempty uint8 vector")
    try:
        result = array.tobytes().decode("ascii" if ascii_only else "utf-8")
    except UnicodeDecodeError as exc:
        raise V12R1ObserverLabelError(f"{label} is not valid text") from exc
    if not result:
        raise V12R1ObserverLabelError(f"{label} must be nonempty")
    return result
...
def _reject_duplicate_pairs(pairs: Sequence[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V12R1ObserverLabelError(
                f"legacy FSM config repeats JSON key {key!r}"
            )
        result[key] = value
    return result
...
def canonical_legacy_fsm_config(runtime_phase_fsm: Any) -> tuple[bytes, str, str]:
    ...
    if config is None or not dataclasses.is_dataclass(config):
        raise V12R1ObserverLabelError("runtime phase FSM config must be a dataclass")
    try:
    ...
    if not module_name or not class_name or "<locals>" in class_name:
        raise V12R1ObserverLabelError("runtime phase FSM identity is unstable")
...
def _event_timestamp(
    ...
    *,
    ...
    if not primary_present and not fallback_present:
        if default is None:
            raise V12R1ObserverLabelError(f"{label} is missing")
        return float(default)
    primary_value = (
        _finite(event[primary], label=f"{label}.{primary}")
        if primary_present else None
    )
    fallback_value = (
        _finite(event[fallback], label=f"{label}.{fallback}")
        if fallback_present and fallback is not None else None
    )
    if (
        ...
        raise V12R1ObserverLabelError(f"{label} timestamp aliases disagree")
...
def _observation_fields(info: Mapping[str, Any]) -> tuple[float, float, float, float]:
    ...
    if not isinstance(observation, Mapping):
        raise V12R1ObserverLabelError("info.observation must be an object")
    return (
...
class ProbeReplayAccumulator:
    ...
    def __init__(
        ...
        *,
        ...
        if self._body_weight_n <= 0.0:
            raise V12R1ObserverLabelError("body_weight_n must be positive")
        if not isinstance(event_contract_id, str) or not event_contract_id:
            raise V12R1ObserverLabelError("event_contract_id must be nonempty")
    ...
    def _append_boundary(
        ...
        if not isinstance(info, Mapping):
            raise V12R1ObserverLabelError("boundary info must be an object")
        ...
        if not isinstance(detector, Mapping):
            raise V12R1ObserverLabelError("info.online_grf_detector must be an object")
        if reset_boundary:
            if detector:
                raise V12R1ObserverLabelError("reset detector stream must be empty")
            load_bw = 0.0
            in_contact = False
        else:
            left = detector.get("left")
            if not isinstance(left, Mapping):
                raise V12R1ObserverLabelError("left detector stream is missing")
        ...
        if reset_boundary and raw_events:
            raise V12R1ObserverLabelError("reset legacy event stream must be empty")
        previous_onset = (
        ...
        for raw_index, raw in enumerate(raw_events):
            ...
            if not isinstance(raw, Mapping):
                raise V12R1ObserverLabelError(f"{label} must be an object")
            side = str(raw.get("side", "")).strip().lower()
            event_type = str(raw.get("event", "")).strip().lower()
            if side not in {"left", "right"}:
                raise V12R1ObserverLabelError(f"{label}.side is unknown")
            if event_type not in {"heel_strike", "toe_off"}:
                raise V12R1ObserverLabelError(f"{label}.event is unknown")
            ...
            if not (
                ...
                raise V12R1ObserverLabelError(f"{label} timestamps are not causal")
            if side != "left":
                continue
            if previous_onset is not None and onset <= previous_onset + TIME_TOLERANCE_S:
                raise V12R1ObserverLabelError(
            ...
            startup_keys = tuple(
                key for key in ("startup_contact", "startup_partial_stance")
    ...
    def append_step(
        ...
        *,
        ...
        if self._closed:
            raise V12R1ObserverLabelError("probe replay accumulator is closed")
        ...
        if not np.all(np.isfinite(actor)):
            raise V12R1ObserverLabelError("actor_observation contains non-finite values")
    ...
    def arrays(self) -> dict[str, np.ndarray]:
        """Close the accumulator and return the exact V12R1 NPZ mapping."""

        if self._closed:
            raise V12R1ObserverLabelError("probe replay accumulator already closed")
        if not self._actors or self.boundary_count != self.step_count + 1:
            raise V12R1ObserverLabelError("replay must contain N states and N+1 boundaries")
        self._closed = True
...
class PureProbeReplayRecorder:
    ...
    def record_reset(
        ...
        *,
        ...
        if self._accumulator is not None:
            raise V12R1ObserverLabelError("probe reset was already recorded")
    ...
    def arrays(self) -> dict[str, np.ndarray]:
        if self._accumulator is None:
            raise V12R1ObserverLabelError("probe reset was not recorded")
...
def write_npz_exclusive(path: str | Path, arrays: Mapping[str, np.ndarray]) -> Path:
    ...
    if os.path.lexists(destination):
        raise V12R1ObserverLabelError(f"refusing to clobber: {destination}")
...
# Explicit runner-facing spelling requested by the V12R1 orchestrator.
...
def _strict_config(config_json: bytes) -> dict[str, Any]:
    ...
    except V12R1ObserverLabelError:
        raise
    except (UnicodeError, json.JSONDecodeError) as exc:
        raise V12R1ObserverLabelError("legacy FSM config JSON is invalid") from exc
    if not isinstance(value, Mapping):
        raise V12R1ObserverLabelError("legacy FSM config JSON must be an object")
    try:
        ...
        raise V12R1ObserverLabelError("legacy FSM config JSON is not canonical") from exc
    if canonical != config_json:
        raise V12R1ObserverLabelError("legacy FSM config JSON bytes are noncanonical")
    if value.get("event_source") != "legacy_events":
        raise V12R1ObserverLabelError("legacy FSM config event source drifted")
    return dict(value)
...
def load_probe_replay_strict(
    ...
    if not source.is_file():
        raise V12R1ObserverLabelError(f"probe replay is missing: {source}")
    try:
    ...
    if actor.ndim != 2 or actor.shape[1] != ACTOR_FEATURE_COUNT or len(actor) < 1:
        raise V12R1ObserverLabelError("actor_observations must have shape (N,35)")
    n_steps = len(actor)
    boundaries = n_steps + 1
    if penetration.shape != (n_steps,):
        raise V12R1ObserverLabelError("previous_penetration_m shape drifted")
    for name in BOUNDARY_ARRAY_DTYPES:
        if arrays[name].shape != (boundaries,):
            raise V12R1ObserverLabelError(f"{name} must have shape (N+1,)")
    if arrays["body_weight_n"].shape != (1,):
        raise V12R1ObserverLabelError("body_weight_n must have shape (1,)")
    ...
    if not np.all(np.isfinite(actor)) or not np.all(np.isfinite(penetration)):
        raise V12R1ObserverLabelError("probe step arrays contain non-finite values")
    if np.any(penetration < 0.0):
        raise V12R1ObserverLabelError("previous penetration must be nonnegative")
    for name in BOUNDARY_ARRAY_DTYPES:
        if arrays[name].dtype.kind == "f" and not np.all(np.isfinite(arrays[name])):
            raise V12R1ObserverLabelError(f"{name} contains non-finite values")
    times = arrays["boundary_time_s"]
    if not np.all(np.diff(times) > TIME_TOLERANCE_S):
        raise V12R1ObserverLabelError("boundary times are not strictly increasing")
    body_weight = _finite(
    ...
    if body_weight <= 0.0:
        raise V12R1ObserverLabelError("body_weight_n must be positive")
    if (
        arrays["legacy_left_normal_grf_bw"][0].tobytes() != np.float32(0.0).tobytes()
        or bool(arrays["legacy_left_in_contact"][0])
    ):
        raise V12R1ObserverLabelError("reset detector boundary is not empty")
    if np.any(arrays["legacy_left_normal_grf_bw"] < 0.0):
        raise V12R1ObserverLabelError("legacy normal GRF must be nonnegative")
    ...
    if observed_hash != expected_hash or len(observed_hash) != 64 or any(
        char not in "0123456789abcdef" for char in observed_hash
    ):
        raise V12R1ObserverLabelError("legacy FSM config SHA-256 mismatch")
    fsm_module = _decode_utf8(
        arrays["legacy_fsm_module_utf8"], label="legacy FSM module"
    )
    fsm_class = _decode_utf8(
        arrays["legacy_fsm_class_utf8"], label="legacy FSM class"
    )
    ...
    if offsets.shape != (boundaries + 1,):
        raise V12R1ObserverLabelError("event boundary offsets must have shape (N+2,)")
    if offsets[0] != 0 or np.any(np.diff(offsets) < 0):
        raise V12R1ObserverLabelError("event boundary offsets are malformed")
    event_count = int(offsets[-1])
    if offsets[1] != 0:
        raise V12R1ObserverLabelError("reset boundary must own zero events")
    for name in EVENT_ARRAY_DTYPES:
        if name == "legacy_left_event_boundary_offsets":
            continue
        if arrays[name].shape != (event_count,):
            raise V12R1ObserverLabelError(f"{name} must have shape (E,)")
    if any(str(value) != "left" for value in arrays["legacy_left_event_side"]):
        raise V12R1ObserverLabelError("event journal must contain only left events")
    if any(
        str(value) not in {"heel_strike", "toe_off"}
        for value in arrays["legacy_left_event_type"]
    ):
        raise V12R1ObserverLabelError("event journal contains an unknown event")
    ...
    if not (
        ...
        raise V12R1ObserverLabelError("event timestamps are non-finite or noncausal")
    if event_count > 1 and not np.all(np.diff(onset) > TIME_TOLERANCE_S):
        raise V12R1ObserverLabelError("left event onsets are not strictly increasing")
    for boundary in range(boundaries):
        start, stop = int(offsets[boundary]), int(offsets[boundary + 1])
        if start < 0 or stop > event_count:
            raise V12R1ObserverLabelError("event offsets escape the journal")
    ...
    optional_float_pairs = (
        ("legacy_left_event_cycle_duration_present", "legacy_left_event_cycle_duration_s"),
        ("legacy_left_event_contact_duration_present", "legacy_left_event_contact_duration_s"),
    )
    for present_name, value_name in optional_float_pairs:
        present = arrays[present_name]
        value = arrays[value_name]
        if not np.all(np.isfinite(value)) or np.any(value < 0.0):
            raise V12R1ObserverLabelError(f"{value_name} is invalid")
    ...
    if (
        ...
        raise V12R1ObserverLabelError("V12R1 replay contract is not available")
    topology = selected_contract.replay_event_topology_gate(
        arrays, n_steps=n_steps
    )
    if topology.get("passed") is not True:
        raise V12R1ObserverLabelError(
            f"V12R1 replay topology gate failed: {topology}"
        )
    return LoadedReplay(
        arrays=arrays,
        config=config,
...
def _boundary_events(replay: LoadedReplay, boundary: int) -> list[dict[str, Any]]:
    ...
    for index in range(start, stop):
        event: dict[str, Any] = {
            ...
            "event_time_s": float(
                arrays["legacy_left_event_onset_time_s"][index]
            ),
    ...
    if not reset:
        detector = {
            "left": {
                ...
                "normal_force": float(
                    arrays["legacy_left_normal_grf_bw"][boundary]
                ) * float(arrays["body_weight_n"][0]),
                "in_contact": bool(
                    arrays["legacy_left_in_contact"][boundary]
                ),
            }
        }
    return {
        ...
        "observation": {
            ...
            "pros_knee_angle_vel": float(
                arrays["pros_knee_speed_rad_s"][boundary]
            ),
            "pros_ankle_angle": float(arrays["pros_ankle_angle_rad"][boundary]),
            "pros_ankle_angle_vel": float(
                arrays["pros_ankle_speed_rad_s"][boundary]
            ),
        },
...
def replay_teacher_views(
    ...
    *,
    ...
    try:
        ...
        raise V12R1ObserverLabelError("legacy FSM reconstruction failed") from exc
    ...
    for boundary in range(replay.n_steps):
        ...
        invariant_equal = (
            student[list(IMMUTABLE_ACTOR_COLUMNS)].tobytes(order="C")
        ...
        invariant_exact += 1
        changed_only += 1
        column_24_changed += int(
            student[24].tobytes() != teacher[24].tobytes()
        )
    ...
    if teacher_array.shape != (replay.n_steps, ACTOR_FEATURE_COUNT):
        raise V12R1ObserverLabelError("teacher replay result shape drifted")
...
def _default_mean_query(module: Any, teacher_view: np.ndarray) -> np.ndarray:
    ...
    if logits.shape != (1, 2 * ACTION_DIM) or not np.all(np.isfinite(logits)):
        raise V12R1ObserverLabelError("H0 policy logits are malformed")
...
def _coverage_from_mapping(value: Any, *, rows: int) -> CoverageResult:
    if isinstance(value, CoverageResult):
        ...
        raise V12R1ObserverLabelError("coverage evaluator returned an unknown value")
    if (
        ...
        raise V12R1ObserverLabelError("coverage arrays are malformed")
    return result


def _default_coverage_evaluator(observations: np.ndarray) -> CoverageResult:
    fitter = importlib.import_module(
        "h0_primary_split_v12r1_recovery_weighted_fitter"
    )
    ...
    if not callable(evaluator):
        raise V12R1ObserverLabelError(
            "V12R1 fitter does not expose observer coverage evaluation"
        )
...
def observer_episode_weights(
    ...
    *,
    ...
    if (
        ...
        raise V12R1ObserverLabelError("observer episode weighting inputs are malformed")
    ...
    normalized[-1] = target_mass - math.fsum(
        float(value) for value in normalized[:-1]
    )
    for _ in range(4):
    ...
    if (
        ...
        raise V12R1ObserverLabelError("observer episode weights are malformed")
...
def label_closed_probe_in_memory(
    ...
    *,
    ...
    if not callable(probe_closure_validator):
        raise V12R1ObserverLabelError("probe closure validator is required")
    contract = contract_module or _contract_module()
    if probe_stage not in tuple(contract.FIT_STAGES):
        raise V12R1ObserverLabelError(f"unknown probe stage: {probe_stage!r}")
    if not isinstance(case_id, str) or not case_id or len(case_id) > 64:
        raise V12R1ObserverLabelError("case_id must be nonempty and <=64 chars")
    replay = load_probe_replay_strict(
        replay_path, contract_module=contract
    )
    ...
    try:
        ...
        raise V12R1ObserverLabelError("pure probe is not validly closed") from exc
    teacher_views = replay_teacher_views(
        replay, phase_fsm_factory=phase_fsm_factory
    )
    ...
    for index, teacher_view in enumerate(teacher_views.teacher_observations):
        ...
        if not np.all(np.isfinite(mean)) or np.any(np.abs(mean) > 1.0):
            raise V12R1ObserverLabelError("H0 mean is non-finite or out of bounds")
        actions.append(mean.copy())
        query_count += 1
    if query_count != replay.n_steps:
        raise V12R1ObserverLabelError("H0 teacher query count drifted")
...
def validate_observer_label_arrays(
    ...
    if set(arrays) != set(LABEL_ARRAY_DTYPES):
        raise V12R1ObserverLabelError("observer label corpus array schema drifted")
    for name, dtype in LABEL_ARRAY_DTYPES.items():
        array = np.asarray(arrays[name])
        if not _dtype_matches(array, dtype):
            raise V12R1ObserverLabelError(f"observer label {name} dtype drifted")
    expected_shapes = {
    ...
    if any(np.asarray(arrays[name]).shape != shape for name, shape in expected_shapes.items()):
        raise V12R1ObserverLabelError("observer label corpus shape drifted")
    for name in (
        "observations",
        "actions",
        "previous_penetration_m",
        ...
        if not np.all(np.isfinite(arrays[name])):
            raise V12R1ObserverLabelError(f"observer label {name} is non-finite")
    if tuple(arrays["actor_feature_names"].astype(str).tolist()) != tuple(
        coherent_teacher.EXPECTED_ACTOR_FEATURE_NAMES
    ):
        raise V12R1ObserverLabelError("observer actor feature layout drifted")
    if np.count_nonzero(arrays["reset_mask"]) != 1 or not bool(
        arrays["reset_mask"][0]
    ):
        raise V12R1ObserverLabelError("observer reset mask drifted")
    if not np.array_equal(arrays["step_indices"], np.arange(1, rows + 1)):
        raise V12R1ObserverLabelError("observer step indices drifted")
...
def _artifact(path: str | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R1ObserverLabelError(f"cannot bind artifact: {path}") from exc
...
def _tree_record(path: str | Path) -> dict[str, Any]:
    try:
        ...
        raise V12R1ObserverLabelError(f"cannot bind artifact tree: {path}") from exc
    ...
    *,
    ...
    def validate(replay: LoadedReplay) -> None:
        ...
        if (
            ...
            raise V12R1ObserverLabelError("probe evidence does not close this replay")
        if replay.event_contract_id != contract.EVENT_CONTRACT_ID:
            raise V12R1ObserverLabelError("probe replay event contract drifted")
...
def run_observer_label_stage(
    *,
    ...
    if stage not in tuple(contract.FIT_STAGES):
        raise V12R1ObserverLabelError(f"unknown label stage: {stage!r}")
    replay_source = _resolve_repo_path(
        contract.PROBE_ROOT / stage / "replay_boundaries.npz"
        if replay_path is None else replay_path
    )
    ...
    if enforce_canonical_destination and destination != canonical_destination:
        raise V12R1ObserverLabelError("observer label destination is not canonical")
    if os.path.lexists(destination):
        raise V12R1ObserverLabelError(
    ...
    summary = {
        ...
        "status": "H0_PRIMARY_SPLIT_V12R1_OBSERVER_LABELS_COMPLETE_UNGATED",
        ...
        "coverage_normalization_mean_sha256": coverage[
            "normalization_mean_sha256"
        ],
        "coverage_normalization_std_sha256": coverage[
    ...
    summary_path = forensic.write_json_exclusive(
        destination / "summary.json", summary
    )
    gate = dict(
...
def _strict_json_mapping(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise V12R1ObserverLabelError(f"{label} is not strict JSON") from exc
    if not isinstance(value, Mapping):
        raise V12R1ObserverLabelError(f"{label} must be an object")
    return dict(value)
...
def verify_observer_label_stage(
    ...
    *,
    ...
    try:
        with np.load(corpus_path, allow_pickle=False) as archive:
            if set(archive.files) != set(LABEL_ARRAY_DTYPES):
                raise V12R1ObserverLabelError("observer corpus NPZ schema drifted")
            arrays = {
                name: np.ascontiguousarray(archive[name].copy())
                for name in archive.files
            }
    ...
    if type(rows) is not int or rows < 1:
        raise V12R1ObserverLabelError("observer labelled row count is malformed")
    ...
    if expected_ood.tobytes(order="C") != arrays["coverage_ood_mask"].tobytes(
        ...
        raise V12R1ObserverLabelError("observer coverage OOD mask drifted")
    ...
    if (
        expected_raw.tobytes(order="C")
        != arrays["raw_sample_weights"].tobytes(order="C")
        or expected_normalized.tobytes(order="C")
        != arrays["normalized_sample_weights"].tobytes(order="C")
    ):
        raise V12R1ObserverLabelError("observer sample weights drifted")
    ...
    if (
        ...
        raise V12R1ObserverLabelError("observer label gate recomputation failed")
    ...
    if forensic.canonical_json_bytes(receipt) != forensic.canonical_json_bytes(
        ...
        raise V12R1ObserverLabelError("minimal observer label receipt drifted")
    current_bindings = (
    ...
    if not current_bindings:
        raise V12R1ObserverLabelError("observer label artifact binding drifted")
    return {
        **receipt,
        "receipt": _artifact(receipt_path),
        "summary": _artifact(summary_path),
