...
from validation import run_h0_primary_grf_split_v4_qualification as runner
...
class QualificationRunnerTests(unittest.TestCase):
    ...
    def test_post_holdout_validator_requires_exact_v4_lineage(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def strict(path: Path) -> dict[str, object]:
                ...
                if resolved == root / contract.HOLDOUT_GATE_PATH:
                    return gate
                if resolved == root / contract.V4_EXECUTION_LEDGER_PATH:
                    return ledger
            ...
            with (
                ...
                mock.patch.object(runner.v4, "verify_lock", return_value={}),
                mock.patch.object(
                    runner.v4,
    ...
    def test_post_holdout_validator_rejects_additional_actor_update(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                runner.validate_v4_post_holdout()

    def test_v3_rollout_engine_binding_is_scoped_and_uses_v4_contract(self) -> None:
    ...
    def test_supervisor_runs_all_baselines_before_any_candidate(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                mock.patch.object(runner.v4, "_strict_mapping", return_value={}),
                mock.patch.object(
                    runner.gates,
