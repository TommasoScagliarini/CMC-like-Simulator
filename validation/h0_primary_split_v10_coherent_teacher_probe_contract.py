"""Pure contract for the V10 coherent-teacher closed-loop probe.

V10 is a development-only, zero-update check.  The deployed environment stays
on the V26 binary-active contract.  A separate, force-free legacy shadow
reconstructs the complete historical H0 semantic block (actor indices 10..24)
solely for querying the frozen H0 teacher.
"""
...
SCHEMA_VERSION = 93
REVISION = "2026-08-07"
PROTOCOL_ID = "AB06_H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_CLOSED_LOOP_PROBE"
PROBE_ID = "H0_V10_COHERENT_LEGACY_TEACHER_V26_ZERO_UPDATE"
...
PREFLIGHT_STATUS = "PASS_H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE_PREFLIGHT"
LOCK_STATUS = "H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE_FROZEN"
CLAIM_STATUS = "H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE_EXECUTION_CLAIMED"
WORKER_CLAIM_STATUS = "H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE_WORKER_CLAIMED"
ROLLOUT_STATUS = "H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE_PERSISTED_UNGATED"
ROLLOUT_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE"
ROLLOUT_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE"
PROTOCOL_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE_PROTOCOL"
PROTOCOL_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V10_COHERENT_TEACHER_PROBE_PROTOCOL"
...
TEACHER_BLOCK_INDICES = tuple(range(10, 25))
OUTSIDE_TEACHER_BLOCK_INDICES = tuple(
    index for index in range(EXPECTED_ACTOR_FEATURES)
    if index not in TEACHER_BLOCK_INDICES
)
...
PREFLIGHT_PATH = PurePosixPath(
    "validation/h0_primary_split_v10_coherent_teacher_probe_preflight_receipt.json"
)
LOCK_PATH = PurePosixPath(
    "validation/h0_primary_split_v10_coherent_teacher_probe_execution_lock.json"
)
...
SOURCE_RELATIVE_PATHS = {
    "contract": "validation/h0_primary_split_v10_coherent_teacher_probe_contract.py",
    "runner": "validation/run_h0_primary_split_v10_coherent_teacher_probe.py",
    "contract_tests": (
        "validation/test_h0_primary_split_v10_coherent_teacher_probe_contract.py"
    ),
    "runner_tests": (
        "validation/test_run_h0_primary_split_v10_coherent_teacher_probe.py"
    ),
    "teacher_helper": "validation/h0_primary_split_v10_coherent_teacher.py",
    "teacher_helper_tests": (
        "validation/test_h0_primary_split_v10_coherent_teacher.py"
    ),
    "environment_config_source": (
        "validation/run_h0_primary_split_v9_causal_teacher.py"
    ),
    "legacy_shadow_source": (
...
AUTHORITY = {
    ...
    "development_coherent_teacher_probe_authorized": True,
...
def canonical_case(case_id: str) -> dict[str, Any]:
    if case_id != CASE_ID:
        raise ValueError(f"unknown V10 coherent-teacher probe case: {case_id!r}")
...
def rollout_gate(summary: Mapping[str, Any]) -> dict[str, Any]:
    """Classify the persisted probe without tolerating partial evidence."""
    ...
    zero_fields = (
        ...
        "historical_time_mismatch_count",
        "legacy_shadow_failure_count",
    ...
    checks = {
        ...
        "complete_semantic_block": summary.get("teacher_block_indices")
        == list(TEACHER_BLOCK_INDICES),
        ...
        "active_cycles": _nonnegative_int(summary.get("phase_valid_cycle_count"))
        and summary["phase_valid_cycle_count"] >= MINIMUM_VALID_CYCLES,
        "legacy_shadow_cycles": _nonnegative_int(
            summary.get("legacy_shadow_valid_cycle_count")
        )
        and summary["legacy_shadow_valid_cycle_count"] >= MINIMUM_VALID_CYCLES,
