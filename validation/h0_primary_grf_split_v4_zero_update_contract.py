"""Pure contract for the qualified H0 V4 zero-update trainer port.

The module is intentionally free of numerical, simulator, and filesystem side
effects.  V4 is a separate one-shot lineage and never consumes the failed V3
...
SCHEMA_VERSION = 1
REVISION = "2026-08-06"
PROTOCOL_ID = "AB06_H0_PRIMARY_GRF_SPLIT_V4_ZERO_UPDATE_PORT"
SOURCE_PROTOCOL_ID = "AB06_H0_PRIMARY_GRF_SPLIT_V4_FULL_MEAN"
QUALIFICATION_PROTOCOL_ID = (
    "AB06_H0_PRIMARY_GRF_SPLIT_V4_AUTONOMOUS_QUALIFICATION"
)
...
LOCK_STATUS = "H0_PRIMARY_SPLIT_V4_ZERO_UPDATE_PORT_FROZEN"
PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V4_ZERO_UPDATE_PORT"
FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V4_ZERO_UPDATE_PORT"
QUALIFICATION_LOCK_STATUS = "H0_PRIMARY_SPLIT_V4_QUALIFICATION_UNLOCKED"
QUALIFICATION_PASS_STATUS = (
    "PASS_H0_PRIMARY_SPLIT_V4_AUTONOMOUS_QUALIFICATION"
)
...
V4_RUN_ROOT_RELATIVE = (
    "validation/h0_primary_grf_split_adaptation_runs/"
    "2026-08-06_h0_primary_split_v4_full_mean"
)
LOCK_RELATIVE_PATH = (
    "validation/h0_primary_grf_split_v4_zero_update_port_lock.json"
)
OUTPUT_ROOT_RELATIVE_PATH = f"{V4_RUN_ROOT_RELATIVE}/zero_update_port"

INPUT_RELATIVE_PATHS = {
    "v4_execution_lock": "validation/h0_primary_grf_split_v4_execution_lock.json",
    "v4_execution_ledger": f"{V4_RUN_ROOT_RELATIVE}/execution_ledger.json",
    "candidate_freeze": f"{V4_RUN_ROOT_RELATIVE}/adaptation/candidate_freeze.json",
    "candidate_receipt": f"{V4_RUN_ROOT_RELATIVE}/adaptation/receipt.json",
    "candidate_offline_gate": (
        f"{V4_RUN_ROOT_RELATIVE}/adaptation/offline_gate.json"
    ),
    "candidate_module_state": (
        f"{V4_RUN_ROOT_RELATIVE}/adaptation/rl_module_target_adapted/"
        "module_state.pkl"
    ),
    ...
    "candidate_module_metadata": (
        f"{V4_RUN_ROOT_RELATIVE}/adaptation/rl_module_target_adapted/metadata.json"
    ),
    "candidate_actor_manifest": (
        f"{V4_RUN_ROOT_RELATIVE}/adaptation/rl_module_target_adapted/"
        "actor_feature_manifest.json"
    ),
    "holdout_receipt": f"{V4_RUN_ROOT_RELATIVE}/holdout/receipt.json",
    "holdout_gate": f"{V4_RUN_ROOT_RELATIVE}/holdout/gate.json",
    "holdout_replay_receipt": (
        f"{V4_RUN_ROOT_RELATIVE}/holdout_replay_receipt.json"
    ),
    "qualification_lock": (
        "validation/h0_primary_grf_split_v4_qualification_lock.json"
    ),
    "qualification_execution_ledger": (
        f"{V4_RUN_ROOT_RELATIVE}/qualification/"
...
for _qualification_case_id in QUALIFICATION_CASE_IDS:
    INPUT_RELATIVE_PATHS[f"qualification_gate_{_qualification_case_id}"] = (
        f"{V4_RUN_ROOT_RELATIVE}/qualification/gates/"
        f"{_qualification_case_id}.json"
    )

SOURCE_RELATIVE_PATHS = {
    "contract": "validation/h0_primary_grf_split_v4_zero_update_contract.py",
    "freezer": "validation/freeze_h0_primary_grf_split_v4_zero_update.py",
    "driver": "validation/run_h0_primary_grf_split_v4_zero_update_port.py",
    "tests": "validation/test_h0_primary_grf_split_v4_zero_update_port.py",
    "v4_freeze_contract": (
        "validation/h0_primary_grf_split_v4_freeze_contract.py"
    ),
    "v4_runner": "validation/run_h0_primary_grf_split_v4_full_mean.py",
    "v4_qualification_contract": (
        "validation/h0_primary_grf_split_v4_qualification_contract.py"
    ),
