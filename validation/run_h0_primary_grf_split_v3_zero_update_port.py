"""One-shot, zero-update trainer port for the qualified H0 V3 actor.

The driver builds a fresh target Algorithm from the frozen V3 configuration,
...
INPUT_PATHS = {
    key: _resolve_relative(value) for key, value in contract.INPUT_RELATIVE_PATHS.items()
}
SOURCE_PATHS = {
...
def _target_training_args(train_module: Any, output_dir: Path) -> Any:
    argv = [
        ...
        "--config",
        str(INPUT_PATHS["v3_training_config"]),
        "--output-dir",
        str(output_dir),
        "--iterations",
        "0",
    ...
    if not bool(args.asymmetric_actor_critic):
        raise ZeroUpdatePortError("V3 target must use asymmetric actor/critic")
    return args
...
def _export_actor(
    *,
    ...
    manifest = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "H0_PRIMARY_SPLIT_V3_ZERO_UPDATE_ACTOR_MANIFEST",
        "protocol_id": contract.PROTOCOL_ID,
        "candidate_id": contract.CANDIDATE_ID,
...
def _runtime_port() -> dict[str, Any]:
    ...
    runtime_env = {"env_vars": {"KMP_DUPLICATE_LIB_OK": "TRUE", "PYTHONWARNINGS": "ignore"}}
    ray.init(
    ...
    try:
        ...
        transplanted, transplant_report = warm_start.transplant_actor_state(
            ...
            source_config=INPUT_PATHS["v3_training_config"],
        ...
        if checkpoint_path.exists():
            raise ZeroUpdatePortError(f"refusing to clobber checkpoint: {checkpoint_path}")
...
def _attempt_claim_payload() -> dict[str, Any]:
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "H0_PRIMARY_SPLIT_V3_ZERO_UPDATE_PORT_ATTEMPT_CLAIMED",
        "protocol_id": contract.PROTOCOL_ID,
...
def execute() -> dict[str, Any]:
    ...
    if os.path.lexists(OUTPUT_ROOT):
        raise ZeroUpdatePortError(f"zero-update output is already claimed: {OUTPUT_ROOT}")
