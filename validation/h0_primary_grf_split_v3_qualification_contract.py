"""Immutable contract for the post-holdout H0 V3 qualification branch.

The module is deliberately pure: it imports no numerical or simulation stack
and performs no filesystem writes.  Execution code, the freezer, and the pure
comparator consume these values so that the six-case matrix and all numerical
caps are fixed before the first qualification rollout.
"""
...
SCHEMA_VERSION = 1
PROTOCOL_ID = "AB06_H0_PRIMARY_GRF_SPLIT_V3_AUTONOMOUS_QUALIFICATION"
...
RUN_ROOT = PurePosixPath(
    ...
    "2026-08-06_h0_primary_split_v3_semantic_replay/qualification"
)
LOCK_PATH = PurePosixPath("validation/h0_primary_grf_split_v3_qualification_lock.json")
...
HOLDOUT_RECEIPT_PATH = PurePosixPath(
    "validation/h0_primary_grf_split_adaptation_runs/"
    "2026-08-06_h0_primary_split_v3_semantic_replay/holdout/receipt.json"
)
CANDIDATE_FREEZE_PATH = PurePosixPath(
    "validation/h0_primary_grf_split_adaptation_runs/"
    "2026-08-06_h0_primary_split_v3_semantic_replay/adaptation/candidate_freeze.json"
)
CANDIDATE_MODULE_PATH = PurePosixPath(
    "validation/h0_primary_grf_split_adaptation_runs/"
    "2026-08-06_h0_primary_split_v3_semantic_replay/adaptation/"
...
_CASE_ROWS = (
    (CASE_IDS[0], "deterministic", CANONICAL_OFFSET_S - 0.2, None,
     _NOISE_ROOT / "qualification_deterministic_all_zero.npz"),
    (CASE_IDS[1], "deterministic", CANONICAL_OFFSET_S, None,
     _NOISE_ROOT / "qualification_deterministic_all_zero.npz"),
    (CASE_IDS[2], "deterministic", CANONICAL_OFFSET_S + 0.2, None,
     _NOISE_ROOT / "qualification_deterministic_all_zero.npz"),
    (CASE_IDS[3], "stochastic", CANONICAL_OFFSET_S, 126,
     _NOISE_ROOT / "qualification_trial_02_standard_normal.npz"),
    (CASE_IDS[4], "stochastic", CANONICAL_OFFSET_S, 127,
     _NOISE_ROOT / "qualification_trial_04_standard_normal.npz"),
    (CASE_IDS[5], "stochastic", CANONICAL_OFFSET_S, 128,
...
# These counters are true hard errors and must be zero for both reference and
# candidate.  Raw SLSQP->bounded-LS recovery is intentionally not present.
...
# These diagnostic/count metrics may be non-zero, but the candidate may never
# exceed its exact condition-matched baseline.
...
def canonical_cases() -> tuple[dict[str, Any], ...]:
    """Return fresh, immutable-in-content descriptions of the six cases."""

    result: list[dict[str, Any]] = []
    for case_id, selection, offset, action_seed, tape in _CASE_ROWS:
        runtime_seed = (
            DETERMINISTIC_RUNTIME_SEED if action_seed is None else int(action_seed)
        )
        result.append(
            {
                "case_id": case_id,
...
def tolerance_rows() -> dict[str, list[dict[str, float | str]]]:
    def convert(rows: tuple[tuple[str, float, float], ...]) -> list[dict[str, float | str]]:
        return [
            {
                "metric": metric,
    ...
    return {"sea": convert(SEA_TOLERANCES), "reserve": convert(RESERVE_TOLERANCES)}
