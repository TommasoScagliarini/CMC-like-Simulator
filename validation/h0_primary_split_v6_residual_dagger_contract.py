"""Pure contract for the one-round V6 V25 residual/DAgger pipeline.

The module is deliberately free of filesystem, OpenSim, Torch, and RLlib
work.  It preregisters exactly one interim P0 fit, exactly two P0 closed-loop
DAgger collections, one final P1 continuation, one immutable P1 freeze, and
six P1 closed-loop development cases.  P0 is never promotable and no second
DAgger round, retry, rescue, or hyper-parameter change is represented here.
"""
...
PIPELINE_ID = "H0_V6_V25_RESIDUAL_ONE_ROUND_DAGGER"

PIPELINE_CLAIM_STATUS = "H0_PRIMARY_SPLIT_V6_PIPELINE_EXECUTION_CLAIMED"
WORKER_CLAIM_STATUS = "H0_PRIMARY_SPLIT_V6_PIPELINE_WORKER_CLAIMED"
PIPELINE_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V6_RESIDUAL_DAGGER_PIPELINE"
PIPELINE_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V6_RESIDUAL_DAGGER_PIPELINE"
P0_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V6_RESIDUAL_P0_INTERIM"
P0_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V6_RESIDUAL_P0_INTERIM"
DAGGER_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V6_DAGGER_COLLECTION"
DAGGER_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V6_DAGGER_COLLECTION"
P1_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V6_RESIDUAL_P1_FINAL_FIT"
P1_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V6_RESIDUAL_P1_FINAL_FIT"
CANDIDATE_FREEZE_STATUS = "H0_PRIMARY_SPLIT_V6_CANDIDATE_FROZEN"
DEVELOPMENT_ROLLOUT_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V6_DEVELOPMENT_ROLLOUT"
DEVELOPMENT_ROLLOUT_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V6_DEVELOPMENT_ROLLOUT"
DEVELOPMENT_PASS_STATUS = "PASS_H0_PRIMARY_SPLIT_V6_DEVELOPMENT"
DEVELOPMENT_FAIL_STATUS = "FAIL_H0_PRIMARY_SPLIT_V6_DEVELOPMENT"

SOURCE_H0_ID = "H0_MARKOV35_PHASE_ALIGNED_SIGMA0005_ITER1_RETRY"
TARGET_CONTRACT_ID = "primary_grf_split_v1+binary_point_v25+functional_contact_fsm_v1"
...
MIN_DAGGER_SAMPLES_PER_CASE = 250
...
# Fixed offline gates.  These are measured on float32 action means and are
# intentionally the same for P0 and the final combined-corpus P1 fit.
OFFLINE_THRESHOLDS = {
    "rmse_max": 0.0015,
...
P0_MODULE_PATH = P0_ROOT / "rl_module_target_v25_residual_p0"
P0_RECEIPT_PATH = P0_ROOT / "receipt.json"
P1_ROOT = ADAPTATION_ROOT / "p1"
P1_MODULE_PATH = ADAPTATION_ROOT / "rl_module_target_v25_residual_p1"
...
def canonical_case(case_id: str, *, dagger: bool = False) -> dict[str, Any]:
    """Return one fresh exact case definition."""

    rows = DAGGER_CASES if dagger else DEVELOPMENT_CASES
    matches = [row for row in rows if row["case_id"] == case_id]
    if len(matches) != 1:
        raise ValueError(
            f"unknown {'DAgger' if dagger else 'development'} case: {case_id!r}"
        )
    return copy.deepcopy(matches[0])


def worker_claim_path(stage_id: str) -> PurePosixPath:
    """Return the unique ordered one-shot claim path for a worker stage."""

    try:
        index = STAGE_IDS.index(stage_id)
    except ValueError as exc:
        raise ValueError(f"unknown V6 pipeline stage: {stage_id!r}") from exc
...
def stage_receipt_path(stage_id: str) -> PurePosixPath:
    ...
    raise ValueError(f"unknown V6 pipeline stage: {stage_id!r}")


def learning_rate(stage: str, epoch: int) -> float:
    """Return the preregistered learning rate for one one-based epoch."""
...
def fit_gate(summary: Mapping[str, Any], *, stage: str) -> dict[str, Any]:
    """Classify one residual fit summary without accepting partial metadata."""
    ...
    dagger_count_valid = (
        dagger_samples == 0
        if stage == "p0"
        else type(dagger_samples) is int
        and len(DAGGER_CASE_IDS) * MIN_DAGGER_SAMPLES_PER_CASE
        <= dagger_samples
        <= len(DAGGER_CASE_IDS) * EXPECTED_STEPS
    )
    ...
    checks = {
        ...
        "sample_count": dagger_count_valid
        and summary.get("sample_count") == teacher_samples + int(dagger_samples),
        "teacher_sample_count": summary.get("teacher_sample_count") == teacher_samples,
        "dagger_sample_count": dagger_count_valid,
...
def dagger_gate(summary: Mapping[str, Any]) -> dict[str, Any]:
    """Accept only a full rollout or an explicitly classified FSM rejection."""
    ...
    mode = summary.get("completion_mode")
    count_valid = (
        type(count) is int
        and MIN_DAGGER_SAMPLES_PER_CASE <= count <= EXPECTED_STEPS
    )
    completion_valid = (mode == "episode_time_limit" and count == EXPECTED_STEPS) or (
        mode == "fsm_event_rejected" and count_valid
    )
    successful_steps = (
        count
        if mode == "episode_time_limit" and type(count) is int
        else count - 1
        if mode == "fsm_event_rejected" and type(count) is int
        else -1
    )
    rejection = summary.get("fsm_event_rejection")
    checks = {
        "schema": summary.get("schema_version") == SCHEMA_VERSION,
        "protocol": summary.get("protocol_id") == PROTOCOL_ID,
        "known_case": known,
        ...
        "rejection_explicit": mode != "fsm_event_rejected"
        or (
            isinstance(rejection, Mapping)
            and type(rejection.get("step")) is int
            and rejection.get("step") == count
            and rejection.get("type") == "ValueError"
            and isinstance(rejection.get("message"), str)
            and rejection["message"].startswith(
                "Actor FSM rejected a V20 active event: "
            )
        ),
        "persisted_samples": count_valid
        and summary.get("persisted_sample_count") == count,
        "audited_successful_steps": successful_steps >= 0
        and summary.get("control_window_count") == successful_steps * 10
        and summary.get("v25_raw_sensor_sample_count") == successful_steps * 10,
        "finite": summary.get("nonfinite_count") == 0,
        "no_clipping": summary.get("action_clipped_values") == 0,
        ...
        "no_prior_invalid_event": summary.get("invalid_event_count") == 0,
        "no_hard_invalid": summary.get("hard_invalid_count") == 0,
        "no_unaccepted_so": summary.get("so_solver_unaccepted_count") == 0,
        "no_sea_fallback": summary.get("sea_plugin_fallback_count") == 0,
        "routing_exact": summary.get("routing_failure_count") == 0,
        "causal_order": summary.get("candidate_selected_before_teacher_count") == count,
        "no_teacher_dependency": summary.get("served_action_teacher_dependency_count")
        == 0,
...
def development_rollout_gate(summary: Mapping[str, Any]) -> dict[str, Any]:
    """Apply the strict P1 closed-loop development gate to one case."""
    ...
    checks = {
        ...
        "raw_samples": summary.get("v25_raw_sensor_sample_count")
        ...
        and float(summary["grf_penetration_max_m"]) < PENETRATION_LIMIT_M,
        "zero_invalids": all(
            type(summary.get(field)) is int and summary.get(field) == 0
