...
from validation import h0_primary_grf_split_v4_freeze_contract as v4
from validation import h0_primary_grf_split_v4_qualification_contract as contract


class QualificationContractTests(unittest.TestCase):
    def test_lineage_and_paths_are_v4_only(self) -> None:
        self.assertEqual(contract.SOURCE_PROTOCOL_ID, v4.PROTOCOL_ID)
        self.assertEqual(contract.CANDIDATE_ID, v4.CANDIDATE_ID)
        self.assertEqual(contract.V4_RUN_ROOT.as_posix(), v4.RUN_ROOT_RELATIVE)
        self.assertEqual(
            contract.V4_EXECUTION_LOCK_PATH.as_posix(),
            v4.LOCK_RELATIVE,
        )
        for value in (
            contract.LOCK_PATH,
            ...
            self.assertIn("v4", value.as_posix())
