"""One-shot, zero-update trainer port for the qualified H0 V4 actor.

The driver builds a fresh target Algorithm from the frozen V4 configuration,
...
def _target_training_args(train_module: Any, output_dir: Path) -> Any:
    argv = [
        ...
        "--config",
        str(INPUT_PATHS["v4_training_config"]),
        "--output-dir",
        str(output_dir),
        "--iterations",
        "0",
    ...
    if not bool(args.asymmetric_actor_critic):
        raise ZeroUpdatePortError("V4 target must use asymmetric actor/critic")
    return args
...
def _export_actor(
    *,
    ...
    manifest = {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "H0_PRIMARY_SPLIT_V4_ZERO_UPDATE_ACTOR_MANIFEST",
        "protocol_id": contract.PROTOCOL_ID,
        "candidate_id": contract.CANDIDATE_ID,
...
def _runtime_port() -> dict[str, Any]:
    ...
    try:
        ...
        transplanted, transplant_report = warm_start.transplant_actor_state(
            ...
            source_config=INPUT_PATHS["v4_training_config"],
...
def _attempt_claim_payload() -> dict[str, Any]:
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": "H0_PRIMARY_SPLIT_V4_ZERO_UPDATE_PORT_ATTEMPT_CLAIMED",
        "protocol_id": contract.PROTOCOL_ID,
