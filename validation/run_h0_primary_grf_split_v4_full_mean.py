"""One-shot H0 primary-split V4 full-mean adaptation and final holdout.
...
class H0PrimarySplitV4Error(RuntimeError):
    """Raised whenever the V4 one-shot boundary cannot be proven."""
...
def _strict_mapping(path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise H0PrimarySplitV4Error(f"invalid strict JSON object: {resolved}") from exc
    if not isinstance(value, Mapping):
        raise H0PrimarySplitV4Error(f"expected JSON object: {resolved}")
...
def _require_finite_json(value: Any, label: str) -> None:
    ...
    if isinstance(value, numbers.Real):
        if not math.isfinite(float(value)):
            raise H0PrimarySplitV4Error(f"{label} contains a non-finite number")
        return
    if isinstance(value, Mapping):
        for key, child in value.items():
            if not isinstance(key, str) or not key:
                raise H0PrimarySplitV4Error(f"{label} has an invalid key")
    ...
    if isinstance(value, list):
        ...
        return
    raise H0PrimarySplitV4Error(f"{label} contains a non-JSON value")
...
def _require_int(value: Any, label: str) -> int:
    if type(value) is not int or value < 0:
        raise H0PrimarySplitV4Error(f"{label} must be a non-negative integer")
    return value


def verify_lock() -> dict[str, Any]:
    if not LOCK.is_file():
        raise H0PrimarySplitV4Error("V4 execution lock is absent")
    observed = _strict_mapping(LOCK)
    from freeze_h0_primary_grf_split_v4_execution import _build_lock_payload

    expected = _build_lock_payload()
    if set(observed) != LOCK_TOP_LEVEL_KEYS or _canonical_bytes(observed) != (
        _canonical_bytes(expected)
    ):
        raise H0PrimarySplitV4Error("V4 execution lock drifted")
    return observed
...
def _validate_v3_terminal_lineage() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for key, expected in contract.V3_TERMINAL_EXPECTATION.items():
        if ledger.get(key) != expected:
            raise H0PrimarySplitV4Error(f"V3 terminal lineage drifted at {key}")
    if (
        ...
        raise H0PrimarySplitV4Error("V3 terminal authority/footer drifted")
    ...
    if (
        ...
        raise H0PrimarySplitV4Error("V3 failed offline gate drifted")
...
def _validate_v3_corpus() -> tuple[dict[str, Any], dict[str, Any], Path]:
    ...
    if (
        ...
        raise H0PrimarySplitV4Error("V3 corpus is not the frozen train-only PASS")
    expected_paths = {
        "corpus": corpus_path,
    ...
    if not isinstance(inputs, Mapping):
        raise H0PrimarySplitV4Error("V4 lock inputs are missing")
    for key, expected in expected_paths.items():
        if not _record_matches(inputs.get(key), expected):
            raise H0PrimarySplitV4Error(f"V4 lock inherited {key} drifted")
...
def _attempt_claim_payload() -> dict[str, Any]:
    return {
        "schema_version": 4,
        "status": "H0_PRIMARY_SPLIT_V4_EXECUTION_ATTEMPT_CLAIMED",
...
def _claim_attempt_root() -> Path:
    ...
    try:
        ...
        raise H0PrimarySplitV4Error(
            f"V4 execution attempt already exists/no retry: {RUN_ROOT}"
        ) from exc
...
def _verify_attempt_claim() -> dict[str, Any]:
    ...
    if _canonical_bytes(observed) != _canonical_bytes(expected):
        raise H0PrimarySplitV4Error("V4 attempt claim drifted")
    return observed
...
def _claim_directory(path: str | Path, expected: Path) -> Path:
    ...
    if destination != expected.resolve():
        raise H0PrimarySplitV4Error(
            f"non-canonical V4 destination: {destination} != {expected.resolve()}"
        )
    ...
    try:
        ...
        raise H0PrimarySplitV4Error(
            f"V4 destination already exists/no-clobber: {destination}"
        ) from exc
    return destination
...
def _load_train_corpus() -> tuple[Any, ...]:
    ...
    with np.load(corpus_path, allow_pickle=False) as archive:
        ...
        if set(archive.files) != expected_keys:
            raise H0PrimarySplitV4Error("V3 corpus NPZ schema drifted")
    ...
    if (
        ...
        raise H0PrimarySplitV4Error("V3 train corpus arrays drifted")
    digests = {
    ...
    if any(manifest.get(key) != digest for key, digest in digests.items()):
        raise H0PrimarySplitV4Error("V3 train corpus digest drifted")
...
def full_mean_update_audit(
    ...
    if set(source_state) != expected_keys or set(candidate_state) != expected_keys:
        raise H0PrimarySplitV4Error("actor state schema drifted")
    ...
    for key in sorted(expected_keys):
        ...
        if source.shape != candidate.shape:
            raise H0PrimarySplitV4Error(f"actor tensor shape drifted: {key}")
...
def adapt_worker(output_dir: str | Path) -> dict[str, Any]:
    ...
    gate = {
        "schema_version": 4,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V4_OFFLINE"
            if passed
            else "FAIL_H0_PRIMARY_SPLIT_V4_OFFLINE"
        ),
        "passed": passed,
        "checks": checks,
    ...
    if not passed:
        raise H0PrimarySplitV4Error("V4 offline adaptation gate failed")
    return receipt
...
def _validate_candidate_receipt() -> tuple[dict[str, Any], Path]:
    ...
    if (
        ...
        raise H0PrimarySplitV4Error("V4 candidate receipt is not a PASS")
    expected_records = {
    ...
    for key, path in expected_records.items():
        if not _record_matches(receipt.get(key), path):
            raise H0PrimarySplitV4Error(f"candidate record drifted: {key}")
    ...
    if not isinstance(source_h0, Mapping) or set(source_h0) != set(
        ...
        raise H0PrimarySplitV4Error("V4 candidate source-H0 schema drifted")
    for key, path in expected_source_h0.items():
        if not _record_matches(source_h0[key], path):
            raise H0PrimarySplitV4Error(f"V4 candidate source H0 drifted: {key}")
    manifest_records = {
        "execution_lock": LOCK,
        "attempt_claim": ATTEMPT_CLAIM,
        "v3_corpus": REPO_ROOT / contract.V3_INPUT_RELATIVE_PATHS["corpus"],
    ...
    for key, path in manifest_records.items():
        if not _record_matches(manifest.get(key), path):
            raise H0PrimarySplitV4Error(f"V4 candidate manifest drifted: {key}")
    hyperparameters = report.get("hyperparameters")
    if (
        ...
        raise H0PrimarySplitV4Error("V4 candidate gate/report/manifest drifted")
...
def _candidate_freeze_payload() -> dict[str, Any]:
    ...
    return {
        "schema_version": 4,
        "status": "H0_PRIMARY_SPLIT_V4_CANDIDATE_FROZEN_BEFORE_HOLDOUT",
        "protocol_id": PROTOCOL_ID,
        "candidate_id": CANDIDATE_ID,
...
def freeze_candidate_before_holdout() -> dict[str, Any]:
    ...
    if any(os.path.lexists(path) for path in forbidden):
        raise H0PrimarySplitV4Error("holdout artifact exists before candidate freeze")
...
def verify_candidate_freeze() -> dict[str, Any]:
    ...
    if set(observed) != contract.CANDIDATE_FREEZE_KEYS or _canonical_bytes(
        ...
        raise H0PrimarySplitV4Error("V4 candidate freeze drifted")
    return observed
...
def _holdout_access_payload() -> dict[str, Any]:
    ...
    return {
        "schema_version": 4,
        "status": "H0_PRIMARY_SPLIT_V4_FINAL_HOLDOUT_ACCESS_CLAIMED",
        "protocol_id": PROTOCOL_ID,
        "candidate_id": CANDIDATE_ID,
...
def claim_holdout_access() -> dict[str, Any]:
    if os.path.lexists(HOLDOUT_REPLAY_DIR) or os.path.lexists(
        ...
        raise H0PrimarySplitV4Error("holdout replay exists before access claim")
...
def verify_holdout_access_claim() -> dict[str, Any]:
    ...
    if set(observed) != contract.HOLDOUT_ACCESS_CLAIM_KEYS or _canonical_bytes(
        ...
        raise H0PrimarySplitV4Error("V4 holdout access claim drifted")
    return observed


@contextlib.contextmanager
def _bind_frozen_v3_holdout_engine() -> Iterator[None]:
    """Bind V3's frozen replay engine to V4 provenance in this worker only."""
    # Validate with the unmodified V3 module first.  Calling the rich V4
    # validators after RUN_ROOT is rebound would make their inherited-corpus
    # audit look under the V4 root.  The engine therefore receives immutable
...
def _validate_engine_replay_receipt() -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    if (
        ...
        raise H0PrimarySplitV4Error("frozen V3 holdout-engine receipt drifted")
...
def holdout_replay_worker() -> dict[str, Any]:
    ...
    if os.path.lexists(HOLDOUT_REPLAY_RECEIPT):
        raise H0PrimarySplitV4Error("V4 holdout wrapper receipt already exists")
    with _bind_frozen_v3_holdout_engine():
        v3.run_replay(
    ...
    wrapper = {
        "schema_version": 4,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V4_REPLAY"
            if passed
            else "FAIL_H0_PRIMARY_SPLIT_V4_REPLAY"
        ),
        "passed": passed,
        "seed": FINAL_HOLDOUT_SEED,
        ...
        "v4_wrapper_runner": source_record(RUNNER),
    ...
    if not passed:
        raise H0PrimarySplitV4Error("V4 holdout replay wrapper failed")
    return wrapper
...
def _validate_holdout_replay_wrapper() -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    if (
        ...
        or wrapper.get("schema_version") != 4
        or wrapper.get("status") != "PASS_H0_PRIMARY_SPLIT_V4_REPLAY"
        ...
        or not _record_matches(wrapper.get("v4_wrapper_runner"), RUNNER)
    ):
        raise H0PrimarySplitV4Error("V4 holdout replay wrapper drifted")
...
def evaluate_final_holdout(output_dir: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise H0PrimarySplitV4Error("holdout replay arrays drifted")
    ...
    gate = {
        "schema_version": 4,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V4_FINAL_HOLDOUT"
            if passed
            else "FAIL_H0_PRIMARY_SPLIT_V4_FINAL_HOLDOUT"
        ),
        "passed": passed,
        "checks": checks,
        "metrics": metrics,
    ...
    if set(receipt) != contract.HOLDOUT_RECEIPT_KEYS:
        raise H0PrimarySplitV4Error("V4 holdout receipt schema disagrees with contract")
    _write_json_exclusive(destination / "receipt.json", receipt)
    if not passed:
        raise H0PrimarySplitV4Error("V4 final holdout gate failed")
    return receipt
...
def execute() -> dict[str, Any]:
    ...
    started = time.time()
    stage = "adaptation"
    status = "FAIL_H0_PRIMARY_SPLIT_V4_OFFLINE"
    passed = False
    error: str | None = None
    try:
        ...
        if completed.returncode != 0:
            raise H0PrimarySplitV4Error(
                f"V4 holdout replay worker exited {completed.returncode}"
            )
        stage = "final_holdout_evaluation"
        evaluate_final_holdout(HOLDOUT_DIR)
        stage = "final_holdout_complete"
        status = "PASS_H0_PRIMARY_SPLIT_V4_FINAL_HOLDOUT"
        passed = True
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.adapt_worker:
        if not args.output_dir:
            raise H0PrimarySplitV4Error("--output-dir is required")
