"""Pure fail-closed gates for autonomous H0 V5 qualification."""
...
try:  # Package import in unit tests; direct import in worker processes.
    from validation import h0_primary_grf_split_v5_qualification_contract as contract
except ImportError:  # pragma: no cover
    import h0_primary_grf_split_v5_qualification_contract as contract


class QualificationGateError(RuntimeError):
    """Raised when a qualification artifact is malformed or non-finite."""
...
def _validate_metrics(summary: Mapping[str, Any], label: str) -> None:
    ...
    for joint in contract.JOINTS:
        ...
        denominator = _counter(
            saturation["sample_count"],
            f"{label}.{joint}.saturation.sample_count",
        )
        count = _counter(
            saturation["count"],
            f"{label}.{joint}.saturation.count",
        )
        fraction = _finite(
            saturation["fraction"],
...
def common_rollout_gate(
    ...
    """Apply fixed physical/layout and complete-window SO gates."""
    ...
    fixed = (
        ("schema_version", data.get("schema_version"), contract.SCHEMA_VERSION),
        ...
        (
            ...
            data.get("phase_fsm_input_mode"),
            contract.PHASE_FSM_INPUT_MODE,
        ),
        ...
        (
            "morphology_weight",
            _finite(data.get("morphology_weight"), f"{role}.morphology_weight"),
    ...
    control_windows = _counter(
        data.get("so_solver_control_window_count"),
        f"{role}.control_windows",
    )
    invocations = _counter(
        data.get("so_solver_bounded_ls_invocation_count"),
        f"{role}.bounded_invocations",
    )
    verified = _counter(
        data.get("so_solver_verified_bounded_ls_count"),
        f"{role}.bounded_verified",
    )
    raw_fallbacks = _counter(
        data.get("raw_so_fallback_count"),
        f"{role}.raw_fallbacks",
    )
    status0 = _counter(
        data.get("so_solver_verified_status0_max_iter_count"),
        f"{role}.status0",
    )
    unsuccessful = _counter(
        data.get("so_solver_bounded_ls_unsuccessful_count"),
        f"{role}.unsuccessful",
    )
    hard = _counter(
        data.get("so_solver_hard_fallback_count"),
        f"{role}.hard",
    )
    checks.extend(
        (
...
def baseline_case_metrics(summary: Mapping[str, Any]) -> dict[str, dict[str, float]]:
    """Return the exact ordered schema embedded in the baseline receipt."""
