"""Freeze and preallocate one-shot post-holdout H0 V5 qualification.

The freezer cannot start a rollout.  It validates V5 holdout continuity,
...
import run_h0_primary_grf_split_v5_qualification as runner  # noqa: E402


DESTINATION = REPO_ROOT / contract.LOCK_PATH
TEST_RECEIPT = VALIDATION_ROOT / "h0_primary_grf_split_v5_qualification_test_receipt.json"
TEST_MODULES = (
    "validation.test_h0_primary_grf_split_v5_qualification_contract",
    "validation.test_compare_h0_primary_grf_split_v5_qualification",
    "validation.test_freeze_h0_primary_grf_split_v5_qualification",
    "validation.test_run_h0_primary_grf_split_v5_qualification",
)
...
def _source_paths() -> dict[str, Path]:
    return {
        ...
        / "h0_primary_grf_split_v5_qualification_contract.py",
        "qualification_comparator": VALIDATION_ROOT
        / "compare_h0_primary_grf_split_v5_qualification.py",
        "qualification_runner": VALIDATION_ROOT
        / "run_h0_primary_grf_split_v5_qualification.py",
        "qualification_freezer": Path(__file__).resolve(),
        "qualification_contract_tests": VALIDATION_ROOT
        / "test_h0_primary_grf_split_v5_qualification_contract.py",
        "qualification_comparator_tests": VALIDATION_ROOT
        / "test_compare_h0_primary_grf_split_v5_qualification.py",
        "qualification_freezer_tests": VALIDATION_ROOT
        / "test_freeze_h0_primary_grf_split_v5_qualification.py",
        "qualification_runner_tests": VALIDATION_ROOT
        / "test_run_h0_primary_grf_split_v5_qualification.py",
        "v5_runner": VALIDATION_ROOT / "run_h0_primary_grf_split_v5_full_mean.py",
        "v5_freeze_contract": VALIDATION_ROOT
        / "h0_primary_grf_split_v5_freeze_contract.py",
...
def _input_paths() -> dict[str, Path]:
    paths = {
        "v5_execution_lock": REPO_ROOT / contract.V5_EXECUTION_LOCK_PATH,
        "v5_execution_ledger": REPO_ROOT / contract.V5_EXECUTION_LEDGER_PATH,
        "v5_holdout_receipt": REPO_ROOT / contract.HOLDOUT_RECEIPT_PATH,
        "v5_holdout_gate": REPO_ROOT / contract.HOLDOUT_GATE_PATH,
        "v5_candidate_freeze": REPO_ROOT / contract.CANDIDATE_FREEZE_PATH,
        "v5_candidate_receipt": v5.ADAPTATION_DIR / "receipt.json",
        "v4_preexecution_failure": REPO_ROOT
        / contract.V4_PREEXECUTION_FAILURE_PATH,
...
def run_and_record_tests() -> dict[str, Any]:
    ...
    payload = {
        "schema_version": 5,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V5_QUALIFICATION_TESTS"
            if completed.returncode == 0
            else "FAIL_H0_PRIMARY_SPLIT_V5_QUALIFICATION_TESTS"
        ),
    ...
    v5._write_json_exclusive(TEST_RECEIPT, payload)
...
def _validate_prerequisites() -> dict[str, Any]:
    post_holdout = runner.validate_v5_post_holdout()
    noise = noise_contract.validate_noise_tapes(REPO_ROOT)
    tests = v5._strict_mapping(TEST_RECEIPT)
    if (
        tests.get("schema_version") != 5
        or tests.get("status") != "PASS_H0_PRIMARY_SPLIT_V5_QUALIFICATION_TESTS"
...
def build_payload(*, require_unoccupied: bool = True) -> dict[str, Any]:
    ...
    payload = {
        ...
        "execution_order": [
            ...
            "all_six_primary_split_v5_candidate_rollouts",
        ],
        "baseline_contract": {
        ...
        "candidate_contract": {
            ...
            "trainable_scope": v5_contract.TRAINABLE_SCOPE,
            "logstd_policy": v5_contract.LOGSTD_POLICY,
            "physics": "primary_grf_split_v1",
            "event_contract_id": contract.EVENT_CONTRACT_ID,
        },
        "tolerances": {
    ...
    v5._require_finite_json(payload, "qualification lock")
    return payload
...
def freeze() -> dict[str, Any]:
    ...
    v5._write_json_exclusive(DESTINATION, payload)
    return payload
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"H0 V5 qualification freeze failed closed: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 2
