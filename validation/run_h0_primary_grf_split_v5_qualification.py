"""Execute the frozen post-holdout autonomous qualification of H0 V5.
...
import compare_h0_primary_grf_split_v5_qualification as gates  # noqa: E402
import h0_primary_grf_split_v5_qualification_contract as contract  # noqa: E402
import run_h0_primary_grf_split_v3_qualification as engine  # noqa: E402
import run_h0_primary_grf_split_v5_full_mean as v5  # noqa: E402
...
def _record(path: str | Path) -> dict[str, Any]:
    return v5.source_record(Path(path).resolve())


def _record_matches(record: Any, path: str | Path) -> bool:
    return isinstance(record, Mapping) and v5._canonical_bytes(record) == (
        v5._canonical_bytes(_record(path))
    )
...
def _frozen_record_path(record: Any, label: str) -> Path:
    ...
    if (
        ...
        != v5._require_int(record["size_bytes"], f"{label}.size")
        or v5.v3.sha256_file(resolved) != record["sha256"]
    ):
        raise QualificationExecutionError(f"{label} integrity mismatch")
    return resolved


def validate_v5_post_holdout() -> dict[str, Any]:
    """Require exact V5 candidate/holdout continuity without opening new data."""

    v5.verify_lock()
    candidate_receipt, candidate = v5._validate_candidate_receipt()
    freeze = v5.verify_candidate_freeze()
    ledger = v5._strict_mapping(REPO_ROOT / contract.V5_EXECUTION_LEDGER_PATH)
    receipt_path = REPO_ROOT / contract.HOLDOUT_RECEIPT_PATH
    gate_path = REPO_ROOT / contract.HOLDOUT_GATE_PATH
    v4_failure_path = REPO_ROOT / contract.V4_PREEXECUTION_FAILURE_PATH
    receipt = v5._strict_mapping(receipt_path)
    gate = v5._strict_mapping(gate_path)
    ...
    if (
        ...
        or not _record_matches(
            receipt.get("v4_preexecution_failure"), v4_failure_path
        )
        ...
        raise QualificationExecutionError("V5 final holdout receipt is not canonical")
    ...
    if (
        ...
        or gate.get("v4_preexecution_failure")
        != receipt["v4_preexecution_failure"]
        ...
        raise QualificationExecutionError("V5 final holdout gate is not canonical")

    if (
        set(ledger) != v5.contract.EXECUTION_LEDGER_KEYS
        or ledger.get("schema_version") != v5.contract.SCHEMA_VERSION
        ...
        or not _record_matches(
            ledger.get("v4_preexecution_failure"), v4_failure_path
        )
        or not _record_matches(
            ledger.get("execution_lock"),
            REPO_ROOT / contract.V5_EXECUTION_LOCK_PATH,
        )
    ):
        raise QualificationExecutionError("V5 terminal execution ledger is not a PASS")

    if (
        freeze.get("schema_version") != 5
        ...
        or not _record_matches(
            freeze.get("v4_preexecution_failure"), v4_failure_path
        )
        or not _record_matches(
            freeze.get("candidate_receipt"),
            v5.ADAPTATION_DIR / "receipt.json",
        )
        ...
        raise QualificationExecutionError("V5 candidate freeze continuity failed")
    manifest_path = candidate / "actor_feature_manifest.json"
    manifest = v5._strict_mapping(manifest_path)
    if (
        ...
        raise QualificationExecutionError("V5 candidate actor manifest drifted")
    return {
        "status": "PASS_H0_PRIMARY_SPLIT_V5_POST_HOLDOUT_PREREQUISITE",
        "execution_ledger": _record(REPO_ROOT / contract.V5_EXECUTION_LEDGER_PATH),
        "holdout_receipt": _record(receipt_path),
        "candidate_freeze": _record(REPO_ROOT / contract.CANDIDATE_FREEZE_PATH),
        "v4_preexecution_failure": _record(v4_failure_path),
        "candidate_actor_digest": freeze["candidate_actor_digest"],
    }


def verify_lock() -> dict[str, Any]:
    observed = v5._strict_mapping(LOCK)
    from freeze_h0_primary_grf_split_v5_qualification import build_payload

    expected = build_payload(require_unoccupied=False)
    if v5._canonical_bytes(observed) != v5._canonical_bytes(expected):
        raise QualificationExecutionError("V5 qualification lock drifted")
    return observed
...
def _validate_baseline_decision() -> dict[str, Any]:
    decision = v5._strict_mapping(REPO_ROOT / contract.DECISION_RECEIPT_PATH)
    expected_keys = {
        "schema_version",
        "status",
        "passed",
    ...
    if (
        ...
        or decision.get("schema_version") != 5
        ...
        raise QualificationExecutionError("V5 baseline/tolerance decision drifted")
    ...
    if (
        ...
        raise QualificationExecutionError("V5 baseline decision record drifted")
    frozen = v5._strict_mapping(baseline_path)
    if (
        frozen.get("schema_version") != 5
        or frozen.get("status")
        != "H0_PRIMARY_SPLIT_V5_QUALIFICATION_BASELINE_FROZEN"
        ...
        raise QualificationExecutionError("V5 baseline receipt drifted")
    return decision
...
def _bind_v3_qualification_engine() -> Iterator[None]:
    """Bind the tested V3 rollout engine to V5 paths in this process only."""
...
def _publish_baseline_and_decision() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for case_id in contract.CASE_IDS:
        ...
        case_metrics[case_id] = gates.baseline_case_metrics(
            v5._strict_mapping(summary_path)
        )
        artifacts[case_id] = {
    ...
    manifest = {
        "schema_version": 5,
        "status": "H0_PRIMARY_SPLIT_V5_QUALIFICATION_BASELINE_EVIDENCE_FROZEN",
    ...
    v5._write_json_exclusive(REPO_ROOT / contract.BASELINE_MANIFEST_PATH, manifest)
    baseline = {
        "schema_version": 5,
        "status": "H0_PRIMARY_SPLIT_V5_QUALIFICATION_BASELINE_FROZEN",
        "passed": True,
    ...
    baseline_path = v5._write_json_exclusive(
        REPO_ROOT / contract.BASELINE_RECEIPT_PATH,
        baseline,
    )
    decision = {
        "schema_version": 5,
        "status": "H0_PRIMARY_SPLIT_V5_QUALIFICATION_BASELINE_TOLERANCE_DECIDED",
        "passed": True,
    ...
    v5._write_json_exclusive(REPO_ROOT / contract.DECISION_RECEIPT_PATH, decision)
    _validate_baseline_decision()
...
def execute() -> dict[str, Any]:
    ...
    try:
        ...
        for case_id in contract.CASE_IDS:
            ...
            baseline = v5._strict_mapping(
                REPO_ROOT
                / contract.rollout_destination("baseline", case_id)
                / "summary.json"
            )
            candidate = v5._strict_mapping(
                REPO_ROOT
    ...
    ledger = {
        ...
        "qualification_lock": _record(LOCK),
        "v5_execution_ledger": _record(
            REPO_ROOT / contract.V5_EXECUTION_LEDGER_PATH
        ),
        "v5_holdout_receipt": _record(REPO_ROOT / contract.HOLDOUT_RECEIPT_PATH),
        "v4_preexecution_failure": _record(
            REPO_ROOT / contract.V4_PREEXECUTION_FAILURE_PATH
        ),
    ...
    v5._write_json_exclusive(REPO_ROOT / contract.EXECUTION_LEDGER_PATH, ledger)
        ...
        if args.worker and args.output_dir:
            ...
            if not failure.exists():
                try:
                    ...
                    pass
        print(
            f"H0 V5 qualification failed closed: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 2
