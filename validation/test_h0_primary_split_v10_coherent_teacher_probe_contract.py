...
def _passing_summary() -> dict[str, object]:
    return {
        ...
        "legacy_shadow_valid_cycle_count": 2,
...
def test_contract_is_single_case_zero_update_and_complete_semantic_block() -> None:
    assert contract.CASE_IDS == ("deterministic_offset_minus_0p20",)
    assert contract.canonical_case(contract.CASE_ID)["action_selection"] == (
        "deterministic"
    )
    assert contract.TEACHER_BLOCK_INDICES == tuple(range(10, 25))
    assert contract.OUTSIDE_TEACHER_BLOCK_INDICES == (
        *range(0, 10),
        *range(25, 35),
    )
...
def test_rollout_gate_rejects_boundary_partial_shadow_or_updated() -> None:
    for field, value, check in (
        ("grf_penetration_max_m", 0.025, "penetration"),
        ("steps", 499, "steps"),
        ("phase_valid_cycle_count", 1, "active_cycles"),
        ("legacy_shadow_valid_cycle_count", 1, "legacy_shadow_cycles"),
        ("actor_updates", 1, "no_updates"),
        (
            "teacher_block_indices",
            list(range(10, 24)),
            "complete_semantic_block",
        ),
        (
        ...
        ("historical_action_mismatch_count", 1, "zero_failures"),
        ("historical_time_mismatch_count", 1, "zero_failures"),
    ):
