"""Run the V5 static/unit preflight and publish its no-clobber receipt.
...
PYTHON_SOURCES = tuple(
    REPO_ROOT / relative
    for relative in contract.V5_SOURCE_RELATIVE_PATHS.values()
    if relative.endswith(".py")
)
TEST_SOURCES = tuple(
    REPO_ROOT / contract.V5_SOURCE_RELATIVE_PATHS[key]
...
class V5PreflightError(RuntimeError):
    """Raised when the V5 preflight is not a clean PASS."""
...
def build() -> dict[str, Any]:
    if os.path.lexists(RECEIPT):
        raise V5PreflightError(f"refusing to clobber preflight receipt: {RECEIPT}")
    if os.path.lexists(runner.LOCK) or os.path.lexists(runner.RUN_ROOT):
        raise V5PreflightError("V5 lock/run must remain absent during preflight")
    runner._validate_v4_preexecution_failure()
    ...
    if ruff is None:
        ...
        if mac_fallback.is_file():
            ...
            raise V5PreflightError("ruff executable is unavailable")
    py_compile_command = [
        sys.executable,
        "-m",
        "py_compile",
    ...
    if os.path.lexists(runner.LOCK) or os.path.lexists(runner.RUN_ROOT):
        raise V5PreflightError("preflight unexpectedly created V5 lock/run")
    ...
    checks = {
        ...
        "v5_lock_absent": not os.path.lexists(runner.LOCK),
        "v5_run_root_absent": not os.path.lexists(runner.RUN_ROOT),
        "v3_seed125_semantic_artifacts_absent": v3_holdout_absent,
        "v4_preexecution_failure_valid": True,
    ...
    receipt = {
        "schema_version": 5,
        "status": (
            "PASS_H0_PRIMARY_SPLIT_V5_PREFLIGHT_TESTS"
            if passed
            else "FAIL_H0_PRIMARY_SPLIT_V5_PREFLIGHT_TESTS"
        ),
        "passed": passed,
        ...
        "tested_sources": {
            ...
            for key, relative in contract.V5_SOURCE_RELATIVE_PATHS.items()
        },
        "lineage": {
            ...
            "v4_preexecution_failure": runner.source_record(
                REPO_ROOT / contract.V4_PREEXECUTION_FAILURE_RELATIVE
            ),
        },
        "actor_updates": 0,
        "critic_updates": 0,
    ...
    if not passed:
        raise V5PreflightError("V5 preflight checks did not all pass")
    return receipt
