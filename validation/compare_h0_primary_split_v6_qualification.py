"""Pure, fail-closed gates for fresh H0 primary-split V6 qualification.

The baseline and candidate deliberately have different event/observation
contracts.  The baseline is the original H0 queried through its historical
analog view and legacy events; the candidate is the frozen P1 queried through
the primary-split view while V25 drives the active FSM.  Only the physical
SEA/reserve envelopes are compared condition by condition.
"""

from __future__ import annotations

import math
import numbers
from collections.abc import Mapping
from typing import Any

try:  # Package import in tests; direct import when invoked as a script.
    from validation import h0_primary_split_v6_qualification_contract as contract
except ImportError:  # pragma: no cover
    import h0_primary_split_v6_qualification_contract as contract


class QualificationGateError(RuntimeError):
    """Raised when qualification evidence is malformed or non-finite."""


def _mapping(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise QualificationGateError(f"{label} must be an object")
    return dict(value)


def _finite(value: Any, label: str, *, minimum: float | None = None) -> float:
    if isinstance(value, bool) or not isinstance(value, numbers.Real):
        raise QualificationGateError(f"{label} must be a finite number")
    result = float(value)
    if not math.isfinite(result) or (minimum is not None and result < minimum):
        raise QualificationGateError(f"{label} is outside its finite domain")
    return result


def _counter(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, numbers.Integral):
        raise QualificationGateError(f"{label} must be a non-negative integer")
    result = int(value)
    if result < 0:
        raise QualificationGateError(f"{label} must be a non-negative integer")
    return result


def _check(name: str, actual: Any, expected: Any, passed: bool) -> dict[str, Any]:
    return {
        "name": name,
        "status": "PASS" if passed else "FAIL",
        "actual": actual,
        "expected": expected,
    }


def _case(case_id: str) -> dict[str, Any]:
    rows = [row for row in contract.canonical_cases() if row["case_id"] == case_id]
    if len(rows) != 1:
        raise QualificationGateError(f"unknown canonical case {case_id!r}")
    return rows[0]


def _metric(value: Any, label: str, *, expected_samples: int | None) -> dict[str, Any]:
    data = _mapping(value, label)
    if set(data) != {"sample_count", "rms", "abs_max"}:
        raise QualificationGateError(f"{label} metric schema drifted")
    sample_count = _counter(data["sample_count"], f"{label}.sample_count")
    if expected_samples is not None and sample_count != expected_samples:
        raise QualificationGateError(
            f"{label}.sample_count={sample_count}, expected {expected_samples}"
        )
    rms = _finite(data["rms"], f"{label}.rms", minimum=0.0)
    abs_max = _finite(data["abs_max"], f"{label}.abs_max", minimum=0.0)
    if abs_max + 1.0e-15 < rms:
        raise QualificationGateError(f"{label}.abs_max is below rms")
    return {"sample_count": sample_count, "rms": rms, "abs_max": abs_max}


def _validate_metrics(summary: Mapping[str, Any], label: str) -> None:
    episode = _mapping(summary.get("episode_metrics"), f"{label}.episode_metrics")
    for metric in ("reserve_norm_nm", "residual_norm_nm"):
        _metric(
            episode.get(metric),
            f"{label}.{metric}",
            expected_samples=contract.EXPECTED_STEPS,
        )

    sea = _mapping(summary.get("sea_episode_metrics"), f"{label}.sea")
    if set(sea) != set(contract.JOINTS):
        raise QualificationGateError(f"{label} SEA joint schema drifted")
    for joint in contract.JOINTS:
        joint_data = _mapping(sea[joint], f"{label}.sea.{joint}")
        if set(joint_data) != {*contract.SEA_SIGNALS, "tau_input_saturated"}:
            raise QualificationGateError(f"{label}.{joint} SEA schema drifted")
        for signal in contract.SEA_SIGNALS:
            observed = _metric(
                joint_data[signal],
                f"{label}.{joint}.{signal}",
                expected_samples=None,
            )
            if observed["sample_count"] <= 0:
                raise QualificationGateError(f"{label}.{joint}.{signal} is empty")
        saturation = _mapping(
            joint_data["tau_input_saturated"],
            f"{label}.{joint}.tau_input_saturated",
        )
        if set(saturation) != {"sample_count", "count", "fraction"}:
            raise QualificationGateError(f"{label}.{joint} saturation schema drifted")
        denominator = _counter(
            saturation["sample_count"], f"{label}.{joint}.saturation.sample_count"
        )
        count = _counter(saturation["count"], f"{label}.{joint}.saturation.count")
        fraction = _finite(
            saturation["fraction"],
            f"{label}.{joint}.saturation.fraction",
            minimum=0.0,
        )
        if (
            denominator <= 0
            or count > denominator
            or fraction > 1.0
            or not math.isclose(
                fraction, count / denominator, rel_tol=0.0, abs_tol=1.0e-15
            )
        ):
            raise QualificationGateError(f"{label}.{joint} saturation is inconsistent")


def _nested_count(summary: Mapping[str, Any], path: str, label: str) -> int:
    joint = path.split(".", 1)[0]
    sea = _mapping(summary.get("sea_episode_metrics"), f"{label}.sea")
    joint_data = _mapping(sea.get(joint), f"{label}.sea.{joint}")
    saturation = _mapping(
        joint_data.get("tau_input_saturated"), f"{label}.{joint}.saturation"
    )
    return _counter(saturation.get("count"), f"{label}.{path}")


def common_rollout_gate(
    summary: Mapping[str, Any],
    *,
    role: str,
    case_id: str,
    candidate_id: str,
) -> dict[str, Any]:
    """Apply role-specific routing and common physical/layout gates."""

    if role not in {contract.BASELINE_ROLE, contract.CANDIDATE_ROLE}:
        raise QualificationGateError(f"unknown role {role!r}")
    if not isinstance(candidate_id, str) or not candidate_id.strip():
        raise QualificationGateError("candidate_id must be a non-empty string")
    data = _mapping(summary, f"{role}/{case_id} summary")
    case = _case(case_id)
    runtime = contract.role_contract(role)
    expected_actor_id = (
        contract.BASELINE_ACTOR_ID if role == contract.BASELINE_ROLE else candidate_id
    )
    checks: list[dict[str, Any]] = []

    identity = {
        "case_id": case_id,
        "action_selection": case["action_selection"],
        "episode_start_offset_s": case["episode_start_offset_s"],
        "action_seed": case["action_seed"],
        "runtime_seed": case["runtime_seed"],
        "sigma": case["sigma"],
    }
    for name, expected in identity.items():
        actual = data.get(name)
        if isinstance(expected, float):
            actual = _finite(actual, f"{role}.{name}")
            passed = math.isclose(actual, expected, rel_tol=0.0, abs_tol=1.0e-12)
        else:
            passed = actual == expected
        checks.append(_check(name, actual, expected, passed))

    fixed = {
        "schema_version": contract.SCHEMA_VERSION,
        "protocol_id": contract.PROTOCOL_ID,
        "role": role,
        "candidate_id": candidate_id,
        "actor_id": expected_actor_id,
        "contract_id": runtime["contract_id"],
        "actor_input_view": runtime["actor_input_view"],
        "observation_semantics": runtime["observation_semantics"],
        "primary_load_contract_id": runtime["primary_load_contract_id"],
        "phase_fsm_input_mode": runtime["phase_fsm_input_mode"],
        "event_contract_id": runtime["event_contract_id"],
        "binary_phase_fsm_mode": runtime["binary_phase_fsm_mode"],
        "steps": contract.EXPECTED_STEPS,
        "end_reason": "episode_time_limit",
        "terminated": False,
        "truncated": True,
        "n_actor": contract.EXPECTED_ACTOR_FEATURES,
        "n_observation": contract.EXPECTED_FULL_FEATURES,
        "observation_dtype": contract.EXPECTED_DTYPE,
        "action_shape": list(contract.EXPECTED_ACTION_SHAPE),
        "action_dtype": contract.EXPECTED_DTYPE,
        "online_grf_applied_sides": ["left"],
        "morphology_weight": contract.MORPHOLOGY_WEIGHT,
        "so_policy_id": contract.SO_POLICY_ID,
        "actor_updates": 0,
        "critic_updates": 0,
        "ppo_updates": 0,
        "protected_trials_opened": [],
    }
    for name, expected in fixed.items():
        actual = data.get(name)
        if name == "morphology_weight":
            actual = _finite(actual, f"{role}.{name}")
        checks.append(_check(name, actual, expected, actual == expected))

    tape_hash = data.get("noise_tape_array_sha256")
    checks.append(
        _check(
            "noise_tape_array_sha256",
            tape_hash,
            "64 lowercase hex characters",
            isinstance(tape_hash, str)
            and len(tape_hash) == 64
            and all(character in "0123456789abcdef" for character in tape_hash),
        )
    )
    cycles = _counter(data.get("phase_valid_cycle_count"), f"{role}.cycles")
    checks.append(
        _check(
            "phase_valid_cycle_count",
            cycles,
            f">={contract.MINIMUM_VALID_CYCLES}",
            cycles >= contract.MINIMUM_VALID_CYCLES,
        )
    )
    penetration = _finite(
        data.get("grf_penetration_max_m"), f"{role}.penetration", minimum=0.0
    )
    checks.append(
        _check(
            "grf_penetration_max_m",
            penetration,
            f"<{contract.PENETRATION_LIMIT_M}",
            penetration < contract.PENETRATION_LIMIT_M,
        )
    )
    raw_samples = _counter(
        data.get("binary_phase_sensor_sample_count"), f"{role}.raw_samples"
    )
    # Case A keeps V25 sampled-but-disabled so both roles see the exact same
    # physical detector stream.  Only the candidate is allowed to execute it.
    expected_samples = contract.EXPECTED_CONTROL_WINDOWS
    checks.append(
        _check(
            "binary_phase_sensor_sample_count",
            raw_samples,
            expected_samples,
            raw_samples == expected_samples,
        )
    )
    for name in (*contract.ZERO_REQUIRED_COUNTS, "runtime_contract_mismatch_count"):
        value = _counter(data.get(name), f"{role}.{name}")
        checks.append(_check(name, value, 0, value == 0))

    control_windows = _counter(
        data.get("so_solver_control_window_count"), f"{role}.control_windows"
    )
    invocations = _counter(
        data.get("so_solver_bounded_ls_invocation_count"), f"{role}.bounded_invocations"
    )
    verified = _counter(
        data.get("so_solver_verified_bounded_ls_count"), f"{role}.bounded_verified"
    )
    raw_fallbacks = _counter(
        data.get("raw_so_fallback_count"), f"{role}.raw_fallbacks"
    )
    status0 = _counter(
        data.get("so_solver_verified_status0_max_iter_count"), f"{role}.status0"
    )
    unsuccessful = _counter(
        data.get("so_solver_bounded_ls_unsuccessful_count"), f"{role}.unsuccessful"
    )
    hard = _counter(
        data.get("so_solver_hard_fallback_count"), f"{role}.hard"
    )
    checks.extend(
        (
            _check(
                "so_solver_control_window_count",
                control_windows,
                contract.EXPECTED_CONTROL_WINDOWS,
                control_windows == contract.EXPECTED_CONTROL_WINDOWS,
            ),
            _check("all_bounded_lsq_verified", verified, invocations, verified == invocations),
            _check(
                "raw_so_fallback_count_traced",
                raw_fallbacks,
                invocations,
                raw_fallbacks == invocations,
            ),
            _check(
                "status0_accounts_for_unsuccessful",
                unsuccessful,
                status0,
                unsuccessful == status0,
            ),
            _check("status0_accounts_for_hard", hard, status0, hard == status0),
        )
    )
    _counter(
        data.get("policy_step_terminal_so_fallback_count"),
        f"{role}.policy_step_terminal_so_fallback_count",
    )
    _validate_metrics(data, role)
    passed = all(check["status"] == "PASS" for check in checks)
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": contract.ROLLOUT_PASS_STATUS if passed else contract.FAIL_STATUS,
        "passed": passed,
        "protocol_id": contract.PROTOCOL_ID,
        "candidate_id": candidate_id,
        "role": role,
        "case_id": case_id,
        "checks": checks,
    }


def _continuous_values(
    summary: Mapping[str, Any],
) -> tuple[dict[str, float], dict[str, float]]:
    episode = _mapping(summary.get("episode_metrics"), "episode_metrics")
    reserve: dict[str, float] = {}
    for metric in ("reserve_norm_nm", "residual_norm_nm"):
        values = _metric(
            episode.get(metric), metric, expected_samples=contract.EXPECTED_STEPS
        )
        for aggregation in contract.CONTINUOUS_AGGREGATIONS:
            reserve[f"{metric}.{aggregation}"] = float(values[aggregation])
    sea_values: dict[str, float] = {}
    sea = _mapping(summary.get("sea_episode_metrics"), "sea_episode_metrics")
    for joint in contract.JOINTS:
        joint_data = _mapping(sea.get(joint), joint)
        for signal in contract.SEA_SIGNALS:
            values = _metric(
                joint_data.get(signal), f"{joint}.{signal}", expected_samples=None
            )
            for aggregation in contract.CONTINUOUS_AGGREGATIONS:
                sea_values[f"{joint}.{signal}.{aggregation}"] = float(
                    values[aggregation]
                )
    return sea_values, reserve


def baseline_case_metrics(summary: Mapping[str, Any]) -> dict[str, dict[str, float]]:
    """Return the exact tolerance-row ordering for a baseline receipt."""

    sea, reserve = _continuous_values(summary)
    if (
        list(sea) != [row[0] for row in contract.SEA_TOLERANCES]
        or list(reserve) != [row[0] for row in contract.RESERVE_TOLERANCES]
    ):
        raise QualificationGateError("baseline metric order drifted")
    return {"sea": sea, "reserve": reserve}


def condition_matched_gate(
    baseline: Mapping[str, Any],
    candidate: Mapping[str, Any],
    *,
    case_id: str,
    candidate_id: str,
) -> dict[str, Any]:
    """Compare one candidate only with its exact condition-matched baseline."""

    baseline_data = _mapping(baseline, "baseline")
    candidate_data = _mapping(candidate, "candidate")
    baseline_common = common_rollout_gate(
        baseline_data,
        role=contract.BASELINE_ROLE,
        case_id=case_id,
        candidate_id=candidate_id,
    )
    candidate_common = common_rollout_gate(
        candidate_data,
        role=contract.CANDIDATE_ROLE,
        case_id=case_id,
        candidate_id=candidate_id,
    )
    checks: list[dict[str, Any]] = []
    for name in (
        "case_id",
        "action_selection",
        "episode_start_offset_s",
        "action_seed",
        "runtime_seed",
        "sigma",
        "noise_tape_file_sha256",
        "noise_tape_array_sha256",
    ):
        actual = candidate_data.get(name)
        expected = baseline_data.get(name)
        checks.append(_check(f"condition.{name}", actual, expected, actual == expected))

    baseline_sea, baseline_reserve = _continuous_values(baseline_data)
    candidate_sea, candidate_reserve = _continuous_values(candidate_data)
    for family, rows, reference, observed in (
        ("sea", contract.SEA_TOLERANCES, baseline_sea, candidate_sea),
        ("reserve", contract.RESERVE_TOLERANCES, baseline_reserve, candidate_reserve),
    ):
        for metric, absolute, relative in rows:
            baseline_value = reference[metric]
            candidate_value = observed[metric]
            tolerance = max(float(absolute), float(relative) * abs(baseline_value))
            cap = baseline_value + tolerance
            item = _check(
                f"{family}.{metric}",
                candidate_value,
                f"<= {cap}",
                candidate_value <= cap,
            )
            item.update(
                {"baseline": baseline_value, "tolerance": tolerance, "cap": cap}
            )
            checks.append(item)

    for name in contract.NONINCREASING_COUNTS:
        baseline_count = _nested_count(baseline_data, name, "baseline")
        candidate_count = _nested_count(candidate_data, name, "candidate")
        checks.append(
            _check(
                f"count.{name}",
                candidate_count,
                f"<= {baseline_count}",
                candidate_count <= baseline_count,
            )
        )

    passed = (
        baseline_common["passed"] is True
        and candidate_common["passed"] is True
        and all(check["status"] == "PASS" for check in checks)
    )
    return {
        "schema_version": contract.SCHEMA_VERSION,
        "status": contract.CASE_PASS_STATUS if passed else contract.FAIL_STATUS,
        "passed": passed,
        "protocol_id": contract.PROTOCOL_ID,
        "candidate_id": candidate_id,
        "case_id": case_id,
        "baseline_common_gate": baseline_common,
        "candidate_common_gate": candidate_common,
        "condition_matched_checks": checks,
    }


__all__ = [
    "QualificationGateError",
    "baseline_case_metrics",
    "common_rollout_gate",
    "condition_matched_gate",
]
