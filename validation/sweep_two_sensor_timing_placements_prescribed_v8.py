"""V8 heel-focused placement sweep for a simple two-sensor gait detector.

V7 showed that moving the forefoot substantially posterior closes the initial
heel-to-toe gap but prevents causal rearming before a later HS.  V8 therefore
returns the forefoot to F79.5/F80 at fixed P35 and varies only two small heel
placement axes.  Each candidate remains exactly one heel sphere plus one
forefoot sphere and is evaluated directly at 10 ms and 1 ms.
"""
...
import sweep_two_sensor_timing_placements_prescribed_v7 as v7  # noqa: E402


DEFAULT_PROTOCOL = (
    VALIDATION_ROOT / "two_sensor_timing_placement_sweep_protocol_v8.json"
)
DEFAULT_OUTPUT_DIR = (
    VALIDATION_ROOT
    / "two_sensor_timing_placement_sweep_runs/"
    "2026-07-22_ab06_50_100_heel_micro_v8"
)
DEFAULT_PLOT_DIR = REPO_ROOT / "plot/07_22_2026_two_sensor_timing_placement_v8"
PROTOCOL_ID = "AB06_TWO_SENSOR_TIMING_PLACEMENT_DEVELOPMENT_2026-07-22_V8"
BASE_PROTOCOL = "validation/two_sensor_timing_placement_sweep_protocol_v7.json"
BASE_PROTOCOL_SHA256 = (
    "0986f2c19cd88d558ee6e143c5c0082b7ada8fadbb3d990aece5229625815ed4"
)
V7_MANIFEST = (
...
HEEL_OFFSETS_MM = (2.0, 2.25)
HEEL_X_DELTAS_MM = (2.5, 2.75)
FOREFOOT_FRACTIONS = (0.795, 0.80)
...
class ProtocolError(v6.ProtocolError):
    """Raised before sampling if the frozen V8 contract drifts."""
...
def load_and_validate_protocol(
    ...
    protocol_path = v6.v1.resolve_repo_path(path).resolve()
    try:
        revision = json.loads(protocol_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ProtocolError(f"cannot load V8 protocol: {protocol_path}") from exc
    if not isinstance(revision, dict):
        raise ProtocolError("V8 protocol root must be an object")
    for key, expected in {
        "schema_version": 8,
        "protocol_id": PROTOCOL_ID,
        "frozen_before_execution": True,
        "stage": "development_heel_micro_multiresolution",
        "base_protocol": {"path": BASE_PROTOCOL, "sha256": BASE_PROTOCOL_SHA256},
        "source_v7_manifest": {"path": V7_MANIFEST, "sha256": V7_MANIFEST_SHA256},
        "threshold_fsm_gate_or_evidence_routing_changed": False,
    }.items():
        if revision.get(key) != expected:
            raise ProtocolError(f"V8 protocol field drifted: {key}")
    ...
    for key, expected in {
        ...
        if grid.get(key) != expected:
            raise ProtocolError(f"V8 grid field drifted: {key}")
    expected_sample = {
    ...
    if revision.get("sampling") != {
        ...
        raise ProtocolError("V8 sampling contract drifted")
    if revision.get("selection") != {
        ...
        raise ProtocolError("V8 selection contract drifted")
    sources = revision.get("sources")
    if not isinstance(sources, dict) or not sources:
        raise ProtocolError("V8 hash-pinned sources are required")
    for label, record in sources.items():
        source_path = v6.v1.resolve_repo_path(str(record.get("path", ""))).resolve()
        if not source_path.is_file():
            raise ProtocolError(f"missing V8 pinned source: {label}")
        if v6.v1._sha256(source_path) != record.get("sha256"):
            raise ProtocolError(f"V8 source hash drift: {label}")
    base_path = v6.v1.resolve_repo_path(BASE_PROTOCOL).resolve()
    if v6.v1._sha256(base_path) != BASE_PROTOCOL_SHA256:
        raise ProtocolError("V7 base protocol drifted")
    if v6.v1._sha256(v6.v1.resolve_repo_path(V7_MANIFEST)) != V7_MANIFEST_SHA256:
        raise ProtocolError("V7 result manifest drifted")
    base = v7.load_and_validate_protocol(base_path)
    base.update(
        {
            "schema_version": 8,
            "protocol_id": PROTOCOL_ID,
            ...
            "_protocol_sha256": v6.v1._sha256(protocol_path),
            "_v8_lineage": {
                "base_protocol": revision["base_protocol"],
                "source_v7_manifest": revision["source_v7_manifest"],
                "scientific_change": "placement_grid_only",
            },
        }
    )
    return base
...
def build_placement_candidates(
...
) -> tuple[v6.v1.OnlineGRFProfile, list[v6.v1.PlacementCandidate], dict[str, Any]]:
    base = v6.v1.load_online_grf_profile(
        v6.v1.resolve_repo_path(str(protocol["detector_template_profile"])).resolve()
    )
    sensors = v6.v1._left_sensor_spheres(base)
    heel_template = sensors["left_heel"]
    toe_template = sensors["left_toe"]
    setup = v6.v1.read_setup_xml(
        v6.v1.resolve_repo_path(str(protocol["setup"])).resolve()
    )
    mesh_path = v6.v1._resolve_left_foot_mesh(setup.model_file.resolve())
    triangles = v6.v1._load_stl_triangles(mesh_path)
    ...
    for fraction in FOREFOOT_FRACTIONS:
        toe_locations[fraction] = v6.v1._derive_forefoot_location(
            triangles,
            toe_template,
            fraction=fraction,
            protrusion_mm=FOREFOOT_PROTRUSION_MM,
        )
    candidates: list[v6.v1.PlacementCandidate] = []
    ...
    for offset_mm in HEEL_OFFSETS_MM:
        for x_mm in HEEL_X_DELTAS_MM:
            ...
            heel_geometry = v6.v3._mesh_geometry(heel_location, radius, triangles)
            for fraction in FOREFOOT_FRACTIONS:
                toe_location, derived = toe_locations[fraction]
                toe_geometry = v6.v3._mesh_geometry(toe_location, radius, triangles)
                ...
                candidates.append(
                    v6.v1.PlacementCandidate(
                        ...
                        selectable=True,
                        role="development_heel_micro_multiresolution_candidate",
                        geometry={
                            **derived,
                            "heel": heel_geometry,
    ...
    v5_toe = toe_locations[0.80][0]
    candidates.append(
        v6.v1.PlacementCandidate(
            candidate_id=v6.V5_COMPARATOR_ID,
    ...
    candidates.append(
        v6.v1.PlacementCandidate(
            candidate_id=v6.CURRENT_COMPARATOR_ID,
    ...
    if len(selectable) != SELECTABLE_COUNT or len(candidates) != PAIR_COUNT:
        raise ProtocolError("V8 candidate grid drifted")
    if len({item.candidate_id for item in candidates}) != PAIR_COUNT:
        raise ProtocolError("V8 candidate IDs are not unique")
    if not all(bool(item.geometry.get("pre_gate_ok")) for item in selectable):
        raise ProtocolError("a selectable V8 geometry failed its pre-gate")
    maximum_offset = max(abs(value) for value in bottom_offsets)
    if maximum_offset > 20.0 + 1e-9:
        raise ProtocolError("V8 bottom-offset pre-gate failed")
    return base, candidates, {
        "mesh": v6.v1._source_record(mesh_path),
        "selectable_candidate_count": SELECTABLE_COUNT,
        "nonselectable_comparator_count": 2,
        "total_pair_count": PAIR_COUNT,
        "sensors_per_pair": 2,
...
def select_multiresolution_winner(
    ...
    if set(runtime) != set(fine) or len(runtime) != PAIR_COUNT:
        raise ProtocolError("V8 multiresolution candidate sets differ")
    selectable_ids = sorted(
        item for item, row in runtime.items() if bool(row.get("selectable"))
    )
    if len(selectable_ids) != SELECTABLE_COUNT:
        raise ProtocolError("V8 selection requires eight candidates")
    current_10 = runtime[v6.CURRENT_COMPARATOR_ID]
    current_1 = fine[v6.CURRENT_COMPARATOR_ID]
    gates: dict[str, Any] = {}
    eligible: list[str] = []
    for candidate_id in selectable_ids:
        gate_10 = v6.strict_gate(
            runtime[candidate_id],
            current_10,
            protocol,
            gate_key="runtime_gate_10ms",
        )
        gate_1 = v6.strict_gate(
            fine[candidate_id],
            current_1,
            protocol,
...
def _plot(
    ...
    fig.suptitle(
        "Two-sensor V8 heel micro-sweep — 1 heel + 1 forefoot, 10 ms / 1 ms"
    )
    fig.tight_layout()
...
def run_sweep(
    ...
    for label, sample_dt_s in (
        ("runtime_10ms", v6.PRIMARY_DT_S),
        ("fine_1ms", v6.FINE_DT_S),
    ):
        streams, access = v6.v4.sample_streams_once(
            protocol,
            base,
            candidates,
    ...
    csv_10 = output_dir / "timing_placement_v8_runtime_10ms_metrics.csv"
    csv_1 = output_dir / "timing_placement_v8_fine_1ms_metrics.csv"
    plot_path = plot_dir / "01_timing_placement_v8_multiresolution.png"
    ...
    manifest = {
        ...
        "protocol": {
            "path": v6.v1._portable_path(Path(str(protocol["_protocol_path"]))),
            "sha256": protocol["_protocol_sha256"],
            "protocol_id": protocol["protocol_id"],
        },
        "lineage": protocol["_v8_lineage"],
        "data_access": {
            "already_open_block_s": [v6.v1.BLOCK_START_S, v6.v1.SEALED_START_S],
            "sealed_block_s": [v6.v1.SEALED_START_S, v6.v1.SEALED_END_S],
        ...
        "artifacts": {
            "runtime_metrics_csv": v6.v1._source_record(csv_10),
            "fine_metrics_csv": v6.v1._source_record(csv_1),
            "multiresolution_plot": v6.v1._source_record(plot_path),
        },
        "non_actions": {
    ...
    safe = v6.v1._json_safe(manifest)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run preregistered two-sensor V8 heel micro-sweep."
    )
