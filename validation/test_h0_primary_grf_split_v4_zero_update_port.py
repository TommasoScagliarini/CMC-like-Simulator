...
from validation import freeze_h0_primary_grf_split_v4_zero_update as freezer
from validation import h0_primary_grf_split_v4_zero_update_contract as contract
from validation import run_h0_primary_grf_split_v4_zero_update_port as driver
...
class ZeroUpdatePortTests(unittest.TestCase):
    def test_contract_is_closed_and_uses_canonical_qualification_evidence(self) -> None:
        self.assertEqual(
            ...
            "PASS_H0_PRIMARY_SPLIT_V4_ZERO_UPDATE_PORT",
        )
    ...
    def _frozen_world(self) -> dict[str, object]:
        ...
        sources = {
            key: root / "sources" / f"{key}.py" for key in contract.SOURCE_RELATIVE_PATHS
        }
        ...
        inputs["v4_training_config"].parent.mkdir(parents=True, exist_ok=True)
        inputs["v4_training_config"].write_text("iterations: 1\n", encoding="utf-8")
        ...
        _write_json(
            ...
            {
                "schema_version": 4,
                "status": "PASS_H0_PRIMARY_SPLIT_V4_OFFLINE",
                "passed": True,
                "actor_updates": 1,
                "critic_updates": 0,
                "ppo_updates": 0,
        ...
        _write_json(
            ...
            {
                "schema_version": 4,
                "status": "PASS_H0_PRIMARY_SPLIT_V4_REPLAY",
                "passed": True,
                "seed": 125,
        ...
        _write_json(
            inputs["v4_execution_lock"],
            {
                ...
                "sources": {
                    "v4": {"runner": driver.source_record(sources["driver"])}
                },
                "inputs": {
                    "h0": {
        ...
        qualification_lock = {
            "schema_version": 4,
            "status": "H0_PRIMARY_SPLIT_V4_QUALIFICATION_UNLOCKED",
            "protocol_id": (
        ...
        _write_json(
            inputs["qualification_decision_receipt"],
            {
                "schema_version": 4,
                "status": (
                    "H0_PRIMARY_SPLIT_V4_QUALIFICATION_"
        ...
        for case_id in contract.QUALIFICATION_CASE_IDS:
            gate = {
                "schema_version": 4,
                "status": "PASS_H0_PRIMARY_SPLIT_V4_QUALIFICATION_CASE",
                "passed": True,
                "case_id": case_id,
            }
        ...
        _write_json(
            inputs["qualification_execution_ledger"],
            {
                ...
                "v4_execution_ledger": driver.source_record(
                    inputs["v4_execution_ledger"]
                ),
                "v4_holdout_receipt": driver.source_record(
                    inputs["holdout_receipt"]
                ),
