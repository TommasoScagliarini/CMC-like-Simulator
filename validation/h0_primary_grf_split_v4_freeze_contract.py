"""Pure no-I/O contract for H0 primary-split V4 full-mean adaptation.

V4 is a new one-shot lineage.  It inherits only the frozen, passing V3
train corpus (seeds 123/124) and the terminal V3 failure receipt.  It never
inherits the failed V3 candidate and it keeps seed 125 procedural: the
freezer may hash the two historical files, but only the post-candidate-freeze
holdout worker may parse them.
"""
...
LOCK_RELATIVE = "validation/h0_primary_grf_split_v4_execution_lock.json"
PREFLIGHT_RECEIPT_RELATIVE = (
    "validation/h0_primary_grf_split_v4_preflight_receipt.json"
)
...
# Train-only design evidence.  It selected the sole V4 design; it is not a
...
V4_SOURCE_RELATIVE_PATHS = {
    "plan_addendum": (
        "reports/plans/2026-08-06_addendum_h0_primary_split_v4_full_mean.md"
    ),
    "freeze_contract": "validation/h0_primary_grf_split_v4_freeze_contract.py",
    "execution_freezer": "validation/freeze_h0_primary_grf_split_v4_execution.py",
    "preflight_builder": (
        "validation/build_h0_primary_grf_split_v4_preflight_receipt.py"
    ),
    "runner": "validation/run_h0_primary_grf_split_v4_full_mean.py",
    "contract_tests": "validation/test_h0_primary_grf_split_v4_contract.py",
    "freezer_tests": (
        "validation/test_freeze_h0_primary_grf_split_v4_execution.py"
    ),
    "runner_tests": "validation/test_h0_primary_grf_split_v4_full_mean.py",
}
