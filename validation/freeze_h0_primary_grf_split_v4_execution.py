"""Publish only the immutable H0 primary-split V4 execution lock.

The freezer hashes the seed-125 historical files as opaque bytes.  It must not
load or interpret either file, create the V4 run root, fit an actor, or invoke
the replay engine.
"""
...
class V4FreezeError(RuntimeError):
    """Raised when the V4 boundary cannot be frozen safely."""
...
def _resolve_relative(relative: str) -> Path:
    ...
    if (
        ...
        raise V4FreezeError(f"non-canonical contract path: {relative!r}")
...
V4_SOURCE_PATHS = _resolve_map(contract.V4_SOURCE_RELATIVE_PATHS)
...
def _record(path: str | Path) -> dict[str, Any]:
    ...
    if not resolved.is_file():
        raise V4FreezeError(f"required frozen file is missing: {resolved}")
...
def _validate_alignment() -> None:
    exact = {
        ...
        "runner": (runner.RUNNER, V4_SOURCE_PATHS["runner"]),
    ...
    for label, (observed, expected) in exact.items():
        if Path(observed).expanduser().resolve() != Path(expected).resolve():
            raise V4FreezeError(f"V4 contract alignment drifted at {label}")
    if (
        ...
        raise V4FreezeError("V4 runner constants drifted from the pure contract")
...
def _validate_train_only_design() -> None:
    ...
    if (
        ...
        raise V4FreezeError("authorized V4 train-only design evidence is invalid")


def _validate_v3_lineage_and_corpus() -> dict[str, Any]:
    v3_lock = v3.verify_lock()
    if v3_lock.get("so_policy_id") != contract.SO_POLICY_ID:
        raise V4FreezeError("V3 status0 policy identity drifted")
    ...
    if (
        ...
        raise V4FreezeError("V3 inherited train corpus drifted")
...
def _validate_preflight_receipt() -> dict[str, Any]:
    ...
    if (
        ...
        or receipt.get("status") != "PASS_H0_PRIMARY_SPLIT_V4_PREFLIGHT_TESTS"
        ...
        raise V4FreezeError("V4 preflight receipt is non-canonical")
    tested_sources = receipt.get("tested_sources")
    if not isinstance(tested_sources, Mapping) or set(tested_sources) != set(
        V4_SOURCE_PATHS
    ):
        raise V4FreezeError("V4 preflight tested-source closure drifted")
    for key, path in V4_SOURCE_PATHS.items():
        if runner._canonical_bytes(tested_sources[key]) != runner._canonical_bytes(
            _record(path)
        ):
            raise V4FreezeError(f"V4 preflight source drifted: {key}")
    expected_lineage = {
    ...
    if not isinstance(lineage, Mapping) or set(lineage) != set(expected_lineage):
        raise V4FreezeError("V4 preflight lineage schema drifted")
    for key, path in expected_lineage.items():
        if runner._canonical_bytes(lineage[key]) != runner._canonical_bytes(
            _record(path)
        ):
            raise V4FreezeError(f"V4 preflight lineage drifted: {key}")
    return receipt
...
def _build_lock_payload() -> dict[str, Any]:
    ...
    payload = {
        "schema_version": 4,
        "status": "H0_PRIMARY_GRF_SPLIT_V4_EXECUTION_FROZEN",
        ...
        "sources": {
            "v4": _record_map(V4_SOURCE_PATHS),
    ...
    if set(payload) != runner.LOCK_TOP_LEVEL_KEYS:
        raise V4FreezeError("V4 runner/freezer lock schemas disagree")
    runner._require_finite_json(payload, "V4 lock")
    return payload


def freeze() -> dict[str, Any]:
    """Create only the no-clobber V4 lock after every prerequisite passes."""
    if os.path.lexists(LOCK):
        raise V4FreezeError(f"refusing to clobber existing V4 lock: {LOCK}")
    if os.path.lexists(RUN_ROOT):
        raise V4FreezeError(
            f"V4 run root must not exist before execution freeze: {RUN_ROOT}"
        )
    payload = _build_lock_payload()
    if os.path.lexists(LOCK) or os.path.lexists(RUN_ROOT):
        raise V4FreezeError("V4 lock/run root appeared during freeze")
    runner._write_json_exclusive(LOCK, payload)
    if os.path.lexists(RUN_ROOT):
        raise V4FreezeError("V4 freezer unexpectedly created the run root")
    runner.verify_lock()
    return payload
