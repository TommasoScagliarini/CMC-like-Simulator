...
from validation import compare_h0_primary_grf_split_v3_qualification as gates
...
def _summary(role: str, case_id: str) -> dict[str, object]:
    ...
    for joint in contract.JOINTS:
        sea[joint] = {
            signal: _metric(1.0, samples=5000 if signal != "tau_spring_rate_nm_s" else 4500)
...
class QualificationComparatorTests(unittest.TestCase):
    ...
    def test_verified_status0_is_not_rejected_as_raw_fallback(self) -> None:
        result = gates.common_rollout_gate(
            _summary("baseline", self.CASE), role="baseline", case_id=self.CASE
        )
    ...
    def test_unaccepted_bounded_ls_closes_common_gate(self) -> None:
        ...
        result = gates.common_rollout_gate(
            summary, role="candidate", case_id=self.CASE
        )
    ...
    def test_condition_matched_caps_are_applied(self) -> None:
        ...
        # Reserve baseline RMS=100 -> max(5 Nm, 5%)=5; 104.9 passes.
        candidate["episode_metrics"]["reserve_norm_nm"]["rms"] = 104.9
        candidate["episode_metrics"]["reserve_norm_nm"]["abs_max"] = 110.0
        # abs-max baseline=110 -> 5.5 cap, so 110 remains comfortably below.
        result = gates.condition_matched_gate(baseline, candidate, case_id=self.CASE)
        self.assertTrue(result["passed"])
        ...
        result = gates.condition_matched_gate(baseline, failing, case_id=self.CASE)
        self.assertFalse(result["passed"])
    ...
    def test_baseline_metric_schema_matches_scaffold_order(self) -> None:
        ...
        self.assertEqual(list(metrics["reserve"]), [row[0] for row in contract.RESERVE_TOLERANCES])
